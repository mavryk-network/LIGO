...
import Morley.Micheline
  (Annotation, MichelinePrimAp (..), MichelinePrimitive, StringEncode (..), MavrykBigNum,
  annotFromText)
...
instance MessagePack MavrykBigNum where
  toObject _ = const ObjectNil
  fromObjectWith _ = fmap StringEncode . withMsgText "MavrykBigNum" \txt ->
...
fromLigoType st = \case
  LTCConstant LigoTypeConstant {..} ->
    -- See: https://gitlab.com/mavryk-network/ligo/-/issues/1478
  ...
  where
    ...
    fromLigoTypeLiteralValue = \case
      ...
      LTLVMumav n -> make' (st, CMav $ show n)

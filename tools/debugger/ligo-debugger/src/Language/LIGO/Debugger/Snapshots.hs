        ...
        (EXEC{}, argEl :& StkEl lam :& _) -> do
          ...
          case newStack of
            StkEl newLam@VLam{} :& stkEls -> do
              ...
              -- function.
              let oldLambdaMeta = view lambdaMetaL lam ?: def
              let appliedArg = LambdaArg
                    { laValue = stkElValue argEl
                    , laType = LigoTypeUnresolved
                      -- ↑ TODO: currently there is no way to properly initialize
                      -- this field, we do not track the necessary info in
                      -- interpreter
                    }
              let newLambdaMeta = addAppliedArg appliedArg oldLambdaMeta
              let embeddedLam = newLam & lambdaMetaL ?~ newLambdaMeta
              logMessage [int|n|
                Embedding new meta with an applied argument
                #{newLambdaMeta}

...
export function defaultModeDetector(filePath) {
  ...
  if (filePath.endsWith(".json") || filePath.endsWith(".abi")) {
    return "json";
  }
  if (filePath.endsWith(".md") || filePath.endsWith(".md.in")) {
    return "markdown";
  }
...
class ModelSessionManager {
  ...
  replaceModelSession(oldPath, newPath) {
    ...
    return new MonacoEditorModelSession(
      newModel,
      this.sessions[oldPath]._remote,
      this.sessions[oldPath]._CustomTab,
      this.sessions[oldPath].decorations
    );
  }
  ...
  set currentModelSession(modelSession) {
    if (modelSession.filePath.indexOf("node_modules") > -1) {
      modelSession.setTopbar({
        title:
          "This file is a dependency and changes may have a great impact on the project. Make sure you know what you are doing before making changes.",
      });
      this._editorContainer.refresh();
    }
  ...
  async newModelSession(filePath, remote = false, mode = this.modeDetector(filePath)) {
    ...
    if (!this.sessions[filePath]) {
      ...
      this.sessions[filePath] = new MonacoEditorModelSession(
        model,
        remote,
        this.CustomTabs[mode],
        this.decorationMap[filePath] || []
      );
    }
  ...
  delete() {
    ...
    const { model } = this.sessions[this.currentFilePath];
    if (this._editor) {
  ...
  updateCompilerBatch(decorationMap, newInfoArr) {
    const newMap = newInfoArr.reduce((prev, cur) => {
      prev[cur.filePath] ? prev[cur.filePath].push(cur) : (prev[cur.filePath] = [cur]);
      return prev;
    }, {});
  ...
  updateDecorations(newValue, pathFromlinter) {
    pathFromlinter
      ? this.updateLinterBatch(this.decorationMap, newValue, pathFromlinter)

...
- `networks`: list of allowed networks. You need to specify unique `id`, `group` for network groups such as `Mavryk` or `T4L3NT`, `name` where `Mainnet` is used for main chain and other names will be joined together as testnets, `fullName` as display name, `url` of the node for specific chain, `explorerUrl`, `symbol` like `XTZ` of the chain coin, and `icon` with `type`. 
...
__Note__: `type` is a `NetworkType` for `mavlet` wallet. We use `@mavrykdynamics/webmavryk-mavlet-wallet` for it, so you should check what version of ` @mavrykdynamics/mavlet-dapp` is currently available and what networks available in this version. You can get network updates info in ` @mavrykdynamics/mavlet-dapp` [changelog](https://github.com/airgap-it/mavlet-sdk/releases) and ` @mavrykdynamics/mavlet-dapp` updates in `webmavryk` [changelog](https://tezoswebmavryk.io/docs/version/). And in case of any updates upgrade dependencies. If network is not available in current version you should use `custom` type.

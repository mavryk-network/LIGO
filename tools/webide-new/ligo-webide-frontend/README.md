...
__Note__: `type` is a `NetworkType` for `beacon` wallet. We use `@mavrykdynamics/taquito-beacon-wallet` for it, so you should check what version of ` @mavrykdynamics/beacon-dapp` is currently available and what networks available in this version. You can get network updates info in ` @mavrykdynamics/beacon-dapp` [changelog](https://github.com/airgap-it/beacon-sdk/releases) and ` @mavrykdynamics/beacon-dapp` updates in `taquito` [changelog](https://tezostaquito.io/docs/version/). And in case of any updates upgrade dependencies. If network is not available in current version you should use `custom` type.

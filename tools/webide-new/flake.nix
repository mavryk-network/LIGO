{
  ...
  outputs = {
    ...
    system: let
      ...
      ligo-binary = {
        "x86_64-linux" = {
          url = "https://gitlab.com/mavryk-network/ligo/-/jobs/4687472710/artifacts/raw/ligo";
    ...
    in rec {
      packages =
        ...
        // (pkgs.lib.optionalAttrs (system == "x86_64-linux") {
          # prebuilt ligo binary and nix-based mavkit-client are only available for x86_64-linux
          ligo-bin = pkgs.runCommand "ligo-bin" {} ''
            install -Dm777 ${pkgs.fetchurl ligo-binary.${system}} $out/bin/ligo
          '';
          inherit mavryk-client;
        });
      tests = {

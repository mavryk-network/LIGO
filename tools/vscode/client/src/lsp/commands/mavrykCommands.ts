...
/**
 * Mavryk toolkit to be used for deploying contracts. Currently deploys them to
 * ecadinfra.
 */
const Mavryk = (network: string): MavrykToolkit | undefined => {
  if (currentlyActiveNetworks.includes(network)) {
    return new MavrykToolkit(`https://${network}.ecadinfra.com`)
  } else {
    return undefined
  }
}
...
export async function executeDeploy(context: LigoContext, client: LigoProtocolClient): Promise<void> {
  ...
  vscode.window.withProgress(
    ...
    async (progress) => {
      const MavrykNetwork = Mavryk(network)
      if (!MavrykNetwork) {
      ...
      await importKey(MavrykNetwork, result);

      progress.report({
        message: 'Originating contract',
      })

      const op = await MavrykNetwork.contract.originate({
 ...
 * `mavkit-client` to deploy the smart contract to the network that will be
...
export async function executeGenerateDeployScript(context: LigoContext, client: LigoProtocolClient): Promise<void> {
  ...
  vscode.window.withProgress(
    ...
    async (progress) => {
      ...
      await importKey(MavrykNetwork, randomPrivateKey);
      progress.report({
        message: 'Calculating burn-cap cost',
      })

      const estimate = await MavrykNetwork.estimate.originate({
        code: JSON.parse(codeJson.result),
        init: JSON.parse(storageJson.result),
      });

      const sourceAccount = vscode.workspace.getConfiguration().get<string>('ligoLanguageServer.mavryk_source_account');

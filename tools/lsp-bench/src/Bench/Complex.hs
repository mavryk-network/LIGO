...
(complexBenchBigFileReferencesEdits, complexBenchBigFileReferencesKeystrokes) =
  ...
  complexBenchBigFileReferences insertFuncName insert =
      benchLspSession ("one_big_file references " <> insertFuncName) projectWithOneBigFile $ do
        ...
        insert doc (1001, 1) "let q1 = 1234 + 5678 + 90"
        refs2 <-
          requestReferences $
        ...
        insert doc (1002, 1) "let q2 = 1234 + 24 + 90"
        refs3 <-
          requestReferences $

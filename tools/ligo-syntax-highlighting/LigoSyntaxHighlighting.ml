...
let vim_syntax_highlighting dir syntaxes =
  let write_file file_name subdir_name contents =
    ...
    if not (Sys.file_exists subdir) then Ligo_unix.mkdir subdir ~perm:0o777;
...
let textmate_syntax_highlighting_impl
    (generate_configuration : bool)
    (dir : string)
    (syntaxes : (string * SyntaxHighlighting.Core.t) list)
    : unit Term.ret
  =
  let go (syntax, textmate) = function
    | Ok () ->
      ...
      | Ok (syntax_highlighting_json, language_conf_json) ->
        ...
        Ok ()
      | Error (SyntaxHighlighting.Core.Referenced_rule_does_not_exist s) ->
        Error (Format.sprintf "Referenced rule '%s' does not exist." s)
      | Error (Meta_name_some_but_empty s) ->
        Error (Format.sprintf "%s.name has no value, but is expected to." s)
      | Error (Begin_cant_be_empty s) ->
        Error (Format.sprintf "%s.begin_ can't be empty." s)
...
let vscode_syntax_highlighting
    : string -> (string * SyntaxHighlighting.Core.t) list -> unit Term.ret
  =
  textmate_syntax_highlighting_impl true

let textmate_syntax_highlighting
    : string -> (string * SyntaxHighlighting.Core.t) list -> unit Term.ret
  =
...
let emacs_syntax_highlighting dir syntaxes =
  ...
  Print.print_lsp fmt;
  List.iter
    (fun ((name, t) : string * SyntaxHighlighting.Core.t) ->
...
let output : string -> string -> string -> string -> _ Term.ret =
 fun vscode_directory vim_directory emacs_directory textmate_directory ->
  if not (Sys.is_directory vscode_directory)
  then `Error (false, "Not a valid directory to output Visual Studio Code files")
  else if not (Sys.is_directory vim_directory)
  then `Error (false, "Not a valid directory to output Vim files")
  else if not (Sys.is_directory emacs_directory)
  then `Error (false, "Not a valid directory to output Emacs files")
  else if not (Sys.is_directory textmate_directory)
  then `Error (false, "Not a valid directory to output TextMate files")
    ...
    let syntaxes =
      ...
      ; (* MAVRYK: PascaLIGO *) "ligo", PascaLIGO.syntax_highlighting
      ]
    in

...
let generate_syntax_highlighting output_directory func file = 
  ...
  | Error Not_a_valid_reference s -> `Error (false, Format.sprintf "Not a valid reference: '%s'." s)
...
let output output_directory = 
  if not (Sys.is_directory output_directory) then 
    ...
    let cameligo_syntax_highlighting = generate_syntax_highlighting output_directory CameLIGO.syntax_highlighting "mligo.tmLanguage.json" in
    match cameligo_syntax_highlighting with 
      `Error e -> `Error e
    | `Ok ->
      let pascaligo_syntax_highlighting = generate_syntax_highlighting output_directory PascaLIGO.syntax_highlighting "ligo.tmLanguage.json" in
      match pascaligo_syntax_highlighting with 
        `Error e -> `Error e
      | `Ok ->
        let reasonligo_syntax_highlighting = generate_syntax_highlighting output_directory ReasonLIGO.syntax_highlighting "religo.tmLanguage.json" in

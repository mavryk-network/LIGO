...
let pos_to_position (pos: Pos.t) : Position.t =
  ...
  Position.create ~line:(pos#line - line_diff) ~character:(pos#point_num - pos#point_bol - character_diff)
...
fun region -> Lsp.Types.Location.create
...
let get_location : Scopes.def -> Location.t =
  function
    | Variable vdef -> vdef.range
    | Type tdef -> tdef.range
    | Module mdef -> mdef.range

let is_reference : Position.t -> Scopes.def -> bool =
  fun pos defintion ->
    let check_pos : Location.t -> bool =
    function

...
let rec instr e =
  let head, inner =
    ...
    | CallIndirect_symbol s ->
      "call_indirect $" ^ s, []
    | DataSymbol s -> "i32.const 0 ;; data symbol", []
    | FuncSymbol s -> "i32.const 0 ;; func symbol", []
...
let instr_list e = Node ("", list instr e)
...
let func_with_name name f =
  ...
  Node ("func" ^ name,
    [Node ("type " ^ var ftype, [])] @
    decls "local" (List.map (fun (_, s) -> s) locals) @
    list instr body
  )

let func_with_name_symbol f =
  let {name; ftype; locals; body} = f in
  Node ("func " ^ name,
    [Node ("type $" ^ string ftype, [])] @
    decls "local" (List.map (fun (_, s) -> s) locals) @
    list instr body
  )
...
let func_with_index off i f =
  match f.it with 
    FuncNoSymbol f -> func_with_name (" $" ^ nat (off + i)) f
  | FuncSymbol f -> func_with_name_symbol f

let func f =
  match f.it with 
    FuncNoSymbol f -> func_with_name "" f
  | FuncSymbol f -> func_with_name_symbol f

let _start x = Node ("start " ^ var x, [])
...
let data_part dinit = 
  ...
  let rec aux result = function 
    (* String s :: rest -> aux (result ^ s) rest
  | Int32 i  :: rest -> aux (result ^ "") rest
  | Int64 i :: rest  -> aux (result ^ "") rest
  | Nativeint i :: rest  -> aux (result ^ "") rest
  | Int16 i :: rest  -> aux (result ^ "") rest
  | Int8 i :: rest  -> aux (result ^ "") rest
  | Float32 f :: rest  -> aux (result ^ "") rest
  | Float64 f :: rest  -> aux (result ^ "") rest
  | Symbol s :: rest  -> aux (result ^ "") rest
  | FunctionLoc l :: rest -> aux (result ^ "") rest *)
  | _ :: rest -> aux (Atom "" :: result) rest
  | [] -> List.rev result
  in
  aux [] dinit.detail
  (* match dinit.detail with  *)


let data i seg =
  let {dinit; dmode} = seg.it in
  Node ("data $" ^ nat i, segment_mode "memory" dmode @ data_part dinit)
...
let module_with_var_opt x_opt m =
  ...
  Node ("module" ^ var_opt x_opt,
    listi typedef (List.map (fun {it; at} -> 
      match it with
        TypeSymbol {tdetails; _} -> {it = tdetails; at}
      | TypeNoSymbol t -> {it = t; at}
    ) m.it.types) @
    imports @
  ...
  try
    ...
    | `Textual ->
      let rec unquote def =
        match def.it with
        | Textual m -> m
        | Encoded (_, bs) -> failwith "ignore" (* Decode.decode "" bs *)
    ...
    | `Binary ->
      let rec unquote def =
        ...
        | Encoded (_, bs) -> failwith "ignore" (* Encode.encode (Decode.decode "" bs) *)
        | Quoted (_, s) -> unquote (Parse.string_to_module s)

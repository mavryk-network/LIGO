...
type vec_laneop = (vec_type, pack_size) memop * int
 ...
 and instr' =
   ...
   | Call_symbol of symbol                    (* call function *)
   | CallIndirect_symbol of symbol            (* call function through table *)
   | LocalGet_symbol of symbol                (* read local variable *)
   | LocalSet_symbol of symbol                (* write local variable *)
   | LocalTee_symbol of symbol                (* write local variable and keep value *)
   | GlobalGet_symbol of symbol               (* read global variable *)
   | GlobalSet_symbol of symbol               (* write global variable *)
...
and start' =
{
 ...
 type sym_info = sym_info' Source.phrase
 and sym_info' = 
 {
   name: string;
   details: sym_info_details;
 }
 and sym_info_details =
  | Function

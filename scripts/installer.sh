...
# curl https://gitlab.com/ligolang/ligo-ci-test/-/raw/dev/scripts/installer.sh | bash
...
if test $# -ne 1 || printf "$1" | tail -n +2 | grep -q '^' || ! (printf "$1" | grep -q \^\\\(next\\\|\[0-9.\]\*\\\)\$); then
  ...
  then
    ...
    docker pull "ligolang/ligo-ci-test:$version"
...
if test "x\$PWD" = "x"; then
  ...
  docker run --rm -v "\$PWD":"\$PWD" -w "\$PWD" ligolang/ligo-ci-test:$version "\$@"
fi
EOF
  fi

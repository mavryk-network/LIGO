...
let dummy_environment_result ?tezos_context () =
  Option.value
    ~default:(Lwt.map Result.ok @@ dummy_environment ())
    (Option.map (fun env -> Lwt.return (Ok env)) tezos_context)
...
let stack_ty_eq
  ...
  let%map Eq, _ = Lwt.return @@ alpha_wrap (X.stack_ty_eq env.tezos_context 0 a b) in
  ...
  let%bind x, _ =
    ...
    @@ alpha_wrap
         (let x = Script_ir_translator.ty_eq ~error_details a b in
          Gas_monad.run env.tezos_context x)
  in
...
let parse_michelson_fail
  ...
  dummy_environment_result ?tezos_context ()
  >>=? fun env ->
  ...
  | Typed descr ->
    Lwt.return
      (alpha_wrap (X.stack_ty_eq env.tezos_context 0 descr.aft aft)
      >>? fun (Eq, _) ->
...
let parse_michelson_data ?tezos_context michelson ty =
  ...
  in
  parse_data env.tezos_context ty michelson ~elab_conf ~allow_forged:true
...
let make_options
  ...
  let%bind tezos_context =
    ...
    in
    Lwt.map
      (fun result -> force_ok ~msg:("bad options " ^ __LOC__) @@ alpha_wrap result)
  ...
  let%map tezos_context =
    Lwt_list.fold_left_s
      (fun ctxt cnt ->
        let%map ctxt, _, _ =
          Lwt.map (fun result ->
              force_ok ~msg:("bad constants " ^ __LOC__) @@ alpha_wrap result)
...
let pack (data_ty : ('a, _) ty) (data : 'a) : bytes tzresult Lwt.t =
  Lwt.bind (dummy_environment ()) (fun env ->
      ...
      >>=?? fun (packed, _) -> Lwt_result_syntax.return packed)
...
let to_hex michelson =
  let open Lwt.Let_syntax in
  let%map bytes = to_bytes michelson in
  Hex.of_bytes bytes

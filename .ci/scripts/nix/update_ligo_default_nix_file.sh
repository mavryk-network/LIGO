...
# Replace SRI for nix
SRI_LIGO_BINARY_HASH=$(nix --extra-experimental-features nix-command hash to-sri --type sha256 $(jq -r .sha256 <<< $(nix run nixpkgs#nix-prefetch-git -- --rev $VERSION --url https://gitlab.com/ligolang/ligo-ci-test.git --fetch-submodules --quiet --no-deepClone)))

...
RUN sed -i 's/"ppx_inline_test" {with-test}/"ppx_inline_test"/' vendors/tezos-ligo/src/lib_stdlib/tezos-stdlib.opam
RUN sed -i 's/"tezos-stdlib"/"tezos-stdlib"\n  "bls12-381-legacy"/' vendors/tezos-ligo/src/lib_protocol_environment/tezos-protocol-environment-structs.opam
RUN sed -i 's/{ >= "2.7.2" }/{ >= "2.7.2" \& < "2.8.0" }/' vendors/tezos-ligo/src/lib_context/tezos-context.opam
RUN sed -i 's/ptime.clock.os/ptime.clock.os\n\t\t\t\t\t\t\t\t\t\t\t\tfmt/' vendors/tezos-ligo/src/lib_stdlib_unix/dune
RUN sed -i 's/"lwt"/"lwt"\n\t"fmt"/' vendors/tezos-ligo/src/lib_stdlib_unix/tezos-stdlib-unix.opam

...
let wasm_code_format =
  ...
  let doc =
    "CODE_FORMAT format that will be used by compile-contract for the resulting \
     webassembly. Currently only 'binary' (default) is supported."
  in
  flag ~doc docv
...
let compile_file =
  ...
  let readme () =
    "This sub-command compiles a contract to Michelson or WebAssembly code. It expects a \

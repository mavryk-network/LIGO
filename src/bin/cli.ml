...
let return = ref Done
let compile_file =
  let f source_file entry_point views syntax protocol_version display_format disable_michelson_typechecking enable_typed_opt michelson_format output_file show_warnings warning_as_error michelson_comments constants file_constants project_root warn_unused_rec backend () =
    let raw_options = Compiler_options.make_raw_options ~entry_point ~syntax ~views ~protocol_version ~disable_michelson_typechecking ~enable_typed_opt ~warning_as_error ~constants ~file_constants ~project_root ~warn_unused_rec ~backend () in
  ...
  let readme () = "This sub-command compiles a contract to Michelson code. It \
  ...
  (f <$> source_file <*> entry_point <*> on_chain_views <*> syntax <*> protocol_version <*> display_format <*> disable_michelson_typechecking <*> enable_michelson_typed_opt <*> michelson_code_format <*> output_file <*> warn <*> werror <*> michelson_comments <*> constants <*> file_constants <*> project_root <*> warn_unused_rec <*> backend)

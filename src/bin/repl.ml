...
let import_file ~raise ~raw_options state file_name module_name =
  ...
  let options = Compiler_options.set_init_env options state.env in
  let module_ =
    let prg = Build.qualified_typed ~raise ~options Env (Build.Source_input.From_file file_name) in
...
let use_file ~raise ~raw_options state file_name =
  ...
  let module' = Build.qualified_typed ~raise ~options Env (Build.Source_input.From_file file_name) in

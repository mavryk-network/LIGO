...
let get_declarations_core (core_prg : Ast_core.program) =
  ...
  *)
  ...
  let func_declarations =
    ...
    @@ Ligo_compile.Of_core.list_declarations core_prg
  in
  let type_declarations =
    List.map ~f:(fun a -> `Type a)
    @@ Ligo_compile.Of_core.list_type_declarations core_prg
  in
  ...
  let mod_declarations =
    ...
    @@ List.filter
         mod_declarations
         ~f:ignore_module_variable_which_is_absolute_path
  in
...
let repl_result_jsonformat = function
  ...
  | Defined_values_core module_ ->
    ...
    let type_declarations =
      Ligo_compile.Of_core.list_type_declarations module_
    in
    let func_defs =
      List.map
        ~f:(fun n -> `Assoc [ "name", Value_var.to_yojson n ])
        func_declarations
    in
    let type_defs =
      List.map
        ~f:(fun n -> `Assoc [ "name", Type_var.to_yojson n ])
        type_declarations
    in
    `Assoc [ "definitions", `List (func_defs @ type_defs) ]
  | Defined_values_typed module' ->
    let func_declarations =
      Ligo_compile.Of_typed.list_declarations false module'
    in
    let type_declarations =
      Ligo_compile.Of_typed.list_type_declarations module'
    in
    let func_defs =
      List.map
        ~f:(fun n -> `Assoc [ "name", Value_var.to_yojson n ])
...
let concat_modules
    ~declaration
    (m1 : Ast_typed.program)
    (m2 : Ast_typed.program)
    : Ast_typed.program
  =
      ...
      (fun ~raise ~catch:_ ->
        ...
        let state =
          { state with
            env
          ; top_level =
              concat_modules ~declaration:true state.top_level typed_prg
          }
        in
...
let import_file ~raise ~raw_options state file_name module_name =
  let file_name =
    ModRes.Helpers.resolve ~file:file_name state.module_resolutions
  in
  let options =
...
let eval display_format state c =
  ...
  | Error (e, _w) ->
    let disp =
      Displayable { value = e; format = Main_errors.Formatter.error_format }
    in
    let out : string =
      match t with
      | Human_readable -> convert ~display_format:t disp
...
let rec loop ~raw_options syntax display_format term history state n =
  ...
  let k, state, out =
    parse_and_eval ~raw_options display_format state command_utf8
  in

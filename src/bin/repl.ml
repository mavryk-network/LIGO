...
(* Helpers *)
let builtin_mod = ["List"; "Bytes"; "Crypto"; "String"; "Set"; "Map"; "Tezos"; "Option"; "Big_map"]
...
let get_declarations_typed typed_prg =
  ...
  *)
  let ignore_module_variable_which_is_absolute_path module_variable =
    ...
    not @@ Caml.Sys.file_exists module_variable in
  let ignore_builtin_mod module_variable = not (List.mem builtin_mod (Format.asprintf "%a" Ast_typed.ModuleVar.pp module_variable) ~equal:String.equal) in
  ...
  let mod_declarations  = List.map ~f:(fun a -> `Module a) @@ List.filter mod_declarations ~f:ignore_builtin_mod in
  func_declarations @ type_declarations @ mod_declarations

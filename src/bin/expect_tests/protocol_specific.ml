...
let%expect_test _ =
  ...
  [%expect {xxx|
    ...
    const a : bls12_381_g1 = [%Michelson {|
      ...
      |}]
    const b : bls12_381_g2 = [%Michelson {|
      ...
      |}]
    const main : ( list (( bls12_381_g1 * bls12_381_g2 )) * bool ) -> ( list (operation) * bool ) = lambda (#1) return let #3 = #1 in
     match #3 with
      | ( p , s ) ->

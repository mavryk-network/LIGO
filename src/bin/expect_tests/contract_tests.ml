...
Not all free variables could be inlined in Tezos.create_contract usage: gen#242. |}];
  ...
  [%expect{|
    ...
    Not all free variables could be inlined in Tezos.create_contract usage: foo#257. |}];
...
let%expect_test _ =
  ...
  [%expect {|
File "../../test/contracts/negative/bad_contract.mligo", line 6, characters 50-57:
  5 | [@entry]
...
An entrypoint must of type "parameter * storage -> operation list * storage". |}];
  ...
  [%expect {|
File "../../test/contracts/negative/bad_contract2.mligo", line 8, characters 50-56:
  7 | [@entry]
...
An entrypoint must of type "parameter * storage -> operation list * storage". |}];
  ...
  [%expect {|
File "../../test/contracts/negative/bad_contract3.mligo", line 8, characters 50-56:
  7 | [@entry]
...
let%expect_test _ =
  ...
  [%expect {|
    File "../../test/contracts/negative/compile_test.mligo", line 19, character 0 to line 27, character 16:
     18 |
     19 | [@entry]
          ^^^^^^^^
     20 | let main (action : parameter) (store : storage) : return =
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
     21 |   ([] : operation list), // No operations
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
     22 |   (match action with
          ^^^^^^^^^^^^^^^^^^^^
     23 |      Increment (n) ->
          ^^^^^^^^^^^^^^^^^^^^^
     24 |        let _ = Test.log "foo" in
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
     25 |        add store n
          ^^^^^^^^^^^^^^^^^^
     26 |    | Decrement (n) -> sub store n
          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
     27 |    | Reset -> 0)
          ^^^^^^^^^^^^^^^^
     28 |
...
let%expect_test _ =
  ...
  [%expect {|
    No entrypoints, invalid contract.
    Hint: If your contract previously used a `main`-style entrypoint, try adding an `entry` annotation to `main`. |}]

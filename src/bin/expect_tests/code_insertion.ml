...
let%expect_test _ =
  ...
  [%expect{xxx|
    File "../../test/contracts/negative/bad_michelson_insertion_1.ligo", line 5, characters 32-40:
      4 |   const f : (nat * nat -> nat)= [%Michelson ({| ADD |} : nat *nat -> nat)];
      5 | } with ((nil: list(operation)), f (p, s))

    Invalid type(s).
    Expected: "( nat * nat )", but got: "nat". |xxx}]

let%expect_test _ =
  run_ligo_bad [ "compile" ; "contract" ; bad_contract "bad_michelson_insertion_2.ligo" ] ;
  [%expect{xxx|
File "../../test/contracts/negative/bad_michelson_insertion_2.ligo", line 4, characters 33-77:
  3 | function main (const (p, s) : nat * nat ) : list (operation)* nat is block {
  4 |   const f : (nat -> nat -> nat)= [%Michelson ({| ADD |} : nat -> nat -> nat)];
  5 | } with ((nil: list(operation)), f (p, s))

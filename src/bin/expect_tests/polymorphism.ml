...
let%expect_test _ =
  run_ligo_good
    ...
    ; "expression"
    ; "cameligo"
    ; "map (fun (f : (string -> int -> int)) -> f \"hello\" 4) (uhms : (string \
       -> int -> int) list)"
    ; "--init-file"
    ; test "map.mligo"
    ];
...
let%expect_test _ =
  run_ligo_good
    ...
    ; "expression"
    ; "cameligo"
    ; "try_transfer (\"tz1KqTpEZ7Yob7QbPE4Hy4Wo8fHG8LhKxZSx\" : address) 0 \
       (Build_state (Map.empty :(address, tokenValue) map))"
    ; "--init-file"
...
let%expect_test _ =
  run_ligo_bad
    [ "compile"; "parameter"; test "unresolved/parameter.mligo"; "p" ];
  [%expect
    {xxx|

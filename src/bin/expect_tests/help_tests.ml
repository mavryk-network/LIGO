...
This sub-command compiles a contract to Michelson or WebAssembly code. It expects a source file and an entrypoint function that has the type of a contract: "parameter * storage -> operations list * storage".

=== flags ===

  [--backend Compile]        . the contract to `michelson`(default) or `wasm`.
  ...
  [--warn-unused-rec]        . warn about unused recursion in a recursive
                               function
  [--wasm-format CODE_FORMAT]
                             . format that will be used by compile-contract for
                               the resulting webassembly. Currently only
                               'binary' (default) is supported.

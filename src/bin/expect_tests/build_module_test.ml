...
let%expect_test _ =
  run_ligo_good
    [ "print"; "dependency-graph"; contract "D.mligo"; "--format"; "json" ];
  [%expect
    {|
    {
      "status": "error",
...
let%expect_test _ =
  run_ligo_good
    [ "print"; "dependency-graph"; contract "Xmain.mligo"; "--format"; "dev" ];
  [%expect
    {|
...
let%expect_test _ =
  run_ligo_bad
    ...
    ; "interpret"
    ; "--init-file"
    ; contract "module_scoping_bug.mligo"
    ; "x"
    ];
  [%expect
    {|

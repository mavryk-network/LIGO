...
let%expect_test _ =
  run_ligo_good
    [ "run"; "interpret"; "t5 1"; "--init-file"; good_test "pm_test.mligo" ];
  [%expect {| 1 |}]

let%expect_test _ =
  run_ligo_good
    [ "run"; "interpret"; "t6 42"; "--init-file"; good_test "pm_test.mligo" ];
  [%expect {| 2 |}]
...
let%expect_test _ =
  run_ligo_good
    [ "info"; "measure-contract"; good_test "nested_record_sum.mligo" ];
  [%expect {| 142 bytes |}]

...
module RawParserType (CST : CST) = struct
  module type RawParser = sig
    ...
    module MenhirInterpreter :
      MenhirLib.IncrementalEngine.EVERYTHING with type token = token

    module Incremental : sig
      val interactive_expr : Lexing.position -> CST.expr MenhirInterpreter.checkpoint
      val contract : Lexing.position -> CST.t MenhirInterpreter.checkpoint
    end
  end
end
...
struct
  ...
  module ParserLexerBuilder =
    Parsing_shared.Common.MakeParser (Config) (Token) (ParErr) (UnitPasses) (TokenPasses)
      (struct
        type tree = CST.t
      end)
      (Parser)
  ...
  let print_list ?(sep = "") out_buffer =
    List.iter ~f:(fun s ->
        Buffer.add_string out_buffer s;
        Buffer.add_string out_buffer sep)


  let tokens_formatter token = Token.to_string ~offsets:false `Point token

  let print_tokens tokens out_buffer =
    tokens |> List.map ~f:tokens_formatter |> print_list ~sep:"\n" out_buffer
  ...
  let test_file file =
    ...
    let%bind error =
      reverse_result @@ replace @@ parse_file (to_string file (Some Blank) Test)
    in
    ...
    let replace =
      Result.map_error ~f:(fun _ -> "FAIL : can't get original file tokens")
    in
    ...
    let replace =
      Result.map_error ~f:(fun _ -> "FAIL : can't get recovered file tokens")
    in
    ...
    print_buffer original_cst_symbols
    @@ to_string file (Some Cst_symbols) Original_generated;
  ...
  let test () =
    ...
    List.iter
      ~f:(fun file ->
        Buffer.add_string result @@ from_result @@ test_file file;

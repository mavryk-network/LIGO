...
then
	...
	Darwin*)
	    ...
	    ;;
	*)
	    LIGO_FILES=$(LC_COLLATE=C.UTF-8 find $DUNE_WORKSPACE_ROOT/src/test/error-recovery/$TEST_TYPE/$DIALECT -name "*.$LIGO_FILE_EXT" -type f -printf "%f\n" | LC_ALL=C sort -fus | uniq)
	    ;;
    esac
fi

...
let transfer ~raise f s () =
  ...
  let storage =
    e_record_ez
      [ ( "tokens"
        , e_big_map [ sender, e_nat 100; from_, e_nat 100; to_, e_nat 100 ] )
      ; ( "allowances"
        , e_big_map
            [ e_record_ez [ "owner", from_; "spender", sender ], e_nat 100 ] )
  ...
  let new_storage =
    e_record_ez
      [ ( "tokens"
        , e_big_map [ sender, e_nat 100; from_, e_nat 90; to_, e_nat 110 ] )
      ; ( "allowances"
        , e_big_map
            [ e_record_ez [ "owner", from_; "spender", sender ], e_nat 90 ] )
      ; "total_supply", e_nat 300
      ]
  in
  ...
  let options =
    Proto_alpha_utils.Memory_proto_alpha.(
      make_options ~env:(test_environment ()) ())
  in
...
let transfer_not_e_allowance ~raise f s () =
  ...
  let storage =
    e_record_ez
      ...
      ; ( "allowances"
        , e_big_map
            [ e_record_ez [ "owner", from_; "spender", sender ], e_nat 0 ] )
      ; "total_supply", e_nat 300
      ]
  in
  let parameter =
...
let transfer_not_e_balance ~raise f s () =
  ...
  let storage =
    e_record_ez
      [ ( "tokens"
        , e_big_map [ sender, e_nat 100; from_, e_nat 0; to_, e_nat 100 ] )
      ; ( "allowances"
        , e_big_map
            [ e_record_ez [ "owner", from_; "spender", sender ], e_nat 100 ] )
      ; "total_supply", e_nat 300
...
let approve ~raise f s () =
  ...
  let storage =
    e_record_ez
      ...
      ; ( "allowances"
        , e_big_map
            [ e_record_ez [ "owner", sender; "spender", from_ ], e_nat 0 ] )
      ; "total_supply", e_nat 300
      ]
  in
  ...
  let new_storage =
    e_record_ez
      ...
      ; ( "allowances"
        , e_big_map
            [ e_record_ez [ "owner", sender; "spender", from_ ], e_nat 100 ] )
      ; "total_supply", e_nat 300
      ]
  in
  let input = e_pair parameter storage in
...
let main =
  test_suite
    ...
    ; test_w
        "approve (unsafe allowance change)"
        (approve_unsafe mfile_FA12 "cameligo")

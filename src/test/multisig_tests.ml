...
let get_program_w f = get_program_w f (Contract "main")
...
let params ~add_warning ~raise counter msg keys is_validl f =
  let _,env = get_program_w ~raise f () in
...
let invalid_1_of_1 ~add_warning ~raise f () =
  let (program,env) = get_program_w ~raise f () in
  let exp_failwith = "Invalid signature" in
  let keys = [gen_keys ()] in
  let test_params = params ~add_warning ~raise 0 empty_message keys [false] f in
  let () = expect_string_failwith ~raise
    (program,env) "main" (e_pair test_params (init_storage 1 0 keys)) exp_failwith in
  ()

(* Provide one valid signature when the threshold is one of one key *)
let valid_1_of_1 ~add_warning ~raise f () =
  let (program,env) = get_program_w ~raise f () in
  let keys = gen_keys () in
  let () = expect_eq_n_trace_aux ~raise [0;1;2] (program,env) "main"
      (fun n ->
        let params = params ~add_warning ~raise n empty_message [keys] [true] f in
...
let valid_2_of_3 ~add_warning ~raise f () =
  let (program,env) = get_program_w ~raise f () in
  let param_keys = [gen_keys (); gen_keys ()] in
  let st_keys = param_keys @ [gen_keys ()] in
  let () = expect_eq_n_trace_aux ~raise [0;1;2] (program,env) "main"
      (fun n ->
        let params = params ~add_warning ~raise n empty_message param_keys [true;true] f in
...
let invalid_3_of_3 ~add_warning ~raise f () =
  let (program,env) = get_program_w ~raise f () in
  ...
  let test_params = params ~add_warning ~raise 0 empty_message param_keys [false;true;true] f in
  let exp_failwith = "Invalid signature" in
  let () = expect_string_failwith ~raise
...
let not_enough_2_of_3 ~add_warning ~raise f () =
  let (program,env) = get_program_w ~raise f () in
  let valid_keys = [gen_keys() ; gen_keys()] in
  let st_keys = gen_keys () :: valid_keys  in
  let test_params = params ~add_warning ~raise 0 empty_message (valid_keys) [true;true] f in
  let exp_failwith = "Not enough signatures passed the check" in
  let () = expect_string_failwith ~raise
...
let main = test_suite "Multisig" [
    test_ww "compile"                       (compile_main       file);
    test_ww "unmatching_counter"            (unmatching_counter file);
    test_ww "valid_1_of_1"                  (valid_1_of_1       file);
    test_ww "invalid_1_of_1"                (invalid_1_of_1     file);
    test_ww "not_enough_signature"          (not_enough_1_of_2  file);
    test_ww "valid_2_of_3"                  (valid_2_of_3       file);
    test_ww "invalid_3_of_3"                (invalid_3_of_3     file);
    test_ww "not_enough_2_of_3"             (not_enough_2_of_3  file);
    test_ww "compile (mligo)"               (compile_main       mfile);
    test_ww "unmatching_counter (mligo)"    (unmatching_counter mfile);
    test_ww "valid_1_of_1 (mligo)"          (valid_1_of_1       mfile);
    test_ww "invalid_1_of_1 (mligo)"        (invalid_1_of_1     mfile);
    test_ww "not_enough_signature (mligo)"  (not_enough_1_of_2  mfile);
    test_ww "valid_2_of_3 (mligo)"          (valid_2_of_3       mfile);
    test_ww "invalid_3_of_3 (mligo)"        (invalid_3_of_3     mfile);
    test_ww "not_enough_2_of_3 (mligo)"     (not_enough_2_of_3  mfile);
    test_ww "compile (religo)"              (compile_main       refile);
    test_ww "unmatching_counter (religo)"   (unmatching_counter refile);
    test_ww "valid_1_of_1 (religo)"         (valid_1_of_1       refile);
    test_ww "invalid_1_of_1 (religo)"       (invalid_1_of_1     refile);
    test_ww "not_enough_signature (religo)" (not_enough_1_of_2  refile);
    test_ww "valid_2_of_3 (religo)"         (valid_2_of_3       refile);
    test_ww "invalid_3_of_3 (religo)"       (invalid_3_of_3     refile);
    test_ww "not_enough_2_of_3 (religo)"    (not_enough_2_of_3  refile);
  ]

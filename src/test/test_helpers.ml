...
let typed_program_with_imperative_input_to_michelson ~raise (program : Ast_typed.program) (entry_point: Ast_typed.expression_variable) (input: Ast_imperative.expression) : Stacking.compiled_expression *  Ast_aggregated.type_expression =
...
let expect ~(raise: (all,Main_warnings.all) raise) ?options program entry_point input expecter =
  let result =
    trace ~raise (test_run_tracer entry_point) @@
    run_typed_program_with_imperative_input ?options program entry_point input in
  expecter result

let expect_fail ~(raise: (all,Main_warnings.all) raise) ?options program entry_point input =
  trace ~raise (test_run_tracer entry_point) @@

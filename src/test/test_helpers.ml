...
let test_w_all name test =
  List.map
    ~f:(fun s ->
      let file =
        "./contracts/" ^ Str.(global_replace (regexp " ") "_" name) ^ "." ^ s
      in
...
let test name f =
  Test (Alcotest.test_case name `Quick @@ fun () -> wrap_test name f)
...
let pack_payload
  ...
  let code =
    ...
    in
    let aggregated =
      Ligo_compile.Of_typed.compile_expression
        ~raise
...
let expect_eq_n_strict_pos_small ?options =
  expect_eq_n_aux ?options [ 1; 2; 10 ]

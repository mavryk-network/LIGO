...
let compile_main ~raise () =
  Test_helpers.compile_main ~raise "./contracts/hashlock.ligo" ()
...
let commit ~raise () =
  ...
  let lock_time =
    Memory_proto_alpha.Protocol.Script_timestamp.add_delta
      now
  ...
  let commit =
    e_record_ez
      [ "date", e_timestamp_z (to_sec lock_time); "salted_hash", salted_hash ]
  in
...
let reveal_no_commit ~raise () =
  let program = get_program ~raise () in
  let empty_message = empty_message in
  let reveal =
    e_record_ez
      [ "hashable", e_bytes_string "hello world"; "message", empty_message ]
  in
  let test_hash_raw = sha_256_hash (Bytes.of_string "hello world") in
  let test_hash = e_bytes_raw test_hash_raw in
  let pre_commits =
...
let reveal_young_commit ~raise () =
  ...
  let options =
    Proto_alpha_utils.Memory_proto_alpha.(
      make_options ~env:(test_environment ()) ~now ~sender:first_contract ())
  in
  expect_string_failwith
    ~raise
    ~options
...
let reveal_breaks_commit ~raise () =
  let program = get_program ~raise () in
  let empty_message = empty_message in
  let reveal =
    e_record_ez
  ...
  let commit =
    e_record_ez
      [ "date", e_timestamp_z (to_sec now); "salted_hash", salted_hash ]
  in
  let commits = e_big_map [ e_address first_committer, commit ] in
  let init_storage = storage test_hash true commits in
  let options =
...
let main =
  test_suite
    ...
    ; test_w "reveal (raise.error if no commitment)" reveal_no_commit
    ; test_w "reveal (raise.error if commit too young)" reveal_young_commit
    ; test_w "reveal (raise.error if breaks commitment)" reveal_breaks_commit
    ; test_w "reveal (raise.error if wrong bytes for hash)" reveal_wrong_commit
    ; test_w "reveal (raise.error if attempt to reuse)" reveal_no_reuse

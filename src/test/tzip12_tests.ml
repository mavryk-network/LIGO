...
let get_program_w f st = get_program_w ~st f (Contract "main")

let compile_main_w ~raise f s () =
  let typed_prg,_   = get_program_w ~raise f s () in
...
let transfer_w ~raise f s () =
  let (program, env) = get_program_w ~raise f s () in
...
let transfer_not_e_allowance_w ~raise f s () =
  let (program, env) = get_program_w ~raise f s () in
  let storage = e_record_ez [
    ("tokens", e_big_map [(sender, e_nat 100); (from_, e_nat 100); (to_, e_nat 100)]);
...
let transfer_not_e_balance_w ~raise f s () =
  let (program, env) = get_program_w ~raise f s () in
  let storage = e_record_ez [
...
let approve_unsafe_w ~raise f s () =
  let (program, env) = get_program_w ~raise f s () in
  let storage = e_record_ez [
    ("tokens", e_big_map [(sender, e_nat 100); (from_, e_nat 100); (to_, e_nat 100)]);
    ("allowances", e_big_map [(e_record_ez [("owner", sender); ("spender", from_)], e_nat 100)]);
...
let get_total_supply_w ~raise f s () =
  let (program, env) = get_program_w ~raise f s () in
  let storage = e_record_ez [
    ("tokens", e_big_map [(sender, e_nat 100); (from_, e_nat 100); (to_, e_nat 100)]);
    ("allowances", e_big_map [(e_record_ez [("owner", sender); ("spender", from_)], e_nat 100)]);
...
let main = test_suite "tzip-12" [
  test_ww "transfer"                          (fun ~add_warning -> transfer_w mfile_FA12 "cameligo");
  test_ww "transfer (not enough allowance)"   (fun ~add_warning -> transfer_not_e_allowance_w mfile_FA12 "cameligo");
  test_ww "transfer (not enough balance)"     (fun ~add_warning -> transfer_not_e_balance_w   mfile_FA12 "cameligo");
  test_ww "approve"                           (fun ~add_warning -> approve_w                  mfile_FA12 "cameligo");
  test_ww "approve (unsafe allowance change)" (fun ~add_warning -> approve_unsafe_w           mfile_FA12 "cameligo");

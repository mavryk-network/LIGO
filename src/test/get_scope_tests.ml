...
let validate_json_file file_name =
  let command_str =
    Format.sprintf "python3 -m jsonschema -i %s %s" file_name schema
  in
...
let schema_test_negative
  ...
  let _test () =
    ...
    let res_str, actual_status =
      ...
      | Ok (res_str, _) ->
        ( res_str
        , true (* Alcotest.fail "None errors are detected in negative test" *) )
  ...
  Alcotest.run
    ...
    ; ( "all_positive"
      , List.map
          ...
          ~f:(fun file ->
            schema_test_positive ~with_types:true ~speed:`Slow file) )
    ; ( "all_negative"
      , List.map
          (files_in_all_dirs

...
let schema_test_positive ?(with_types=false) ?(speed=`Quick) source_file =
  let _test () =
    let temp_file_name = Caml.Filename.temp_file ~temp_dir:"./" "get_scope_test" ".json" in
...
let schema_test_negative ?(with_types=false) ?(speed=`Quick)
  ...
  let _test () =
    ...
    let res_str, actual_status =
      match InfoApi.get_scope options source_file json () with
      | Ok    (res_str, _) -> res_str, true (* Alcotest.fail "None errors are detected in negative test" *)
      | Error (res_str, _) -> res_str, false
    in
    match expected_status with
    | Some expected_status -> Alcotest.(check bool) "Unexpected status" expected_status actual_status;
    | None -> ();
    write res_str;
    validate_json_file temp_file_name;
    ...
    | None -> if List.is_empty errors
              then Alcotest.fail "Expect non empty list of errors"
    | Some error_cnt ->
...
let files_in_dir dir_path =
  ...
  |> List.filter
         ~f:(fun x -> match (Filename.split_extension x) with
                        _, Some "mligo" | _, Some "jsligo"
                      | _, Some "ligo"  | _, Some "religo" -> true
                      | _ -> false
         )
...
let main =
  ...
  Alcotest.run "get-scope json validation tests" [
    "positive", [
    ...
    "negative", [
      ...
      schema_test_negative ~with_types:false ~error_cnt:2
        "error-recovery/multiple_errors/2_errors.mligo";
      schema_test_negative ~with_types:false ~error_cnt:3
        "error-recovery/multiple_errors/3_errors.mligo";
    ];

    "all_positive",
      List.map (files_in_all_dirs [
            ...
            "contracts/tuples_no_annotation.jsligo"; (* syntax error: wrong brackets + untyped recursion *)
            "contracts/parametric_types.jsligo";      (* TODO: syntax error *)
            "contracts/double_fold_converter.religo"; (* TODO: abstracter: unknown constant *)
            "contracts/modules.religo";               (* TODO: syntax error *)
            "contracts/heap.ligo";                    (* TODO: syntax error in case *)
            "contracts/k.ligo";                       (* TODO: syntax error in case *)
            "contracts/existential.mligo";            (* TODO: syntax error: missing type annotation *)
            "contracts/heap-instance.ligo";           (* TODO: syntax error in case *)
            "contracts/bad_timestamp.ligo";           (* TODO: self ast imperative: bad timestamp *)
          ])
    ...
    "all_negative",
      List.map (files_in_all_dirs [
            ...
            "error-recovery/simple/jsligo/missing_semicolon_in_top_level.jsligo"; (* not negative *)

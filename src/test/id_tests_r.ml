...
let get_program = get_program "./contracts/id.religo"

let compile_main ~raise () =
  Test_helpers.compile_main ~raise "./contracts/id.religo" ()
...
let buy_id_sender_addr ~raise () =
  ...
  let param =
    e_record_ez
      [ "profile", owner_website
      ; "initial_controller", e_typed_none (t_address ())
      ]
  in
  let new_storage =
    e_record_ez
...
let update_details_owner ~raise () =
  ...
  let new_storage =
    e_record_ez
      [ ( "identities"
        , e_big_map [ e_int 0, id_details_1; e_int 1, id_details_2_diff ] )
      ; "next_id", e_int 2
      ; "name_price", e_mutez 1000000
      ; "skip_price", e_mutez 1000000
      ]
...
let update_owner ~raise () =
  ...
  in
  let param =
    e_record_ez [ "id", e_int 1; "new_owner", e_address owner_addr ]
  in
  let () =
    expect_eq
      ~raise

    ...
    let () =
      ...
      let options =
        Proto_alpha_utils.Memory_proto_alpha.(
          make_options ~env:(test_environment ()) ~amount ~sender:second_contract ())
      in
      expect_eq_n_pos_small ~raise ~options program "buy_single" make_input make_expected
    in
    let () =
      let amount =
        trace_option ~raise (test_internal "getting amount for run")
      ...
      @@ expect_eq_n_pos_small ~options program "buy_single" make_input make_expected
    in
    ()
  in
  ()
    ...
    let () =
      ...
      let options =
        Proto_alpha_utils.Memory_proto_alpha.(
          make_options ~env:(test_environment ()) ~amount ~sender:second_contract ())
      in
      expect_eq_n_pos_small ~raise ~options program "main" make_input make_expected
    in
    let () =
      let amount =
        trace_option ~raise (test_internal "getting amount for run")

...
let card_ty =
  t_record_ez [ "card_owner", t_address (); "card_pattern", t_nat () ]
...
let card_pattern (coeff, qtt) =
  e_record_ez [ "coefficient", coeff; "quantity", qtt ]


let card_pattern_ty =
  t_record_ez [ "coefficient", t_tez (); "quantity", t_nat () ]
...
let storage cards_patterns cards next_id =
  e_record_ez
    [ "cards", cards; "card_patterns", cards_patterns; "next_id", next_id ]
...
let basic a b cards next_id =
  let card_patterns = List.map ~f:card_pattern_ez [ 100, a; 20, b ] in
...
let transfer ~raise () =
  ...
  let () =
    let make_input n =
      let transfer_action =
        e_record_ez
          [ "card_to_transfer", e_nat 0; "destination", e_address second_owner ]
      in
      let storage = basic 100 1000 (cards_ez first_owner n) (2 * n) in
    ...
    let () =
      ...
      let options =
        Proto_alpha_utils.Memory_proto_alpha.(
          make_options ~env:(test_environment ()) ~amount ~sender ())
      in
  ...
  let () =
    ...
    let make_expecter : int -> Ast_core.expression -> unit =
     fun n result ->
      ...
      in
      let expected_storage =
        Test_helpers.expression_to_core ~raise expected_storage
      in
      trace_option ~raise (test_internal __LOC__)

...
let params_w ~raise counter payload keys is_validl f =
...
let not_enough_1_of_2 ~add_warning ~raise f () =
  let program = get_program_w ~raise f () in
  let exp_failwith = "Not enough signatures passed the check" in
  let keys = gen_keys () in
  let test_params = params_w ~raise 0 empty_payload [keys] [true] f in
...
let unmatching_counter ~add_warning ~raise f () =
  ...
  let keys = gen_keys () in
  let test_params = params_w ~raise 1 empty_payload [keys] [true] f in
  let () = expect_string_failwith ~raise
...
let invalid_1_of_1 ~add_warning ~raise f () =
  ...
  let keys = [gen_keys ()] in
  let test_params = params_w ~raise 0 empty_payload keys [false] f in
  let () = expect_string_failwith ~raise
    program "main" (e_pair test_params (init_storage 1 0 keys)) exp_failwith in
  ()
...
let valid_1_of_1 ~add_warning ~raise f () =
  ...
  let () = expect_eq_n_trace_aux ~raise [0;1;2] program "main"
      (fun n ->
        let params = params_w ~raise n empty_payload [keys] [true] f in
...
let valid_2_of_3 ~add_warning ~raise f () =
  ...
  let () = expect_eq_n_trace_aux ~raise [0;1;2] program "main"
      (fun n ->
        let params = params_w ~raise n empty_payload param_keys [true;true] f in
...
let invalid_3_of_3 ~add_warning ~raise f () =
  ...
  let test_params = params_w ~raise 0 empty_payload param_keys [false;true;true] f in
  let exp_failwith = "Invalid signature" in
  let () = expect_string_failwith ~raise
...
let not_enough_2_of_3 ~add_warning ~raise f () =
  ...
  let test_params = params_w ~raise 0 empty_payload (valid_keys) [true;true] f in
  let exp_failwith = "Not enough signatures passed the check" in
  let () = expect_string_failwith ~raise

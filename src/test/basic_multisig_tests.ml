...
let op_list ~raise =
  ...
  let source =
    Trace.trace_alpha_tzresult ~raise (fun _ ->
        Main_errors.test_internal __LOC__)
  ...
  let destination =
    ...
    @@ Signature.Public_key_hash.of_b58check
         "tz1PpDGHRXFQq3sYDuH8EpLWzPm5PFpe1sLE"
  in
  let operation
      : _
      Memory_proto_alpha.Protocol.Script_typed_ir.internal_operation_contents
    =
  ...
  let opbytes =
    let contents =
      Memory_proto_alpha.Protocol.Apply_internal_results
      .packed_internal_operation
        internal_operation
    in
    Data_encoding.Binary.to_bytes_exn
      Memory_proto_alpha.Protocol.Apply_internal_results
      .internal_operation_encoding
      contents
  in
  Ast_imperative.(
...
let params ~raise counter payload keys is_validl f =
  ...
  in
  let signed_msgs =
    List.fold ~f:aux ~init:[] (List.rev @@ List.zip_exn keys is_validl)
  in
  e_record_ez
    [ "counter", e_nat counter
    ; "payload", payload
    ; ( "signatures"
      , e_typed_list signed_msgs (t_pair (t_key_hash (), t_signature ())) )
    ]
...
let valid_2_of_3 ~raise f () =
  ...
  let () =
    expect_eq_n_trace_aux
      ...
      (fun n ->
        let params =
          params ~raise n empty_payload param_keys [ true; true ] f
        in
...
let invalid_3_of_3 ~raise f () =
  ...
  let test_params =
    params ~raise 0 empty_payload param_keys [ false; true; true ] f
  in
  let exp_failwith = "Invalid signature" in
  let () =
    expect_string_failwith

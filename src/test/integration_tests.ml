...
let bytes_arithmetic ~raise f : unit =
  let program = type_file ~raise f in
  let foo = e_bytes_string ~loc "0f00" in
  let foototo = e_bytes_string ~loc "0f007070" in
  let toto = e_bytes_string ~loc "7070" in
  let empty = e_bytes_string ~loc "" in
  let tata = e_bytes_string ~loc "ff7a7aff" in
  let at = e_bytes_string ~loc "7a7a" in
  let ba = e_bytes_string ~loc "ba" in
...
let balance_constant ~raise f : unit =
  let program = type_file ~raise f in
  let input = e_tuple ~loc @@ List.Ne.of_list [ e_unit ~loc; e_mutez ~loc 0 ] in

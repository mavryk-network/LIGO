...
let compile_groups ~raise filename grp_list =
  let aux
      :  (syntax * group_name * Environment.Protocols.t) * (lang * string)
      -> unit
    =
   fun ((syntax, grp, protocol_version), (lang, contents)) ->
    ...
    | Object ->
      ...
      in
      let mini_c =
        Ligo_compile.Of_aggregated.compile_expression
          ~raise
          aggregated_with_unit
      in
...
let get_all_md_files () =
  ...
  let ic =
    Ligo_unix.open_process_in "find ./gitlab-pages/docs -iname \"*.md\""
  in
    ...
    try
      while true do
        ...
        then (
          ...
          if not (SnippetsGroup.is_empty grp)
          then all_input := md_file :: !all_input)
      done
    with

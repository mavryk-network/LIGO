...
let execute_command command : int * string =
  ...
  let exit_code =
    ...
    | End_of_file ->
      ...
      (match status with
      | Ligo_unix.WEXITED code | Ligo_unix.WSIGNALED code | Ligo_unix.WSTOPPED code ->
        code)
  in
...
let get_groups md_file : snippetsmap =
    ...
    | Some "shell" ->
      let () =
        ...
        List.iter
          ~f:(fun arg ->
            match arg with
            | Md.Field "" | Md.Field "skip" | Md.Field "run" -> ()
  ...
  in
  let (_nb_shell_blocks : int), grp_map =
    List.fold_left ~f:aux ~init:(0, SnippetsGroup.empty) code_blocks
  in
  grp_map
...
let write_if_different (filename : string) ~(contents : string) : bool =
  let old_contents, same =
    ...
    with
    | _ ->
      "<error>", false (* if there is any error while reading, assume we need to write. *)
  in
  if same
  then false
  else (
   ...
   fun ((syntax, grp, protocol_version), (lang, contents)) ->
    ...
    then false
      ...
      let dirname =
        if String.equal
             ".md"
             (String.sub md_filename ~pos:(String.length md_filename - 3) ~len:3)
        then (
          let d, b =
            Filename.split
              (String.sub md_filename ~pos:0 ~len:(String.length md_filename - 3))
          in
          Filename.of_parts [ d; "src"; b ])
        else failwith "Unexpected Markdown filename: does not end with .md"
      in
      let extension =
        match syntax with
        | CameLIGO -> ".mligo"
  ...
  let aux
   ...
   fun ((syntax, grp, protocol_version), (lang, contents)) ->
    ...
    | Shell ->
      ...
      @@
      let running_in_CI =
        match Sys.getenv "GITLAB_CI" with
        | Some "true" -> true
        | _ -> false
      in
      if running_in_CI
      then (
        ...
        if exit_code = 0
        then Format.eprintf "The shell command exited successfully"
        else (
          let () =
            Format.eprintf
              "The shell command exited with code %d, output:\n%s\n"
              exit_code
              output
          in
        ...
        Format.eprintf
          "Would run shell command if env var GITLAB_CI was true: %s\n"
          contents
  in
...
let write_all_to_files ~raise md_filenames () =
  ...
  then
    failwith
      ("Some documentation snippets from the following .md were written to external \
        files; please simply re-run `dune runtest'.\n"
      ^ String.concat ~sep:"\n" updates)
  else ()

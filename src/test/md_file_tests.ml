...
let compile_groups ~raise filename grp_list =
  ...
  let aux : (syntax * group_name * Environment.Protocols.t) * (lang * string) -> unit =
    fun ((syntax , grp, protocol_version) , (lang , contents)) ->
      ...
      let raw_options : Compiler_options.raw = Compiler_options.default_raw_options in
      let warn_unused_rec = raw_options.warn_unused_rec && not (Syntax_types.equal meta.syntax Syntax_types.JsLIGO) in

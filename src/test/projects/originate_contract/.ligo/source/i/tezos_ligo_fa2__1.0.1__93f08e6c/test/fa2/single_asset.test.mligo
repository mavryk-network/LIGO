...
let test_atomic_tansfer_success =
  ...
  let (t_addr,_,_) = Test.originate FA2_single_asset.main initial_storage 0mav in
...
let test_atomic_transfer_failure_not_operator =
  ...
  let (t_addr,_,_) = Test.originate FA2_single_asset.main initial_storage 0mav in
  let contr = Test.to_contract t_addr in
  let result = Test.transfer_to_contract contr (Transfer transfer_requests) 0mav in
  match result with
...
let test_empty_transfer_and_balance_of =
  ...
  let owner3 = List_helper.nth_exn 2 owners in
  let op1    = List_helper.nth_exn 0 operators in
  let (callback_addr,_,_) = Test.originate Callback.main ([] : nat list) 0mav in

...
let test =
  ...
  let (addr, code, size) = Test.originate_from_file cut "main" ([] : string list) init_storage 0mav in
  ...
  let _ = Test.transfer_exn addr param 10mav in
  ...
  match (Test.transfer addr param 10mav : test_exec_result) with
...
let test2 =
  ...
  let overide_default_amounts = [ 8000mav ; 100mumav ] in // the [i]th element of the list overwrite default balance of the [i]th account
  ...
  let () = assert (Test.michelson_equal (Test.compile_value (Test.get_balance bsa2)) (tz 3800000000000n)) in
  let () = assert (Test.michelson_equal (Test.compile_value (Test.get_balance bsa3)) (tz 3800000000000n)) in
  ()

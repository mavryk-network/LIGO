let test1 =
  ...
  let ret = Test.run_exn (fun (x:r_comb) -> x.one ) v in
  assert (Test.michelson_equal d_one ret)

let test2 =
  let x1 = Test.compile_value (1,2) in
  let x2 = Test.run_exn (fun () -> (1,2)) () in
...
let test3 =
  let x1 =
    type r_comb = [@layout comb] { one : int ; two : nat ; three : string ; four : bytes ; five : unit } in
    Test.run_exn (fun () -> ({one = 1 ; two = 2n ; three = "a" ; four = 0xFF00 ; five = ()}:r_comb)) ()
  in
  let x2 =
    type r_tree = [@layout tree] { one : int ; two : nat ; three : string ; four : bytes ; five : unit } in
    Test.run_exn (fun () -> ({one = 1 ; two = 2n ; three = "a" ; four = 0xFF00 ; five = ()}:r_tree)) ()
  in

...
let test =
  ...
  in
  let orig =  Test.Next.Originate.contract (contract_of C) big_list 0mav in
  let _caching =
    (* some caching is happening on the first transaction *)
    Test.Next.Typed_address.transfer orig.taddr (Main true) 0mav
  in
  let tx1 = Test.Next.Typed_address.transfer orig.taddr (Main false) 0mav in
  let tx2 = Test.Next.Typed_address.transfer orig.taddr (Main true) 0mav in
  let tx3 = Test.Next.Typed_address.transfer orig.taddr (Main true) 0mav in

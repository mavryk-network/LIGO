...
let _tester (a : (Adder parameter_of, int) typed_address) (_ : (Adder parameter_of, int) michelson_contract) (_ : int) : unit =
  ...
  (* Test 1 *)
  let _ = Test.transfer_to_contract_exn c (Add 0) 0mav in
  let () = assert (Test.get_storage a = 0) in
  (* Test 2 *)
  let _ = Test.transfer_to_contract_exn c (Add 1) 0mav in 
  let () = assert (Test.get_storage a = 1) in
  ()

let test =
    Test.originate_and_mutate_all (contract_of Adder) 0 0mav _tester

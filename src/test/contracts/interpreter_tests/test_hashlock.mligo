...
let test_commit =
  let () = Test.reset_state_at (0 : timestamp) 10n ([] : mav list) in
  ...
  let {addr = typed_addr; code = _; size = _} = Test.originate_from_file "../hashlock.mligo" init_storage 0mav in
  let contr = Test.to_contract typed_addr in
  let parameter = Commit salted_hash in
  let () = Test.set_source first_committer in
  let lock_time = Mavryk.get_now () + 15 + 86_400 in
  let _ = Test.transfer_to_contract_exn contr parameter 0mav in
...
let test_reveal_no_commit =
  ...
  let () = Test.set_source first_committer in
  match Test.transfer_to_contract contr parameter 0mav with
...
let test_reveal_young_commit =
  let () = Test.reset_state_at (3_600 : timestamp) 10n ([] : mav list) in
  let first_committer = Test.nth_bootstrap_account 0 in
  let packed_sender = Bytes.pack first_committer in
  let hashable = [%bytes "hello world"] in
  let salted_hash = Crypto.sha256 (Bytes.concat hashable packed_sender) in
...
let test_reveal_breaks_commit =
  let () = Test.reset_state_at (86_400 : timestamp) 10n ([] : mav list) in
  let first_committer = Test.nth_bootstrap_account 0 in
  let packed_sender = Bytes.pack first_committer in
  let hashable = [%bytes "hello world"] in

...
let test_commit =
  let () = State.reset_at (0 : timestamp) 10n ([] : mav list) in
  ...
  let {taddr = typed_addr; code = _; size = _} = Originate.from_file "../hashlock.mligo" init_storage 0mav in
  ...
  let lock_time = Mavryk.get_now () + 10 + 86_400 in
  let _ = Contract.transfer_exn contr parameter 0mav in
...
let test_reveal_no_commit =
  ...
  let () = State.set_source first_committer in
  match Contract.transfer contr parameter 0mav with
...
let test_reveal_young_commit =
  let () = State.reset_at (3_600 : timestamp) 10n ([] : mav list) in
  let first_committer = Account.address 0 in
  let packed_sender = Bytes.pack first_committer in
  let hashable = [%bytes "hello world"] in
  let salted_hash = Crypto.sha256 (Bytes.concat hashable packed_sender) in
...
let test_reveal_breaks_commit =
  let () = State.reset_at (86_400 : timestamp) 10n ([] : mav list) in
  let first_committer = Account.address 0 in
  let packed_sender = Bytes.pack first_committer in
  let hashable = [%bytes "hello world"] in

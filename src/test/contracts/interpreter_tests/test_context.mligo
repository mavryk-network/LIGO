...
let test_contract =
  ...
  let _ = Test.transfer_exn orig.addr (Main 5) 123mav in
  let () = Test.save_context () in
  let _ = Test.transfer_exn orig.addr (Main 5) 123mav in
  let () = Test.log (Test.get_storage orig.addr) in
...
let test_move =
  ...
  let _ = Test.reset_state 4n ([] : mav list) in  
  let to_ = Test.nth_bootstrap_account 2 in
  let () = Test.log (Test.get_balance_of_address to_) in
  let () = Test.save_context () in
  let _ = Test.transfer_exn (Test.cast_address to_: implicit_address) () 100mav in
  let () = Test.log (Test.get_balance_of_address to_) in
  let () = Test.restore_context () in
  let () = Test.log (Test.get_balance_of_address to_) in
  ()

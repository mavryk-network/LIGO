...
let test =
  ...
  let _ = Test.Contract.transfer_exn c () 1000000mav in
  ...
  let orig = Test.Originate.contract (contract_of C) 41 5mav in

  let () = Test.IO.log "BALANCE AND VOTING POWER AFTER ORIGINATE" in
  let () = Test.IO.log(Test.Address.get_balance a) in
  let () = Test.IO.log(Test.State.get_voting_power pkh) in
  let _ = Test.Typed_address.transfer orig.taddr (Main 1) 3mav in

...
let test =
  ...
  let {taddr = typed_addr; code = _; size} = Test.Originate.contract (contract_of C) None 0mav in
  ...
  let _ = Test.Typed_address.transfer_exn typed_addr (Main Two) 10mav in
  ...
  match (Test.Typed_address.transfer typed_addr (Main One) 10mav : test_exec_result) with
...
let test2 =
  ...
  // some balance tests:
  let tz = fun (n:nat) ->
    Test.Michelson.run (fun (x : unit -> nat) -> x () * 1mumav) (fun (_ : unit) -> n)
  in
  let () = assert ((Test.Address.get_balance bsa0) = 2000mav) in
  let () = assert ((Test.Address.get_balance bsa1) = 0mumav) in
  let () = assert (Test.Compare.eq (Test.Michelson.eval (Test.Address.get_balance bsa1)) (tz 0n)) in
  let () = assert ((Test.Address.get_balance bsa2) = 3800000mav) in
  let () = assert ((Test.Address.get_balance bsa3) = 3800000000000mumav) in
  ()

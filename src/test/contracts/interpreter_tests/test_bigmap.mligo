...
let test =
  ...
  let _ = Test.Originate.contract (contract_of C) init 0mav in
  let init = Big_map.add 32 42n Big_map.empty in
  let orig = Test.Originate.contract (contract_of C) init 0mav in
  ...
  let _ = Test.Contract.transfer_exn ctr (Main (21, 42n)) 0mav in
  let _ = Test.Contract.transfer_exn ctr (Main (3, 42n)) 0mav in

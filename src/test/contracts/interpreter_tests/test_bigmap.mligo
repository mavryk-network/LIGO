...
let @test =
  ...
  let (_taddr, _, _) = Test.@originate main init 0tez in
  ...
  let _ = Test.transfer_to_contract_exn ctr (21, 42n) 0tez in
  let _ = Test.transfer_to_contract_exn ctr (3, 42n) 0tez in

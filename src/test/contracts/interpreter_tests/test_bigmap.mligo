...
let test =
  ...
  let _ = Test.originate (contract_of C) init 0mav in
  let init = Big_map.add 32 42n Big_map.empty in
  let orig = Test.originate (contract_of C) init 0mav in
  ...
  let _ = Test.transfer_to_contract_exn ctr (Main (21, 42n)) 0mav in
  let _ = Test.transfer_to_contract_exn ctr (Main (3, 42n)) 0mav in

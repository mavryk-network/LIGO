...
let test =
  let {taddr; code = _; size = _} = Test.Originate.contract (contract_of C) Big_map.empty 0mav in
  let y : nat = 1n in
  let _ = Test.Typed_address.transfer_exn taddr (Main (21, (fun (x : nat) -> x * 2n + y))) 0mav in
  let _y : nat = 100n in

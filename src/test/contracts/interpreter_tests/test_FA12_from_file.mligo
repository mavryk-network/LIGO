#import "../FA1.2.mligo" "FA12"

let originate storage balance =
  let storage = Test.eval storage in
  let addr, ctr, size = Test.originate_from_file "../FA1.2.mligo" "main" [] storage balance in
  ((Test.cast_address addr : (FA12.parameter, FA12.storage) typed_address), ctr, size)

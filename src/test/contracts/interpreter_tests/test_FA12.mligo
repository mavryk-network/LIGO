...
let test_transfer =
  let () = Test.State.reset 10n ([] : mav list) in
  ...
  let {taddr = typed_addr; code = _; size = _} = Test.Originate.contract (contract_of C) storage 0mav in
  ...
  let _ = Test.Contract.transfer_exn contr (Main parameter) 0mav in
...
let test_transfer_not_e_allowance =
  ...
  let parameter = Transfer { address_from = from_; address_to = to_; value = 10n } in
  let () = Test.State.set_source sender_ in
  match Test.Contract.transfer contr (Main parameter) 0mav with
...
let test_get_allowance =
  ...
  let from_ = Test.Account.address 1 in
  let to_ = Test.Account.address 2 in
  let {taddr = dummy_typed_addr; code = _; size = _} = Test.Originate.contract dummy_contract 0n 0mav in

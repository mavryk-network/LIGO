...
let test =
    ...
    let orig = Test.Originate.contract (contract_of C) initial_storage 0mav in
    let () = Test.State.set_source a1 in
    let _ = Test.Typed_address.transfer_exn orig.taddr (Main (a1, 1234n)) 1mumav in
    ...
    let _ = Test.Typed_address.transfer_exn orig.taddr (Main (a1, 4321n)) 1mumav in
    let () = assert (ns = Big_map.literal [((a1, 0n), 42n); ((a1, 1n), 1234n)]) in
    let ns = Test.Typed_address.get_storage orig.taddr in

...
let test_foo =
  let c : (unit, unit) michelson_contract =
    Test.Michelson.Contract.from_file "contract_under_test/compiled.mv" in
  let s = Test.Michelson.decompile (Test.Michelson.parse "Unit") in
  let a = Test.Originate.michelson c s 0mav in
  Test.IO.log a

let test_bar =
  let c : (unit, (int,string) map) michelson_contract =
    Test.Michelson.Contract.from_file
      "contract_under_test/other_compiled.mv" in

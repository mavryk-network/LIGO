...
let main (p,s: mystruct * storage ) : operation list * storage = [ Mavryk.emit "%foo" p; ], s

let test_foo =
  let (ta, _, _) = Test.originate main () 0mav in
  let _ = Test.transfer_to_contract_exn (Test.to_contract ta) {num1 = 1n ; num2 = 2n} 0mav in

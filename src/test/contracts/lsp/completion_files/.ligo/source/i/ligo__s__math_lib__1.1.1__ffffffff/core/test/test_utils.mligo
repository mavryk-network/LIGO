...
let test =
  let _test_is_implicit =
    ...
    let add_negative_tests_from_inputs
      (inputs : address list)
      (tests : (address * bool) list)
    : (address * bool) list =
      let fill_expectation_false ((acc, elt) : (address * bool) list * address)
      : (address * bool) list = (elt, false) :: acc in
      List.fold fill_expectation_false inputs tests in
    let add_positive_tests_from_inputs
      (inputs : address list)
      (tests : (address * bool) list)
    : (address * bool) list =
      let fill_expectation_true ((acc, elt) : (address * bool) list * address)
      : (address * bool) list = (elt, true) :: acc in
      List.fold fill_expectation_true inputs tests in
    let run_tests (all_tests : (address * bool) list) (f : (address) -> bool)
    : unit =
  ...
  let _test_bytes_is_adddress =
    ...
    let payload : bytes =
      0x050a000000160000430d6ec166d9c623104776aaad3bb50615c6791f in

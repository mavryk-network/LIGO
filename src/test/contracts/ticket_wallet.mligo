...
Modelled after:
  ...
  https://gitlab.com/mavos/mavos/-/blob/95a072715b/tests_python/contracts_alpha/mini_scenarios/ticket_wallet_fungible.mv
...
let main (p : parameter) (storage : storage) : operation list * storage =
  begin
    ...
    (match p with
       Receive ticket ->
         let ((ticketer, _), ticket) = Mavryk.Ticket.read ticket in
         ...
         let ticket =
           match old_ticket with
             None -> ticket
           | Some old_ticket ->
               (match Mavryk.Ticket.join (ticket, old_ticket) with
     ...
     | Send send ->
         begin
           Assert.assert (Mavryk.get_sender () = manager);
           let (ticket, tickets) =
           ...
           (match ticket with
            ...
            | Some ticket ->
                let ((_, (_, total_amt)), ticket) = Mavryk.Ticket.read ticket in
                ...
                (match Mavryk.Ticket.split ticket (send_amt, keep_amt) with
                 ...
                 | Some split_tickets ->
                     ...
                     let (_, tickets) =
                       Big_map.get_and_update
                         ...
                         tickets in
                     let op =
                       Mavryk.Operation.transaction send_ticket 0mumav send.destination in
                     ([op],
                      {
                       manager = manager;
                       tickets = tickets

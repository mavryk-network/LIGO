...
let buy (parameter, storage: (bytes * address option) * storage) =
  let void: unit = assert (Mavryk.get_amount () = storage.2.0) in
...
let update_details (parameter, storage: (id * bytes option * address option) * storage) =
  ...
  let is_allowed: bool =
    if
      ...
      | Some id_details -> (Mavryk.get_sender () = id_details.controller) || (Mavryk.get_sender () = id_details.owner)
...
let skip (p,s: unit * storage) =
  let void: unit = assert (Mavryk.get_amount () = storage.2.1) in
  let identities, last_id, prices = storage in

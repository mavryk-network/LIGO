...
function main (var (_, s) : parameter * storage) : return is
  block {
...
function set_ (var (n, m) : int * foo) : foo is block {
  m[23] := n
} with m

function add (var (n, m) : int * foo) : foo is set_ ((n,m))
...
function mutimaps (var (m, n) : foo * foo) : foo is block {
  var bar : foo := m;
  bar[42] := 0;

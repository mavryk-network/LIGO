      ...
      let parameter_ty, storage_ty =
        trace_option
          ...
          let* { type1 = input_ty; _ } =
            Ast_aggregated.get_t_arrow exp.type_expression
          in
...
let compile_expression ~raise ~options
    : Ast_typed.expression -> Ast_aggregated.expression
  =
 fun e ->
 ...
 fun c entry contract param ->
  let entry_point =
    trace_option
      ...
      (Ast_typed.get_entry contract entry)
  in
...
let apply_to_entrypoint_view ~raise ~options
 ...
 fun prg ->
  ...
  in
  let tuple_view =
    Ast_typed.ez_e_a_record ~layout:L_comb (List.mapi ~f:aux views_info)
  in
...
let list_declarations (only_ep : bool) (m : Ast_typed.program)
    : Value_var.t list
  =
  List.fold_left
    ~f:(fun prev el ->
      ...
      | D_value { binder; attr; expr } when attr.public && not attr.hidden ->
        ...
        then
          if is_some
               (Ast_typed.Misc.get_type_of_contract

...
let build_contract ~raise
    ...
    -> ?has_env_comments:bool -> ?disable_typecheck:bool
    -> ?constants:string list -> ?tezos_context:_
    -> Stacking.compiled_expression
    -> (Value_var.t * Stacking.compiled_expression) list
    -> _ Michelson.michelson
  =
 fun ~protocol_version
  ...
  then contract
  else if Environment.Protocols.equal
  ...
  then (
    ...
    let tezos_context =
      List.fold_left
        constants
        ~init:environment.tezos_context
        ~f:(fun ctxt cnt ->
          let ctxt, _, _ =
       ...
       fun c ->
        let map, _ =
          Trace.trace_tzresult_lwt
            ~raise
            (typecheck_contract_tracer protocol_version contract)
          @@ Proto_alpha_utils.Memory_proto_alpha.typecheck_map_contract
               ~environment
               c
        in
        map
        ...
        ~f:(fun ctxt cnt ->
          let ctxt, _, _ =
            Proto_pre_alpha_utils.Trace.trace_alpha_tzresult_lwt
              ~raise
              (typecheck_contract_tracer protocol_version contract)
            @@ Proto_pre_alpha_utils.Memory_proto_alpha.register_constant
                 ctxt
                 cnt
          in
          ctxt)

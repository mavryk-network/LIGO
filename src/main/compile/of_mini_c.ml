...
let dummy : Stacking.meta =
  { location = Location.dummy; env = []; binder = None }


let dummy_locations :
      'l 'p. ('l, 'p) Micheline.node -> (meta, 'p) Micheline.node
  =
...
let optimize_for_contract ~raise options e : type_expression * anon_function =
  let input_ty, _ =
    ...
    @@ Self_mini_c.get_t_function e.type_expression
  in
  ...
  let optimized =
    trace ~raise self_mini_c_tracer
    @@ Self_mini_c.contract_check ~options contract
  in
  input_ty, optimized
...
let compile_contract ~raise
 ...
 fun ~options e ->
  ...
  let de_bruijn =
    ...
    @@ Scoping.translate_closed_function
 ...
 fun ~options e ->
  ...
  let view =
    { view with
      body =
        trace ~raise self_mini_c_tracer
        @@ Self_mini_c.all_expression options view.body
    }
  in
  let protocol_version = options.backend.protocol_version in
  let de_bruijn =
  ...
  in
  let l, r =
    trace ~raise self_mini_c_tracer @@ Self_mini_c.get_t_pair input_ty
  in
  ...
  let expr_ty =
    Micheline.(Prim (dummy, "lambda", [ input_ty; output_ty ], []))
  in
  let expr_ty = dummy_locations expr_ty in
  ({ expr_ty; expr } : Stacking.Program.compiled_expression)
...
let compile_expression ~raise
 ...
 fun ~options e ->
  let e =
    trace ~raise self_mini_c_tracer @@ Self_mini_c.all_expression options e
  in
  let protocol_version = options.backend.protocol_version in
  let expr =
    trace ~raise scoping_tracer

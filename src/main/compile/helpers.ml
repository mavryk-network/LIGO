...
let parse_and_abstract_reasonligo ~raise buffer file_path =
  let raw =
    trace ~raise parser_tracer @@ Parsing.Reasonligo.parse_file buffer file_path
  in
  let imperative =
    trace ~raise cit_reasonligo_tracer @@ Tree_abstraction.Reasonligo.compile_program raw
  in
  imperative


let parse_and_abstract_expression_reasonligo ~raise buffer =
  let raw = trace ~raise parser_tracer @@ Parsing.Reasonligo.parse_expression buffer in
  let imperative =
...
let parse_and_unify_cameligo ~raise buffer file_path =
  let cst = trace ~raise parser_tracer @@ Parsing.Cameligo.parse_file buffer file_path in
  let unified =
    trace ~raise unification_tracer @@ Unification.Cameligo.compile_program cst
  in
  unified


let parse_and_unify_pascaligo ~raise buffer file_path =
  let cst = trace ~raise parser_tracer @@ Parsing.Pascaligo.parse_file buffer file_path in
  let unified =
    trace ~raise unification_tracer @@ Unification.Pascaligo.compile_program cst
  in
  unified


let parse_and_unify_jsligo ~raise buffer file_path =
  let cst = trace ~raise parser_tracer @@ Parsing.Jsligo.parse_file buffer file_path in
  let unified =
    trace ~raise unification_tracer @@ Unification.Jsligo.compile_program cst
  in
  unified


let parse_and_unify_reasonligo ~raise _buffer _file_path =
  trace ~raise unification_tracer
  @@ fun ~raise -> raise.error @@ [ Unification.Errors.unsupported_syntax "ReasonLIGO" ]


let parse_and_unify ~raise ~meta (c_unit : Buffer.t) (source_filename : string)
    : Ast_unified.program
  =
  let parse_and_unify =
    match meta.syntax with
...
let parse_and_abstract_expression ~raise ~(meta : meta) buffer =
  let parse_and_abstract =
    ...
    | ReasonLIGO -> parse_and_abstract_expression_reasonligo
...
let parse_and_abstract_string_reasonligo ~raise buffer =
  let raw = trace ~raise parser_tracer @@ Parsing.Reasonligo.parse_string buffer in
  let imperative =
    trace ~raise cit_reasonligo_tracer @@ Tree_abstraction.Reasonligo.compile_program raw
  in
  imperative
...
let parse_and_abstract_string ~raise (syntax : Syntax_types.t) buffer =
  let parse_and_abstract =
    ...
    | ReasonLIGO -> parse_and_abstract_string_reasonligo
...
let pretty_print_reasonligo_cst = Parsing.Reasonligo.pretty_print_cst
...
let pretty_print_reasonligo = Parsing.Reasonligo.pretty_print_file

...
let type_file ~raise ~(options : Compiler_options.t) f stx form
    : Ast_typed.program
  =
  let meta = Of_source.extract_meta stx in
  let c_unit, _ =
    Of_source.preprocess_file ~raise ~options:options.frontend ~meta f
  in
...
let compile_contract_input ~raise ~options parameter storage syntax init_prog =
  ...
  in
  let imperative =
    Of_c_unit.compile_contract_input ~raise ~meta parameter storage
  in

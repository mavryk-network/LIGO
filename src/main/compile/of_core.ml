...
let list_declarations (m : Ast_core.program) : Value_var.t list =
  List.fold_left
    ~f:(fun prev el ->
      ...
      | D_irrefutable_match _ | D_type _ | D_module _ | D_open _ | D_include _ -> prev)
    ~init:[]
    m
  ...
  List.fold_left
    ~f:(fun prev el ->
      ...
      | D_irrefutable_match { pattern; _ } ->
        ...
        vars @ prev
      | D_value _ | D_type _ | D_module _ | D_open _ | D_include _ -> prev)
    ~init:[]
    m
...
let list_type_declarations (m : Ast_core.program) : Type_var.t list =
  List.fold_left
    ~f:(fun prev el ->
      ...
      | D_value _ | D_irrefutable_match _ | D_module _ | D_type _ | D_open _ | D_include _
        -> prev)
    ~init:[]
    m
...
let list_mod_declarations (m : Ast_core.program) : Module_var.t list =
  List.fold_left
    ~f:(fun prev el ->
      ...
      | D_value _ | D_irrefutable_match _ | D_type _ | D_open _ | D_include _ -> prev)
    ~init:[]
    m

...
let rec error_ppformat
    :  display_format:string display_format -> Format.formatter -> Types.all
    -> unit
  =
 fun ~display_format f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Build_error_tracer err ->
      BuildSystem.Errors.error_ppformat ~display_format f err
    ...
    | `Test_expected_to_fail ->
      Format.fprintf f "test was expected to fail but did not"
    ...
    | `Main_invalid_syntax_name syntax ->
      Format.fprintf
        f
        "@[<hv>Invalid syntax option: '%s'. @.Use 'pascaligo', 'cameligo', \
         'reasonligo', or 'jsligo'. @]"
        syntax
    ...
    | `Main_invalid_extension extension ->
      Format.fprintf
        f
        "@[<hv>Invalid file extension '%s'. @.Use '.ligo' for PascaLIGO, \
         '.mligo' for CameLIGO, '.religo' for ReasonLIGO, '.jsligo' for \
    ...
    | `Main_could_not_serialize errs ->
      ...
      Format.fprintf
        ...
        (Memory_proto_alpha.Client.Michelson_v1_error_reporter.report_errors
           ...
           ?parsed:None)
        errs
    | `Check_typed_arguments_tracer
        (Simple_utils.Runned_result.Check_parameter, err) ->
      Format.fprintf
        f
        "@[<hv>Invalid command line argument. @.The provided parameter does \
         not have the correct type for the given entrypoint.@ %a@]"
        (error_ppformat ~display_format)
        err
    | `Check_typed_arguments_tracer
        (Simple_utils.Runned_result.Check_storage, err) ->
      Format.fprintf
        f
        "@[<hv>Invalid command line argument. @.The provided storage does not \
         have the correct type for the contract.@ %a@]"
        (error_ppformat ~display_format)
        err
    ...
    | `Main_entrypoint_not_a_function ->
      Format.fprintf
        f
        "@[<hv>Invalid command line argument. @.The provided entrypoint is not \
         a function.@]"
    | `Main_view_not_a_function var ->
      Format.fprintf
        f
        "@[<hv>Invalid command line argument. @.View \"%a\" is not a \
         function.@]"
    ...
    | `Main_invalid_balance a ->
      Format.fprintf
        f
        "@[<hv>Invalid command line option \"--balance\". @.The provided \
         balance \"%s\" is invalid. Use an integer instead. @]"
        a
    | `Main_invalid_amount a ->
      Format.fprintf
        f
        "@[<hv>Invalid command line option \"--amount\". @.The provided amount \
         \"%s\" is invalid. Use an integer instead. @]"
        a
    | `Main_invalid_source a ->
      Format.fprintf
        f
        "@[<hv>Invalid command line option \"--source\". @.The provided source \
         address \"%s\" is invalid. A valid Tezos address is a string prefixed \
         by either tz1, tz2, tz3 or KT1 and followed by a Base58 encoded hash \
         and terminated by a 4-byte checksum.@]"
        a
    | `Main_invalid_sender a ->
      Format.fprintf
        f
        "@[<hv>Invalid command line option \"--sender\". @.The provided sender \
         address \"%s\" is invalid. A valid Tezos address is a string prefixed \
         by either tz1, tz2, tz3 or KT1 and followed by a Base58 encoded hash \
         and terminated by a 4-byte checksum.@]"
        a
    | `Main_invalid_timestamp t ->
      Format.fprintf
        f
        "@[<hv>Invalid command line option \"--now\". @.The provided now \
         \"%s\" is invalid. It should use RFC3339 notation in a string, or the \
         number of seconds since Epoch.@]"
        t
    | `Main_cannot_open_global_constants s ->
      Format.fprintf
        f
        "@[<hv>Cannot open global constants file. @.Check that the provided \
         file \"%s\" exists.@]"
        s
    | `Main_cannot_parse_global_constants (fn, s) ->
      Format.fprintf
        f
        "@[<hv>Cannot parse global constants file: %s. @.Check that the \
         provided file consists of JSON list of strings (one string per \
         Michelson constant). @.JSON Error: %s@]"
        fn
        s
    ...
    | `Error_of_execution_tracer errs ->
      ...
      Format.fprintf
        ...
        errs
    | `Preproc_tracer e ->
      Preprocessing.Errors.error_ppformat ~display_format f e
    ...
    | `Cit_pascaligo_tracer e ->
      List.iter
        ~f:(Tree_abstraction.Pascaligo.Errors.error_ppformat ~display_format f)
        e
    | `Cit_cameligo_tracer e ->
      List.iter
        ~f:(Tree_abstraction.Cameligo.Errors.error_ppformat ~display_format f)
        e
    | `Cit_reasonligo_tracer e ->
      List.iter
        ~f:(Tree_abstraction.Reasonligo.Errors.error_ppformat ~display_format f)
        e
    | `Cit_jsligo_tracer e ->
      List.iter
        ~f:(Tree_abstraction.Jsligo.Errors.error_ppformat ~display_format f)
        e
    | `Self_ast_imperative_tracer e ->
      Self_ast_imperative.Errors.error_ppformat ~display_format f e
    | `Desugaring_tracer e ->
      Desugaring.Errors.error_ppformat ~display_format f e
    | `Checking_tracer e -> Checking.Errors.error_ppformat ~display_format f e
    | `Self_ast_typed_tracer e ->
      Self_ast_typed.Errors.error_ppformat ~display_format f e
    | `Aggregation_tracer e ->
      Aggregation.Errors.error_ppformat ~display_format f e
    | `Self_ast_aggregated_tracer e ->
      Self_ast_aggregated.Errors.error_ppformat ~display_format f e
    | `Self_mini_c_tracer e ->
      Self_mini_c.Errors.error_ppformat ~display_format f e
    ...
    | `Main_interpret_target_lang_failwith (loc, calltrace, v) ->
      ...
      then
        ...
        Format.fprintf
          f
          "@[<v 4>%a@.An uncaught error occured:@.Failwith: \
           %a@.Trace:@.%a@.%a@]"
          Snippet.pp
          loc
          Tezos_utils.Michelson.pp
    ...
    | `Main_interpret_boostrap_not_enough loc ->
      Format.fprintf
        f
        "@[<hv>%a@.We need at least two boostrap accounts for the default \
         source and baker@]"
        Snippet.pp
        loc
    ...
    | `Main_interpret_modules_not_supported loc ->
      Format.fprintf
        ...
        Snippet.pp
        loc
    | `Main_decompile_michelson e ->
      Stacking.Errors.error_ppformat ~display_format f e
    | `Main_decompile_mini_c e ->
      Spilling.Errors.error_ppformat ~display_format f e
    | `Main_decompile_aggregated e ->
      Aggregation.Errors.error_ppformat ~display_format f e
    | `Main_decompile_typed e ->
      Checking.Errors.error_ppformat ~display_format f e
    | `Main_view_rule_violated loc ->
      Format.fprintf
        f
        "@[<hv>%a@.View rule violated:\n\
        \      - Tezos.create_contract ; Tezos.set_delegate and \
         Tezos.transaction cannot be used because they are stateful (expect in \
         lambdas)\n\
        \      - Tezos.self can't be used because the entry-point does not \
         make sense in a view@.@]"
        Snippet.pp
        loc
    | `Repl_unexpected ->
      Format.fprintf f "unexpected error, missing expression?"
...
let rec error_json : Types.all -> Simple_utils.Error.t list =
 fun a ->
  ...
  | `Main_typecheck_contract_tracer (_p, _c, _) ->
    let content =
      make_content ~message:"Could not typecheck michelson code" ()
    in
    [ make ~stage:"michelson contract build" ~content ]
  | `Main_could_not_serialize _errs ->
    let content =
      make_content ~message:"Could not serialize michelson code" ()
    in
  ...
  | `Main_cannot_open_global_constants _ ->
    let content =
      make_content ~message:"cannot open global constants file" ()
    in
    [ make ~stage:"global constants parsing" ~content ]
  | `Main_cannot_parse_global_constants _ ->
    let content =
      make_content ~message:"cannot parse global constants file" ()
    in
    [ make ~stage:"global constants parsing" ~content ]
  ...
  | `Main_entrypoint_not_a_function ->
    let content =
      make_content ~message:"given entrypoint is not a function" ()
    in
  ...
  | `Cit_pascaligo_tracer e ->
    List.map ~f:Tree_abstraction.Pascaligo.Errors.error_json e
  | `Cit_cameligo_tracer e ->
    List.map ~f:Tree_abstraction.Cameligo.Errors.error_json e
  | `Cit_reasonligo_tracer e ->
    List.map ~f:Tree_abstraction.Reasonligo.Errors.error_json e
  | `Cit_jsligo_tracer e ->
    List.map ~f:Tree_abstraction.Jsligo.Errors.error_json e

...
let rec error_ppformat
 ...
 fun ~display_format ~no_colour f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Main_invalid_source a ->
      Format.fprintf
        ...
        "@[<hv>Invalid command line option \"--source\". @.The provided source address \
         \"%s\" is invalid. A valid Tezos address is a string prefixed by either mv1, \
    ...
    | `Resolve_config_type_uncaught (locs_and_types, pp_typ) ->
      let pp_elt f (loc, typ) =
        Format.fprintf f "Unexpected type: %a\n%a\n" pp_typ typ snippet_pp loc
      in
      Format.fprintf
        f

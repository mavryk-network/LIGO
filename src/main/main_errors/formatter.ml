  ...
  | Human_readable | Dev ->
    (match a with
     ...
     | `Check_typed_arguments_tracer (Simple_utils.Runned_result.Check_parameter, err) ->
       Format.fprintf
         f
         "@[<hv>Invalid command line argument. @.The provided parameter does not have \
          the correct type for the given entrypoint.@ %a@]"
         (error_ppformat ~display_format)
         err
     ...
     | `Main_invalid_amount a ->
       Format.fprintf
         f
         "@[<hv>Invalid command line option \"--amount\". @.The provided amount \"%s\" \
          is invalid. Use an integer instead. @]"
         a
     ...
     | `Main_required_output_file ->
       Format.fprintf
         f
         "@[<hv>Missing required --output[o] <filename>, which is required when \
          targeting wasm. @]"
     ...
     | `Main_view_rule_violated loc ->
       Format.fprintf
         ...
         \      - Tezos.create_contract ; Tezos.set_delegate and Tezos.transaction \
          cannot be used because they are stateful (expect in lambdas)\n\
         \      - Tezos.self can't be used because the entry-point does not make sense \
          in a view@.@]"
         Snippet.pp
         loc

...
let rec error_ppformat : display_format:string display_format ->
  ...
  | Human_readable | Dev -> (
    ...
    | `Main_unparse_tracer errs ->
      ...
      (Tezos_client_015_PtLimaPt.Michelson_v1_error_reporter.report_errors ~details:true ~show_source:true ?parsed:(None)) errs
    ...
    | `Main_view_not_a_function var -> Format.fprintf f "@[<hv>Invalid command line argument. @.View \"%a\" is not a function.@]" Ligo_prim.Value_var.pp var

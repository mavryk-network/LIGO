...
let get_storage ~raise ~loc ~calltrace ctxt addr =
  ...
  in
  let script_ty = Trace.trace_tzresult_lwt ~raise (throw_obj_exc loc calltrace) @@
  ...
  in
  let (_, storage_ty, _, _) = Trace.trace_alpha_tzresult ~raise (throw_obj_exc loc calltrace) @@
...
let get_delegate ~raise ~loc ~calltrace (ctxt :context) contract =
  Trace.trace_tzresult_lwt ~raise (throw_obj_exc loc calltrace) @@ Tezos_protocol_008_PtEdo2Zk.Protocol.Alpha_services.Contract.delegate_opt Tezos_alpha_test_helpers.Block.rpc_ctxt ctxt.threaded_context contract
...
let init_ctxt ~raise ?(loc=Location.generated) ?(calltrace=[]) ?(initial_balances=[]) ?(n=2) bootstrapped_contracts =
  ...
  match acclst with
  | baker::source::_ ->
    { threaded_context ; baker ; source ; bootstrapped = acclst ; last_originations = [] ; storage_tys ; parameter_tys ; alpha_context ; next_bootstrapped_contracts = [] ; storage_keys = [] }
  | _ ->

...
let set_big_map ~raise (ctxt : context) id version k_ty v_ty =
  ...
  let key_type = Proto_alpha_utils.Trace.trace_alpha_tzresult ~raise
                   ...
                   Tezos_protocol_011_PtHangzH.Protocol.Michelson_v1_primitives.prims_of_strings key_type in
  ...
  let value_type = Proto_alpha_utils.Trace.trace_alpha_tzresult ~raise
                     ...
                     Tezos_protocol_011_PtHangzH.Protocol.Michelson_v1_primitives.prims_of_strings value_type in
...
let get_contract_rejection_data :
  ...
  fun errs ->
    let open Tezos_protocol_011_PtHangzH.Protocol in
...
let script_of_compiled_code ~raise ~loc ~calltrace (contract : unit Tezos_utils.Michelson.michelson) (storage : unit Tezos_utils.Michelson.michelson) : Tezos_protocol_011_PtHangzH.Protocol.Alpha_context.Script.t  =
...
let script_repr_of_compiled_code ~raise ~loc ~calltrace (contract : unit Tezos_utils.Michelson.michelson) (storage : unit Tezos_utils.Michelson.michelson) : Tezos_protocol_011_PtHangzH.Protocol.Script_repr.t  =
  let contract = ligo_to_canonical ~raise ~loc ~calltrace contract in
  let storage = ligo_to_canonical ~raise ~loc ~calltrace storage in
...
let extract_lazy_storage_diff_from_result :
  type a .
    a Tezos_raw_protocol_011_PtHangzH.Apply_results.contents_result ->
    Tezos_raw_protocol_011_PtHangzH.Alpha_context.Lazy_storage.diffs option list =
  fun x ->
...
let convert_lazy_storage_diffs (lazy_storage_diffs : Tezos_raw_protocol_011_PtHangzH.Alpha_context.Lazy_storage.diffs) =
...
let upd_context_of_receipts ~raise (ctxt : context) (op : Tezos_raw_protocol.Apply_results.packed_operation_metadata) =
  ...
  let bigmaps = let get_id id = Z.to_int (Tezos_raw_protocol_011_PtHangzH.Lazy_storage_kind.Big_map.Id.unparse_to_z id) in
...
let bake_op ~raise ~loc ~calltrace (ctxt:context) operation =
  ...
  | Ok incr ->
    let last_op = get_last_operation_result incr in
...
let transfer ~raise ~loc ~calltrace (ctxt:context) ?entrypoint dst parameter amt : add_operation_outcome =
  ...
  let operation : Tezos_raw_protocol_011_PtHangzH__Alpha_context.packed_operation = Trace.trace_tzresult_lwt ~raise (throw_obj_exc loc calltrace) @@
    (* TODO: fee? *)
...
let init_ctxt ~raise ?(loc=Location.generated) ?(calltrace=[]) ?(initial_balances=[]) ?(n=2) bootstrapped_contracts =
  ...
  *)
  ...
  let alpha_context,_,_ = alpha_context_of_block ~raise ~loc ~calltrace threaded_context in
  match acclst with
  | baker::source::_ ->
    {
      threaded_context ; baker ; source ; bootstrapped = acclst ; last_originations = [] ;
      storage_tys ; parameter_tys ; alpha_context ; next_bootstrapped_contracts = [] ; bigmaps = []
    }
  | _ ->

...
type state =
  { mavryk_context : Mavryk_state.context
...
let make_state ~raise ~(options : Compiler_options.t) =
  ...
  let%map mavryk_context = Mavryk_state.init_ctxt ~raise [] in
  let mod_res = Option.bind ~f:ModRes.make options.frontend.project_root in
  { mavryk_context; mod_res; heap = Heap.empty; print_values = true }
...
module Command = struct
  type 'a mavryk_command =
    ...
    | Nth_bootstrap_contract :
        int
        -> Mavryk_protocol.Protocol.Alpha_context.Contract.t mavryk_command
    ...
    | Reset_state :
        ...
        -> unit mavryk_command
    | Get_state : unit -> Mavryk_state.context mavryk_command
    | External_call :
        Location.t
        ...
        * Z.t
        -> [ `Exec_failed of Mavryk_state.state_error | `Exec_ok of Z.t ] mavryk_command
    | State_error_to_value : Mavryk_state.state_error -> LT.value mavryk_command
    | Get_storage :
        Location.t * Ligo_interpreter.Types.calltrace * LT.value
        -> LT.value mavryk_command
    | Get_size : LT.value -> LT.value mavryk_command
    | Get_balance :
        Location.t * Ligo_interpreter.Types.calltrace * LT.value
        -> LT.value mavryk_command
    | Get_last_originations : unit -> LT.value mavryk_command
    | Get_last_events : string * LT.type_expression -> LT.value mavryk_command
    | Compile_contract_from_file : string * Z.t option -> LT.value mavryk_command
    | Read_contract_from_file :
        Location.t * LT.calltrace * string
        -> LT.value mavryk_command
    | Run : Location.t * LT.func_val * LT.value -> LT.value mavryk_command
    | Eval :
    ...
    | Run_Michelson :
        ...
        * (LT.value * Ast_aggregated.type_expression) list
        -> LT.value mavryk_command
    | Compile_contract : Location.t * LT.value * LT.value -> LT.value mavryk_command
    | Compile_ast_contract : Location.t * LT.value -> LT.value mavryk_command
    | Decompile :
    ...
    | Inject_script :
        ...
        -> LT.value mavryk_command
    | Set_source : LT.value -> unit mavryk_command
    | Set_baker : Location.t * LT.calltrace * LT.value -> unit mavryk_command
    | Get_voting_power :
        Location.t
        * Ligo_interpreter.Types.calltrace
    ...
    | Get_total_voting_power :
        ...
        -> LT.value mavryk_command
    | Get_bootstrap : Location.t * LT.calltrace * LT.value -> LT.value mavryk_command
    | Sign : Location.t * LT.calltrace * string * bytes -> LT.value mavryk_command
    | Add_cast :
    ...
    | Add_account :
        ...
        * Mavryk_protocol.Protocol.Alpha_context.public_key
        -> unit mavryk_command
    | New_account : unit -> LT.value mavryk_command
    | Baker_account : LT.value * LT.value -> unit mavryk_command
    | Register_delegate :
        Location.t
        * Ligo_interpreter.Types.calltrace
        * Mavryk_protocol.Protocol.Alpha_context.public_key_hash
    ...
    | Register_constant :
        ...
        -> string mavryk_command
    | Constant_to_Michelson :
        Location.t * Ligo_interpreter.Types.calltrace * string
        -> LT.mcode mavryk_command
    | Register_file_constants :
        Location.t * Ligo_interpreter.Types.calltrace * string
        -> LT.value mavryk_command
    | Push_context : unit -> unit mavryk_command
    | Pop_context : unit -> unit mavryk_command
    | Drop_context : unit -> unit mavryk_command

  type 'a t =
    | Mavryk : 'a mavryk_command -> 'a t
  ...
  let eval_tezos
      : type a.
        ...
        -> a mavryk_command
        -> Mavryk_state.context
        -> execution_trace ref option
        -> (a * Mavryk_state.context) Lwt.t
    =
   fun ~raise ~options command ctxt _log ->
    ...
    | Set_big_map (id, kv, bigmap_ty) ->
      ...
      let ctxt = Mavryk_state.set_big_map ~raise ctxt (Z.to_int id) kv k_ty v_ty in
      Lwt.return ((), ctxt)
    | Nth_bootstrap_contract n ->
      let contract = Mavryk_state.get_bootstrapped_contract ~raise n in
    ...
    | Nth_bootstrap_typed_address (loc, n) ->
      ...
      let storage_ty =
        ...
        @@ List.Assoc.find
             ~equal:Mavryk_state.equal_account
    ...
    | Reset_state (loc, initial_timestamp, calltrace, n, amts) ->
      ...
      let amts =
        List.map
          ~f:(fun x ->
            let x = Trace.trace_option ~raise (corner_case ()) @@ LC.get_mumav x in
            Z.to_int64 x)
          amts
      in
    ...
    | External_call (loc, calltrace, { address; entrypoint }, param, amt) ->
      let%map x =
        Mavryk_state.transfer ~raise ~loc ~calltrace ctxt address ?entrypoint param amt
      in
      (match x with
    ...
    | State_error_to_value errs ->
      ...
      | Ecoproto_error (Script_interpreter.Runtime_contract_error contract_failing)
        ...
        let contract_failing =
          LT.V_Ct (C_address (Mavryk_state.contract_of_hash ~raise contract_failing))
        in
        (match rest with
        | Ecoproto_error (Script_interpreter.Reject (_, x, _)) :: _ ->
          let code = Mavryk_state.canonical_to_ligo x in
    ...
    | Get_balance (loc, calltrace, addr) ->
      ...
      let%map balance = Mavryk_state.get_balance ~raise ~loc ~calltrace ctxt addr in
    ...
    | Get_storage (loc, calltrace, addr) ->
      ...
      in
      let%map storage', ty = Mavryk_state.get_storage ~raise ~loc ~calltrace ctxt addr in
      let storage =
        storage'
      ...
      let ret =
        match
          List.Assoc.find ~equal:Mavryk_state.equal_account ctxt.internals.storage_tys addr
        with
        | Some ast_ty ->
          LT.V_Michelson
    ...
    | Get_size contract_code ->
      ...
      | LT.V_Michelson_contract contract_code ->
        let%map s = Ligo_compile.Of_michelson.measure ~raise contract_code in
    ...
    | Run (loc, f, v) ->
      ...
      let%bind { micheline_repr = { code = arg_code; _ }; _ } =
      ...
      ret, ctxt
    | Eval (loc, v, expr_ty) ->
      let%map value = Michelson_backend.compile_value ~raise ~options ~loc v expr_ty in
    ...
    | Compile_contract (loc, v, vs) ->
      ...
      in
      Lwt.return (LT.V_Ast_contract { main; views }, ctxt)
    | Compile_ast_contract (loc, v) ->
      let%map contract =
        match v with
        | LT.V_Ast_contract { main = ast_aggregated; views = `None } ->
          let%bind mavryk_context = Mavryk_state.get_alpha_context ~raise ctxt in
    ...
    | Inject_script (loc, calltrace, code, storage, amt) ->
      Mavryk_state.originate_contract ~raise ~loc ~calltrace ctxt (code, storage) amt
    | Set_source source ->
    ...
    | Set_baker (loc, calltrace, baker_policy) ->
      ...
      in
      let baker_policy = Mavryk_state.baker_policy ~raise ~loc ~calltrace baker_policy in
      Lwt.return ((), { ctxt with internals = { ctxt.internals with baker_policy } })
    | Get_voting_power (loc, calltrace, key_hash) ->
      let%map vp = Mavryk_state.get_voting_power ~raise ~loc ~calltrace ctxt key_hash in
      LT.V_Ct (LT.C_nat (Z.of_int64 vp)), ctxt
    | Get_total_voting_power (loc, calltrace) ->
      let%map tvp = Mavryk_state.get_total_voting_power ~raise ~loc ~calltrace ctxt in
        ...
        let%map sk, pk =
          let pkh =
            Mavryk_state.implicit_account
              ...
              x
          in
          Mavryk_state.get_account ~raise ~loc ~calltrace pkh
        in
        let record =
    ...
    | Sign (loc, calltrace, sk, data) ->
      let signature = Mavryk_state.sign_message ~raise ~loc ~calltrace data sk in
    ...
    | Add_cast (loc, addr, ty) ->
      ...
      in
      let storage_tys =
        List.Assoc.add ~equal:Mavryk_state.equal_account ctxt.internals.storage_tys addr ty
      in
      let internals = { ctxt.internals with storage_tys } in
      Lwt.return ((), { ctxt with internals })
    ...
    | Add_account (loc, calltrace, sk, pk) ->
      ...
      Mavryk_state.add_account ~raise ~loc ~calltrace sk pk pkh;
      Lwt.return ((), ctxt)
    | New_account () ->
      let sk, pk = Mavryk_state.new_account () in
      let value = LC.v_pair (V_Ct (C_string sk), V_Ct (C_key pk)) in
      Lwt.return (value, ctxt)
    | Baker_account (acc, opt) ->
      let mav = Trace.trace_option ~raise (corner_case ()) @@ LC.get_option opt in
      let mav =
        Option.map
          ~f:(fun v -> Trace.trace_option ~raise (corner_case ()) @@ LC.get_mumav v)
          mav
      in
    ...
    | Register_delegate (loc, calltrace, pkh) ->
      let%map ctxt = Mavryk_state.register_delegate ~raise ~loc ~calltrace ctxt pkh in
      let value = LC.v_unit () in
      value, ctxt
    | Stake (loc, calltrace, pkh, amt) ->
      let%map ctxt = Mavryk_state.stake ~raise ~loc ~calltrace ctxt pkh amt in
      let value = LC.v_unit () in
      value, ctxt
    | Bake_until_n_cycle_end (loc, calltrace, n) ->
      let%map ctxt =
        Mavryk_state.bake_until_n_cycle_end ~raise ~loc ~calltrace ctxt (Z.to_int n)
      in
      let value = LC.v_unit () in
      value, ctxt
    | Register_constant (loc, calltrace, code) ->
      let%map hash, ctxt =
        Mavryk_state.register_constant
          ~raise
          ~loc
          ~calltrace
    ...
    | Constant_to_Michelson (loc, calltrace, code) ->
      let code = Mavryk_state.parse_constant ~raise ~loc ~calltrace code in
      Lwt.return (code, ctxt)
    | Register_file_constants (loc, calltrace, fn) ->
      let%map hashes, ctxt =
        Mavryk_state.register_file_constants
          ~raise
          ~loc
          ~calltrace
          ~source:ctxt.internals.source
      ...
      hashes, ctxt
    | Push_context () ->
      Mavryk_state.contexts := ctxt :: !Mavryk_state.contexts;
      Lwt.return ((), ctxt)
    | Pop_context () ->
      Lwt.return
      @@
      (match !Mavryk_state.contexts with
      | [] -> (), ctxt
      | ctxt :: ctxts ->
        Mavryk_state.contexts := ctxts;
        (), ctxt)
    | Drop_context () ->
      Lwt.return
      @@
  ...
  let eval
   ...
   fun ~raise ~options command state log ->
    ...
    match command with
    | Mavryk mavryk_cmd ->
      let%map ret, ctxt = eval_tezos ~raise ~options mavryk_cmd state.mavryk_context log in
      ret, { state with mavryk_context = ctxt }
    | Check_obj_ligo e ->
      let _ =
        Trace.trace ~raise Main_errors.self_ast_aggregated_tracer
...
let rec eval
 ...
 fun ~raise ~options e state log ->
  ...
  match e with
  | Bind (e', f) ->
    let%bind v, state = eval ~raise ~options e' state log in

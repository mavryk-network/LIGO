...
type bootstrap_contract =
  int
  * unit Mavryk_utils.Michelson.michelson
  * unit Mavryk_utils.Michelson.michelson
...
type mavryk_op = Mavryk_raw_protocol.Alpha_context.packed_operation
...
(*
  ...
  - Raw: Mavryk state as represented in the mavryk code-base (same types)
...
type ligo_repr = unit Mavryk_utils.Michelson.michelson
...
let script_of_compiled_code
    ...
    ~loc
    ~calltrace
    (contract : unit Mavryk_utils.Michelson.michelson)
    (storage : unit Mavryk_utils.Michelson.michelson)
...
let bake_ops
    ...
    -> (Mavryk_alpha_test_helpers.Incremental.t -> mavryk_op) list
...
let bake_op
    :  raise:r -> loc:Location.t -> calltrace:calltrace -> context -> mavryk_op
    -> add_operation_outcome Lwt.t
  =
...
let init
  ...
  let accounts =
    List.map2_exn accounts initial_balances ~f:(fun account balance ->
        let balance = Option.map ~f:Mav.of_mumav_exn balance in
  ...
  let baker_accounts =
    List.map baker_accounts ~f:(fun (sk, pk, amt) ->
        ...
        let balance =
          match amt with
          | None -> Mav.of_mumav_exn 4_000_000_000_000L
          | Some v -> Mav.of_mumav_exn v
        in
        let account = Account.{ sk; pk; pkh } in

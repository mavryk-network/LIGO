...
let rec apply_operator ~raise ~steps ~(options : Compiler_options.t)
 ...
 fun loc calltrace expr_ty env c operands ->
  ...
  | C_TEST_WRAP_OP_TRANSFER, [ V_Ct (C_contract contract); V_Michelson (Ty_code { micheline_repr = { code = param ; _ } ; _ } | Untyped_code param); V_Ct (C_mutez amount); V_Ct (C_address source) ] ->

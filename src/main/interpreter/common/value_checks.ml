...
type protocolInfo = { sha256hash : string option; uri : Uri.t; scheme : string }
...
let uri_check (b : bytes) =
  ...
  if (List.mem ~equal:String.equal valid_protocols scheme) then
    return { sha256hash = None; uri; scheme }
...
let tzip16_check ~(raise : (Main_errors.all, Main_warnings.all) Simple_utils.Trace.raise) ~loc (v : value) =
  ...
  let* _, root = of_option ~error:(`Metadata_no_empty_key loc) @@
    List.find ~f:(fun (k, _) -> String.equal k "") values in
  let* { uri ; scheme; _ } = of_option ~error:(`Metadata_not_valid_URI (loc, Bytes.to_string root)) @@
    uri_check root in
  match scheme with
  | "tezos-storage" -> (
      ...
      | None ->
        ...
        let* () = of_option ~error:(`Metadata_slash_not_valid_URI (loc, location)) @@
          Simple_utils.Option.some_if (not @@ String.contains location '/') () in
        let location = Uri.pct_decode location in
        let* _, json = of_option ~error:(`Metadata_tezos_storage_not_found (loc, location)) @@
          List.find ~f:(fun (k, _) -> String.equal k location) values in

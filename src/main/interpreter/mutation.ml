...
let get_syntax ~raise ~deprecated syntax loc =
  match syntax with
  | Some syntax -> syntax
  | None ->
    ...
    | Some r ->
      ...
      let syntax =
        Simple_utils.Trace.to_stdlib_result
          (Syntax.of_string_opt ~deprecated (Syntax_types.Syntax_name "auto") (Some file))
      in
      (match syntax with
      | Ok (r, _) -> r
...
let mutate_some_contract
 ...
 fun ~raise ?syntax z main ->
  ...
  let f (e, (l, m)) =
    let syntax = get_syntax ~raise ~deprecated:true syntax l in
...
let mutate_some_value
 ...
 fun ~raise ?syntax loc z v v_type ->
  ...
  let module Fuzzer = Fuzz.Ast_aggregated.Mutator in
  let f (e, (loc, m)) =
    let syntax = get_syntax ~raise ~deprecated:true syntax loc in
    let s = Fuzz.Ast_aggregated.expression_to_string ~syntax m in
    e, (loc, m, s)
  in
...
let rec value_gen
 ...
 fun ~raise ?(small = true) ?known_addresses type_expr ->
  ...
  then
    QCheck.Gen.(
      (if small then map Bytes.of_string small_string else map Bytes.of_string string)
  ...
  then
    QCheck.Gen.(
      (if small then small_int else int) >>= fun n -> return (v_int (Z.of_int n)))
  ...
  then (
    ...
    | Some type_value_r ->
      QCheck.Gen.(
        (if small then small_list else list) (value_gen ~raise ~small type_value_r)
  ...
  then (
    ...
    | Some rows ->
      ...
      let rec gen l : (string * LT.value) list QCheck.Gen.t =
        ...
        | (label, expr) :: tl ->
          QCheck.Gen.(
            expr >>= fun row_el -> gen tl >>= fun r -> return ((label, row_el) :: r))
      in
  ...
  then (
    ...
    | Some (type_value_k, type_value_v) ->
      QCheck.Gen.(
        ...
        >>= fun l -> return (v_map l))
    | None ->
      raise.error (Errors.generic_error type_expr.location "Expected big_map type"))
  else
    raise.error

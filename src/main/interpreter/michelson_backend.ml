...
let compile_contract_w ~raise source_file entry_point =
...
let compile_expression_w ~raise ~loc ~calltrace syntax exp_as_string source_file subst_lst =
  let open Ligo_compile in
  let options = Compiler_options.make () in
  let (decl_list,env) = match source_file with
    | Some init_file -> Build.build_mini_c_w ~raise ~options syntax Env init_file

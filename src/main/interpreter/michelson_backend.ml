...
let make_function in_ty out_ty arg_binder body subst_lst =
  ...
  Ast_aggregated.e_a_lambda {result=typed_exp'; output_type = out_ty ; binder=Ligo_prim.Param.make arg_binder in_ty} in_ty out_ty
...
and compile_non_func_value ~raise ~options ~run_options ~loc : Ligo_interpreter.Types.value ->
  ...
  | V_Construct (ctor, arg) when Option.is_some (get_t_option ty) ->
    ...
    | "Some" ->
      ...
      let arg = compile_non_func_value ~raise ~options ~run_options ~loc arg option_ty in
  ...
  | V_Construct (ctor, arg) when is_t_sum ty ->
    ...
    let arg = compile_non_func_value ~raise ~options ~run_options ~loc arg ty' in
  ...
  | V_Record map when is_t_record ty ->
     ...
     Layout.record_to_pairs ~raise (fun (t, v) -> compile_non_func_value ~raise ~options ~run_options ~loc v t) (fun e1 e2 -> Tezos_micheline.Micheline.Prim ((), "Pair", [e1 ; e2], [])) (fun es -> Tezos_micheline.Micheline.Prim ((), "Pair", es, [])) map_ty map_kv
  | V_List lst ->
    ...
    let lst = List.map ~f:(fun v -> compile_non_func_value ~raise ~options ~run_options ~loc v lst_ty) lst in
  ...
  | V_Map map when is_t_map ty ->
    ...
    let map = List.map ~f:(fun (k, v) ->
        let k = compile_non_func_value ~raise ~options ~run_options ~loc k k_ty in
        let v = compile_non_func_value ~raise ~options ~run_options ~loc v v_ty in
        (k, v)) map in
...
let run_michelson_func
  ...
  let { code = arg; code_ty = arg_ty; _ } =
    compile_simple_value ~raise ~options ~run_options ~loc arg arg_ty
  in

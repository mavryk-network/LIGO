...
let rec apply_comparison ~raise
    :  Location.t -> calltrace -> Ast_aggregated.type_expression -> value
    -> value -> int
  =
 fun loc calltrace type_ operand operand' ->
  ...
  | V_List xs, V_List ys ->
    let type_ =
      trace_option
        ~raise
        (Errors.generic_error ~calltrace loc "Expected list type")
  ...
  | V_Set s, V_Set s' ->
    let type_ =
      trace_option
        ~raise
        (Errors.generic_error ~calltrace loc "Expected set type")
      @@ AST.get_t_set type_
    in
    List.compare
      (apply_comparison ~raise loc calltrace type_)
      (List.dedup_and_sort
         ~compare:(apply_comparison ~raise loc calltrace type_)
         s)
      (List.dedup_and_sort
         ~compare:(apply_comparison ~raise loc calltrace type_)
         s')
  ...
  | V_Record r, V_Record r' ->
    ...
    let rec aux (row_kv : (Label.t * row_element) list) =
      ...
      | (label, ({ associated_type; _ } : row_element)) :: row_kv ->
        ...
        (match
           apply_comparison ~raise loc calltrace associated_type value_a value_b
         with
        | 0 -> aux row_kv
        | c -> c)
    in
  ...
  | V_Construct (ctor_a, args_a), V_Construct (ctor_b, args_b) ->
    ...
    in
    let order =
      AST.Helpers.kv_list_of_t_sum ~layout fields |> List.map ~f:fst
    in
    let ith_a, _ =
...
let rec apply_operator ~raise ~steps ~(options : Compiler_options.t)
    :  Location.t -> calltrace -> AST.type_expression -> env
    -> Constant.constant' -> (value * AST.type_expression * Location.t) list
    -> value Monad.t
  =

...
and try_eval ~raise ~add_warning ~steps ~options expr env state r =
  Monad.eval ~raise ~add_warning ~options (eval_ligo ~raise ~add_warning ~steps ~options expr [] env) state r
...
let eval ~raise ~add_warning ~steps ~options expr =
  let initial_state = Execution_monad.make_state ~raise ~options in
  let res,_ctxt = Monad.eval ~raise ~add_warning ~options (eval_ligo ~raise ~add_warning ~steps ~options expr [] Env.empty_env) initial_state None in
  res

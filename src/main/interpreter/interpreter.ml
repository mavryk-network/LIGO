...
module Michelson_backend = Michelson_backend
module Michelson_to_value = Michelson_to_value

module type MutationType = sig
  open Simple_utils.Trace
  open Errors
  ...
  val mutate_some_contract
    :  raise:(interpreter_error, _) raise
  ...
  val save
    :  dir:string
    -> mutation:Location.t * expression * string
    -> string Execution_monad.t
end

module Make (Mutation : MutationType) = struct
...
pattern_env_extend_ [locs,env] [pattern] [ty] [value]
  ...
  the execution monad
*)
  let rec pattern_env_extend_
      ~(no_colour : bool)
      ~(attributes : ValueAttr.t)
      ~(mut : bool)
  ...
  let rec apply_comparison ~no_colour ~raise
   ...
   fun loc calltrace type_ operand operand' ->
    ...
    | V_Construct (ctor_a, args_a), V_Construct (ctor_b, args_b) ->
      ...
      let ith_a, _ =
        List.findi_exn order ~f:(fun _i (Label l) -> String.equal l ctor_a)
      in
      let ith_b, _ =
        List.findi_exn order ~f:(fun _i (Label l) -> String.equal l ctor_b)
      in
  ...
  let apply_comparison ~no_colour ~raise
   ...
   fun loc calltrace type_ c operand operand' ->
    let open Monad in
    let cmpres =
      apply_comparison ~no_colour ~raise loc calltrace type_ operand operand'
    in
...
let deref_env env =
  ...
  let rec apply_operator ~raise ~steps ~(options : Compiler_options.t)
   ...
   fun loc calltrace expr_ty env c operands ->
    ...
    | C_MAP_UPDATE, [ k; option; V_Map kvs ] ->
      ...
      | Some None ->
        ...
        @@ V_Map
             (List.dedup_and_sort ~compare
             @@ List.Assoc.remove ~equal:LC.equal_value kvs k)
      | _ -> assert false)
    ...
    *)
    ...
    | C_TEST_SAVE_MUTATION, [ V_Ct (C_string dir); V_Mutation ((_, _, _) as mutation) ] ->
      (try
         let* file_path = Mutation.save ~dir ~mutation in
    ...
    | ( C_TEST_DECOMPILE
      ...
      let () =
        trace_option
          ...
          @@ Format.asprintf
               "This Michelson value has assigned type '%a', which does not coincide \
    ...
    | C_TEST_DECOMPILE, [ V_Michelson (Untyped_code code) ] ->
      ...
      let code_ty =
        Tezos_micheline.Micheline.map_node (fun _ -> ()) (fun s -> s) code_ty
      in
      let>> v = Decompile (code, code_ty, expr_ty) in
      return v

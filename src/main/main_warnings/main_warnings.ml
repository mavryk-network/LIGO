...
let warn_bad_self_type t1 t2 loc =
  `Self_ast_aggregated_warning_bad_self_type (t1, t2, loc)
 ...
 fun ~display_format f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Use_meta_ligo loc ->
      Format.fprintf
        f
        "@[<hv>%a@ You are using Michelson failwith primitive (loaded from \
         standard library).@.Consider using `Test.failwith` for throwing a \
    ...
    | `Michelson_typecheck_failed_with_different_protocol (user_proto, errs) ->
      ...
      Format.fprintf
        f
        "@[<hv>Warning: Error(s) occurred while type checking the produced \
         michelson contract:@.%a@.Note: You compiled your contract with \
         protocol %s although we internally use protocol %s to typecheck the \
         produced Michelson contract@.so you might want to ignore this error \
         if related to a breaking change in protocol %s@.@]"
    ...
    | `Checking_ambiguous_constructor (loc, tv_chosen, tv_possible) ->
      Format.fprintf
        f
        "@[<hv>%a@ Warning: The type of this value is ambiguous: Inferred type \
    ...
    | `Main_view_ignored loc ->
      Format.fprintf
        f
        "@[<hv>%a@ Warning: This view will be ignored, command line option \
         override [@ view] annotation@.@]"
        Snippet.pp
        loc
    | `Self_ast_typed_warning_unused (loc, s) ->
      Format.fprintf
        f
        "@[<hv>%a:@.Warning: unused variable \"%s\".@.Hint: replace it by \
         \"_%s\" to prevent this warning.\n\
         @]"
        Snippet.pp
        loc
        s
    ...
    | `Self_ast_typed_warning_unused_rec (loc, s) ->
      Format.fprintf
        f
        "@[<hv>%a:@.Warning: unused recursion .@.Hint: remove recursion from \
         the function \"%s\" to prevent this warning.\n\
         @]"
        Snippet.pp
        loc
        s
    | `Self_ast_imperative_warning_layout (loc, Label s) ->
      Format.fprintf
        f
        "@[<hv>%a@ Warning: layout attribute only applying to %s, probably \
         ignored.@.@]"
        Snippet.pp
        loc
        s
    | `Self_ast_imperative_warning_deprecated_polymorphic_variable (loc, name)
      ...
      Format.fprintf
        f
        "@[<hv>%a@ Warning: %a is not recognize as a polymorphic variable \
         anymore. If you want to make a polymorphic function, please consult \
    ...
    | `Self_ast_imperative_warning_deprecated_constant (l, curr, alt, ty) ->
      Format.fprintf
        f
        "@[<hv>%a@ Warning: the constant %a is soon to be deprecated. Use \
         instead %a : %a. @]"
        Snippet.pp
        l
    ...
    | `Jsligo_deprecated_failwith_no_return loc ->
      Format.fprintf
        f
        "@[<hv>%a@.Deprecated `failwith` without `return`: `failwith` is just \
         a function.@.Please add an explicit `return` before `failwith` if you \
         meant the built-in `failwith`.@.For now, compilation proceeds adding \
         such `return` automatically.@.@]"
        Snippet.pp
        loc
    ...
    | `Self_ast_aggregated_warning_bad_self_type (got, expected, loc) ->
      Format.fprintf
        f
        "@[<hv>%a@ Warning: Tezos.self type annotation.@.Annotation \"%a\" was \
         given, but contract being compiled would expect \"%a\".@.Note that \
         \"Tezos.self\" refers to the current contract, so the parameters \
    ...
    | `Deprecated_reasonligo ->
      Format.fprintf
        f
        "@[Reasonligo is depreacted, support will be dropped in a few \
         versions.@.@]")
...
let to_warning : all -> Simple_utils.Warning.t =
 fun w ->
    ...
    let message =
      Format.asprintf
        "You are using Michelson failwith primitive (loaded from standard \
         library).@.Consider using `Test.failwith` for throwing a testing \
         framework failure.@."
    in
  ...
  | `Checking_ambiguous_constructor (location, tv_chosen, tv_possible) ->
    let message =
      Format.asprintf
        "Warning: The type of this value is ambiguous: Inferred type is %a but \
  ...
  | `Self_ast_typed_warning_unused (location, variable) ->
    let message =
      Format.sprintf
        "@.Warning: unused variable \"%s\".@.Hint: replace it by \"_%s\" to \
  ...
  | `Self_ast_typed_warning_unused_rec (location, s) ->
    let message =
      Format.sprintf
        "Warning: unused recursion .@.Hint: remove recursion from the function \
         \"%s\" to prevent this warning.\n"
        s
    in
    let content = make_content ~message ~location () in
    make ~stage:"parsing command line parameters" ~content
  | `Self_ast_imperative_warning_layout (location, Label s) ->
    ...
    let content = make_content ~message ~location () in
    make ~stage:"typer" ~content
  | `Self_ast_imperative_warning_deprecated_polymorphic_variable
      (location, variable) ->
    let variable = Format.asprintf "%a" Type_var.pp variable in
    let message =
      Format.sprintf
        "Warning: %s is not recognize as a polymorphic variable anymore. If \
  ...
  | `Self_ast_imperative_warning_deprecated_constant (location, curr, alt, ty)
    ...
    let message =
      Format.asprintf
        "Warning: the constant %a is soon to be deprecated. Use instead %a : \
         %a."
        Ast_imperative.PP.expression
        curr
        Ast_imperative.PP.expression
  ...
  | `Jsligo_deprecated_failwith_no_return location ->
    let message =
      Format.sprintf
        "Deprecated `failwith` without `return`: `failwith` is just a \
         function.@.Please add an explicit `return` before `failwith` if you \
         meant the built-in `failwith`.@.For now, compilation proceeds adding \
         such `return` automatically.@"
    in
    let content = make_content ~message ~location () in
  ...
  | `Self_ast_aggregated_warning_bad_self_type (got, expected, location) ->
    let message =
      Format.asprintf
        "Warning: Tezos.self type annotation.@.Annotation \"%a\" was given, \
         but contract being compiled would expect \"%a\".@.Note that \
         \"Tezos.self\" refers to the current contract, so the parameters \
         should be generally the same."
        Ast_aggregated.PP.type_expression
        got

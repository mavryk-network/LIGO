...
let return_good ?output_file v =
  let fmt : Format.formatter =
    ...
    | Some file_path ->
      Format.formatter_of_out_channel @@ Out_channel.create file_path
...
let makeCommand cmd =
  ...
  | false ->
    let pathVars =
      ...
      |> Array.to_list
      |> List.map (fun e ->
             match Str.split (Str.regexp ("=" [@reason.raw_literal "="])) e with
             | k :: v :: _rest -> Some (k, v)
             | _ -> None)
    ...
    | [] ->
      fprintf
        stderr
        ("No %s bin path found" [@reason.raw_literal "No %s bin path found"])
        cmd;
      exit (-1)
    | h :: _ ->
      Filename.concat h (sprintf ("%s.cmd" [@reason.raw_literal "%s.cmd"]) cmd))
...
let run_command (cmd : command) =
  ...
  then (
    ...
    let env =
      Ligo_unix.environment ()
      |> Array.filter ~f:(fun kv ->
             not (Str.string_match (Str.regexp "_=") kv 0))
      |> Array.append [| "_=" |]
    in

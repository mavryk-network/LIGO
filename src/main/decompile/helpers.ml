let specialise_and_print_pascaligo p =
  ...
  let source =
    Parsing.Pascaligo.pretty_print
      Parsing.Pascaligo.CST.
        { decl = cst; eof = Lexing_pascaligo.Token.ghost_eof }
  in
  source
...
let specialise_and_print_reasonligo m =
  ...
  let source = Parsing.Reasonligo.pretty_print cst in
  source


let specialise_and_print_expression_reasonligo expression =
  let cst = Tree_abstraction.Reasonligo.decompile_expression expression in
  let source = Parsing.Reasonligo.pretty_print_expression cst in
  source

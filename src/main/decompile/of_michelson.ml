...
let decompile_value ~raise (output_type:Ast_aggregated.type_expression) (ty, value) =
  ...
  let pat        = Expand_pattern.decompile aggregated in
...
let decompile_value_from_contract_execution ~raise (output_type: Ast_aggregated.type_expression) runned_result =
  ...
  | Fail s -> Fail s
  | Success ex_ty_value ->
    let Ligo_prim.Arrow.{ type1 = _ ; type2 = return_type } = trace_option ~raise main_entrypoint_not_a_function @@ Ast_aggregated.get_t_arrow output_type in

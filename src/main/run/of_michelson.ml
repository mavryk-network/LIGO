...
type dry_run_options =
  { parameter_ty :
      (Stacking.Program.meta, string) Tezos_micheline.Micheline.node option
...
let run_contract
      ...
      let y =
        ...
        then y
          ...
          Tezos_micheline.Micheline.Prim
            (-1, Michelson_v1_primitives.T_pair, y :: ys, [])
      in
      x, y
  ...
  let top_level =
    ...
    let entrypoints =
      Script_typed_ir.
        { root = Script_typed_ir.no_entrypoints
        ; original_type_expr = Int (0, Z.zero)
        }
    in
  ...
  let res =
    ...
    @@ Memory_proto_alpha.failure_interpret
         ?options
         descr
         input
         (EmptyCell, EmptyCell)
  ...
  | Memory_proto_alpha.Fail expr ->
    let expr =
      ...
      @@ Memory_proto_alpha.Protocol.Michelson_v1_primitives.strings_of_prims
           expr
    in
    Fail expr
...
let run_function
  ...
  let tezos_context =
    Option.map
      ~f:(fun ({ tezos_context; _ } : options) -> tezos_context)
      options
  in
  let input =
    Trace.trace_tzresult_lwt ~raise Errors.parsing_input_tracer
    @@ Memory_proto_alpha.parse_michelson_data
         ?tezos_context
         input_michelson
         input_ty
  in
...
let run_failwith
  ...
  match expr with
  | Success _ ->
    raise.error Errors.main_unknown
    (* TODO : simple_fail "an error of execution was expected" *)
  | Fail res -> res
...
let run_no_failwith
  ...
  match expr with
  | Success tval -> tval
  | Fail _ -> raise.error Errors.main_unknown


(* TODO : simple_fail "unexpected error of execution" *)

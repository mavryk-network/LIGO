...
let get : options:Compiler_options.t -> unit -> t = fun ~options () ->
  ...
  let curry_content_core = compile ~options (test_module ^ (def "CURRY") ^ std ^ lib ^ backend) in
  let curry_content_typed = type_ ~options curry_content_core in
  let uncurry_content_core = compile ~options (test_module ^ (def "UNCURRY") ^ std ^ lib ^ backend) in
  ...
  let typed_mod_def =
    ...
    [Location.wrap @@ D_module { module_binder = binder_curry ; module_attr ;
     ...
     Location.wrap @@ D_module { module_binder = binder_uncurry ; module_attr ;
      module_ = Location.wrap @@ M_struct uncurry_content_typed }] in
  let core_mod_def =
    let open Ligo_prim.Module_expr in
    ...
    [Location.wrap @@ D_module { module_binder = binder_curry ; module_attr ;
     ...
     Location.wrap @@ D_module { module_binder = binder_uncurry ; module_attr ;
        module_ = Location.wrap @@ M_struct uncurry_content_core }] in
  let uncurry =
    let prelude_core = Helpers.get_aliases_prelude binder_uncurry uncurry_content_typed in
    let options = Compiler_options.set_init_env options (Environment.append options.middle_end.init_env typed_mod_def) in
    let prelude_typed = type_ ~options prelude_core in
    { prelude_core ; prelude_typed ; content_typed = uncurry_content_typed } in
  let curry =
    let prelude_core = Helpers.get_aliases_prelude binder_curry curry_content_typed in
    let options = Compiler_options.set_init_env options (Environment.append options.middle_end.init_env typed_mod_def) in
    let prelude_typed = type_ ~options prelude_core in
    { prelude_core ; prelude_typed ; content_typed = curry_content_typed } in

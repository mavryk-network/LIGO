...
let type_ ~options x =
  match
    Simple_utils.Trace.to_stdlib_result
      (Ligo_compile.Of_core.typecheck ~options Env x)
  with
  | Ok (x, _w) -> x
  | Error (e, _w) ->
    let error_msg =
...
let get : options:Compiler_options.t -> unit -> t =
 fun ~options () ->
  ...
  let typed_mod_def =
    ...
    let module_attr =
      Ast_typed.TypeOrModuleAttr.{ public = true; hidden = true }
    in
    [ Location.wrap
      @@ D_module
  ...
  let uncurry =
    let prelude_core =
      Helpers.get_aliases_prelude binder_uncurry uncurry_content_typed
    in
    let options =
  ...
  let curry =
    let prelude_core =
      Helpers.get_aliases_prelude binder_curry curry_content_typed
    in
    let options =
      Compiler_options.set_init_env
        options

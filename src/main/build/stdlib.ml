...
let empty =
  let e = { prelude = [] ; content_core = [] ; content_typed = [] } in
...
module Cache = struct
  ...
  *)
  module LanguageMap = Simple_utils.Map.Make(struct
    ...
    let compare (pa,ta) (pb,tb) = Int.( abs (Environment.Protocols.compare pa pb) + abs (compare_bool ta tb) )
  end)
...
let compile ~options x =
  match Simple_utils.Trace.to_stdlib_result (Ligo_compile.Utils.core_program_string ~options CameLIGO x) with
  | Ok (x,_w) -> x
  | Error (e,_w) ->
...
let type_ ~options x =
  match Simple_utils.Trace.to_stdlib_result (Ligo_compile.Of_core.typecheck ~options Env x) with
  | Ok (x,_w) -> x
  | Error (e,_w) ->
    let error_msg = Format.asprintf "%a" (Main_errors.Formatter.error_ppformat ~display_format:Human_readable) e in
    failwith ("Error typing the stdlib: " ^ error_msg)

let stdlib : options:Compiler_options.t -> unit -> t = fun ~options () ->
  ...
  let curry =
    let content_core = compile ~options (test_module ^ (def "CURRY") ^ std ^ lib) in
    let content_typed = type_ ~options content_core in
    let prelude = Helpers.get_aliases_prelude binder_curry content_typed in
    { prelude ; content_core ; content_typed } in
  let uncurry =
    let content_core = compile ~options (test_module ^ (def "UNCURRY") ^ std ^ lib) in
    let content_typed = type_ ~options content_core in
    let prelude = Helpers.get_aliases_prelude binder_uncurry content_typed in
    { prelude ; content_core ; content_typed } in
  let typed_mod_def =
    ...
    [
      Location.wrap @@ D_module { module_binder = binder_curry ; module_attr ;
        module_ = Location.wrap @@ M_struct curry.content_typed } ;
      Location.wrap @@ D_module { module_binder = binder_uncurry ; module_attr ;
        module_ = Location.wrap @@ M_struct uncurry.content_typed }
    ]
  in
  let core_mod_def =
    let open Ligo_prim.Module_expr in
    ...
    [
      Location.wrap @@ D_module { module_binder = binder_curry ; module_attr ;
        module_ = Location.wrap @@ M_struct curry.content_core } ;
      Location.wrap @@ D_module { module_binder = binder_uncurry ; module_attr ;
        module_ = Location.wrap @@ M_struct uncurry.content_core }
    ]
  in
...
let get ~options : t =
  if options.Compiler_options.middle_end.no_stdlib then
    ...
    | None ->
      let lib = stdlib ~options () in
      cache_ref := LanguageMap.add k (lib) @@ !cache_ref;
      lib
    | Some typed -> typed

let select_prelude (stx : Syntax_types.t) (lib:t) : Ast_core.program =
  match stx with
  | CameLIGO -> lib.curry.prelude
  | (PascaLIGO | JsLIGO | ReasonLIGO) -> lib.uncurry.prelude

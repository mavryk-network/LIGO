  ...
  fun mod_binder prg ->
    ...
    let attr = Ast_core.ValueAttr.
      { inline = true ; no_mutation = true ; view = false ;
        public = true ; hidden = true ; thunk = false ; entry = false }
    in
...
let inject_declaration ~options ~raise : Syntax_types.t -> Ast_core.program -> Ast_core.program = fun syntax prg ->
  let inject_arg_declaration arg =
    ...
    let attr = ValueAttr.{ inline = false ; no_mutation = true ; view = false ; public = false ; hidden = false ; thunk = false ; entry = false } in
    let d = Location.wrap @@ D_value { binder = Binder.make (Ligo_prim.Value_var.of_input_var "cli_arg") None ; expr ; attr } in
    d::prg
  in

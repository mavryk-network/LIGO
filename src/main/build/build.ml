...
let rec build_contract_aggregated ~raise : options:Compiler_options.t -> string list -> string list -> Source_input.file_name -> _ =
  fun ~options entry_points cli_views file_name ->
    let entry_points = List.map ~f:Value_var.of_input_var entry_points in
    let typed_prg = qualified_typed ~raise ~options Ligo_compile.Of_core.Env file_name in
    let entry_point, typed_contract =
    ...
    let entry_point = Option.value_exn entry_point in
    let _, typed_views =
      let form =
...
and build_contract_stacking ~raise : options:Compiler_options.t -> string list -> string list -> Source_input.file_name -> Value_var.t * (Stacking.compiled_expression * _ ) * ((Value_var.t * Stacking.compiled_expression) list * _) =
  fun ~options entry_point cli_views file_name ->
    let entry_point, _, aggregated, agg_views = build_contract_aggregated ~raise ~options entry_point cli_views file_name in
    ...
    entry_point, (contract,aggregated),(views,agg_views)

(* building a contract in michelson *)
and build_contract ~raise ~options entry_point views file_name =
  let entry_point, (contract,_),(views,_) = build_contract_stacking ~raise ~options entry_point views file_name in
...
and build_contract_meta_ligo ~raise ~options entry_point views file_name =
  let entry_point, (_,contract),(_,views) = build_contract_stacking ~raise ~options entry_point views file_name in
  entry_point, contract,views

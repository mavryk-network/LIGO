...
module type Params = sig
  ...
  val meta : File_metadata.t
  val env  : Environment.t
end
...
module M (Params : Params) =
  struct
    ...
    let preprocess : file_name -> compilation_unit * meta_data * (file_name * module_name) list =
      fun file_name ->
      let meta = trace ~raise meta_tracer @@ File_metadata.extract "auto" file_name in
    ...
    let compile : AST.environment -> file_name -> meta_data -> compilation_unit -> (AST.t * AST.environment) =
      ...
      let inferred = Ligo_compile.Of_core.infer ~raise ~options ~env ast_core in
      let Ast_typed.Module_Fully_Typed ast_typed,ast_typed_env = Ligo_compile.Of_core.typecheck ~raise ~add_warning ~options ~meta ~env Ligo_compile.Of_core.Env inferred in
...
let dependency_graph ~raise ~add_warning : options:Compiler_options.t -> meta:File_metadata.t -> env:Environment.t -> file_name -> _ =
  fun ~options ~meta ~env file_name ->
...
let infer_contract ~raise ~add_warning : options:Compiler_options.t -> meta:File_metadata.t -> env:Environment.t -> string -> Ligo_compile.Of_core.form -> file_name -> _ =
  fun ~options ~meta ~env _syntax _entry_point main_file_name ->
...
let type_contract ~raise ~add_warning : options:Compiler_options.t -> meta:File_metadata.t -> env:Environment.t -> Ligo_compile.Of_core.form -> file_name -> _ =
  fun ~options ~meta ~env _entry_point file_name ->
    let open Build(struct
      let raise = raise
      let add_warning = add_warning
      let options = options
...
let combined_contract ~raise ~add_warning : options:Compiler_options.t -> meta:File_metadata.t -> env:Environment.t -> file_name -> _ =
  fun ~options ~meta ~env file_name ->
    let open BuildSystem.Make(Infer(struct
      let raise = raise
      let add_warning = add_warning
    ...
    let contract,env = Ligo_compile.Of_core.typecheck ~raise ~add_warning ~options ~meta ~env Env contract in
    contract, env

let build_mini_c ~raise ~add_warning : options:Compiler_options.t -> meta:File_metadata.t -> env:Environment.t -> _ -> file_name -> Mini_c.toplevel_statement list * Ast_typed.environment =
  fun ~options ~meta ~env entry_point file_name ->
    let open Build(struct
      let raise = raise
      let add_warning = add_warning
      let options = options
...
let build_expression ~raise ~add_warning : options:Compiler_options.t -> meta:File_metadata.t -> env:Environment.t -> string -> file_name option -> _ =
  fun ~options ~meta ~env expression file_name ->
    let (module_,env) = match file_name with
      | Some init_file ->
         let contract, env = combined_contract ~raise ~add_warning ~options ~meta ~env init_file in
         (contract, env)
      | None -> (Module_Fully_Typed [],env) in
    let typed_exp,_     = Ligo_compile.Utils.type_expression ~raise ~options ~meta ~env expression in
    ...
    let _,module_       = trace ~raise self_ast_typed_tracer @@ Self_ast_typed.morph_module env module_ in
...
let build_contract ~raise ~add_warning : options:Compiler_options.t -> meta:File_metadata.t -> env:Environment.t -> _ -> file_name -> _ =
  fun ~options ~meta ~env entry_point file_name ->
    let mini_c,e   = build_mini_c ~raise ~add_warning ~options ~meta ~env (Ligo_compile.Of_core.Contract entry_point) file_name in
...
let build_views ~raise ~add_warning :
  options:Compiler_options.t -> meta:File_metadata.t -> string -> string list * Ast_typed.environment -> file_name -> (string * Stacking.compiled_expression) list =
  fun ~options ~meta main_name (declared_views,env) source_file ->
    let views =
    ...
    in
    let f view_name =
      let mini_c,_   = build_mini_c ~raise ~add_warning:(fun _ -> ()) ~options ~meta ~env (Ligo_compile.Of_core.View (main_name,view_name)) source_file in
      let michelson  = Ligo_compile.Of_mini_c.aggregate_and_compile_contract ~raise ~options mini_c view_name in
      (view_name, michelson)
    in
    List.map ~f views

let build_contract_use ~raise ~add_warning : options:Compiler_options.t -> meta:File_metadata.t -> env:Environment.t -> file_name -> Mini_c.program * _ * _ =
  fun ~options ~meta ~env file_name ->
    let open Build(struct
      let raise = raise
      let add_warning = add_warning
    ...
    let _,contract = trace ~raise self_ast_typed_tracer @@ Self_ast_typed.morph_module init_env contract in

...
module type Monad = sig
  ...
  val get_list
    :  ?n:int
    -> 'a t
    -> ((Location.t * Ast_typed.expression) list * 'a) list
...
module Lst : Monad = struct
  ...
  let ( let* ) x f =
    List.concat
      (List.map
         ~f:(fun (l, x) ->
           List.concat (List.map ~f:(fun (l', x) -> [ l @ l', x ]) (f x)))
         x)


  let get_one ?(n = 0) l =
    Option.value (List.nth l n) ~default:(List.last_exn l)
...
module Monad_context (M : Monad) = struct
  ...
  let bind_map_ne_list
      : _ -> 'a Simple_utils.List.Ne.t -> 'b Simple_utils.List.Ne.t t
    =

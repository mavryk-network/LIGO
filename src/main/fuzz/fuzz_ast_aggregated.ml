...
let get_mutation_id (loc, expr, _) =
  let s =
    Format.asprintf "%a%a" Location.pp loc Ast_aggregated.PP.expression expr
  in
  let hash =
...
let add_line : Buffer.t -> string -> unit =
 fun buffer s -> Buffer.add_string buffer (Format.asprintf "%s\n" s)
...
let buffer_of_mutation : mutation -> Buffer.t =
 fun (loc, _, expr) ->
    ...
    let s =
      ...
      then (
        ...
        let postfix =
          String.sub
            line
            ~pos:(r#stop#offset `Point)
            ~len:(String.length line - r#stop#offset `Point)
        in
        prefix ^ expr ^ postfix)
      else (
        let line = In_channel.input_line_exn ic in
  ...
  | None ->
    failwith
      "Error when constructing buffer from mutation: file location was expected"
...
let map_constant cons_name arguments final_type_expression =
  ...
  let possible_const =
    ...
    | [ e1; e2 ] ->
      ...
      if is_t_nat t1 && is_t_nat t2 && is_t_nat t3
      then [ C_ADD; C_MUL; C_DIV; C_OR; C_AND; C_XOR ]
      else if is_t_int t1 && is_t_int t2 && is_t_int t3
      ...
      else if is_t_nat t1 && is_t_int t2 && is_t_int t3
      then [ C_ADD; C_MUL; C_SUB ]
      else if Ast_aggregated.equal_type_expression t1 t2 && is_t_bool t3
      then compare
      else []
    | _ -> []
  in
  if List.mem possible_const cons_name ~equal:Constant.equal_constant'
  then possible_const
  else [ cons_name ]
...
module Mutator = struct
  ...
  let combine
    ...
    =
   fun a al b bl ->
    List.map ~f:(fun (b, m) -> a, b, m) bl
    @ List.map ~f:(fun (a, m) -> a, b, m) al
  ...
  let combine_list
   ...
   fun a al ->
    ...
    @@ List.mapi
         ~f:(fun i ali ->
           List.map
             ~f:(fun (v, m) -> List.take a i @ [ v ] @ List.drop a (i + 1), m)
             ali)
         al
   ...
   fun e' ->
    ...
    | E_let_in { let_binder; rhs; let_result; attr } ->
      ...
      then
        ...
        let+ rhs, let_result, mutation =
          combine rhs (self rhs) let_result (self let_result)
        in
        return @@ E_let_in { let_binder; rhs; let_result; attr }, mutation
    ...
    | E_constant c ->
      ...
      let cb =
        List.map
          ~f:(fun (cc, b) ->
            ( cc.Constant.cons_name
            , Option.some_if
                (b && not (Location.is_dummy_or_generated e'.location))
                (e'.location, return @@ E_constant cc) ))
          cb
      in
      let argumentsm =
        combine_list c.arguments (List.map ~f:self c.arguments)
      in
    ...
    | (E_let_mut_in _ | E_for_each _ | E_deref _ | E_while _ | E_for _) as e' ->
      [ return e', None ]
  ...
  let some_mutate_expression ?(n = 0) (expr : Ast_aggregated.expression) =
    List.nth
      (List.filter_map
         ~f:(fun (v, i) -> Option.map i ~f:(fun m -> v, m))

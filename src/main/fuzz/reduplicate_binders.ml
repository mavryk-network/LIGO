...
let rec reduplicate ~raise : expression -> expression =
 fun exp ->
  ...
  let return expression_content : expression =
    { exp with expression_content }
  in
  let binder_remove_counter b =
    Binder.set_var b (Binder.apply remove_counter b)
  in
  let param_remove_counter b =
    Param.set_var b (remove_counter @@ Param.get_var b)
  in

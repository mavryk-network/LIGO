...
let rec reduplicate ~raise : expression -> expression =
 fun exp ->
  ...
  let remove_counter v =
    if Value_var.is_generated v
    then v
    else Value_var.(of_input_var ~loc:(get_location v) (to_name_exn v))
  in
  let return expression_content : expression = { exp with expression_content } in
  let binder_remove_counter b = Binder.set_var b (Binder.apply remove_counter b) in
  let param_remove_counter b = Param.set_var b (remove_counter @@ Param.get_var b) in
  match exp.expression_content with
  | E_literal l -> return (E_literal l)
  | E_constant { cons_name; arguments } ->
    let arguments = List.map ~f:self arguments in
    return (E_constant { cons_name; arguments })
  | E_variable v -> return (E_variable (remove_counter v))
  ...
  | E_for_each { fe_binder = binder1, binder2; collection; collection_type; fe_body } ->
    let fe_binder = remove_counter binder1, Option.map ~f:remove_counter binder2 in

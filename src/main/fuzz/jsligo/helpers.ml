  ...
  let rec map_type_expression : mapper -> type_expr -> 'b monad =
   fun f t ->
    ...
    | TSum { value; region } ->
      ...
      let value =
        { value with variants = { value.variants with value = variants } }
      in
    ...
    | TDisc t ->
      ...
      let obj (element : field_decl reg ne_injection reg) =
        ...
        let element =
          { element with value = { element.value with ne_elements } }
        in
  ...
  let rec map_expression : mapper -> expr -> expr monad =
   fun f e ->
    ...
    | ETernary { value; region } ->
      ...
      return
      @@ ETernary { value = { value with condition; truthy; falsy }; region }
   ...
   fun f s ->
    ...
    | SLet { value; region } ->
      ...
      let map_binding ({ value; region } : val_binding Region.reg) =
        ...
        ok
          ({ value = { value with lhs_type; expr }; region }
            : val_binding Region.reg)
      in
    ...
    | SSwitch { value; region } ->
      let map_case = function
        | Switch_case c ->
          ...
          let* statements =
            bind_map_option (bind_map_npseq self) c.statements
          in
          ok @@ Switch_case { c with expr; statements }
        | Switch_default_case d ->
          let* statements =
            bind_map_option (bind_map_npseq self) d.statements
          in

...
module Fold_helpers (M : Monad) = struct
  ...
  let rec map_type_expression : mapper -> type_expr -> 'b monad =
   fun f t ->
    ...
    | TSum { value; region } ->
      let aux (e : variant reg) =
        let app (x : type_expr par reg) =
          ...
          ok { x with value = { x.value with inside } }
        in
    ...
    | (TVar _ | TArg _ | TInt _ | TString _) as e -> ok @@ e
   ...
   fun f e ->
    ...
    | ECond { value; region } ->
      ...
      let test' =
        ...
        | `Parens { region; value } ->
          `Parens Region.{ region; value = { value with inside = test_expr' } }
      in
    ...
    | EList (ECons { value; region }) ->
      let { lbracket = _; lexpr; comma = _; ellipsis = _; rexpr; rbracket = _ } =
        value
      in
    ...
    | ELetIn { value; region } ->
      let { kwd_let = _; kwd_rec = _; binding; semi = _; body; attributes = _ } =
        value
      in
    ...
    | EModIn { value; region } ->
      ...
      let { kwd_module = _; name = _; eq = _; lbrace = _; module_; rbrace = _ } =
        mod_decl
      in

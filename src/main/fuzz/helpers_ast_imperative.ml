...
module Fold_helpers (M : Monad) = struct
  ...
  let constructor
      : ('a -> 'b monad) -> 'a Constructor.t -> 'b Constructor.t monad
    =
  ...
  let application
      : ('a -> 'b monad) -> 'a Application.t -> 'b Application.t monad
    =
   fun f { lamb; args } ->
    let* lamb = f lamb in
    let* args = f args in
  ...
  let conditional
      : ('a -> 'b monad) -> 'a Conditional.t -> 'b Conditional.t monad
    =
  ...
  let for_each
      : ('a -> 'b monad) -> 'a For_each_loop.t -> 'b For_each_loop.t monad
    =
  ...
  let declaration_type
      : ('a -> 'b monad) -> 'a Type_decl.t -> 'b Type_decl.t monad
    =
  ...
  let rec map_expression : 'err exp_mapper -> expression -> expression monad =
   fun f e ->
    ...
    | ( E_literal _
      ...
      | E_raw_code _
      | E_skip _
      | E_module_accessor _ ) as e' -> return e'

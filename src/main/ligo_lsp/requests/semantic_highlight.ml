...
let mk_diff (tokens : int array) (start : Position.t) : unit =
  let len = Array.length tokens in
  let rec go (i : int) (last_line : int) (last_start_char : int) : unit =
    if i < len
    then (
      ...
      if delta_line < 0 || delta_start_char < 0 || length < 0
      then
        Format.eprintf
          "Got negative delta (%d, %d) or length at %d:%d (length %d) with token type %d \
           (modifier %d) at group %d.\n\
           %!"
          delta_line
          delta_start_char
          line
  ...
  in
  go 0 start.line (start.character + 1)
...
let semantic_tokens (cst : Dialect_cst.t) (range : Range.t) : int array =
  ...
  let outside (reg : Simple_utils.Region.t) : bool =
    not Range.(inside ~big:range ~small:(of_region reg))
  in
  let () =
    ...
    | CameLIGO cst ->
      ...
      let rec collect (type_override : env) (Some_node (node, sing)) : unit fold_control =
        ...
        (* Literals *)
        | S_attribute -> terminal Decorator node
        | S_ctor -> terminal EnumMember node
        | S_field_name -> terminal Property node
        | S_language -> terminal Macro node
        | S_module_name -> terminal Namespace node
        | S_string -> terminal String node
        | S_type_name -> terminal TypeParameter node
        | S_variable -> terminal type_override node
        | S_verbatim -> terminal String node
    ...
    | JsLIGO cst ->
      ...
      let rec collect (type_override : env) (Some_node (node, sing)) : unit fold_control =
        ...
        | S_fun_name -> terminal Function node
        | S_intf_name -> terminal Interface node
        | S_language -> terminal Macro node
        | S_namespace_name -> terminal Namespace node
        | S_property_name -> terminal Property node
        | S_type_ctor -> terminal TypeParameter node
        | S_type_name -> terminal TypeParameter node
        | S_type_var -> terminal TypeParameter node
        | S_variable -> terminal type_override node
        (* Other literals *)
        ...
        | S_value_decl ->
          ...
          (* If we have more than one binder, it's a function. *)
          fold_
            (collect (if List.is_empty tl then Normal else FunDef))
            (hd -| S_reg S_val_binding);
          fold_collect Normal tl (S_list (S_array_2 (S_comma, S_reg S_val_binding)));
          Stop
        | S_arrow_fun_expr ->
          let { type_vars; parameters; rhs_type; arrow; fun_body } = node in
          fold_collect Normal type_vars (S_option S_type_vars);
          ...
          Stop
        | S_function_expr ->
          let ({ kwd_function; type_vars; parameters; rhs_type; fun_body }
                : function_expr)
            =
            node
          in
    ...
    | PascaLIGO cst ->
      ...
      let rec collect (type_override : env) (Some_node (node, sing)) : unit fold_control =
        ...
        | S_wrap _ when outside node#region -> Stop
        (* Keywords *)
        | S_kwd_and -> keyword node
        | S_kwd_begin -> keyword node
        | S_kwd_big_map -> keyword node
        | S_kwd_block -> keyword node
        | S_kwd_case -> keyword node
        | S_kwd_const -> keyword node
        | S_kwd_contains -> keyword node
        | S_kwd_down -> keyword node
        | S_kwd_else -> keyword node
        | S_kwd_end -> keyword node
        | S_kwd_for -> keyword node
        | S_kwd_from -> keyword node
        ...
        | S_kwd_if -> keyword node
        | S_kwd_in -> keyword node
        | S_kwd_is -> keyword node
        | S_kwd_list -> keyword node
        | S_kwd_map -> keyword node
        | S_kwd_mod -> keyword node
        | S_kwd_module -> keyword node
        | S_kwd_nil -> keyword node
        | S_kwd_not -> keyword node
        | S_kwd_of -> keyword node
        | S_kwd_or -> keyword node
        | S_kwd_patch -> keyword node
        | S_kwd_record -> keyword node
        | S_kwd_recursive -> keyword node
        | S_kwd_remove -> keyword node
        | S_kwd_set -> keyword node
        | S_kwd_skip -> keyword node
        | S_kwd_step -> keyword node
        | S_kwd_then -> keyword node
        | S_kwd_to -> keyword node
        | S_kwd_type -> keyword node
        | S_kwd_var -> keyword node
        | S_kwd_while -> keyword node
        | S_kwd_with -> keyword node
        (* Symbols *)
        | S_arrow -> operator node
        | S_assign -> operator node
        | S_caret -> operator node
        | S_colon -> operator node
        | S_comma -> operator node
        | S_sharp -> operator node
        ...
        | S_variable -> terminal type_override node
        | S_attribute -> terminal Decorator node
        | S_collection -> terminal Keyword node
        | S_language -> terminal Macro node
        | S_string -> terminal String node
        | S_verbatim -> terminal String node
        | S_wrap (S_tuple_2 (S_lexeme, S_hex)) -> terminal Number node
        ...
        | S_fun_decl ->
          let { kwd_recursive
            ...
            node
          in
          fold_collect Normal kwd_recursive (S_option S_kwd_recursive);
          fold_collect Normal kwd_function S_kwd_function;
          fold_collect FunDef fun_name S_variable;
          fold_collect Normal type_params (S_option (S_reg (S_chevrons S_type_params)));
          fold_collect FunArg parameters S_parameters;
          fold_collect Normal ret_type (S_option S_type_annotation);
          fold_collect Normal kwd_is S_kwd_is;
          fold_collect Normal return S_expr;
          fold_collect Normal terminator (S_option S_semi);
          Stop
        | S_fun_expr ->
          let ({ kwd_function; type_params; parameters; ret_type; kwd_is; return }
                : fun_expr)
            =
            node
          in
          ...
          Stop
        | S_call ->
          let expr, call_args = node.value in
          fold_collect FunApp expr S_expr;
          fold_collect FunArg call_args S_call_args;
          Stop
        (* Fix iteration order for module body. *)
        | S_module_body ->
          let { enclosing; declarations } = node in
          (match enclosing with
          | Braces (kwd_block, lbrace, rbrace) ->
            fold_collect Normal kwd_block (S_option S_kwd_block);
            fold_collect Normal lbrace S_lbrace;
            fold_collect Normal declarations S_declarations;
            fold_collect Normal rbrace S_rbrace
          | BeginEnd (kwd_begin, kwd_end) ->
            fold_collect Normal kwd_begin S_kwd_begin;
            fold_collect Normal declarations S_declarations;
            fold_collect Normal kwd_end S_kwd_end);
          Stop
        (* Fix iteration order for block. *)
        | S_block ->
          ...
          | Braces (kwd_block, lbrace, rbrace) ->
            fold_collect Normal kwd_block (S_option S_kwd_block);
            fold_collect Normal lbrace S_lbrace;
            fold_collect Normal statements S_statements;
            fold_collect Normal terminator (S_option S_semi);
            fold_collect Normal rbrace S_rbrace
          | BeginEnd (kwd_begin, kwd_end) ->
            fold_collect Normal kwd_begin S_kwd_begin;
...
let on_req_semantic_tokens_range (path : Path.t) (range : Range.t)
  ...
  let diff_data = Array.copy data in
  let () = mk_diff diff_data range.start in
  let@ () =
    send_debug_msg
      (Format.asprintf
         "Got tokens: %a\nFrom: %a"
         ...
         data)
  in
  return (Some (SemanticTokens.create ~data:diff_data ()))

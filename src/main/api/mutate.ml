...
let mutate_ast (raw_options : Raw_options.t) source_file display_format seed no_colour () =
  ...
  let syntax = Syntax.of_string_opt ~raise ~support_pascaligo:raw_options.deprecated (Syntax_name syntax) None in
  let buffer =
    Decompile.Of_imperative.decompile imperative_prg syntax
  in
  buffer

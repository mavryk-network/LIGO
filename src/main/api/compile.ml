...
let contract ?werror source_file entry_point declared_views syntax infer protocol_version display_format disable_typecheck michelson_format =
    ...
    format_result ?werror ~display_format (Formatter.Michelson_formatter.michelson_format michelson_format) get_warnings @@
      ...
      let code,env = Build.build_contract ~raise ~add_warning ~options ~meta ~env entry_point source_file in
      let views =
        Build.build_views ~raise ~add_warning ~options ~meta entry_point (declared_views,env) source_file
      in
...
let expression expression syntax infer protocol_version init_file display_format without_run michelson_format werror =
    ...
    format_result ~werror ~display_format (Formatter.Michelson_formatter.michelson_format michelson_format) get_warnings @@
      ...
      let options =
        ...
        Compiler_options.make ~infer ~protocol_version ()
      in
      let meta     = trace ~raise Main_errors.meta_tracer @@ File_metadata.make syntax init_file in
      let env      = Helpers.make_env ~options ~meta () in
      let (mini_c_exp,_), decl_list = Build.build_expression ~raise ~add_warning ~options ~meta ~env expression init_file in
...
let parameter source_file entry_point expression syntax infer protocol_version amount balance sender source now display_format michelson_format werror =
    ...
    format_result ~werror ~display_format (Formatter.Michelson_formatter.michelson_format michelson_format) get_warnings @@
      ...
      let meta     = trace ~raise Main_errors.meta_tracer @@ File_metadata.extract syntax source_file in
      let env      = Helpers.make_env ~options ~meta () in
      let typed_prg,env   = Build.combined_contract ~raise ~add_warning ~options ~meta ~env source_file in
      ...
      let typed_param,_    = Ligo_compile.Utils.type_expression ~raise ~options ~meta ~env expression in

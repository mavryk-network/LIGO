...
let contract
  ...
  let source = match source with
    ...
    | Text (source_code, _syntax) -> BuildSystem.Source_input.(Raw { id = ""; code = source_code })
  in
...
let storage
  ...
  let entry_point = Value_var.of_input_var ~loc entry_point in
  let app_typed_prg =
    Build.qualified_typed ~raise ~options Ligo_compile.Of_core.Env (Build.Source_input.From_file source_file)
  in

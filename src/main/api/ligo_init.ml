...
let list' ~kind =
  List.sort ~compare:String.compare @@ Map.keys (determine_map ~kind)
...
let new_project'
  ...
  in
  let _ =
    Cli_helpers.run_command (Constants.git_clone ~project_url ~project_name)
  in
  ...
  in
  let _ =
    OS.Dir.delete ~recurse:true @@ Fpath.(v "." / project_name / ".git")
  in
  [ project_name ]

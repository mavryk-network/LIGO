...
let dependency_graph source_file syntax display_format =
    ...
    format_result ~display_format (BuildSystem.Formatter.graph_format) get_warnings @@
      ...
      let g,_     = Build.dependency_graph ~raise ~add_warning ~options ~meta ~env source_file in
      (g,source_file)
...
let ast_core source_file syntax infer protocol_version display_format =
    ...
    if infer then
      (* Do the same thing as for print_ast_typed, but only infer the main module
        ...
        in
        let inferred_core = Build.infer_contract ~raise ~add_warning ~options ~meta ~env syntax Env source_file in
        inferred_core
    else
...
let mini_c source_file syntax infer protocol_version display_format optimize =
    ...
    fun ~raise ->
      ...
      let meta     = trace ~raise meta_tracer @@ File_metadata.extract syntax source_file in
      let env      = Helpers.make_env ~options ~meta () in
      let mini_c,_ = Build.build_mini_c ~raise ~add_warning ~options ~meta ~env Env source_file in
      match optimize with

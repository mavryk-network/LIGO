...
let test (raw_options : Raw_options.t) source_file display_format () =
  ...
  let typed = Build.qualified_typed ~raise ~options Env (Build.Source_input.From_file source_file) in
...
let test_expression (raw_options : Raw_options.t) expr source_file display_format () =
  ...
  let init_prg =
    let f : Source_input.file_name -> Ast_typed.program =
     fun filename -> Build.qualified_typed ~raise ~options Env (Build.Source_input.From_file filename)
    in
...
let dry_run
  ...
  let typed_prg = Build.qualified_typed ~raise ~options Env (Build.Source_input.From_file source_file) in
  let aggregated_prg =
  ...
  in
  let options =
    Run.make_dry_run_options ~raise { now; amount; balance; sender; source; parameter_ty }
  in
  let runres =
    Run.run_contract ~raise ~options compile_exp.expr compile_exp.expr_ty args_michelson
  in
...
let evaluate_call
  ...
  in
  let Compiler_options.{ entry_point; _ } = options.frontend in
  let init_prog = Build.qualified_typed ~raise Env ~options (Build.Source_input.From_file source_file) in

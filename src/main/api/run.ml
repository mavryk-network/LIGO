...
let test (raw_options : Raw_options.t) source_file display_format () =
  ...
  in
  let options =
    Compiler_options.make ~protocol_version ~syntax ~raw_options ()
  in
...
let dry_run
  ...
  in
  let mini_c_prg =
    Compile.Of_aggregated.compile_expression ~raise aggregated_prg
  in
  let compile_exp =
  ...
  let parameter_ty =
    ...
    in
    Option.map ~f:fst
    @@ Self_michelson.fetch_contract_ty_inputs compile_exp.expr_ty
  in
  let compiled_input =
...
let interpret
  ...
  in
  let compiled_exp =
    Compile.Of_mini_c.compile_expression ~raise ~options mini_c_exp
  in
  let options =
    Run.make_dry_run_options
      ~raise
...
let evaluate_call
  ...
  in
  let core_param =
    Compile.Of_imperative.compile_expression ~raise imperative_param
  in
  let app = Compile.Of_core.apply entry_point core_param in
  let typed_app =
    Compile.Of_core.compile_expression ~raise ~options ~init_prog app
  in
  let app_aggregated =
  ...
  in
  let app_mini_c =
    Compile.Of_aggregated.compile_expression ~raise app_aggregated
  in
  let michelson =

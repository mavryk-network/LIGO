...
let dry_run
  ...
  let entry_point =
    List.map ~f:(Value_var.of_input_var ~loc:Location.dummy) entry_point
  in
...
let evaluate_call
  ...
  let entry_point =
    List.map ~f:(Value_var.of_input_var ~loc:Location.generated) entry_point
  in
  let entry_point =
    Self_ast_typed.get_final_entrypoint_name ~raise entry_point init_prog
  in

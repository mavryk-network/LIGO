...
let measure_contract source_file entry_point declared_views infer protocol_version display_format werror =
    ...
    format_result ~werror ~display_format Formatter.contract_size_format get_warnings @@
      ...
      let views = Build.build_views ~raise ~add_warning ~options entry_point (declared_views,e) source_file in

...
let measure_contract (raw_options : Compiler_options.raw) source_file display_format () =
    ...
    format_result ~warning_as_error ~display_format Formatter.contract_size_format @@
      ...
      let michelson,e,entry_point =  Build.build_contract ~raise ~options entry_point source_file in
      let views = Build.build_views ~raise ~options entry_point (views,e) source_file in

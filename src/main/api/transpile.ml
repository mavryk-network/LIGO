...
let contract source_file new_syntax syntax display_format () =
  ...
  @@ fun ~raise ->
  let syntax =
    Syntax.of_string_opt ~raise (Syntax_name syntax) (Some source_file)
  in
  let options =
    Compiler_options.make ~raw_options:(Raw_options.make ()) ~syntax ()
  in
...
let expression expression new_syntax syntax display_format () =
  ...
  in
  let imperative =
    Compile.Of_c_unit.compile_expression ~raise ~meta c_unit_expr
  in
  ...
  let buffer =
    Decompile.Of_imperative.decompile_expression imperative n_syntax
  in
  buffer

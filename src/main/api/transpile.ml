...
let contract source_file new_syntax syntax new_dialect display_format =
    ...
    format_result ~display_format (Parsing.Formatter.ppx_format) get_warnings @@
      ...
      let dialect    = File_metadata.Dialect_name new_dialect in
      let buffer     = Decompile.Of_imperative.decompile ~raise ~dialect imperative (Syntax_name new_syntax) in
      buffer
...
let expression expression new_syntax syntax new_dialect display_format =
    format_result ~display_format (Parsing.Formatter.ppx_format) (fun _ -> []) @@
      ...
      (* Compiling chain *)
      let options       = Compiler_options.make () in
      let meta          = trace ~raise meta_tracer @@ File_metadata.make syntax None in
      ...
      (* Decompiling chain *)
      let dialect       = File_metadata.Dialect_name new_dialect in
      let n_syntax      = trace ~raise meta_tracer @@ File_metadata.syntax_to_variant ~dialect (Syntax_name new_syntax) None in

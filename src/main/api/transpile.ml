...
let contract source_file new_syntax syntax display_format no_colour () =
  ...
  @@ fun ~raise ->
  let syntax = Syntax.of_string_opt ~raise ~deprecated:true (Syntax_name syntax) (Some source_file) in
  ...
  let buffer =
    Decompile.Of_imperative.decompile ~raise ~deprecated:true imperative (Syntax_name new_syntax)
  in
  buffer
...
let expression expression new_syntax syntax display_format no_colour () =
  ...
  @@ fun ~raise ->
  (* Compiling chain *)
  let syntax = Syntax.of_string_opt ~raise ~deprecated:true (Syntax_name syntax) None in
  let options = Compiler_options.make ~raw_options:(Raw_options.make ()) ~syntax () in
  ...
  (* Decompiling chain *)
  let n_syntax = Syntax.of_string_opt ~raise ~deprecated:true (Syntax_name new_syntax) None in

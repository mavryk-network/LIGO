...
let declarations_jsonformat (source_file, decls) : json =
  ...
  let json_decl =
    List.map ~f:(fun decl -> `String (Value_var.to_name_exn decl)) decls
  in
...
module Michelson_formatter = struct
  ...
  let source_type_to_json (source_type : Ast_typed.type_expression)
  ...
  let shrink (node : (Mini_c.meta, 'p) Tezos_micheline.Micheline.node)
      : shrunk_result
    =
    ...
    let type_index t =
      match
        List.findi (TypeSet.elements type_set) ~f:(fun _ x ->
            0 = TypeOrd.compare x t)
      with
      | Some (i, _) -> i
      (* impossible... *)
      | None -> 0
                  ...
                  Some
                    (`A
                      (List.map env ~f:(function
                          | None -> `Null
                          | Some binder_meta ->
                            variable_meta_to_json binder_meta))) )
            ])
  ...
  let pp_result_json michelson_comments ppf result =
    let json =
      get_json ?comment:(comment_encoding michelson_comments) result.michelson
    in
    let json =
  ...
  let result_ppformat
    ...
    | Human_readable | Dev ->
      let m =
        Format.asprintf "%a\n" (mich_pp michelson_format michelson_comments) a
      in
  ...
  let result_jsonformat michelson_format michelson_comments (a : shrunk_result)
    ...
    | `Json ->
      let code_as_str =
        Format.asprintf "%a" (pp_result_json michelson_comments) a
      in
  ...
  let shrunk_result_format : michelson_format -> _ -> shrunk_result format =
   fun mf michelson_comments ->
    ...
    ; to_json =
        result_jsonformat mf (convert_michelson_comments michelson_comments)
    }
  ...
  let michelson_format
   ...
   fun michelson_format michelson_comments ->
    Display.map
      ~f:shrink
      (shrunk_result_format michelson_format michelson_comments)
  ...
  let michelson_constant_jsonformat michelson_format a : json =
    ...
    | `Text ->
      let code_as_str =
        Format.asprintf "%a" (pp_comment ~comment:(fun _ -> None)) a
      in
      `Assoc [ "text_code", `String code_as_str ]
    | `Hex ->
  ...
  let michelson_constant_ppformat ~display_format f (hash, a) =
    ...
    | Human_readable | Dev ->
      ...
      in
      let code_no_newlines =
        Str.global_replace (Str.regexp_string "\\n") "\n" code
      in
      let hash =
        Format.asprintf
          "%a"
          Memory_proto_alpha.Protocol.Script_expr_hash.pp
          hash
      in
      Format.fprintf
        f
        "Michelson constant as JSON string:@.%s@.This string can be passed in \
         `--constants` argument when compiling a contract.@.@.Remember to \
         register it in the network, e.g.:@.> tezos-client register global \
         constant %s from bootstrap1@.@.Constant hash:@.%s"
        code
        code_no_newlines
        hash

...
module Make (Params : Woo_helpers.PARAMS) = struct
  ...
  let destruct prefix : W.variant -> P.structure_item list =
   fun variant ->
    ...
    let vars =
      lts |> List.map ~f:(fun (l, _tes) -> l) |> List.map ~f:String.lowercase
    in
    let single_case
        :  W.label * W.type_expression list
        -> string * string list * P.expression
      =
     fun (lbl, tes) ->
      let l = List.length tes in
      let body =
        e_apply (e_var @@ String.lowercase lbl) @@ e_tuple @@ e_vars "x" l
      in

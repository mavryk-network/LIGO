...
module Make (Params : Woo_helpers.PARAMS) = struct
  ...
  let matching ?wrap_get ~namer case
      : W.variant -> W.labelled_types -> P.structure_item
    =
   fun variant (base_label, _ty) ->
    ...
    let cases =
      List.map ~f:(case ~polymorphic ~base_label) constructor_declarations'
    in
    let body =
      match wrap_get with
      ...
      | Some wrap ->
        let lhs =
          A.pexp_apply
            ...
            wrap
            [ ( Nolabel
              , A.pexp_ident ~loc @@ Location.mkloc (Longident.Lident "x") loc )
            ]
        in
      ...
      then p_constructor ~polymorphic current_label
        ...
        let pattern =
          ...
          then p_var variable_name
            ...
            let patterns =
              List.map ~f:(fun name -> A.pvar ~loc name) pattern_names
            in
  ...
  let matching_full ?wrap_get ~namer rhs : W.variant -> P.structure_item list =
   fun variant ->
    ...
    let constructor_declarations = SMap.to_kv_list constructor_declarations in
    let matching_lst =
      List.map ~f:(matching_single variant) constructor_declarations
    in
    matching_lst

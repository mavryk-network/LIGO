...
module SMap = struct
  ...
  let to_kv_list : 'a t -> (string * 'a) list =
   fun m -> List.rev (to_kv_list_rev m)
end
...
let c_decls
    : (string * type_expression list) list -> type_expression list SMap.t
  =
 fun cds ->
  List.fold_left
    ~f:(fun old (key, data) -> SMap.set ~key ~data old)
    ~init:SMap.empty
    cds

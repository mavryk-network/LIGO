...
module Generate (Params : PARAMS) = struct
  ...
  let p_record_pun lst =
    A.ppat_record
      ~loc
      (List.map ~f:(fun (x : string) -> lident x, p_var x) lst)
      Closed
  ...
  let e_record lst =
    A.pexp_record
      ~loc
      (List.map ~f:(fun (var, expr) -> lident var, expr) lst)
      None
  ...
  let e_property ~record ~(label : string) =
    A.pexp_field ~loc record (lident label)
  ...
  let e_match
      :  polymorphic:bool -> P.expression
      -> (string * string list * P.expression) list -> P.expression
    =
        ...
        then p_constructor ~polymorphic constructor
          ...
          let pattern =
            ...
            then p_var @@ List.hd_exn params
              ...
              let patterns =
                List.map ~f:(fun name -> A.pvar ~loc name) params
              in

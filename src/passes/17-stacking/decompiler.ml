...
let rec decompile_value ~(raise : (stacking_error, _) raise)
 ...
 fun ty value ->
  ...
  | Prim (_, "pair", ts, _), Prim (_, "Pair", vs, _) ->
    let els =
      List.map ~f:(fun (t, v) -> decompile_value ~raise t v) (List.zip_exn ts vs)
    in
    let rec aux l : value =
      match l with
  ...
  | Prim (_, "chain_id", _, _), String (_, id) ->
    ...
    *)
    D_string id
  | Prim (_, "key_hash", [], _), String (_, n) -> D_string n
  | Prim (_, "key", [], _), String (_, n) -> D_string n
  ...
  | Prim (_, "bytes", [], _), Bytes (_, b) -> D_bytes b
  | Prim (_, "bls12_381_g1", [], _), Bytes (_, b) -> D_bytes b
  | Prim (_, "bls12_381_g2", [], _), Bytes (_, b) -> D_bytes b
  | Prim (_, "bls12_381_fr", [], _), Bytes (_, b) -> D_bytes b
  | Prim (_, "address", [], _), String (_, s) -> D_string s
  | Prim (_, "unit", [], _), Prim (_, "Unit", [], _) -> D_unit
  | Prim (_, "option", [ _ ], _), Prim (_, "None", [], _) -> D_none
  ...
  | Prim (_, "big_map", [ k_ty; v_ty ], _), Seq (_, lst) ->
    ...
    in
    D_big_map lst'
  | Prim (_, "big_map", [ _; _ ], _), Int (_, v) -> D_nat v
  | Prim (_, "list", [ ty ], _), Seq (_, lst) ->
    let lst' = List.map ~f:(decompile_value ~raise ty) lst in
    D_list lst'
  | Prim (_, "set", [ ty ], _), Seq (_, lst) ->
    ...
    in
    let lst'' =
      let aux t = decompile_value ~raise ty t in
      List.map ~f:aux lst'
    in
    D_set lst''

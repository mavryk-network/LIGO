...
let rec to_list_pattern ~(raise : raise) simple_pattern : _ AST.Pattern.t =
  ...
  | SP_Constructor (Label "#NIL", _, _) -> Location.wrap @@ AST.Pattern.P_list (List [])
...
and to_original_pattern ~raise simple_patterns (ty : AST.type_expression) =
  ...
  | [] ->
    raise.error @@ Errors.corner_case "edge case: to_original_pattern empty patterns"
  ...
  let redundant, case, _ =
    List.fold_left
      ...
      then false, case + 1, [ vector ]
        ...
        let redundant_case_found = not @@ algorithm_Urec matrix ~raise vector in
...
let check_anomalies
  ...
  ~syntax
  ~loc
  (eqs : (AST.type_expression AST.Pattern.t * AST.type_expression * AST.expression) list)
  t
  =

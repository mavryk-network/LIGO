...
and to_original_pattern ~raise simple_patterns (ty : AST.type_expression) =
  ...
  | _ ->
    ...
    | AST.T_record { fields; _ } ->
      ...
      then Location.wrap @@ P_tuple ps
      else Location.wrap @@ P_record (Record.of_list (List.zip_exn labels ps))
...
let rec algorithm_I ~(raise : raise) matrix n ts =
  ...
  then
    ...
    if (not @@ LSet.is_empty constructors)
       && LSet.equal constructors complete_signature
    then
      LSet.fold
        (fun ck p ->
          if Option.is_some p
      ...
      | Some ps ->
        ...
        then (
          ...
          let missing_constructors =
            LSet.diff complete_signature constructors
          in
          let cs =
            LSet.fold
              (fun c cs ->
...
let check_anomalies
    ...
    ~loc
    (eqs :
      (AST.type_expression AST.Pattern.t * AST.type_expression * AST.expression)
      list)
    t
  =

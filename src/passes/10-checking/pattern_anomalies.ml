...
let rec to_simple_pattern ty_pattern =
  ...
  | P_tuple ps ->
    ...
    let ps = List.mapi ps ~f:(fun i p ->
      let row_elem = Option.value_exn ~here:[%here] (LMap.find_opt (Label (Int.to_string i)) row.content) in
  ...
  | P_record (labels, ps) ->
    ...
    let ps = List.map label_ps ~f:(fun (label, p) ->
      let row_elem = Option.value_exn ~here:[%here] (LMap.find_opt label row.content) in
      to_simple_pattern (p, row_elem.associated_type))
    in
    List.concat ps

...
let list_constructors = Label.Set.of_list [ cons_label; nil_label ]
...
let get_variant_nested_type label (row : AST.t_sum) =
  let row_elem : _ Rows.Elem.t = Map.find_exn row.fields label in
  row_elem.content.associated_type
...
let rec destructure_type (t : AST.type_expression) =
  ...
  | AST.T_record { fields; _ } ->
    Map.fold
      fields
      ~init:[]
      ~f:(fun ~key:_ ~data:(row_elem : _ Rows.Elem.t) ts ->
        ts @ destructure_type row_elem.content.associated_type)
  | _ -> [ t ]


let rec to_simple_pattern
    (ty_pattern : _ option Pattern.t * AST.type_expression)
  =
  ...
  | P_variant (c, p) ->
    let p_ty =
      get_variant_nested_type
        c
        (Option.value_exn ~here:[%here] (C.get_t_sum ty))
    in
    [ SP_Constructor (c, to_simple_pattern (p, p_ty), ty) ]
  | P_tuple ps ->
    let types = Option.value_exn ~here:[%here] (C.get_t_tuple ty) in
    let ps =
      List.map2_exn ps types ~f:(fun p ty -> to_simple_pattern (p, ty))
    in
    List.concat ps
  | P_record (labels, ps) ->
    ...
    let ps =
      List.map label_ps ~f:(fun (label, p) ->
          let row_elem =
            Option.value_exn ~here:[%here] (Map.find row.fields label)
          in
          to_simple_pattern (p, row_elem.content.associated_type))
    in
    List.concat ps


let are_keys_numeric keys =
  List.for_all keys ~f:(fun l ->
      Option.is_some @@ int_of_string_opt @@ Label.to_string l)


let rec to_list_pattern ~(raise : raise) simple_pattern : _ Pattern.t =
  match simple_pattern with
  | SP_Wildcard _ -> Location.wrap @@ Pattern.P_var wild_binder
  | SP_Constructor (Label "#NIL", _, _) ->
    Location.wrap @@ Pattern.P_list (List [])
  | SP_Constructor (Label "#CONS", sps, t) ->
    ...
    Location.wrap @@ Pattern.P_list (Cons (hd, tl))
...
and to_original_pattern ~raise simple_patterns (ty : AST.type_expression) =
  ...
  | [ SP_Constructor (c, sps, t) ] ->
    let t =
      get_variant_nested_type c (Option.value_exn ~here:[%here] (C.get_t_sum t))
    in
  ...
  | _ ->
    ...
    | AST.T_record { fields; _ } ->
      let kvs =
        List.sort (Map.to_alist fields) ~compare:(fun (l1, _) (l2, _) ->
            Label.compare l1 l2)
      in
      let labels, tys = List.unzip kvs in
      let tys = List.map tys ~f:(fun ty -> ty.content.associated_type) in
      let _, ps =
      ...
      in
      Location.wrap @@ P_record (labels, ps)
    | AST.T_tuple types ->
      let _, ps =
        List.fold_left types ~init:(simple_patterns, []) ~f:(fun (sps, ps) t ->
            let n = List.length @@ destructure_type t in
            let sps, rest = List.split_n sps n in
            rest, ps @ [ to_original_pattern ~raise sps t ])
      in
...
let get_all_constructors (t : AST.type_expression) =
  ...
  then list_constructors
    ...
    | Some tsum -> Map.key_set tsum.fields
    | None -> Label.Set.empty)


let get_constructors_from_1st_col matrix =
  List.fold_left matrix ~init:Label.Set.empty ~f:(fun s row ->
      match row with
...
let rec algorithm_Urec ~(raise : raise) matrix vector =
  ...
  then false
    ...
    | SP_Wildcard t :: q2_n ->
      ...
      then
        Set.fold complete_signature ~init:false ~f:(fun b c ->
      ...
      else algorithm_Urec ~raise (default_matrix matrix) q2_n
...
let rec algorithm_I ~(raise : raise) matrix n ts =
  ...
  then
    ...
    let complete_signature = get_all_constructors t in
    if (not @@ Set.is_empty constructors)
       && Set.equal constructors complete_signature
    then
      Set.fold complete_signature ~init:None ~f:(fun p ck ->
          if Option.is_some p
          then p
          else (
...
let missing_case_analysis ~raise matrix t =
  ...
  | Some sps ->
    let ps = List.map sps ~f:(fun sp -> to_original_pattern ~raise sp t) in
    Some ps
  | None -> None
      ...
      ~f:(fun (redundant_case_found, case, matrix) vector ->
        ...
        then false, case + 1, [ vector ]
          ...
          let redundant_case_found =
            not @@ algorithm_Urec matrix ~raise vector
          in

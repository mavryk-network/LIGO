...
module PP_DEBUG = struct
  let pp_typed_pattern ppf ((p, t) : typed_pattern) =
    Format.fprintf
      ppf
      "(%a : %a)"
      (Pattern.pp O.PP.type_expression_option)
      p
...
let is_product' : _ Pattern.t -> bool =
 fun p ->
  match p.wrap_content with
  | P_tuple _ -> true
  | P_record _ -> true
...
let get_pattern_type ~raise (eqs : equations) =
  ...
  | ((_, t) :: _, _) :: _ -> t
  | ([], _) :: _ ->
    raise.error @@ corner_case __LOC__ (* typed_pattern list empty *)
...
let extract_variant_type ~raise
    : pattern -> Label.t -> O.type_expression -> O.type_expression
  =
 fun _ label t ->
  match t.type_content with
  | T_sum row ->
    (match Map.find row.fields label with
    | Some t -> t.content.associated_type
    | None -> raise.error @@ corner_case __LOC__)
  | O.T_constant
      { injection = Literal_types.List; parameters = [ proj_t ]; language = _ }
    ->
    (match label with
...
**)
let rec substitute_var_in_body ~raise
  ...
  let aux ~raise : unit -> O.expression -> bool * unit * O.expression =
   fun () exp ->
    ...
    | O.E_matching m ->
          ...
          let cases =
            List.fold_right cases ~init:[] ~f:(fun case cases ->
                ...
                then case :: cases
                  ...
                  let body =
                    substitute_var_in_body ~raise to_subst new_var case.body
                  in
                  { case with body } :: cases))
          in
          Match_variant { cases; tv }
        | Match_tuple { binders; body; tv } ->
          if List.exists binders ~f:(fun binder ->
                 Value_var.equal to_subst (Binder.get_var binder))
          then m.cases
          else (
            let body = substitute_var_in_body ~raise to_subst new_var body in
            Match_tuple { binders; body; tv })
      ...
      ret
        false
        { exp with expression_content = O.E_matching { matchee; cases } }
    | E_literal _
    | E_constant _
    | E_variable _
    | E_application _
...
**)
let group_equations ~raise : equations -> equations Label.Map.t =
 fun eqs ->
  let aux
      :  typed_pattern list * O.expression -> equations Label.Map.t
      -> equations Label.Map.t
    =
   fun (pl, body) m ->
    ...
    | P_variant (label, p_opt) ->
      ...
      Map.update m label ~f:(upd proj_t p_opt)
    | P_list (List []) ->
      let label = Label.of_string "Nil" in
      let proj_t = extract_variant_type ~raise phd label t in
      Map.update m label ~f:(upd proj_t (Location.wrap Pattern.P_unit))
    | P_list (Cons (p_hd, p_tl)) ->
      let label = Label.of_string "Cons" in
      let pattern =
        Location.wrap ~loc:phd.location @@ Pattern.P_tuple [ p_hd; p_tl ]
      in
      let proj_t = extract_variant_type ~raise phd label t in
      Map.update m label ~f:(upd proj_t pattern)
    | _ -> raise.error @@ corner_case __LOC__
  in
  List.fold_right ~f:aux ~init:Label.Map.empty eqs


let rec match_ ~raise
    : err_loc:Location.t -> matchees -> equations -> rest -> O.expression
  =
 fun ~err_loc ms eqs def ->
  ...
  | _ ->
    ...
    let aux
        (part_eq : equations)
        ((def, _, _) :
          O.expression_content * O.type_expression option * Location.t)
      =
      let r = consvar ~raise ~err_loc ms part_eq def in
      r.expression_content, Some r.type_expression, r.location
    in
    let r, t, location =
      List.fold_right ~f:aux ~init:(def, None, Location.generated) leq
    in
...
and consvar ~raise
 ...
 fun ~err_loc ms eqs def ->
  ...
  else ctor_rule ~raise ~err_loc ms eqs def


and var_rule ~raise
    :  err_loc:Location.t -> typed_pattern option -> matchees -> equations
    -> rest -> O.expression
  =
 fun ~err_loc product_opt ms eqs def ->
  ...
  | mhd :: mtl ->
    ...
    | Some shape -> product_rule ~raise ~err_loc shape ms eqs def
    | None ->
      let aux
       ...
       fun (pl, body) ->
        ...
        | (phd, _) :: ptl ->
          ...
          | P_var b ->
            let body' =
              substitute_var_in_body ~raise (Binder.get_var b) mhd body
            in
            ptl, body'
          | P_unit -> ptl, body
          | _ -> raise.error @@ corner_case __LOC__)
  ...
  | mhd :: mtl ->
    ...
    let grouped_eqs =
      ...
      | Some _ when Option.is_some (O.get_t_option matchee_t) ->
        List.map
          ~f:(fun label -> label, Map.find eq_map label)
          [ Label.of_string "Some"; Label.of_string "None" ]
      | Some rows ->
        let eq_opt_map =
          Map.mapi rows.fields ~f:(fun ~key:label ~data:_ ->
              Map.find eq_map label)
        in
        Map.to_alist @@ eq_opt_map
      | None ->
        ...
        | None ->
          raise.error
          @@ corner_case __LOC__ (* should be caught when typing the matchee *))
    in
    let present =
      List.filter_map
        ~f:(fun (c, eq_opt) ->
...
and product_rule ~raise
    :  err_loc:Location.t -> typed_pattern -> matchees -> equations -> rest
    -> O.expression
  =
 fun ~err_loc product_shape ms eqs def ->
  ...
  | mhd :: _ ->
    ...
    in
    let aux
        : typed_pattern list * O.expression -> typed_pattern list * O.expression
      =
     fun (pl, body) ->
      match pl with
      | (prod, t) :: ptl ->
        let var_filler = make_var_pattern (Value_var.fresh ~name:"_" ()), t in
    ...
    let matchee_t = get_pattern_type ~raise eqs in
    let eqs' = List.map ~f:aux eqs in
    let fields = Label.Map.of_alist_exn lb in
...
let compile_matching
    ...
    matchee
    (eqs :
      (O.type_expression option Pattern.t * O.type_expression * O.expression)
      list)
  =
  let eqs =
    List.map
      ~f:(fun (pattern, pattern_ty, body) -> [ pattern, pattern_ty ], body)
      eqs
  in
  let missing_case_default =
    let fs =
      O.make_e
        (O.E_literal
           (Literal_value.Literal_string Backend.Michelson.fw_partial_match))
        (O.t_string ())
    in
    let t_fail =
      ...
      O.t_for_all
        ...
        (O.t_for_all
           b
           Type
           (O.t_arrow (O.t_variable a ()) (O.t_variable b ()) ()))
    in

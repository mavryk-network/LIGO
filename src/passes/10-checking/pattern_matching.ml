...
`Sugar` match expression being 'pattern matching' expression and `Core`/`Typed` being 'case expressions'. (* Needs to change *)
...
Option patterns are treated as the variant type `Some a | None` would be (* can be BYE BYE if you want *)
...
type typed_pattern = pattern * O.type_expression (* once moved, you will have this "by default" *)
type equations = (typed_pattern list * (I.expression * Context.typing_context (* bye bye *) )) list
type type_fun = raise:typer_error raise -> Context.typing_context -> ?tv_opt:O.type_expression -> I.expression -> O.expression (* BYE BYE *)
...
let assert_body_t ~raise : body_t:O.type_expression option -> Location.t -> O.type_expression -> unit = (* BYE BYE *)
  fun ~body_t loc t ->
    match body_t with
...
let extract_record_type ~raise : pattern -> O.label -> O.type_expression -> O.type_expression = (* PARTIAL BYE BYE (should never throw) *)
  fun p label t ->
  match t.type_content with
  | T_record rows -> (
    match O.LMap.find_opt label rows.content with
...
**)
let rec substitute_var_in_body ~raise : I.expression_variable -> O.expression_variable -> I.expression -> (bool * I.expression) =
  fun to_subst new_var body -> 
    let aux ~raise : bool -> I.expression -> bool * bool * I.expression =
      fun has_subst exp ->
        ...
        | I.E_matching m -> (
          ...
          let cases, has_subst = List.fold_right
            ~f:(fun (case : _ I.match_case) (cases, has_subst) ->
              match Stage_common.Helpers.fold_pattern aux false case.pattern with
              | true -> case::cases, has_subst
              | false ->
                let has_subst',body = substitute_var_in_body ~raise to_subst new_var case.body in
                let has_subst = has_subst' || has_subst in
                { case with body }::cases, has_subst
            )
            ~init:([],has_subst)
            m.cases
          in
...
let rec match_ ~raise : err_loc:Location.t -> type_f:type_fun -> body_t:O.type_expression option -> matchees -> equations -> rest -> O.expression =
  ...
  | [] , eqs when List.for_all ~f:(fun (ps,_) -> List.length ps = 0) eqs ->
    raise.raise @@ redundant_pattern err_loc (* BYE BYE after case analysis *)
  | _ ->
    let leq = partition (fun (pl,_) -> is_var (fst @@ List.hd_exn pl)) eqs in
    let aux = fun (part_eq:equations) ((def,body_t,_):O.expression_content * O.type_expression option * Location.t) ->
      let r = consvar ~raise ~err_loc ~type_f ~body_t ms part_eq def in
      let () = assert_body_t ~raise ~body_t r.location r.type_expression in
      (r.expression_content , Some r.type_expression, r.location)
    in
    let (r,t,location) = List.fold_right ~f:aux ~init:(def,body_t,Location.generated) leq in
       ...
       let aux : typed_pattern list * (I.expression * Context.typing_context) ->
        ...
        | (phd,t)::ptl -> (
          ...
          | (P_var b, t) ->
            ...
            let env' = if has_subst then Context.Typing.add_value context mhd t else Context.Typing.add_value context b.var t in (* BYE BYE *)
            (ptl , (body',env'))
          | (P_unit, t) ->
  ...
  | mhd::mtl ->
    ...
    let aux_p : O.type_expression option -> O.label * equations -> O.type_expression option * O.matching_content_case  =
      fun body_t (constructor,eq) ->
        ...
        let () = assert_body_t ~raise ~body_t nested.location nested.type_expression in (* BYE BYE*)
    ...
    let (body_t,present_cases) = List.fold_map present ~f:aux_p ~init:body_t in
    let body_t = Option.value_exn body_t in
...
let compile_matching ~raise (* BYE BYE *) ~err_loc (* BYE BYE *) ~type_f (* BYE BYE*) ~body_t (* BYE BYE*) matchee (eqs:equations) =

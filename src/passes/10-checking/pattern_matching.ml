...
**)
let rec substitute_var_in_body ~raise : Value_var.t -> Value_var.t -> O.expression -> O.expression =
    ...
    let aux ~raise : unit -> O.expression -> bool * unit * O.expression =
      fun () exp ->
        ...
        | (E_literal _ | E_constant _ | E_variable _ | E_application _ | E_lambda _ |
           E_type_abstraction _ | E_recursive _ | E_let_in _  | E_let_pattern_in _| E_mod_in _ |
            ...
            | P_tuple ps , t ->
              let aux i p =
                ...
                let v = match p.wrap_content with
                  | P_var _ -> p
                  | _ -> make_var_pattern (Value_var.fresh ~loc:p.location ~name:"_" ())
                in
                (v,field_t)
              in
              List.mapi ~f:aux ps

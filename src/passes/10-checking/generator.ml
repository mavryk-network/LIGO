      ...
      | Error (`Storage_does_not_match (ep_1, storage_1, ep_2, storage_2)) ->
        C.raise
          ...
          @@ Format.asprintf
               ...
               Type.pp
               storage_2)
      | Error `Optional_type_entrypoint ->
        C.raise (corner_case @@ Format.asprintf "@[<hv>Dunno wat to do :( @]")
      | Ok (p, s) -> return (p, s)
    in
    let type_binder = default_parameter_var in
    let parameter_type_decl = Signature.S_type (type_binder, Some parameter_type) in
    let contract_type = Type.build_entry_type parameter_type storage_type in
    let contract_decl =
      Signature.S_value
        (default_built_entrypoint_var, Some contract_type, Context.Attr.default)
    in
    let views_type = Type.t_views ~loc:Location.generated storage_type () in
    let views_decl =
      Signature.S_value (default_views_var, Some views_type, Context.Attr.default)
    in
    let mcontract_type =
      Type.t_pair ~loc:Location.generated contract_type views_type ()
    in
    let mcontract_decl =
      Signature.S_value (default_contract_var, Some mcontract_type, Context.Attr.default)
    in

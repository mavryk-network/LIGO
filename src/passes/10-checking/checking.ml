...
and infer_t_insts
  ...
  ~loc
  app_context
  ((tc, t) : O.expression_content * O.type_expression)
  =
  match t with
  | { type_content = T_for_all _; type_meta = _; orig_var = _; location = _ } ->
    ...
    let args =
      match App_context.pop app_context with
      | None -> []
      | Some args -> args
    in
    ...
    (match type_no_arrows.type_content with
     | T_constant { injection = External s; parameters = _; _ }
       when String.is_prefix s ~prefix:"u_" ->
       let args = O.Helpers.destruct_tuples args in
       let t, table, ot =
         Constant_typers.external_typers ~raise ~options loc s args last
       in
       ...
       let table =
         List.fold_right
           z
           ~f:(fun (av, t) table -> Inference.TMap.add av t table)
           ~init:table
       in
...
and type_expression ~raise ~options
  : context -> ?tv_opt:O.type_expression -> I.expression -> O.expression
  =
 fun (app_context, context) ?tv_opt e ->
  Context.Hashes.set_context context;
  let self ?(raise = raise) ?(context = app_context, context) ?tv_opt e =
    type_expression ~raise ~options context ?tv_opt e
  in
  let return expr tv =
    let () =
      match tv_opt with
      | None -> ()
      | Some tv' -> assert_type_expression_eq ~raise e.location (tv', tv)
    in
    let location = e.location in
    make_e ~location expr tv
  in
  let return_e (expr : O.expression) =
  ...
  | E_variable name ->
    let tv' =
      ...
      @@ Typing_context.get_value context name
    in
    let tc, tv =
      infer_t_insts ~raise ~options ~loc:e.location app_context (E_variable name, tv')
    in
    return tc tv
  | E_literal Literal_unit -> return (E_literal Literal_unit) (t_unit ())
  | E_literal (Literal_string s) -> return (E_literal (Literal_string s)) (t_string ())
  | E_literal (Literal_key s) -> return (E_literal (Literal_key s)) (t_key ())
  ...
  | E_literal (Literal_bytes s) -> return (E_literal (Literal_bytes s)) (t_bytes ())
  | E_literal (Literal_int n) -> return (E_literal (Literal_int n)) (t_int ())
  | E_literal (Literal_nat n) -> return (E_literal (Literal_nat n)) (t_nat ())
  | E_literal (Literal_timestamp n) ->
    return (E_literal (Literal_timestamp n)) (t_timestamp ())
  | E_literal (Literal_mutez n) -> return (E_literal (Literal_mutez n)) (t_mutez ())
  | E_literal (Literal_address s) -> return (e_address s) (t_address ())
  | E_literal (Literal_operation op) -> return (e_operation op) (t_operation ())
  | E_literal (Literal_bls12_381_g1 b) -> return (e_bls12_381_g1 b) (t_bls12_381_g1 ())
  | E_literal (Literal_bls12_381_g2 b) -> return (e_bls12_381_g2 b) (t_bls12_381_g2 ())
  | E_literal (Literal_bls12_381_fr b) -> return (e_bls12_381_fr b) (t_bls12_381_fr ())
  | E_literal (Literal_chest _ | Literal_chest_key _) ->
    failwith "chest / chest_key not allowed in the syntax (only tests need this type)"
  | E_constructor { constructor = Label s as constructor; element }
    when String.equal s "M_left" || String.equal s "M_right" ->
    let t =
      trace_option ~raise (michelson_or_no_annotation constructor e.location) @@ tv_opt
    in
     ...
     | T_sum c ->
       ...
       let () =
         assert_type_expression_eq
           ~raise
           expr'.location
           (associated_type, expr'.type_expression)
       in
  ...
  | E_constructor { constructor; element } ->
      ...
      | None ->
        ...
        (match matching_t_sum with
         | (v_ty, tvl, c_arg_t, sum_t) :: ignored ->
           let () =
             warn_ambiguous_constructor ~raise e.location (v_ty, c_arg_t) ignored
           in
           tvl, c_arg_t, sum_t
    ...
    let table =
      Inference.infer_type_application
        ~raise
        ~loc:element.location
        avs
        Inference.TMap.empty
        c_arg_t
        c_arg.type_expression
    in
    let () =
      if Option.is_none tv_opt
      then
        trace_option ~raise (not_annotated e.location)
        @@
        if List.for_all avs ~f:(fun v -> O.Helpers.TMap.mem v table)
        then Some ()
        else None
    in
    ...
    let () =
      assert_type_expression_eq ~raise c_arg.location (c_t, c_arg.type_expression)
    in
  ...
  | E_record m ->
    let field_types_opt =
      ...
      let* x =
        match List.zip (List.map ~f:snd x) (O.LMap.to_list m) with
        | Ok x -> Some x
    ...
    let _, lmap =
      O.LMap.fold_map
        ~f:(fun (Label k) e i ->
          ...
          in
          ( i + 1
          , ({ associated_type = get_type e; michelson_annotation = None; decl_pos }
              : O.row_element) ))
        m'
        ~init:0
    in
    let record_type =
      match Typing_context.get_record lmap context with
  ...
  | E_record_accessor { record; path } ->
    ...
    let aux (prev : O.expression) (a : I.label) : O.expression =
      let property = a in
      let r_tv =
        trace_option ~raise (expected_record e.location @@ get_type prev)
        @@ get_t_record prev.type_expression
      in
      let tv =
        trace_option ~raise (bad_record_access property prev e.location)
        @@ O.LMap.find_opt property r_tv.content
      in
      let location = e.location in
      make_e
        ~location
        (E_record_accessor { record = prev; path = property })
        tv.associated_type
    in
    let e = aux e' path in
  ...
  | E_record_update { record; path; update } ->
    ...
    let tv =
      ...
      | T_record { content; _ } ->
        let O.{ associated_type; _ } =
          trace_option ~raise (bad_record_access path record update.location)
          @@ O.LMap.find_opt path content
        in
        associated_type
      | _ ->
        failwith
          (Format.asprintf
             "Update an expression which is not a record %a"
  ...
  (* Data-structure *)
  | E_lambda lambda ->
    let lambda, lambda_type =
      type_lambda ~raise ~options ~loc:e.location ~tv_opt (app_context, context) lambda
    in
  ...
  | E_constant
      { cons_name = (C_LIST_ITER | C_MAP_ITER | C_SET_ITER | C_ITER) as opname
      ; arguments =
          [ ({ expression_content =
                 I.E_lambda
                   { binder = { var = lname; ascr = None; attributes }
                   ; output_type = None
                   ; result
                   }
             ; location = _
    ...
    let lambda' =
      make_e
        (E_lambda
           { binder = { var = lname; ascr = Some input_type; attributes }; result = body })
        tv_lambda
    in
  ...
  | E_constant
      { cons_name = C_LOOP_LEFT as opname
      ; arguments =
          [ ({ expression_content =
                 I.E_lambda
                   { binder = { var = lname; ascr = None; attributes }
    ...
    let lambda' =
      make_e
        (E_lambda
           { binder = { var = lname; ascr = Some input_type; attributes }; result = body })
        (t_arrow input_type output_type ())
    in
    ...
    let tv =
      ...
      | None ->
        (match cst with
         ...
         | _ ->
           failwith
             "Only C_SET_ADD and C_CONS are possible because those were the two cases \
              matched above")
    in
  ...
  | E_constant { cons_name = C_MAP_ADD as cst; arguments = [ key; value; map ] } ->
    ...
    let map' =
      try_with
        (fun ~raise ~catch:_ ->
          self ~raise ~context:(app_context, context) ~tv_opt:tv map)
        (fun ~catch:_ _ ->
          let tv =
            match tv_opt with
            | Some tv -> tv
  ...
  | E_constant { cons_name; arguments } ->
    ...
    let tv_lst = List.map ~f:get_type lst' in
    let name', tv = type_constant ~raise ~options cons_name e.location tv_lst tv_opt in
  ...
  | E_application { lamb = ilamb; args = _ } ->
    ...
    let lamb =
      self ~context:(App_context.push tv_opt args_ty app_context, context) lamb
    in
    (* Arguments are checked *)
    let (_ : _ Inference.TMap.t) =
      Inference.infer_type_applications
        ~raise
        ~loc:e.location
        []
        lamb.type_expression
        args_ty
        tv_opt
    in
    (* Re-build term (i.e. re-add applications) *)
    let app =
      trace_option ~raise (should_be_a_function_type lamb.type_expression ilamb)
      @@ O.Helpers.build_applications_opt lamb args
    in
    return_e app
  (* Advanced *)
  | E_matching { matchee; cases } ->
    let matchee' = self ~context:(app_context, context) matchee in
    let cases = List.mapi ~f:(fun i x -> i, x) cases in
    (* index the cases to keep the order in which they are written *)
    let type_cases
      ~raise
      (cases : (int * (S.expression, S.type_expression) S.match_case) List.Ne.t)
      =
      let cases = List.Ne.to_list cases in
      List.fold_map cases ~init:tv_opt ~f:(fun tv_opt (i, { pattern; body }) ->
        let context, pattern =
          type_pattern ~raise pattern matchee'.type_expression context
        in
        match tv_opt with
        | Some tv_opt ->
          let body =
            self ~raise ~context:(App_context.create (Some tv_opt), context) ~tv_opt body
          in
    ...
    let eqs =
      ...
      List.map ~f:(fun (p, p_ty, body, _) -> p, p_ty, body)
      @@ List.sort infered_eqs ~compare:(fun (_, _, _, a) (_, _, _, b) -> Int.compare a b)
    in
    ...
    (match matchee.expression_content with
     | E_variable matcheevar ->
       let case_exp =
         Pattern_matching.compile_matching ~raise ~err_loc:e.location matcheevar eqs
       in
  ...
  | E_let_in { let_binder = { var; ascr; attributes }; rhs; let_result; attr } ->
    ...
    let context =
      List.fold av ~f:(fun c v -> Typing_context.add_type_var c v ()) ~init:context
    in
    ...
    return
      (E_let_in { let_binder = { var; ascr = tv; attributes }; rhs; let_result; attr })
      let_result.type_expression
  ...
  | E_raw_code { language; code } ->
    let code, type_expression =
      trace_option ~raise (not_annotated code.location)
  ...
  | E_recursive { fun_name; fun_type; lambda } ->
    ...
    let e' =
      List.fold_left av ~init:e' ~f:(fun e v -> Typing_context.add_type_var e v ())
    in
    let lambda, lambda_type =
      type_lambda ~raise ~loc:e.location ~options ~tv_opt (app_context, e') lambda
    in
  ...
  | E_module_accessor { module_path; element } ->
    let f acc el =
      trace_option
        ~raise
        (unbound_module_variable el (I.ModuleVar.get_location el))
        (Typing_context.get_module acc el)
    in
    let module_env = List.fold ~init:context ~f module_path in
    let tv' =
      trace_option ~raise (unbound_variable element e.location)
      @@ Typing_context.get_value module_env element
    in
    let tc, tv =
      infer_t_insts
        ~raise
        ...
        ~loc:e.location
        app_context
        (E_module_accessor { module_path; element }, tv')
    in
    return tc tv
  | E_assign { binder; expression } ->
    let variable_type =
      trace_option
        ~raise
        (unbound_variable binder.var (O.ValueVar.get_location binder.var))
      @@ Typing_context.get_value context binder.var
    in
    let binder = { binder with ascr = Some variable_type } in
    let expression = self expression in
    let expression_type = expression.type_expression in
    let () =
      assert_type_expression_eq ~raise e.location (variable_type, expression_type)
    in
...
and type_pattern
  ~raise
  (pattern : I.type_expression I.pattern)
  (expected_typ : O.type_expression)
  context
  =
  match pattern.wrap_content, expected_typ.type_content with
  | I.P_unit, O.T_constant { injection = Stage_common.Constant.Unit; _ } ->
    context, Location.wrap ~loc:pattern.location O.P_unit
  | I.P_unit, _ -> raise.error (wrong_type_for_unit_pattern pattern.location expected_typ)
  | I.P_var v, _ ->
    ( Context.Typing.add_value context v.var expected_typ
    , Location.wrap ~loc:pattern.location (O.P_var { v with ascr = Some expected_typ }) )
  | ( I.P_list (I.Cons (hd, tl))
    , O.T_constant { injection = Stage_common.Constant.List; parameters; _ } ) ->
    let list_elt_typ = List.hd_exn parameters in
    (* TODO: dont use _exn*)
  ...
  | ( I.P_list (I.List lst)
    ...
    (* TODO: dont use _exn*)
    let context, lst =
      List.fold_right lst ~init:(context, []) ~f:(fun pattern (context, lst) ->
  ...
  | I.P_record (labels, patterns), O.T_record record_type ->
    ...
    then raise.error @@ pattern_do_not_conform_type pattern expected_typ
      ...
      let label_patterns = List.zip_exn labels patterns in
      (* TODO: dont use _exn*)
      let label_patterns =
        List.sort ~compare:(fun (l1, _) (l2, _) -> O.compare_label l1 l2) label_patterns
      in
      let context, labels, patterns =
        List.fold_right
          label_patterns
          ~init:(context, [], [])
          ~f:(fun (label, pattern') (context, labels, patterns) ->
          let c = O.LMap.find_opt label label_map in
          let c =
            trace_option ~raise (pattern_do_not_conform_type pattern expected_typ) c
          in
          ...
          context, label :: labels, pattern :: patterns)
      in
...
and type_lambda ~raise ~options ~loc ~tv_opt (ac, e) { binder; output_type; result } =
  ...
  in
  let ascr_i_t = Option.map ~f:(evaluate_type ~raise e) binder.ascr in
  let ascr_o_t = Option.map ~f:(evaluate_type ~raise e) output_type in
  let input_type =
    match ascr_i_t, top_i_t with
    | Some t1, Some t2 ->
      assert_type_expression_eq ~raise loc (t1, t2);
      Some t2
    | _, Some t -> Some t
    | Some t, _ -> Some t
    | _ -> None
  ...
  in
  let input_type =
    trace_option ~raise (missing_funarg_annotation binder.var) input_type
  in
  ...
  let output_type = body.type_expression in
  ( ({ binder = { binder with ascr = Some input_type }; result = body } : O.lambda)
  , t_arrow input_type output_type () )
...
and type_constant
  ...
  (name : I.constant')
  (loc : Location.t)
  (lst : O.type_expression list)
  (tv_opt : O.type_expression option)
  : O.constant' * O.type_expression
  =
...
and type_module_expr ~raise ~init_context ~options
  : I.module_expr -> typing_context * O.module_expr
  =
 fun m_expr ->
  let return x =
  ...
  let access_module ctxt v =
    trace_option
      ~raise
      (unbound_module_variable v (I.ModuleVar.get_location v))
      (Typing_context.get_module ctxt v)
  in
  ...
  | I.M_module_path path ->
    let _ctxt : typing_context =
      List.fold ~f:access_module ~init:init_context (List.Ne.to_list path)
    in
...
and type_declaration
  :  raise:(typer_error, _) raise -> options:Compiler_options.middle_end -> typing_context
  -> I.declaration -> typing_context * O.declaration
  =
 fun ~raise ~options c d ->
  ...
  | Declaration_type { type_binder; type_expr; type_attr = { public; hidden } } ->
    ...
    return env'
    @@ Declaration_type { type_binder; type_expr = tv; type_attr = { public; hidden } }
  | Declaration_constant { binder = { ascr; var; attributes }; attr; expr } ->
    ...
    let expr =
      ...
      @@ type_expression ~options ?tv_opt:tv (App_context.create tv, env) expr
    in
    ...
    return c
    @@ Declaration_constant { binder = { ascr = tv; var; attributes }; expr; attr }
...
and type_module ~raise ~options ~init_context (p : I.module_) : O.module_ =
  ...
  let _c, lst =
    List.fold_map ~f:(type_declaration ~raise ~options) ~init:init_context p
  in
  lst


let type_program ~raise ~options ?env m =
  type_module ~raise ~options ~init_context:(Typing_context.init ?env ()) m


let type_declaration ~raise ~options ?env d =
  snd @@ type_declaration ~raise ~options (Typing_context.init ?env ()) d
...
let type_expression ~raise ~options ?env ?tv_opt e =
  type_expression
    ~raise
    ~options
    (App_context.create tv_opt, Typing_context.init ?env ())
    ?tv_opt
    e

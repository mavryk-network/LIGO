...
and infer_expression ~(raise : raise) ~options ~ctx (expr : I.expression)
  ...
  let ctx, type_, expr' =
    ...
    | E_let_in { let_binder; rhs; let_result; attributes } ->
      ...
      let ctx, _sigs, let_binder = check_pattern ~raise ~options ~ctx ~mut:false let_binder rhs_type in
  ...
  | E_let_mut_in { let_binder; rhs; let_result; attributes } ->
    ...
    let ctx, _sigs, let_binder = check_pattern ~raise ~options ~ctx ~mut:true let_binder rhs_type in
    let ctx, res_type, let_result =
      Context.enter ~ctx ~mut:false ~in_:(fun ctx ->
          infer
            ~ctx 
...
and infer_pattern
  ...
  in
  let infer ~ctx pat = infer_pattern ~raise ~options ~ctx ~mut pat in
  let check ~ctx pat type_ = check_pattern ~raise ~options ~ctx ~mut pat type_ in
...
and check_pattern
  ...
  let return content =
    return @@ (Location.wrap ~loc content : O.type_expression O.Pattern.t)
  in
  let self ?(raise = raise) = check_pattern ~raise ~options ~mut in
  let infer ~ctx pat = infer_pattern ~raise ~options ~ctx pat ~mut in
  let ctx, sigs, pat =
...
and check_cases
  ...
  let ctx, cases =
    List.fold_map ~init:ctx cases ~f:(fun ctx { pattern; body } ->
        ...
        let ctx, _, pattern = check_pattern ~raise ~options ~ctx ~mut:false pattern matchee_type in
...
and infer_declaration ~(raise : raise) ~options ~ctx (decl : I.declaration)
  ...
  let ctx, (sig_items : Signature.item list), decl' =
    ...
    | D_pattern { pattern; expr; attr } ->
      ...
      let attr = type_value_attr attr in
      let matchee_type = Context.apply ctx matchee_type in
      let ctx, sigs ,pattern = check_pattern ~raise ~options ~ctx ~mut:false pattern matchee_type in
      if debug then Format.printf "Ctx After Decl: %a\n" Context.pp ctx;
      ( ctx
      , sigs
      , let%bind expr = expr

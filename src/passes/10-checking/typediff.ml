...
let rows_to_te_list : rows -> ty_expr list =
 fun r ->
...
let rows_to_te_array : rows -> ty_expr array =
 fun r -> Array.of_list @@ List.rev @@ rows_to_te_list r


module rec TeArg : sig
  val weight : Define.change -> int
  val test : unit -> type_expression -> type_expression -> (unit, unit) result
  ...
  let rec weight : Define.change -> int = function
    ...
    | Change (te1, te2, _) ->
      ...
      | T_record r1, T_record r2 ->
        ...
        total_weight
      | T_record r, _ | _, T_record r ->
        1 + (List.length @@ rows_to_te_list r)
...
let get_diff : type_expression -> type_expression -> t =
 fun t1 t2 ->
  List.rev
  @@
  ...
  | T_record r1, T_record r2
    when Record.is_tuple r1.fields && Record.is_tuple r2.fields ->
...
module PP = struct
  ...
  *)
  ...
  let t (no_colour : bool) ppf (patch : t) : unit =
    if not no_colour then ANSI.add_ansi_marking ppf;
    match patch with
    | [] -> Format.fprintf ppf ""
    | _ ->
      Format.fprintf

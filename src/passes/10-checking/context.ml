...
module HMap = Simple_utils.Map.Make (struct
  type t = type_expression

  let compare t1 t2 =
    Int.compare (Hash.hash_type_expression t1) (Hash.hash_type_expression t2)
end)
...
module Typing = struct
  ...
  let empty : t =
    { values = Types.ValueMap.empty
    ; types = Types.TypeMap.empty
    ; modules = Types.ModuleMap.empty
    }
  ...
  module PP = struct
    ...
    and context ppf { values; types; modules } =
      fprintf
        ppf
        "context:@,{[@[<v 2>@,%a; @,%a; @,%a; ]}@]"
        (print_list value_binding)
        (ValueMap.to_kv_list values)
        (print_list type_binding)
        (TypeMap.to_kv_list types)
        (print_list module_binding)
  ...
  let union : t -> t -> t =
   fun a b ->
    ...
    in
    Types.
      { values = ValueMap.merge merger a.values b.values
      ; types = TypeMap.merge a.types b.types
      ; modules = ModuleMap.merge a.modules b.modules
      }
  ...
  let add_value : t -> Ast_typed.expression_variable -> Ast_typed.type_expression -> t =
   fun c ev te ->
    let values = Types.ValueMap.add ev te c.values in
    { c with values }


  let add_type : t -> Ast_typed.type_variable -> Ast_typed.type_expression -> t =
   fun c tv te ->
  ...
  let add_type_var : t -> Ast_typed.type_variable -> unit -> t =
   fun c tv () -> add_type c tv (Ast_typed.t_variable tv ())


  (* we use type_var while we don't have kind checking *)
  let add_kind : t -> Ast_typed.type_variable -> unit -> t =
   fun c tv () -> add_type_var c tv ()


  let add_module : t -> Ast_typed.module_variable -> t -> t =
   fun c mv te ->
  ...
  let get_value (e : t) =
    List.Assoc.find ~equal:Ast_typed.ValueVar.equal @@ Types.ValueMap.to_kv_list e.values


  let get_type (e : t) =
    List.Assoc.find ~equal:Ast_typed.TypeVar.equal @@ Types.TypeMap.to_kv_list e.types


  let get_module (e : t) =
    List.Assoc.find ~equal:Ast_typed.ModuleVar.equal
    @@ Types.ModuleMap.to_kv_list e.modules


  let get_type_vars : t -> Ast_typed.type_variable list =
   fun { values = _; types; modules = _ } ->
    fst @@ List.unzip @@ Types.TypeMap.to_kv_list types
      ...
      let f : t -> Ast_typed.declaration -> t =
       fun acc d ->
        ...
        | Declaration_module { module_binder; module_; module_attr = { public; _ } } ->
          ...
          then (
            let context =
              context_of_module_expr ~outer_context:(union acc outer_context) module_
            in
    ...
    let rec rec_aux e =
      ...
      | None ->
        ...
        List.fold_left
          ...
          ~init:None
          (Types.ModuleMap.to_kv_list modules)
    in
    rec_aux e
end

  ...
  memoize2
    ...
    (fun ctor ctxt ->
      (let filter_tsum (var, type_) =
         ...
         match type_.type_content with
         | T_sum m ->
           (match Map.find m.fields ctor with
           | Some { content = { associated_type; _ }; _ } ->
...
let get_record
  ...
  =
  let record_hashable : type_expression Rows.fields hashable =
    (module struct
      type t = type_expression Rows.fields

      let equal = Rows.equal_fields equal_type_expression
      let hash = Hash.of_fold (Rows.hash_fold_fields hash_fold_type_expression)
    end)
  in
  memoize2 record_hashable hashable (fun fields ctx ->
  ...
  and type_expr ~ctx t : Kind.t option =
    ...
    let rec loop (t : type_expression) ~ctx =
      ...
      | T_sum rows | T_record rows ->
        if Map.for_all rows.fields ~f:(fun (row_elem : _ Rows.Elem.t) ->
               match self row_elem.content.associated_type with
               | Some Type -> true
               | _ -> false)
        then return Type
        else None
      | T_tuple tup ->
        if List.for_all tup ~f:(fun type_ ->
...
module Elaboration = struct
  ...
  and row_apply ctx (row : _ Rows.t) = Rows.map (t_apply ctx) row
  ...
  let rec e_apply ctx expr =
    ...
    | E_skip -> E_skip
    | E_cond cond -> E_cond (Conditional.map self cond)
    | E_sequence seq -> E_sequence (Sequence.map self seq)
    | E_tuple tup -> E_tuple (Tuple.map self tup)
    | E_set set_expr -> E_set (Set_expr.map self set_expr)
    | E_map map_expr -> E_map (Map_expr.map self map_expr)
    | E_big_map map_expr -> E_big_map (Map_expr.map self map_expr)
    | E_list list_expr -> E_list (List_expr.map self list_expr)
  ...
  and matching_expr_apply ctx match_expr =
    ...
    | Match_record { fields; body; tv } ->
      Match_record
        { fields = Map.map ~f:(fun binder -> binder_apply ctx binder) fields
        ; body = e_apply ctx body
        ; tv = t_apply ctx tv
        }
    | Match_tuple { binders; body; tv } ->
      Match_tuple
        { binders = List.map ~f:(binder_apply ctx) binders
        ; body = e_apply ctx body
        ; tv = t_apply ctx tv
        }
  ...
  let all_lmap lmap ~raise = Map.map ~f:(fun t -> t ~raise) lmap
  ...
  let type_pass ~raise (type_ : type_expression) : unit =
    ...
    let rec loop type_ =
      ...
      | T_record rows | T_sum rows ->
        Map.iter rows.fields ~f:(fun row_elem ->
            loop row_elem.content.associated_type)
      | T_arrow { type1; type2 } ->
        loop type1;
        loop type2
      | T_singleton _ -> ()
      ...
      | T_tuple tup -> List.iter tup ~f:loop
    in
    loop type_
  ...
  let rec expression_pass ~raise expr =
    ...
    | E_record record -> Map.iter record ~f:self
    ...
    | E_sequence { expr1; expr2 } ->
      self expr1;
      self expr2
    | E_tuple tup -> List.iter ~f:self tup
    | E_set set_expr -> List.iter ~f:self set_expr
    | E_list list_expr -> List.iter ~f:self list_expr
    | E_map map_expr | E_big_map map_expr ->
      List.iter map_expr ~f:(fun (expr1, expr2) ->
          self expr1;
          self expr2)
  ...
  and matching_expr_pass ~raise match_expr =
    ...
    | Match_record { fields; body; tv } ->
      type_pass ~raise tv;
      Map.iter fields ~f:(binder_pass ~raise);

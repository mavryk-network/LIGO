...
let[@landmark] insert_at t ~at ~hole =
  let t1, t2 = split_at t ~at in
  t1 |@ hole |@ t2


let[@landmark] add_exists_eq t evar kind type_ =
  let t1, t2 = split_at t ~at:(C_exists_var (evar, kind)) in
  t1 |@ of_list [ C_exists_eq (evar, kind, type_) ] |@ t2


let[@landmark] rec apply t (type_ : type_expression) : type_expression =
  let self = apply t in
...
let[@landmark] get_signature t ((local_module, path) : Module_var.t List.Ne.t) =
...
module Elaboration = struct
  ...
  let[@landmark] rec t_apply ctx (type_ : type_expression) : type_expression =
  ...
  let[@landmark] rec e_apply ctx expr =
  ...
  and[@landmark] declaration_apply ctx decl : declaration =
  ...
  let[@landmark] type_pass ~raise (type_ : type_expression) : unit =
    let fail () =
  ...
  let[@landmark] rec expression_pass ~raise expr =
  ...
  and[@landmark] decl_pass ~raise (decl : decl) = declaration_pass ~raise decl

  and[@landmark] declaration_pass ~raise (decl : declaration) =
    match decl.wrap_content with
...
let[@landmark] decl_enter ~ctx ~in_ =
  let ctx, pos = mark ctx in
...
module Generalization = struct
  let[@landmark] generalize_type ~tvars type_ =
  ...
  let[@landmark] unsolved { items; solved } =
    let solved =
      List.fold items ~init:solved ~f:(fun solved item ->
        match item with
        | C_exists_eq (evar, kind, type_) ->

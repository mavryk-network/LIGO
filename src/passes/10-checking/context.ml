...
and signature_of_module : ctx:t -> Ast_typed.module_ -> Signature.t =
 fun ~ctx module_ ->
  ...
  | decl :: module_ ->
    ...
    let sig_ = signature_of_module ~ctx:(add_signature_items ctx sig_item) module_ in
    ...
    Environment.fold env ~init:empty ~f:(fun ctx decl ->
        ...
        | D_pattern { pattern; expr = _; attr = _ } ->
          List.fold (Ast_typed.Pattern.binders pattern) ~init:ctx ~f:(fun ctx x ->
              add_imm ctx (Binder.get_var x) (Binder.get_ascr x))
...
module Elaboration = struct
  ...
  and pattern_apply ctx (binder : 'a Ast_typed.Pattern.t) =
    Pattern.map (t_apply ctx) binder
  ...
  and declaration_apply ctx decl : declaration =
    ...
    | D_value { binder; expr; attr } ->
      return
      @@ D_value { binder = binder_apply ctx binder; expr = e_apply ctx expr; attr }
    | D_pattern { pattern; expr; attr } ->
      let pattern = Ast_typed.Pattern.map (t_apply ctx) pattern in
      return @@ D_pattern { pattern; expr = e_apply ctx expr; attr }

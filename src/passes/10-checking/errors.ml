...
type typer_error =
  ...
  | `Typer_cannot_unify of
    bool * Location.t * Ast_typed.type_expression * Ast_typed.type_expression
 ...
 fun ~display_format f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Typer_bad_collect_loop (t, loc) ->
      Format.fprintf
        f
        "@[<hv>%a@.Bounded loop over a value with an incorrect type.@.Expected \
         a value with type: \"list\", \"set\" or \"map\", but got a value of \
         type \"%a\". @]"
        Snippet.pp
        loc
        Ast_typed.PP.type_expression
    ...
    | `Typer_expected_map (loc, e) ->
      Format.fprintf
        f
        "@[<hv>%a@.Incorrect argument.@.Expected a map, but got an argument of \
         type \"%a\". @]"
        Snippet.pp
        loc
        Ast_typed.PP.type_expression
...
let rec error_json : typer_error -> Simple_utils.Error.t =
 fun e ->
  ...
  | `Typer_cannot_unify_diff_layout (location, type1, type2, layout1, layout2)
    ...
    let message =
      Format.asprintf
        "Invalid type(s)@.Cannot unify %a with %a due to differing layouts (%a \
         and %a)."
        Ast_typed.PP.type_expression
        type1
        Ast_typed.PP.type_expression
  ...
  | `Typer_missing_funarg_annotation v ->
    let message =
      Format.asprintf
        "Missing a type annotation for argument \"%a\"."
        Value_var.pp
        v
    in
  ...
  | `Typer_unbound_mut_variable (v, location) ->
    ...
    let content = make_content ~message ~location () in
    make ~stage ~content
  | `Typer_mismatching_for_each_collection_type
      (location, collection_type, type_) ->
    let message =
      Format.asprintf
  ...
  | `Typer_michelson_or_no_annotation (c, location) ->
    let message =
      Format.asprintf
        "Incorrect usage of type \"michelson_or\".@.The contructor \"%a\" must \
  ...
  | `Typer_match_error (expected, actual, location) ->
    let message =
      Format.asprintf
        "Pattern matching over an expression of an incorrect type.@.Type \
  ...
  | `Typer_bad_collect_loop (t, location) ->
    let message =
      Format.asprintf
        "Bounded loop over a value with an incorrect type.@.Expected a value \
         with type: \"list\", \"set\" or \"map\", but got a value of type \
         \"%a\"."
        Ast_typed.PP.type_expression
        (type_improve t)
    in
  ...
  | `Typer_comparator_composed (location, _a) ->
    let message =
      Format.asprintf
        "Invalid arguments.@.Only composed types of not more than two element \
  ...
  | `Typer_expected_variant (location, t) ->
    let message =
      Format.asprintf
        "Invalid argument.@.Expected a variant, but got an argument of type \
         \"%a\"."
        Ast_typed.PP.type_expression
        (type_improve t)
    in
  ...
  | `Typer_wrong_param_number (location, name, expected, actual) ->
    let message =
      Format.asprintf
        "Function \"%s\" called with wrong number of arguments.@.Expected %d \
  ...
  | `Typer_expected_map (location, e) ->
    let message =
      Format.asprintf
        "Incorrect argument.@.Expected a map, but got an argument of type \
         \"%a\"."
        Ast_typed.PP.type_expression
        (type_improve e)
    in
  ...
  | `Typer_expected_option (location, e) ->
    let message =
      Format.asprintf
        "Incorrect argument.@.Expected an option, but got an argument of type \
         \"%a\"."
        Ast_typed.PP.type_expression
        (type_improve e)
    in
  ...
  | `Typer_typeclass_error (location, exps, acts) ->
    ...
    let message =
      Format.asprintf
        "Invalid arguments.@.Expected an argument of type %a, but got an \
         argument of type %a."
        (list_sep (printl Ast_typed.PP.type_expression) (const " or "))
        exps
        (list_sep Ast_typed.PP.type_expression (const ", "))
  ...
  | `Typer_expected (location, exps, acts) ->
    ...
    let message =
      Format.asprintf
        "Cannot match arguments for operation.@.Expected arguments with \

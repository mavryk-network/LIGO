...
let rec type_mapper ~f (t : Ast_typed.type_expression) =
    ...
    let fields =
      Map.map
        ...
        ~f:(fun
           ...
           ->
          { content =
              { Rows.Elem.associated_type = self associated_type; decl_pos }
  ...
  | T_sum { fields; attributes } ->
    ...
    in
    return @@ T_sum { fields; attributes }
  | _ -> t
...
let decompile_pattern_to_string ~syntax:_ _pattern =
  assert false
  (* let open Tree_abstraction in *)
  (* let p = Pattern.map Decompiler.decompile_type_expression_option pattern in
  let s =
    match syntax with
    | Some Syntax_types.JsLIGO -> Jsligo.decompile_pattern_to_string p
    | Some CameLIGO -> Cameligo.decompile_pattern_to_string p
    | Some ReasonLIGO -> Reasonligo.decompile_pattern_to_string p
    | Some PascaLIGO -> Pascaligo.decompile_pattern_to_string p
    | None -> Cameligo.decompile_pattern_to_string p
  in
  s *)
    ...
    | `Typer_pattern_missing_cases (loc, syntax, ps) ->
      let ps =
        List.fold ps ~init:"" ~f:(fun s p ->
            let s' =
              decompile_pattern_to_string ~syntax p
            in
            s ^ "- " ^ s' ^ "\n")
      in
      Format.fprintf
...
let rec error_jsonformat : typer_error -> Yojson.Safe.t =
 fun a ->
  ...
  | `Typer_pattern_missing_cases (loc, syntax, ps) ->
    ...
    let patterns =
      List.map ps ~f:(fun p ->
          `String (decompile_pattern_to_string ~syntax p))
    in
    let content =
      `Assoc
        [ "message", message

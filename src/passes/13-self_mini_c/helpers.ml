...
type 'err mapperb = raise:'err raise -> expression -> (expression * bool)

let rec mapb_expression ~raise : 'err mapperb -> expression -> expression = fun f e ->
  let self = mapb_expression ~raise f in
  let e', b = f ~raise e in
  let return content = { e' with content } in
  if b then e' else
  match e'.content with
  | E_variable _ | E_literal _ | E_raw_michelson _

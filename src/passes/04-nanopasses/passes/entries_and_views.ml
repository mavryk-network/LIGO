...
module EntriesSet = Set.Make (struct
  type t = Ligo_prim.Value_var.t [@@deriving ord, sexp]
end)
...
let get_views () =
  ...
  let views =
    List.map ~f:(Ligo_prim.Value_var.of_input_var ~loc:Location.generated) views
  in
  views
...
let rec strip_attribute ~raise ~pred d =
  ...
  match get_d d with
  | D_attr (attr, d) -> if pred attr then self d else make_d ~loc (D_attr (attr, self d))
  | D_export d -> make_d ~loc (D_export (self d))
  | d -> make_d ~loc d
...
let rec toplevel_wrap ~raise ~when_ ~wrap d =
  ...
  match get_d d with
  | D_attr (attr, d) -> make_d ~loc (D_attr (attr, self d))
  | D_export d -> make_d ~loc (D_export (self d))
  | D_const { pattern; _ }
  | D_let { pattern = pattern, _; _ }
  | D_multi_const ({ pattern; _ }, _) ->
    let entries = get_pattern_binders pattern in
    if when_ entries then wrap ~loc entries d else d
  | d -> make_d ~loc d
    ...
    | Top_level (prg : program) ->
      ...
      let prg =
        let wrap ~loc entries d =
          List.iter
            ~f:(fun entry -> _entries := EntriesSet.remove !_entries entry)
            entries;
      ...
      let prg =
        let pass =
          let pred ({ key; _ } : Attribute.t) =
            (not (List.is_empty (get_views ()))) && String.equal key "view"
          in
          strip_attribute ~raise ~pred
        in
        apply_pass ~pass prg
      ...
      let prg =
        ...
        let when_ binders =
          ...
          && List.exists binders ~f:(fun v ->
                 List.mem ~equal:Ligo_prim.Value_var.equal (get_views ()) v)
        in
        let pass = toplevel_wrap ~raise ~when_ ~wrap in
        apply_pass ~pass prg
      in

...
let field_of_property ~raise : expr Object_.property -> (Label.t, expr) Field.t =
 ...
 | F_Name l, Some expr ->
   Complete (l, expr)
 | F_Name l, None ->
   Punned (Location.wrap ~loc:Location.generated l)
 | _, Some expr -> raise.error @@ unsupported_object_field expr
 | _, None -> raise.error @@ unsupported_object_field (e_string ~loc:Location.generated "TODO")
...
let field_update_of_property ~raise : expr Object_.property -> expr Update.field =
  ...
  | F_Name l, Some expr ->
    Full_field
      { field_lhs = [ FieldName l ]
      ; field_lens = Lens_Id
      ; field_rhs = expr
      }
  | F_Name l, None ->
    Pun (Location.wrap ~loc:Location.generated l)
  | _, Some expr -> raise.error @@ unsupported_object_field expr
  | _, None -> raise.error @@ unsupported_object_field (e_string ~loc:Location.generated "TODO")

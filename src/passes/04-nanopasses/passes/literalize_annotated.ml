...
let compile ~raise =
  let expr : _ expr_ -> expr =
   fun e ->
    ...
    | E_annot (ae, ty) as e ->
      (match get_e ae with
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Key_hash) && Caml.(Ligo_string.get_type lit = `Standard) ->
        ...
        in
        e_key_hash ~loc s
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Signature) && Caml.(Ligo_string.get_type lit = `Standard) ->
        let s = Ligo_string.extract lit in
        let _ =
          trace_option ~raise (bad_format_literal ae s) (Signature.of_b58check_opt s)
        in
        e_signature ~loc (Ligo_string.extract lit)
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Key) && Caml.(Ligo_string.get_type lit = `Standard) ->
        let s = Ligo_string.extract lit in
        let _ =
          trace_option
            ~raise
      ...
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Chain_id) && Caml.(Ligo_string.get_type lit = `Standard) ->
        e_chain_id ~loc (Ligo_string.extract lit)
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Address) && Caml.(Ligo_string.get_type lit = `Standard) ->
      ...
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Bytes) && Caml.(Ligo_string.get_type lit = `Standard) ->
        let str = Ligo_string.extract lit in

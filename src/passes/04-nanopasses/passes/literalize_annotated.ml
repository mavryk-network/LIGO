...
let compile ~raise =
  let expr : _ expr_ -> expr =
   fun e ->
    ...
    | E_annot (ae, ty) as e ->
      (match get_e ae with
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Key_hash) ->
        ...
        in
        e_key_hash ~loc s
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Signature) ->
        let s = Ligo_string.extract lit in
        let _ =
          trace_option ~raise (bad_format_literal ae s) (Signature.of_b58check_opt s)
        in
        e_signature ~loc (Ligo_string.extract lit)
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Key) ->
        let s = Ligo_string.extract lit in
        let _ =
          trace_option
            ~raise
      ...
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Chain_id) ->
        e_chain_id ~loc (Ligo_string.extract lit)
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Address) ->
      ...
      | E_literal (Literal_string lit) when is_ty_var ty (to_string Bytes) ->
        let str = Ligo_string.extract lit in

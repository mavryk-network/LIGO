...
let tuple_to_list (p : pattern) =
  let loc, p = destruct_p p in
  let rec finishes_in_ellipsis (ps : pattern element_pattern list) : pattern List.Ne.t option =
    match ps with
    | [] -> None
  ...
  | P_tuple_with_ellipsis ps -> (
      ...
      | Some ps ->
        let list_pattern = List.Ne.fold_right1 ps ~f:(fun p q -> make_p ~loc (P_list (Cons (p, q)))) in
        true, list_pattern
    )
  | P_typed (t, p) -> false, make_p ~loc (P_typed (t, p))
  | _ -> true, make_p ~loc p
...
let remove_ellipsis ~raise (p : pattern) =
  ...
  | P_tuple_with_ellipsis l ->
    let l = List.map ~f:(fun { pattern ; ellipsis } ->
        if ellipsis then raise.error (invalid_list_pattern_match (get_p_loc pattern)) else pattern) l in
...
let compile ~raise =
  ...
  let expr : _ expr_ -> expr =
   fun e ->
    ...
    | E_match_tc39 { subject
                   ; match_clauses = AllClauses (({ filter ; clause_expr }, []), None) } ->
      let clause =

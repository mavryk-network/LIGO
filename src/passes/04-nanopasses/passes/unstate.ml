...
let rec decl : declaration -> Statement_result.t =
 fun d ->
  ...
  | D_import (Import_all_as _ | Import_selected _) | D_module_include _ -> failwith "can't parse"
  | D_multi_var _ | D_multi_const _ | D_type_abstraction _  -> failwith "removed in previous passes"

...
let compile ~raise =
  let switch_to_unit =
    let instruction : (instruction, expr, pattern, statement, block) Types.instruction_ -> instruction =
      fun i ->
        let loc = Location.get_location i in
        let i = Location.unwrap i in
        match i with
        | I_switch { switchee; cases } ->
          let f (c : (expr, block) Switch.case) : (expr, block) Switch.case =
            let update_block b =
            ...
            in
            match c with
            | Switch_case (e, Some b) ->
              Switch.Switch_case (e, Some (update_block b))
            | Switch_default_case (Some b) ->
              Switch.Switch_default_case (Some (update_block b))
            | _ -> c
          in
          let cases = List.Ne.map f cases in
          make_i ~loc (I_switch { switchee; cases })
        | _ -> make_i ~loc i
    in
    Fold { Morphing.idle_fold with instruction }
  in
  let filter_continue_break =
    let instruction : (instruction, expr, pattern, statement, block) Types.instruction_ -> unit =
      fun i ->
        let loc = Location.get_location i in
        let i = Location.unwrap i in
        match i with

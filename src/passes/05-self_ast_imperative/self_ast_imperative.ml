...
let all_exp ~raise ~(lang:File_metadata.v_syntax) = 
  List.map 
...
let all_module ~raise ~add_warning ~(lang:File_metadata.v_syntax) init =
...
let all_expression ~raise ~(lang:File_metadata.v_syntax) init =

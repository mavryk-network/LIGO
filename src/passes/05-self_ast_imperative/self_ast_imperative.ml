...
let all_ty ~raise =
  List.map ~f:(fun el -> Helpers.Type_expression el)
  @@ all_type_expression_mapper ~raise


let all_program ~raise ~js_style_no_shadowing init =
  let all_p =
    List.map ~f:Helpers.map_program @@ all_exp ~raise ~js_style_no_shadowing
  in
...
let decompile_imperative init =
  let all_p =
    List.map ~f:Helpers.map_program

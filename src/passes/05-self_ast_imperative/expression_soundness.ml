...
let reserved_names_exp ~raise : expression -> expression = fun exp ->
  match exp.expression_content with
  | E_let_in {let_binder ; attributes ; _ } when not (List.mem ~equal:String.equal attributes "hidden") ->
...
let reserved_names_mod ~raise : module_ -> module_ = fun m ->
  let aux  = function
    ...
    | {wrap_content = Declaration_constant { attr ; _ }; _} when List.mem ~equal:String.equal attr "hidden" ->
      ()

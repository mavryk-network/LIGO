...
let check_linearity_patterns ~raise : expression -> unit =
 fun exp ->
  ...
  | E_matching x ->
    ...
    let rec aux : Value_var.t list -> type_expression option Pattern.t -> Value_var.t list
      =
     fun vlst p ->
      ...
      | P_list (Cons (p1, p2)) -> List.fold ~init:vlst ~f:aux [ p1; p2 ]
      | P_list (List lst) | P_tuple lst -> List.fold ~init:vlst ~f:aux lst

...
let peephole_expression ~raise : expression -> expression =
 fun e ->
  ...
  | E_ascription { anno_expr = e'; type_annotation = t } as ec ->
    ...
    | E_literal (Literal_string s), T_variable tv
      when Type_var.equal tv v_key_hash ->
      return @@ E_literal (Literal_key_hash (Ligo_string.extract s))
    | E_literal (Literal_string s), T_variable tv
      when Type_var.equal tv v_signature ->
  ...
  | E_raw_code
      { language
      ; code = { expression_content = E_literal (Literal_string code); _ }
      }

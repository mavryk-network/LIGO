...
let error_ppformat
  :  display_format:string display_format -> Format.formatter -> self_ast_imperative_error
  -> unit
  =
  ...
  | Human_readable | Dev ->
    (match a with
     ...
     | `Self_ast_imperative_long_constructor (c, e) ->
       Format.fprintf
         f
         "@[<hv>%a@ Ill-formed data constructor \"%s\".@.Data constructors have a \
          maximum length of 32 characters, which is a limitation imposed by annotations \
          in Tezos. @]"
         Snippet.pp
         e.location
         c

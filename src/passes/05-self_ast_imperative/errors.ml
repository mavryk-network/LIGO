 ...
 fun ~display_format f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Self_ast_imperative_duplicate_parameter exp ->
      Format.fprintf
        f
        "@[<v>%a@.Duplicated variable name in function parameter.@.Hint: \
         Change the name.@]"
        Snippet.pp
        exp.location
    | `Self_ast_imperative_long_constructor (c, e) ->
      Format.fprintf
        f
        "@[<hv>%a@ Ill-formed data constructor \"%s\".@.Data constructors have \
         a maximum length of 32 characters, which is a limitation imposed by \
         annotations in Tezos. @]"
        Snippet.pp
        e.location
        c
    | `Self_ast_imperative_bad_timestamp (t, e) ->
      Format.fprintf
        f
        "@[<hv>%a@ Ill-formed timestamp \"%s\".@.At this point, a string with \
         a RFC3339 notation or the number of seconds since Epoch is expected. \
         @]"
        Snippet.pp
        e.location
        t
    | `Self_ast_imperative_bad_format_literal e ->
      Format.fprintf
        f
        "@[<hv>%a@ Ill-formed literal \"%a\".@.In the case of an address, a \
         string is expected prefixed by either tz1, tz2, tz3 or KT1 and \
         followed by a Base58 encoded hash and terminated by a 4-byte \
         checksum.@.In the case of a key_hash, signature, or key a Base58 \
         encoded hash is expected. @]"
        Snippet.pp
        e.location
    ...
    | `Self_ast_imperative_bad_conversion_bytes e ->
      Format.fprintf
        f
        "@[<hv>%a@ Ill-formed bytes literal.@.Example of a valid bytes \
         literal: \"ff7a7aff\". @]"
        Snippet.pp
        e.location
    | `Self_ast_imperative_vars_captured vars ->
      let pp_var ppf ((decl_loc, var) : Location.t * Value_var.t) =
        Format.fprintf
          ppf
          "@[<hv>%a@ Invalid capture of non-constant variable \"%a\", declared \
           at@.%a@]"
          Snippet.pp
      ...
      Format.fprintf
        f
        "%a"
        (PP_helpers.list_sep pp_var (PP_helpers.tag "@."))
        vars
    | `Self_ast_imperative_const_assigned (loc, var) ->
      Format.fprintf
        f
        "@[<hv>%a@ Invalid assignment to constant variable \"%a\", declared \
         at@.%a@]"
        Snippet.pp
        loc
        Value_var.pp
...
let error_json : self_ast_imperative_error -> Simple_utils.Error.t =
 fun e ->
  ...
  | `Self_ast_imperative_non_linear_record e ->
    let message =
      Format.sprintf "Duplicated record field@.Hint: Change the name."
    in
  ...
  | `Self_ast_imperative_reserved_name (str, location) ->
    let message =
      Format.sprintf "Reserved name %S.@.Hint: Change the name." str
    in
    let content = make_content ~message ~location () in
    make ~stage ~content
  | `Self_ast_imperative_non_linear_pattern t ->
  ...
  | `Self_ast_imperative_duplicate_parameter exp ->
    let message =
      Format.sprintf
        "Duplicated variable name in function parameter.@.Hint: Change the \
         name."
    in
  ...
  | `Self_ast_imperative_long_constructor (c, e) ->
    let message =
      Format.sprintf
        " Ill-formed data constructor \"%s\".@.Data constructors have a \
         maximum length of 32 characters, which is a limitation imposed by \
         annotations in Tezos."
        c
    in
    let location = e.location in
  ...
  | `Self_ast_imperative_bad_timestamp (t, e) ->
    let message =
      Format.sprintf
        "Ill-formed timestamp \"%s\".@.At this point, a string with a RFC3339 \
  ...
  | `Self_ast_imperative_bad_format_literal e ->
    let message =
      Format.asprintf
        "Ill-formed literal \"%a\".@.In the case of an address, a string is \
         expected prefixed by either tz1, tz2, tz3 or KT1 and followed by a \
         Base58 encoded hash and terminated by a 4-byte checksum.@.In the case \
         of a key_hash, signature, or key a Base58 encoded hash is expected."
        Ast_imperative.PP.expression
        e
    in
    let location = e.location in
  ...
  | `Self_ast_imperative_bad_conversion_bytes e ->
    let message =
      Format.sprintf
        "Ill-formed bytes literal.@.Example of a valid bytes literal: \
         \"ff7a7aff\"."
    in
    let location = e.location in
    let content = make_content ~message ~location () in
  ...
  | `Self_ast_imperative_const_assigned (location, var) ->
    let message =
      Format.asprintf
        "Invalid assignment to constant variable \"%a\""
        Value_var.pp
        var
    in
    let content = make_content ~message ~location () in

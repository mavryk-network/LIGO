...
let layout_type_expression ~raise : type_expression -> type_expression =
 fun e ->
  ...
  | T_sum cmap ->
    let (_ : unit list) =
      List.map
        ~f:(fun (label, ({ attributes; _ } : _ Rows.Elem.t)) ->
          if attributes
             |> List.map ~f:is_layout
             |> List.exists ~f:Option.is_some
          then (
            let () =
              raise.Simple_utils.Trace.warning
              @@ Main_warnings.warn_layout e.location label
            in
            ())
          else ())
        cmap.fields

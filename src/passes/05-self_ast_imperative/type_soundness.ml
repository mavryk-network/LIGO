...
let linearity_type_parameters
 ...
 fun ~raise x ->
  ...
  | T_abstraction { ty_binder; type_; _ } ->
    let aux : type_expression -> Type_var.t list -> Type_var.t list =
     fun ty acc ->
    ...
    if List.contains_dup ~compare:Type_var.compare lst
    then raise.error (non_linear_type_decl x)
...
let predefined_names ~(raise : ([< Errors.self_ast_imperative_error ], _) Trace.raise) m =
  (fun x ->
    checks_predefined ~raise x;
    x)
    m


let linearity ~(raise : ([< Errors.self_ast_imperative_error ], _) Trace.raise) m =
  (fun x ->

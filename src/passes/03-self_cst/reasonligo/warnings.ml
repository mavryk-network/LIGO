...
type self_cst_reasonligo_warning = [
  `Shadow_reserved_name of variable
]

let shadow_reserved_name var : self_cst_reasonligo_warning = `Shadow_reserved_name var

let error_ppformat :
  display_format:string display_format ->
  Format.formatter ->
  self_cst_reasonligo_warning ->
  unit =
...
let error_jsonformat : self_cst_reasonligo_warning -> Yojson.Safe.t =
  function
  | `Shadow_reserved_name var ->

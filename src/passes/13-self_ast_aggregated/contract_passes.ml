...
let self_typing ~raise
 ...
 fun dat e ->
  ...
  | ( E_constant { cons_name = C_SELF; arguments = [ entrypoint_exp ] }
    , { type_content =
          T_constant
            { language = _
            ; injection = Ligo_prim.Literal_types.Contract
    ...
    let entrypoint =
      ...
      | E_literal (Literal_string ep) ->
        check_entrypoint_annotation_format
          ~raise
          (Ligo_string.extract ep)
          entrypoint_exp
      | _ ->
        raise.error @@ Errors.entrypoint_ann_not_literal entrypoint_exp.location
    in
    let entrypoint_t =
      match dat.parameter.type_content with
      | T_sum _ as t
        when String.equal "default" (String.uncapitalize entrypoint) ->
        { dat.parameter with type_content = t }
      | T_sum cmap ->
        let content = Map.to_alist cmap.fields in
        let content =
          List.map
            ~f:
              (fun ( entrypoint
                   , ({ content = { associated_type; _ }; michelson_annotation } :
                       _ Rows.Elem.t) ) ->
              ( annotation_or_label michelson_annotation entrypoint
              , associated_type ))
            content
        in
        let associated_type =
          trace_option
            ~raise
            (Errors.unmatched_entrypoint entrypoint_exp.location)
    ...
    let () =
      ...
      then raise.Simple_utils.Trace.warning @@ bad_self_err ()
    in
    true, dat, e
  | _ -> true, dat, e


let entrypoint_typing ~raise
    : contract_type -> expression -> bool * contract_type * expression
  =
 fun dat e ->
  match e.expression_content with
  | E_constant
      { cons_name = C_CONTRACT_ENTRYPOINT_OPT | C_CONTRACT_ENTRYPOINT
      ; arguments = [ entrypoint_exp; _ ]
      } ->
    let _ =
      match entrypoint_exp.expression_content with
      | E_literal (Literal_string ep) ->

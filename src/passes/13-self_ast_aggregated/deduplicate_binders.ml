...
module Scope : sig
  ...
  let make_swapper (scope : t) : swapper =
    ...
    { value =
        (fun map v -> Option.value ~default:v @@ VMap.find_opt v map) value
    ; type_ =
        (fun map v -> Option.value ~default:v @@ TMap.find_opt v map) type_
...
let rec swap_expression : Scope.swapper -> expression -> expression =
 fun swaper e ->
  ...
  | E_for_each
      { fe_binder = binder1, binder2; collection; collection_type; fe_body } ->
...
and matching_cases : Scope.swapper -> matching_expr -> matching_expr =
 fun swaper me ->
  let self = swap_expression swaper in
  let self_type = swap_type_expression swaper in
  let return x = x in
  ...
  | Match_tuple { binders; body; tv } ->
    let binders = List.map binders ~f:(Binder.map self_type) in
    let body = self body in
    let tv = self_type tv in
...
let rec expression : Scope.t -> expression -> Scope.t * expression =
 fun scope e ->
  ...
  let self_type ?(scope = scope) = type_expression scope in
  let return ?(scope = scope) expression_content =
    scope, { e with expression_content }
  in
  match e.expression_content with
  | E_literal literal -> return @@ E_literal literal
  | E_constant { cons_name; arguments } ->
  ...
  | E_matching { matchee; cases } ->
    ...
    return @@ E_matching { matchee; cases }
  | E_record record ->
    let record = record |> Map.map ~f:self |> Map.map ~f:snd in
    return @@ E_record record
  | E_tuple tuple ->
    let tuple = tuple |> List.map ~f:self |> List.map ~f:snd in
    return @@ E_tuple tuple
  | E_accessor { struct_; path } ->
...
and matching_cases : Scope.t -> matching_expr -> matching_expr =
 fun scope me ->
  ...
  | Match_record { fields; body; tv } ->
    ...
    return @@ Match_record { fields; body; tv }
  | Match_tuple { binders; body; tv } ->
    let scope, binders = List.fold_map binders ~f:binder_new ~init:scope in
    let _, body = self ~scope body in
    let tv = self_type tv in
    return @@ Match_tuple { binders; body; tv }

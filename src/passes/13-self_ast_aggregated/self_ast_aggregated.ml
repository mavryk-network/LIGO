...
let eta_reduce : Ast_aggregated.expression -> Ast_aggregated.expression option =
 fun e ->
  ...
  | E_lambda
    ...
    | Some y
      when Param.is_imm binder && Value_var.equal (Param.get_var binder) y ->
      Some lamb
    | _ -> None)
  | _ -> None
...
let get_abstraction_tuple e =
  ...
  | E_lambda { binder; result; _ } ->
    ...
    | E_matching
        { matchee = { expression_content = E_variable u; _ }
        ; cases = Match_tuple { binders; body; _ }
        }
      when Value_var.equal u var && Param.is_imm binder ->
      let acc = List.map ~f:Binder.get_var binders in
      Some (acc, body)
    | _ -> None)
  | _ -> None
...
let get_application_tuple e =
  let open Ast_aggregated in
  match e.expression_content with
  | E_application { lamb; args = { expression_content = E_tuple acc; _ } } ->
    Some (acc, lamb)
  | _ -> None
...
let get_app_and_abs_tuple e =
  ...
  let%bind arguments, lamb = get_application_tuple e in
  let%bind vars, expr = get_abstraction_tuple lamb in
  return (arguments, vars, expr)
...
let rec get_applications acc e =
  let open Ast_aggregated in
  match acc, e.expression_content with
  | _, E_application { lamb; args } -> get_applications (args :: acc) lamb
  | [], _ -> None
  | _, _ -> Some (acc, e)
...
let get_apps_and_abs e =
  let open Option.Let_syntax in
  let%bind arguments, lamb = get_applications [] e in
  let%bind vars, expr = get_abstractions [] lamb in
  return (arguments, vars, expr)
...
let get_const acc e =
  ...
  | E_constant { cons_name; arguments = tuple; _ } ->
    let rec f b e v =
      ...
      | _ -> false
    in
    (match List.fold2 tuple acc ~f ~init:true with
...
let inline_let
    : bool ref -> Ast_aggregated.expression -> Ast_aggregated.expression
  =
 fun changed e ->
  match e.expression_content with
  | E_let_in { let_binder; rhs; let_result; attributes = { thunk = true; _ } } ->
    let e2' =
      Subst.subst_expression
        ~body:let_result
        ~x:(Binder.get_var let_binder)
        ~expr:rhs
    in
    changed := true;
    e2'
  | E_application _ ->
    ...
    | Some (arguments, vars, const) ->
      (match get_const vars const with
      | Some (cons_name, len) when len = List.length arguments ->
        changed := true;
        { e with expression_content = E_constant { cons_name; arguments } }
      | Some _ | None -> e)
    | None -> e)
  | _ -> e
...
let inline_lets
    : bool ref -> Ast_aggregated.expression -> Ast_aggregated.expression
  =
 fun changed -> Helpers.map_expression (inline_let changed)
...
let all_contract ~raise parameter storage prg =
  ...
  let all_p =
    List.map ~f:(fun pass -> Helpers.fold_map_expression pass contract_type)

...
let inline_thunk
 ...
 fun changed e ->
  ...
  | E_let_in { let_binder; rhs; let_result; attr = { thunk = true; _ } } ->
    ...
    return_changed e
  | E_application
      { lamb = { expression_content = E_lambda { binder; result }; _ }; args }
    ...
    let e =
      Subst.subst_expression ~body:result ~x:(Param.get_var binder) ~expr:args
    in
...
let remove_check_self : Ast_aggregated.expression -> Ast_aggregated.expression =
  let f e =
    let open Ast_aggregated in
    match
      e.expression_content, Ast_aggregated.get_t_option e.type_expression
    with
...
let all_aggregated_expression ~raise e =
  ...
  let e =
    Helpers.map_expression (Contract_passes.self_literal_typing ~raise) e
  in
  e
...
let all_contract ~raise parameter storage prg =
  ...
  let all_p =
    List.map ~f:(fun pass -> Helpers.map_expression pass)
    @@ contract_passes_map ~raise
  in
  let prg = List.fold ~f:(fun x f -> f x) all_p ~init:prg in
  let prg =
    Helpers.map_expression Pattern_matching_simpl.peephole_expression prg
  in
  prg

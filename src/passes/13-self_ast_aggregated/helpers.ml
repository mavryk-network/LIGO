...
let rec fold_expression : 'a 'err. ('a, 'err) folder -> 'a -> expression -> 'a =
 fun f init e ->
  ...
  | E_matching { matchee = e; cases } ->
    ...
    res
  | E_record m -> Map.fold ~f:(fun ~key:_ ~data acc -> self acc data) ~init m
...
and fold_cases : 'a 'err. ('a, 'err) folder -> 'a -> matching_expr -> 'a =
 fun f init m ->
  match m with
...
let rec map_expression : 'err mapper -> expression -> expression =
 fun f e ->
  ...
  | E_let_in let_in ->
    let let_in = Let_in.map self self_type let_in in
    return @@ E_let_in let_in
  | E_lambda l ->
...
let rec fold_map_expression
 ...
 fun f a e ->
  ...
  then init, e'
    ...
    | E_let_in let_in ->
      let res, let_in = Let_in.fold_map self idle init let_in in
...
and fold_map_cases : 'a fold_mapper -> 'a -> matching_expr -> 'a * matching_expr
 ...
 fun f init m ->
  ...
  | Match_tuple { binders; body; tv } ->
    let res, body = fold_map_expression f init body in
...
module Free_variables : sig
  ...
  let rec get_fv_expr : expression -> VarSet.t =
   fun e ->
    ...
    | E_literal _ | E_raw_code _ -> VarSet.empty
    ...
    | E_record m ->
      ...
      let res = Map.data res in
      unions res
    | E_tuple tuple ->
      unions (List.map ~f:self tuple)
  ...
  and get_fv_cases : matching_expr -> VarSet.t =
   fun m ->
    ...
    | Match_tuple { binders; body; _ } ->
      ...
      List.fold_right ~f:VarSet.remove ~init:(get_fv_expr body) vars

...
let rec uncurry_lambda (depth : int) (expr : expression)
    : Value_var.t list * expression
  =
...
let rec usage_in_expr (f : Value_var.t) (expr : expression) : usage =
  ...
  | E_matching
      { matchee; cases = Ast_aggregated.Match_variant { cases; tv = _ } } ->
    usages
      (self matchee
      :: List.map
           ~f:(fun { constructor = _; pattern; body } ->
             self_binder [ pattern ] body)
           cases)
  | E_matching
      { matchee; cases = Ast_aggregated.Match_record { fields; body; tv = _ } }
    ->
    usages
      [ self matchee
      ; self_binder (List.map ~f:Binder.get_var (Map.data fields)) body
      ]
  | E_matching { matchee; cases = Match_tuple { binders; body; _ } } ->
    usages
      [ self matchee; self_binder (List.map ~f:Binder.get_var binders) body ]
  | E_record fields -> usages (List.map ~f:self (Map.data fields))
  | E_tuple tuple -> usages (List.map ~f:self tuple)
...
let uncurried_rows (depth : int) (args : type_expression list) : _ Rows.t =
  ...
  let fields =
    Label.Map.of_alist_exn
      (List.mapi
         ~f:(fun i (label, ty) ->
           ( label
           , ({ content = { associated_type = ty; decl_pos = i }
         ...
         | _ ->
           ...
           @@ Format.asprintf
                ...
                depth))
  in
  { fields; attributes = { layout = L_comb } }
...
let uncurry_rhs (depth : int) (expr : expression) =
  ...
  let fields =
    try
      Label.Map.of_alist_exn
        (List.zip_exn labels (List.map2_exn ~f:Binder.make vars arg_types))
    with
    | _ ->
      failwith
      @@ Format.asprintf
  ...
  let result =
    { expression_content =
        E_matching
          { matchee; cases = Match_record { fields; body; tv = record_tv } }
    ; location = Location.generated
...
let rec uncurry_in_expression
  ...
  | E_matching { matchee; cases = Match_tuple { binders; body; tv } } ->
    let matchee = self matchee in
    let body =
      self_binder (List.map ~f:Binder.get_var binders) body
    in
    return (E_matching { matchee; cases = Match_tuple { binders; body; tv } })
  | E_record fields ->
...
let uncurry_expression (expr : expression) : expression =
  ...
  map_expression
    (fun expr ->
      match expr.expression_content with
      | E_recursive
          { fun_name; fun_type; lambda = { binder = _; result } as lambda } ->
        ...
        | Application depth ->
          ...
          then (
            ...
            let binder_types =
              List.map ~f:(fun t -> Value_var.fresh (), t) arg_types
            in
            (* An variable for each function argument *)
            let args =
              List.map ~f:(fun (b, t) -> e_a_variable b t) binder_types
            in
            ...
            let record =
              E_record
                ...
                | _ ->
                  failwith
                  @@ Format.asprintf
                       "Uncurry: mismatching number of arguments, expr: %a, \
                        type %a\n\
                        %!"
                       PP.expression
                       expr
            ...
            let rhs =
              ...
              ; type_expression =
                  { type_content =
                      T_arrow { type1 = record_type; type2 = ret_type }
                  ; orig_var = None
                  ; location = Location.generated
                  ; source_type = None
                  }
            ...
            let result =
              e_a_let_in
                (Binder.make fun_name rhs.type_expression)
                rhs
                result
                attr
            in

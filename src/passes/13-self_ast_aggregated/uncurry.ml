  ...
  let fields =
    try
      let f : e_variable -> type_expression -> _ Pattern.t = fun v ty ->
        Pattern.var_pattern ~loc:Location.generated (Binder.make v ty)
      in
         ...
         (match usage_in_expr fun_name result with
          ...
          | Application depth ->
            ...
            then
              ...
              (* Construct the let *)
              let result = e_a_let_in (Pattern.var_pattern (Binder.make fun_name rhs.type_expression)) rhs result attr in
              let f (var, t) result =

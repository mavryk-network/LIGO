 ...
 fun ~display_format f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Self_ast_aggregated_create_contract_lambda (_cst, e) ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid usage of Tezos.create_contract.@.The first argument \
    ...
    | `Self_ast_aggregated_bad_single_arity (c, e) ->
      Format.fprintf
        f
        "@[<hv>%a@ Ill-formed \"%a\" expression@.One function argument is \
         expected. @]"
        Snippet.pp
        e.location
...
let error_json : self_ast_aggregated_error -> Simple_utils.Error.t =
 fun e ->
  ...
  | `Self_ast_aggregated_create_contract_lambda (_cst, e) ->
    ...
    let message =
      Format.sprintf
        "Invalid usage of Tezos.create_contract.@.The first argument must be \
         an inline function."
    in
    let content = make_content ~message ~location () in
    make ~stage ~content

...
let apply_table_expr table (expr : AST.expression) =
  let apply_table_type u =
    List.fold_right
      table
      ~f:(fun (v, t) u -> AST.Helpers.subst_type v t u)
      ~init:u
  in
  let (), e =
    fold_map_expression
      (fun () e ->
        let e =
          { e with type_expression = apply_table_type e.type_expression }
        in
        let return expression_content =
          true, (), { e with expression_content }
        in
        ...
        | E_matching { matchee; cases = Match_variant { cases; tv } } ->
          ...
          @@ E_matching
               { matchee
               ; cases = Match_variant { cases; tv = apply_table_type tv }
               }
        | E_matching { matchee; cases = Match_record { fields; body; tv } } ->
          let fields = Map.map ~f:(Binder.map apply_table_type) fields in
          return
          @@ E_matching
               { matchee
               ; cases = Match_record { fields; body; tv = apply_table_type tv }
               }
        | E_matching { matchee; cases = Match_tuple { binders; body; tv } } ->
          let binders = List.map ~f:(Binder.map apply_table_type) binders in
          return
          @@ E_matching
               { matchee
               ; cases = Match_tuple { binders; body; tv = apply_table_type tv }
               }
...
let rec subst_external_type et t (u : AST.type_expression) =
  ...
  | T_sum row ->
    let row = AST.Rows.map (self et t) row in
  ...
  let (), e =
    fold_map_expression
      (fun () e ->
        let e =
          { e with
            type_expression = subst_external_type et t e.type_expression
          }
        in
        let return expression_content =
          true, (), { e with expression_content }
        ...
        | E_type_inst { forall; type_ } ->
          return
          @@ E_type_inst { forall; type_ = subst_external_type et t type_ }
        | E_lambda { binder; output_type; result } ->
        ...
        | E_matching { matchee; cases = Match_variant { cases; tv } } ->
          ...
          @@ E_matching
               { matchee
               ; cases =
                   Match_variant { cases; tv = subst_external_type et t tv }
               }
        | E_matching { matchee; cases = Match_record { fields; body; tv } } ->
          let fields =
            Map.map ~f:(Binder.map (subst_external_type et t)) fields
          in
          return
          @@ E_matching
               { matchee
               ; cases =
                   Match_record
                     { fields; body; tv = subst_external_type et t tv }
               }
        | E_matching { matchee; cases = Match_tuple { binders; body; tv } } ->
          let binders =
            List.map ~f:(Binder.map (subst_external_type et t)) binders
          in
          return
          @@ E_matching
               { matchee
               ; cases =
                   Match_tuple
                     { binders; body; tv = subst_external_type et t tv }
               }
        | E_assign { binder; expression } ->
...
let evaluate_external_typer typed rhs =
  ...
  | T_constant { injection = External _; parameters = _; _ } ->
    let _, external_type_for =
      AST.Helpers.destruct_arrows_n typed (List.length l)
    in
 ...
 fun data expr ->
  ...
  | E_let_in { let_binder; rhs; let_result; attributes } ->
    ...
    let binder_instances =
      Data.instances_lookup (Binder.get_var let_binder) data
    in
    let build_let
        (lid : Value_var.t)
      ...
      let data, rhs =
        ...
        | E_recursive
            { fun_name; fun_type = _; lambda = { binder; output_type; result } }
          ->
          let lambda =
            Lambda.
              { binder
              ; output_type
              ; result =
                  Subst.replace (Subst.replace result fun_name vid) lid vid
              }
          in
          let data =
            Data.instance_add lid { vid; type_instances; type_ } data
          in
          ( data
          , { rhs with
              expression_content =
                E_recursive
                  { fun_name = vid; fun_type = rhs.type_expression; lambda }
            } )
        | _ -> data, rhs
      in
      ...
      ( data
      , AST.e_a_let_in
          (Binder.make let_binder rhs.type_expression)
          rhs
          let_result
    ...
    let data, expr =
      ...
      | _ ->
        List.fold_right
          binder_instances
          ~f:(build_let @@ Binder.get_var let_binder)
  ...
  | E_record lmap ->
    let data, lmap =
      Map.fold
        ~init:(data, Label.Map.empty)
        ~f:(fun ~key ~data (acc, map) ->
          let acc, data = self acc data in
          acc, Map.set map ~key ~data)
        lmap
    in
    data, return (E_record lmap)
  | E_tuple tuple ->
    let ret, tuple = List.fold_map ~f:self ~init:data tuple in
    ret, return @@ E_tuple tuple
  ...
  | E_type_inst _ ->
    let rec aux type_insts (e : AST.expression) =
      ...
      | _ ->
        raise.error
          (Errors.corner_case
             "Monomorphisation: cannot resolve non-variables with \
              instantiations")
    in
 ...
 fun data m ->
  ...
  | Match_variant { tv; cases } ->
    let aux { AST.constructor; pattern; body } (data, r) =
      ...
      data, { AST.constructor; pattern; body } :: r
    in
  ...
  | Match_tuple { binders; body; tv } ->
    let res, body = mono_polymorphic_expression ~raise data body in
    res, Match_tuple { binders; body; tv }
...
let check_if_polymorphism_present ~raise e =
  ...
  let rec check_type_expression ~loc (te : AST.type_expression) =
    ...
    | T_record row | T_sum row ->
      let _ = AST.Rows.map (check_type_expression ~loc) row in
      ()
    | T_tuple tuple ->
      let _ = Tuple.map (check_type_expression ~loc) tuple in
      ()
    | T_arrow _ -> ()
    | T_singleton _ -> ()

      ...
      (fun () e ->
        ...
        | E_raw_code { language; code = { expression_content = m; _ } as expr }
          ...
          let tuple =
            { expr with
              expression_content =
                E_record (Record.record_of_tuple (code :: args))
            }
          in
  ...
  | T_sum { fields; layout } ->
    let fields =
      AST.(
        Record.map
          ~f:
            (fun Rows.{ associated_type; michelson_annotation; decl_pos }
                 : row_element ->
  ...
  let (), e =
    fold_map_expression
      (fun () e ->
        ...
        | E_matching { matchee; cases = Match_record { fields; body; tv } } ->
          let fields =
            Record.map ~f:(Binder.map (subst_external_type et t)) fields
          in
          return
          @@ E_matching
               { matchee
...
let check_if_polymorphism_present ~raise e =
  ...
  let rec check_type_expression ~loc (te : AST.type_expression) =
    ...
    | T_record { fields; _ } | T_sum { fields; _ } ->
      Record.LMap.iter
        (fun _ (re : AST.row_element) ->
          check_type_expression ~loc re.associated_type)
        fields
    | T_arrow _ -> ()
    | T_singleton _ -> ()

...
let is_generated_partial_match : expression -> bool =
 fun exp ->
  ...
  | E_application { lamb = { expression_content = E_variable v; _ }; args = e }
    ...
    | Some fw ->
      String.equal fw (Ligo_string.extract Backend.Michelson.fw_partial_match)
    | None -> false)
  | _ -> false
...
let make_le : _ matching_content_variant -> (Label.t * Value_var.t) list =
 fun ml ->
  List.map
    ~f:(fun (m : _ matching_content_case) -> m.constructor, m.pattern)
    ml.cases


let substitute_var_in_body
    : Value_var.t -> Value_var.t -> expression -> expression
  =
...
let compress_matching : expression -> expression =
 fun exp ->
  let aux
      : bool * simpl_map -> expression -> bool * (bool * simpl_map) * expression
    =
  ...
  in
  let simplify exp =
    let (has_been_simpl, _), exp =
      fold_map_expression aux (false, SimplMap.empty) exp
    in
    has_been_simpl, exp
  in

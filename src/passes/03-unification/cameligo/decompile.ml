...
and decompile_var v = ghost_ident @@ Format.asprintf "%a" AST.Variable.pp v
...
and sig_entry
 ...
 fun se ->
  ...
  | AST.S_value (v, ty) -> CST.(S_Value (w (ghost_val, decompile_var v, ghost_colon, ty)))
  | AST.S_type (v, ty) -> CST.(S_Type (w (ghost_type, decompile_tvar v, ghost_eq, ty)))
  | AST.S_type_var v -> CST.(S_TypeVar (w (ghost_type, decompile_tvar v)))

...
module TODO_unify_in_cst = struct
  ...
  let module_alias ~loc alias binders =
    let loc_path =
      List.fold (nseq_to_list binders) ~init:Location.generated ~f:(fun acc (x : m_var) ->
...
let rec compile_type_expression : CST.type_expr -> AST.type_expr =
 fun te ->
  ...
  | TRecord t ->
    ...
    let fields =
      ...
      let compile_field_decl
          : int -> CST.field_decl -> AST.type_expr option Non_linear_rows.row
        =
 ...
 fun decl ->
  ...
  | TypeDecl d ->
    ...
    let params : t_var nseq option =
      let compile_params : CST.type_vars -> t_var nseq =
       fun p ->

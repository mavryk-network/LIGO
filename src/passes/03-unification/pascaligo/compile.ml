...
(* MAVRYK: PascaLIGO. [nseq] is kept as the tuple ['a * 'a list]; upstream removed
   [Simple_utils.List.Ne] and the [Utils.nseq_*] operations. Provide the type and
   the tuple-nseq operations the PascaLIGO builder needs. *)
type 'a nseq = 'a CST.nseq
...
(* MAVRYK: PascaLIGO. The CST [variable] became [Var | Esc] (escaped ids).
   [get_var] extracts the wrap (for region/string); [value_evar]/[type_evar]
   build the AST's escaped-var payloads (Raw/Esc), fed to the
   [e_variable_esc]/[t_var_esc]/[p_var_esc] combinators. *)
...
let type_evar : CST.variable -> AST.Ty_escaped_var.t = function
  ...
  | CST.Esc v -> Esc (Ligo_prim.Type_var.of_input_var ~loc:(w_snd v) (w_fst v))
    ...
    *)
    ...
    | Some label ->
      let carg =
        ...
        | None -> p_unit ~loc
      in
      p_variant ~loc (Label.of_string label) (Some carg)
...
module TODO_unify_in_cst = struct
   ...
   fun init field_path ->
    ...
    let f = function
      | (FieldName name : CST.selection) ->
        let name, loc = w_split (get_var name) in
       ...
       fun x ->
        ...
        | CST.Complete { field_lhs; field_lens; field_rhs; attributes } ->
          ...
          let field_lhs : AST.expr AST.Selection.t list =
            ...
            | CST.E_Var x ->
              let label = w_fst (get_var x) in
              [ FieldName (Label.of_string label) ]
            | CST.E_Proj
                { region = _
                ; value = { record_or_tuple = CST.E_Var label; selector = _; field_path }
                } ->
              let r = Selection.FieldName (Label.of_string @@ w_fst (get_var label)) in
              r
              :: List.map (nsepseq_to_list field_path) ~f:(function
                     | FieldName name ->
                       Selection.FieldName (Label.of_string (w_fst (get_var name)))
  ...
  let declarations_as_program decls =
    make_prg (Nseq.to_list @@ Nseq.map pe_declaration decls)
end
...
let rec compile_type_expression : CST.type_expr -> AST.ty_expr =
 fun te ->
  ...
  | T_App t ->
    ...
    let type_args =
      Nseq.to_ne_list (Nseq.map self @@ Nseq.nsepseq_to_nseq (r_fst ttuple).inside)
    in
  ...
  | T_Cart t ->
    ...
    let te = self te in
    let hd, tl = Nseq.map self @@ Nseq.nsepseq_to_nseq tes in
    t_prod (Nseq.to_ne_list (te, hd :: tl)) ~loc
  | T_Fun t ->
  ...
  | T_ModPath t ->
    ...
    let module_path =
      Nseq.map
        ...
        (Nseq.nsepseq_to_nseq t.module_path)
    in
  ...
  | T_Record t ->
    ...
    let fields =
      let destruct CST.{ field_type; field_name; attributes } =
        ( Label.of_string (w_fst (get_var field_name))
...
and compile_pattern : CST.pattern -> AST.pattern =
 fun p ->
  ...
  | P_Mumav p ->
    let (_s, z), loc = w_split p in
    p_literal ~loc (Literal_mumav (TODO_do_in_parsing.six_to_z z))
  | P_Mav p ->
    let (_s, q), loc = w_split p in
    let mumav_int64 = Z.to_int64 Q.(to_bigint (mul (of_int 1_000_000) q)) in
    p_literal ~loc (Literal_mumav (Z.of_int64 mumav_int64))
  | P_Nat p ->
    let (_s, z), loc = w_split p in
    ...
    let fields =
      let translate_field_assign : CST.field_pattern -> (Label.t, AST.pattern) AST.Field.t
        ...
        | Complete { field_lhs; field_lens = _; field_rhs; attributes } ->
          TODO_do_in_parsing.weird_attributes attributes;
          let lhs, _ =
            w_split (get_var (TODO_do_in_parsing.compile_pattern_record_lhs field_lhs))
          in
  ...
  | P_Typed p ->
    ...
    p_typed ~loc ty ptrn
  | P_Var p ->
    let loc = w_snd (get_var p) in
    p_var_esc ~loc (value_evar p)
  | P_Verbatim p ->
    let s, loc = w_split p in
    p_literal ~loc (Literal_string (Standard s))
...
and compile_block : CST.block -> AST.block =
 fun b ->
  let stmts = Nseq.map compile_statement @@ Nseq.nsepseq_to_nseq b.statements in
  block_of_statements (Nseq.to_ne_list stmts)
...
and compile_case : type a b. (a -> b) -> a CST.case -> (_, _, b) AST.Case.t =
 fun f c ->
  ...
  let cases = Nseq.map (compile_case_clause f <@ r_fst) @@ Nseq.nsepseq_to_nseq c.cases in
  { expr; cases = Nseq.to_ne_list cases }
...
and compile_for_set_or_list
  ...
  =
 fun s ->
  let s_var = get_var s.var in
  let var = TODO_do_in_parsing.var ~loc:(w_snd s_var) (w_fst s_var) in
  let for_kind =
...
and compile_instruction : CST.instruction -> AST.instruction =
 fun i ->
  ...
  | I_For i ->
    let i, loc = r_split i in
    let i_index = get_var i.index in
    let index = TODO_do_in_parsing.var ~loc:(w_snd i_index) (w_fst i_index) in
...
and extract_tuple : 'a. ('a, CST.comma) sepseq CST.par CST.reg -> 'a nseq option =
 fun t -> Option.map t.value.inside ~f:(fun x -> Nseq.nsepseq_to_nseq x)
...
and compile_expression : CST.expr -> AST.expr =
 fun e ->
  ...
  match e with
  | E_Var var ->
    let loc = w_snd (get_var var) in
    e_variable_esc ~loc (value_evar var)
  | E_Par par ->
  ...
  | E_Mumav m ->
    ...
    e_literal ~loc (Literal_mumav (Z.of_int64 m))
  | E_Mav m ->
    let (_, q), loc = w_split m in
    let mumav_int64 = Z.to_int64 Q.(to_bigint (mul (of_int 1_000_000) q)) in
    e_literal ~loc (Literal_mumav (Z.of_int64 mumav_int64))
  ...
  | E_Tuple lst ->
    ...
    let nseq = Nseq.map self (Nseq.nsepseq_to_nseq npseq.inside) in
    e_tuple (Nseq.to_ne_list nseq) ~loc
  | E_Record record ->
  ...
  | E_Ctor ctor ->
    (* MAVRYK: PascaLIGO. A nullary constructor is a bare [E_constr] (as in CameLIGO);
       wrapping it in [E_ctor_app (_, None)] clashes with standalone_constructor_removal
       in the composed nanopass fold. Applied constructors go through [E_App] below. *)
  ...
  | E_App app ->
    ...
    (match func with
    | E_Ctor ctor ->
      (* MAVRYK: PascaLIGO. Build [E_applied_constructor] directly (as CameLIGO does),
         rather than [E_ctor_app (E_constr …, args)] whose inner [E_constr] is rewritten
         by standalone_constructor_removal in the composed nanopass fold. *)
      let constructor =
        let ctor, _loc = w_split ctor in
        Label.of_string ctor
      in
      let args =
        Option.map args.value.inside ~f:(fun x ->
            Nseq.to_ne_list (Nseq.map self @@ Nseq.nsepseq_to_nseq x))
      in
      let element =
        match args with
        | None -> e_unit ~loc
        | Some [ one ] -> one
        | Some lst -> e_tuple ~loc lst
      in
  ...
  | E_MapLookup mlu ->
    ...
    let keys = Nseq.map (self <@ extract_key) mlu.keys in
    e_map_lookup { map; keys = Nseq.to_ne_list keys } ~loc
  | E_Map m ->
    let m, loc = r_split m in
  ...
  | E_Block be ->
    ...
    let block =
      ...
      let block = Nseq.map compile_statement @@ Nseq.nsepseq_to_nseq b.statements in
      make_b ~loc (Nseq.to_ne_list block)
    in
...
and compile_declaration : CST.declaration -> AST.declaration =
 fun decl ->
  ...
  | D_Type d ->
    ...
    let dname = get_var d.name in
    let name = TODO_do_in_parsing.tvar ~loc:(w_snd dname) (w_fst dname) in
    let params =
      Option.map
  ...
  | D_Fun d ->
    ...
    let is_rec =
      ...
      | None -> false
    in
    let d_fun_name = get_var d.fun_name in
    let fun_name = TODO_do_in_parsing.var ~loc:(w_snd d_fun_name) (w_fst d_fun_name) in
    let type_params = Option.map ~f:compile_type_params d.type_params in
    let parameters =
...
and compile_module : CST.module_expr -> AST.mod_expr =
 fun m ->
  ...
  | M_Body m ->
    let m, loc = r_split m in
    let ds = Nseq.map compile_declaration m.declarations in
  ...
  | M_Path m ->
    ...
    let module_path =
      Nseq.map
        ...
        (Nseq.nsepseq_to_nseq m.module_path)
    in
    let field = TODO_do_in_parsing.mvar ~loc:(w_snd m.field) (w_fst m.field) in
    m_path (Nseq.to_ne_list (Nseq.append module_path (field, []))) ~loc
  | M_Var m ->

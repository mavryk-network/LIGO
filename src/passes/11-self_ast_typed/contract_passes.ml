...
let empty_env =
  { env = MVarMap.empty
  ; used_var = VVarSet.empty
  ; used_mut_var = VVarSet.empty
  }
...
and get_fv expr =
  ...
  let get_fv_lambda Lambda.{ binder; output_type; result } =
    ...
    ( (match Param.get_mut_flag binder with
      | Immutable ->
        { env with
          used_var = VVarSet.remove (Param.get_var binder) @@ env.used_var
        }
      | Mutable ->
        { env with
          used_mut_var =
            VVarSet.remove (Param.get_var binder) @@ env.used_mut_var
        })
  ...
  | E_record record ->
    let env, record =
      Map.fold
        record
        ~init:(empty_env, record)
        ~f:(fun ~key:label ~data:expr (env, record) ->
          let env', expr = self expr in
          merge_env env env', Map.set record ~key:label ~data:expr)
    in
    return env @@ E_record record
  ...
  | E_let_in { let_binder; rhs; let_result; attributes } ->
    ...
    let env =
      { env with
        used_var = VVarSet.remove (Binder.get_var let_binder) env.used_var
      }
    in
  ...
  | E_let_mut_in { let_binder; rhs; let_result; attributes } ->
    ...
    let env =
      { env with
        used_mut_var =
          VVarSet.remove (Binder.get_var let_binder) env.used_mut_var
      }
    in
    let env', rhs = self rhs in
    return (merge_env env env')
  ...
  | E_for_each { fe_binder; collection; collection_type; fe_body } ->
    ...
    @@ E_for_each { fe_binder; collection; collection_type; fe_body }
  | E_skip -> return empty_env E_skip
  | E_cond { condition; then_clause; else_clause } ->
    let env1, condition = self condition in
    let env2, then_clause = self then_clause in
    let env3, else_clause = self else_clause in
    return (unions [ env1; env2; env3 ])
    @@ E_cond { condition; then_clause; else_clause }
  | E_sequence { expr1; expr2 } ->
    let env1, expr1 = self expr1 in
    let env2, expr2 = self expr2 in
    return (merge_env env1 env2) @@ E_sequence { expr1; expr2 }
  | E_tuple tuple ->
    let envs, tuple = tuple |> List.map ~f:self |> List.unzip in
    return (unions envs) @@ E_tuple tuple
  | E_set set_expr ->
    let envs, set_expr = set_expr |> List.map ~f:self |> List.unzip in
    return (unions envs) @@ E_set set_expr
  | E_list list_expr ->
    let envs, list_expr = list_expr |> List.map ~f:self |> List.unzip in
    return (unions envs) @@ E_list list_expr
  | E_map map_expr ->
    let envs, map_expr =
      map_expr
      |> List.map ~f:(fun (key, val_) ->
             let kenv, key = self key in
             let venv, val_ = self val_ in
             merge_env kenv venv, (key, val_))
      |> List.unzip
    in
    return (unions envs) @@ E_map map_expr
  | E_big_map map_expr ->
    let envs, map_expr =
      map_expr
      |> List.map ~f:(fun (key, val_) ->
      ...
      |> List.unzip
    in
    return (unions envs) @@ E_big_map map_expr


and get_fv_cases : matching_expr -> env * matching_expr =
 fun m ->
  match m with
  | Match_variant { cases; tv } ->
    let aux { constructor; pattern; body } =
      let env, body = get_fv body in
      ( { env with used_var = VVarSet.remove pattern @@ env.used_var }
      , { constructor; pattern; body } )
    in
    let envs, cases = List.unzip @@ List.map ~f:aux cases in
    unions envs, Match_variant { cases; tv }
  | Match_record { fields; body; tv } ->
    let pattern = Map.data fields |> List.map ~f:Binder.get_var in
    let env, body = get_fv body in
    ( { env with
        used_var = List.fold_right pattern ~f:VVarSet.remove ~init:env.used_var
      }
    , Match_record { fields; body; tv } )
  | Match_tuple { binders; body; tv } ->
    ...
    let env, body = get_fv body in
    ( { env with
        used_var = List.fold_right vars ~f:VVarSet.remove ~init:env.used_var
      }
    , Match_tuple { binders; body; tv } )
...
and get_fv_module (env : env) acc = function
  ...
  | ({ Location.wrap_content = D_module { module_binder; module_; module_attr }
     ; _
     } as hd)
    :: tl ->
    (match MVarMap.find_opt module_binder env.env with
    | Some env' ->
      ...
      get_fv_module
        env
        ({ hd with
           wrap_content = D_module { module_binder; module_; module_attr }
         }
        :: acc)
        tl
...
and get_fv_module_expr env x =
  ...
  | M_module_path path ->
    let rec push_env (name, name_lst) toto =
      ...
      | hd :: tl ->
        { empty_env with
          env = MVarMap.singleton name @@ push_env (hd, tl) toto
        }
    in
...
let remove_unused ~raise : contract_pass_data -> program -> program =
 fun contract_pass_data prg ->
  ...
  let aux = function
    | { Location.wrap_content = D_value { binder; _ }; _ } ->
      not (Value_var.equal (Binder.get_var binder) contract_pass_data.main_name)
    | _ -> true
  in
...
let remove_unused_for_views ~raise ~(view_names : Value_var.t list)
    : program -> program
  =
 fun prg ->
  ...
  let pred _ = function
    | { Location.wrap_content = D_value { binder; _ }; _ } ->
      is_view_name @@ Binder.get_var binder
    | _ -> false
  in
  let idx, _ =
    trace_option ~raise (Errors.corner_case "View not found")
    @@ List.findi prg_decls ~f:pred
  in
  ...
  in
  let view_decls =
    List.map view_decls ~f:(fun decl -> Location.wrap (D_value decl))
  in
  let _, prg_decls =
    trace_option ~raise (Errors.corner_case "View not found")
    @@ Simple_utils.List.uncons prg_decls

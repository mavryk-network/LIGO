...
let check_view_type ~raise : err_data:(Location.t*Ast_typed.expression_variable*Ast_typed.expression_variable) -> contract_type -> view_type -> unit =
  fun ~err_data:(loc,view_name,entry_name) {storage = c_storage ; _} {arg ; storage = v_storage ; return} ->
    let () = trace_option ~raise (storage_view_contract loc view_name c_storage entry_name v_storage) @@

...
let rec fold_expression : 'a folder -> 'a -> expression -> 'a =
 fun f init e ->
  ...
  | E_skip -> init
  | E_cond cond -> Conditional.fold self init cond
  | E_tuple tuple -> Tuple.fold self init tuple
  | E_sequence seq -> Sequence.fold self init seq
  | E_set set_expr -> Set_expr.fold self init set_expr
  | E_list list_expr -> List_expr.fold self init list_expr
  | E_map map_expr | E_big_map map_expr -> Map_expr.fold self init map_expr
...
and fold_cases : 'a folder -> 'a -> matching_expr -> 'a =
 fun f init m ->
  match m with
  | Match_variant { cases; tv = _ } ->
    ...
    res
  | Match_record { fields = _; body; tv = _ } -> fold_expression f init body
  | Match_tuple { body; _ } -> fold_expression f init body
...
let rec iter_type_expression : ty_mapper -> type_expression -> unit =
 fun f t ->
  ...
  | T_sum row | T_record row ->
    Map.iter row.fields ~f:(fun row_elem ->
        self row_elem.content.associated_type)
  | T_arrow { type1; type2 } ->
    self type1;
    self type2
  | T_singleton _ -> ()
  | T_abstraction { type_; _ } -> self type_
  | T_for_all { type_; _ } -> self type_
  | T_tuple tuple -> List.iter tuple ~f:self
...
let rec map_expression : 'err mapper -> expression -> expression =
 fun f e ->
  ...
  | (E_deref _ | E_skip | E_literal _ | E_variable _ | E_raw_code _) as e' ->
    return e'
...
and map_cases : 'err mapper -> matching_expr -> matching_expr =
 fun f m ->
  match m with
...
module Free_variables : sig
  ...
  (* TODO: Alistair. Remove the use of [Caml] here + not OCaml style *)
  ...
  let rec get_fv_expr : expression -> moduleEnv' =
   fun e ->
    ...
    | E_record record -> record |> Map.map ~f:self |> Map.data |> unions
    | E_update { struct_; update; path = _ } ->
    ...
    | E_cond { condition; then_clause; else_clause } ->
      [ condition; then_clause; else_clause ] |> List.map ~f:self |> unions
    | E_sequence { expr1; expr2 } -> unions [ self expr1; self expr2 ]
    | E_tuple tuple -> tuple |> List.map ~f:self |> unions
    | E_set set_expr -> set_expr |> List.map ~f:self |> unions
    | E_list list_expr -> list_expr |> List.map ~f:self |> unions
    | E_map map_expr | E_big_map map_expr ->
      map_expr
      |> List.map ~f:(fun (key, val_) -> unions [ self key; self val_ ])
      |> unions


  and get_fv_cases : matching_expr -> moduleEnv' =
   fun m ->
    match m with
    | Match_variant { cases; tv = _ } ->
      let aux { constructor = _; pattern; body } =
        ...
        { modVarSet
        ; moduleEnv
        ; varSet = VarSet.remove pattern @@ varSet
        ; mutSet
        }
      in
    ...
    | Match_record { fields; body; tv = _ } ->
      ...
      { modVarSet
      ; moduleEnv
      ; varSet = List.fold_right pattern ~f:VarSet.remove ~init:varSet
      ; mutSet
      }
    | Match_tuple { binders; body; _ } ->
      ...
      { modVarSet
      ; moduleEnv
      ; varSet = List.fold_right vars ~f:VarSet.remove ~init:varSet
      ; mutSet
      }

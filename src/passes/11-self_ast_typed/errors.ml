...
type self_ast_typed_error =
  ...
  | `Self_ast_typed_bad_contract_io of
    Value_var.t * Ast_typed.expression * Location.t
  ...
  | `Self_ast_typed_storage_view_contract of
    ...
    * Ast_typed.type_expression
    * Ast_typed.type_expression
  | `Self_ast_typed_view_io of
    Location.t * Ast_typed.type_expression * [ `In | `Out ]
 ...
 fun ~display_format f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Self_ast_typed_annotated_declaration_shadowed loc ->
      Format.fprintf
        f
        "@[<hv>%a@.This declaration holds an annotation and is later \
         shadowed.@]"
        Snippet.pp
        loc
    | `Self_ast_typed_storage_view_contract (loc, main_name, view_name, ct, vt)
      ...
      Format.fprintf
        f
        "@[<hv>%a@.Invalid view argument.@.View '%a' has storage type '%a' and \
         contract '%a' has storage type '%a'.@]"
        Snippet.pp
        loc
        Value_var.pp
        view_name
    ...
    | `Self_ast_typed_view_io (loc, got, arg) ->
      ...
      Format.fprintf
        ...
        got
        s
    | `Self_ast_typed_recursive_call_is_only_allowed_as_the_last_operation
        (_name, loc) ->
      Format.fprintf
        f
        "@[<hv>%a@.Recursive call not in tail position. @.The value of a \
         recursive call must be immediately returned by the defined function. \
         @]"
        Snippet.pp
        loc
    | `Self_ast_typed_bad_self_type (expected, got, loc) ->
      Format.fprintf
        ...
        "@[<hv>%a@.Invalid type annotation.@.\"%a\" was given, but \"%a\" was \
         expected.@.Note that \"Tezos.self\" refers to this contract, so the \
    ...
    | `Self_ast_typed_bad_format_entrypoint_ann (ep, loc) ->
      Format.fprintf
        ...
        "@[<hv>%a@.Invalid entrypoint \"%s\". One of the following patterns is \
         expected:@.* \"%%bar\" is expected for entrypoint \"Bar\"@.* \
    ...
    | `Self_ast_typed_entrypoint_ann_not_literal loc ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid entrypoint value.@.The entrypoint value must be a \
         string literal. @]"
        Snippet.pp
        loc
    | `Self_ast_typed_unmatched_entrypoint loc ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid entrypoint value.@.The entrypoint value does not \
         match a constructor of the contract parameter. @]"
        Snippet.pp
        loc
    | `Self_ast_typed_nested_bigmap loc ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid big map nesting.@.A big map cannot be nested inside \
         another big map. @]"
        Snippet.pp
        loc
    ...
    | `Self_ast_typed_bad_contract_io (entrypoint, _, location) ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid type for entrypoint \"%a\".@.An entrypoint must of \
         type \"parameter * storage -> operation list * storage\". @]"
        Snippet.pp
        location
        Value_var.pp
        entrypoint
    ...
    | `Self_ast_typed_expected_list_operation (entrypoint, got, e) ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid type for entrypoint \"%a\".@.An entrypoint must of \
         type \"parameter * storage -> operation list * storage\".@.We \
         expected a list of operations but we got %a@]"
        Snippet.pp
        e.location
        Value_var.pp
        entrypoint
    ...
    | `Self_ast_typed_expected_same_entry (entrypoint, t1, t2, e) ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid type for entrypoint \"%a\".@.The storage type \
         \"%a\" of the function parameter must be the same as the storage type \
    ...
    | `Self_ast_typed_expected_pair_out loc ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid entrypoint.@.Expected a tuple of operations and \
         storage as return value.@]"
        Snippet.pp
        loc)
...
let error_json : self_ast_typed_error -> Simple_utils.Error.t =
 fun e ->
  ...
  | `Self_ast_typed_annotated_declaration_shadowed location ->
    ...
    make ~stage ~content
  | `Self_ast_typed_storage_view_contract
      (location, main_name, view_name, ct, vt) ->
    let message =
      Format.asprintf
        "Invalid view argument.@.View '%a' has storage type '%a' and contract \
  ...
  | `Self_ast_typed_recursive_call_is_only_allowed_as_the_last_operation
    ...
    let message =
      Format.sprintf
        "Recursive call not in tail position. @.The value of a recursive call \
  ...
  | `Self_ast_typed_bad_self_type (expected, got, location) ->
    let message =
      Format.asprintf
        "Invalid type annotation.@.\"%a\" was given, but \"%a\" was \
         expected.@.Note that \"Tezos.self\" refers to this contract, so the \
         parameters should be the same."
        Ast_typed.PP.type_expression
        got
  ...
  | `Self_ast_typed_bad_format_entrypoint_ann (ep, location) ->
    let message =
      Format.sprintf
        "Invalid entrypoint \"%s\". One of the following patterns is \
         expected:@.* \"%%bar\" is expected for entrypoint \"Bar\"@.* \
         \"%%default\" when no entrypoint is used."
        ep
    in
  ...
  | `Self_ast_typed_entrypoint_ann_not_literal location ->
    let message =
      Format.sprintf
        "Invalid entrypoint value.@.The entrypoint value must be a string \
         literal."
    in
    let content = make_content ~message ~location () in
    make ~stage ~content
  | `Self_ast_typed_unmatched_entrypoint location ->
    let message =
      Format.sprintf
        "Invalid entrypoint value.@.The entrypoint value does not match a \
  ...
  | `Self_ast_typed_nested_bigmap location ->
    let message =
      Format.sprintf
        "Invalid big map nesting.@.A big map cannot be nested inside another \
         big map."
    in
    let content = make_content ~message ~location () in
    make ~stage ~content
  ...
  | `Self_ast_typed_bad_contract_io (entrypoint, _, location) ->
    let message =
      Format.asprintf
        "Invalid type for entrypoint \"%a\".@.An entrypoint must of type \
  ...
  | `Self_ast_typed_expected_list_operation (entrypoint, got, e) ->
    ...
    let message =
      Format.asprintf
        "Invalid type for entrypoint \"%a\".@.An entrypoint must of type \
         \"parameter * storage -> operation list * storage\".@.We expected a \
         list of operations but we got %a"
        Value_var.pp
        entrypoint
        Ast_typed.PP.type_expression
        got
  ...
  | `Self_ast_typed_expected_same_entry (entrypoint, t1, t2, e) ->
    ...
    let message =
      Format.asprintf
        "Invalid type for entrypoint \"%a\".@.The storage type \"%a\" of the \
         function parameter must be the same as the storage type \"%a\" of the \
         return value."
        Value_var.pp
        entrypoint
        Ast_typed.PP.type_expression
  ...
  | `Self_ast_typed_expected_pair_in (location, t) ->
    ...
    in
    let message =
      Format.sprintf "Invalid %s.@.Expected a tuple as argument." ep
    in
    let content = make_content ~message ~location () in
    make ~stage ~content
  | `Self_ast_typed_expected_pair_out location ->
    let message =
      Format.sprintf
        "Invalid entrypoint.@.Expected a tuple of operations and storage as \
         return value."
    in
    let content = make_content ~message ~location () in
    make ~stage ~content

...
(* TODO: is this dead? *)
let rec check_recursive_call ~raise : Value_var.t -> bool -> expression -> unit =
 fun n final_path e ->
  ...
  | E_mod_in { rhs = _; let_result; _ } ->
    check_recursive_call ~raise n final_path let_result
  | E_raw_code _ -> ()
  | E_constructor { element; _ } -> check_recursive_call ~raise n false element
  | E_matching { matchee; cases } ->
    check_recursive_call ~raise n false matchee;
    check_recursive_call_in_matching ~raise n final_path cases
  | E_record elm ->
    List.iter ~f:(check_recursive_call ~raise n false)
    @@ Record.LMap.to_list elm
...
let check_rec_binder_shadowed ~fun_name ~(lambda : _ Lambda.t) =
  ...
  let is_binder_shadowed_in_body =
    not @@ List.mem fv fun_name ~equal:var_equal
  in
  var_equal fun_name (Param.get_var lambda.binder) || is_binder_shadowed_in_body


let check_tail_expression ~raise : expression -> expression =
 fun e ->
  ...
  | E_recursive { fun_name; fun_type = _; lambda } as e ->
    ...
    let () =
      ...
      else check_recursive_call ~raise fun_name true lambda.result
    in
    return e
  | e -> return e
  ...
  then
    raise.warning
      (`Self_ast_typed_warning_unused_rec
        ( Value_var.get_location fun_name
        , Format.asprintf "%a" Value_var.pp fun_name ))
  else ()

...
let rec is_dup (t : type_expression) =
  ...
  | T_record row | T_sum row ->
    Map.for_all row.fields ~f:(fun row_elem ->
        is_dup row_elem.content.associated_type)
  | T_tuple types -> List.for_all types ~f:is_dup
  | T_arrow _ -> true
  | T_variable _ -> true
...
let rec muchuse_of_expr expr : muchuse =
  ...
  | E_record re ->
    Record.fold
      (fun acc x -> muchuse_union (muchuse_of_expr x) acc)
      muchuse_neutral
      re
    ...
    muchuse_unions
      ...
      ; List.fold_left
          ...
          ~f:(fun muchuse (binder, type_) ->
            muchuse_of_binder binder type_ muchuse)
      ]
  | E_while { cond; body } ->
    muchuse_unions [ muchuse_of_expr cond; muchuse_of_expr body ]
  | E_skip -> muchuse_neutral
  | E_cond cond ->
    cond
    |> Conditional.map muchuse_of_expr
    |> Conditional.fold muchuse_union muchuse_neutral
  | E_sequence seq ->
    seq
    |> Sequence.map muchuse_of_expr
    |> Sequence.fold muchuse_union muchuse_neutral
  | E_tuple tuple ->
    tuple
    |> Tuple.map muchuse_of_expr
    |> Tuple.fold muchuse_union muchuse_neutral
  | E_set set_expr ->
    set_expr
    |> Set_expr.map muchuse_of_expr
    |> Set_expr.fold muchuse_union muchuse_neutral
  | E_list list_expr ->
    list_expr
    |> List_expr.map muchuse_of_expr
    |> List_expr.fold muchuse_union muchuse_neutral
  | E_map map_expr | E_big_map map_expr ->
    map_expr
    |> Map_expr.map muchuse_of_expr
    |> Map_expr.fold muchuse_union muchuse_neutral
...
and muchuse_of_cases = function
  ...
  | Match_tuple x -> muchuse_of_tuple x
...
and muchuse_of_variant { cases; tv } =
  ...
  | None ->
    ...
    | Some tv' ->
      let get_c_body (case : _ matching_content_case) =
        case.constructor, (case.body, case.pattern)
      in
      let c_body_lst = Label.Map.of_alist_exn (List.map ~f:get_c_body cases) in
      let get_case c = Map.find_exn c_body_lst (Label c) in
      let match_nil, _ = get_case "Nil" in
      let match_cons, v = get_case "Cons" in
      muchuse_max
        (muchuse_of_binder v (t_pair tv' tv) (muchuse_of_expr match_cons))
        (muchuse_of_expr match_nil))
  | Some ts ->
    let case_ts ({ constructor; _ } : _ matching_content_case) =
      let row_element = Map.find_exn ts.fields constructor in
      row_element.content.associated_type
    in
...
and muchuse_of_record { body; fields; _ } =
  let typed_vars = Map.data fields in
  List.fold_left
    ~f:(fun (c, m) b ->
      muchuse_of_binder (Binder.get_var b) (Binder.get_ascr b) (c, m))
    ~init:(muchuse_of_expr body)
    typed_vars


and muchuse_of_tuple { binders; body; _ } =
  List.fold_left binders ~init:(muchuse_of_expr body) ~f:(fun (c, m) b ->
      muchuse_of_binder (Binder.get_var b) (Binder.get_ascr b) (c, m))
...
let muchused_helper (muchuse : muchuse) = function
  | m ->
    let map, muchused =
      List.fold_right ~f:muchuse_declaration m ~init:muchuse
    in
    (*Put the variable in order : *)
    map, List.rev muchused

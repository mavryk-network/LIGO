...
let all_module_passes ~add_warning ~warn_unused_rec ~raise = [
  ...
  Helpers.map_module @@ Recursion.remove_rec_expression ~warn_unused_rec ~add_warning ;
  Helpers.map_module @@ Pattern_matching_simpl.peephole_expression ~raise ;
]

let all_expression_passes ~add_warning ~warn_unused_rec ~raise = [
  Helpers.map_expression @@ Recursion.check_tail_expression ~raise ;
  Helpers.map_expression @@ Recursion.remove_rec_expression ~warn_unused_rec ~add_warning ;
...
let all_module ~add_warning ~warn_unused_rec ~raise init =
  List.fold ~f:(|>) (all_module_passes ~add_warning ~warn_unused_rec ~raise) ~init

let all_expression ~add_warning ~warn_unused_rec ~raise init =
  List.fold ~f:(|>) (all_expression_passes ~add_warning ~warn_unused_rec ~raise) ~init

...
and translate_constant (expr : I.constant) (ty : I.type_expression) env :
  ...
  let special : (_ O.static_args * I.expression list) option =
    ...
    | C_VIEW -> (
      ...
      | { content = E_literal (Literal_string view_name); type_expression = _; location=_ ; fvs = _} :: arguments ->
    ...
    | C_CONTRACT_ENTRYPOINT ->
      let* a = Mini_c.get_t_contract ty in
      (match expr.arguments with
       | { content = E_literal (Literal_string annot); type_expression = _; location = _ ; fvs = _} :: arguments ->
         let annot = Ligo_string.extract annot in
    ...
    | C_CREATE_CONTRACT ->
      (match expr.arguments with
       | { content= E_closure body ; type_expression = closure_ty ; location =_  ; fvs = _} :: arguments ->

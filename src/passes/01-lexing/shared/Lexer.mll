...
module Make (Options : Options.S) (Token : Token.S) =
  struct
    ...
    let mk_mumav nat state buffer =
      ...
      | Some mumav_64 ->
          ...
          then fail region Non_canonical_zero
              ...
              (Token.mk_mumav nat ~suffix mumav_64 region, state)

    (* Integral Mav *)

    let mk_mav nat suffix state buffer =
      let state, Region.{region; _} = state#sync buffer
      and mav = Q.of_string nat in
      let mumav_bigint = Q.(to_bigint (mul (of_int 1_000_000) mav)) in
      if Z.fits_int64 mumav_bigint then
        if   Q.equal mav Q.zero && String.(nat <> "0")
        then fail region Non_canonical_zero
        else (Token.mk_mav nat ~suffix mav region, state)
      else fail region Overflow_mumav

    (* Mav as a decimal number *)
    ...
    let mk_mav_dec integral fractional suffix state buffer =
      ...
      let q_mav       = Q.make numerator denominator in
      ...
      if Z.equal (Q.den q_mumav) Z.one then
        try
          ...
          then fail region Non_canonical_zero
               ...
               Token.mk_mav lexeme ~suffix q_mav region, state
...
| decimal tz_or_mav { mk_mav_dec integral

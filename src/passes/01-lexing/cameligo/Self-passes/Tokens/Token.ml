...
module T =
  struct
    ...
    | Ident    of (id_kind * lexeme Wrap.t)
    ...
    let wrap_of        = wrap "of"
    let wrap_or        = wrap "or"
    let wrap_originate = wrap "originate"
    ...
    let mk_Entry     region = Entry     (wrap_entry     region)
    ...
    let mk_View      region = View      (wrap_view      region)
    ...
    let ghost_entry     = wrap_entry     Region.ghost
    ...
    let ghost_view      = wrap_view      Region.ghost
    ...
    let ghost_Entry     = Entry     ghost_entry
    ...
    let ghost_View      = View      ghost_view
    ...
    let wrap_ident kind lexeme region =
      kind, Wrap.wrap lexeme region
    ...
    let ghost_ident  k i = wrap_ident    k i Region.ghost
    ...
    let ghost_Ident  k i = Ident    (ghost_ident k i)
    ...
    | Ident (kind, t) ->
        t#region,
        (match kind with
           Plain   -> sprintf "Ident %S" t#payload
         | Escaped -> sprintf "Ident \"@%s\"" t#payload)
    | UIdent t ->
    ...
    | Of        t -> t#region, "Of"
    | Or        t -> t#region, "Or"
    | Originate t -> t#region, "Originate"
    | Rec       t -> t#region, "Rec"
    | Sig       t -> t#region, "Sig"
    | Struct    t -> t#region, "Struct"
    | Test      t -> t#region, "Test"
    | Then      t -> t#region, "Then"
    | To        t -> t#region, "To"
    | Try       t -> t#region, "Try"
    | Type      t -> t#region, "Type"
    | Val       t -> t#region, "Val"
    | View      t -> t#region, "View"
    ...
    let mk_ident kind value region =
      match kind with
        Escaped -> Ident (kind, wrap value region)
      | Plain ->
          match SMap.find keywords value with
            Some mk_kwd -> mk_kwd region
          |        None -> Ident (kind, wrap value region)
    ...
    let start_with_hex = function
      UIdent t | Ident (_,t) -> (
        try
          let head = String.get t#payload 0 in
          List.mem hex_digits head ~equal:Char.equal

  ...
  | TModA ma ->
    ...
    let rec aux : Module_var.t list -> CST.type_expr -> AST.type_expression =
     fun acc exp ->
      ...
      | _ ->
        raise.error
          (expected_access_to_variable (CST.type_expr_to_region ma.field))
    in
...
let rec compile_expression
    ~(raise : (Errors.abs_error, _) Simple_utils.Trace.raise)
    ?fun_rec
    : CST.expr -> AST.expr
  =
 fun e ->
     ...
     fun acc exp ->
      ...
      | EModA ma ->
        aux
          ...
          ma.value.field
      | _ ->
        raise.error (expected_access_to_variable (CST.expr_to_region ma.field))
    in
  ...
  | EFun func ->
    ...
    let ({ binders; type_params; lhs_type; body; arrow = _; attributes = _ }
          : CST.fun_expr)
      =
      func
    in
    ...
    let expr = e_lambda ~loc binder lhs_type expr in
    ...
    let expr =
      ...
      | Some fun_binder ->
        ...
        let fun_type =
          trace_option
            ~raise
            (untyped_recursive_fun @@ Value_var.get_location fun_binder)
          @@ lhs_type
        in
        let lambda = Lambda.map Fun.id (fun x -> Option.value_exn x) lambda in
        e_recursive
          ~loc:(Value_var.get_location fun_binder)
          fun_binder
          fun_type
          lambda
      | None -> expr
    ...
    let expr =
      Option.value_map
        ...
        ~f:(fun tp ->
          let type_vars =
            List.Ne.map compile_type_var @@ npseq_to_ne_list tp.type_vars
          in
          List.Ne.fold_right
  ...
  | EConstr constr ->
    ...
    let args_o =
      Option.map ~f:(compile_tuple_expression <@ List.Ne.singleton) args_o
    in
    let args =
      Option.value
  ...
  | ECond cond ->
    ...
    let else_clause =
      Option.map
        ~f:(fun ((_, b) : _ * CST.branch) ->
          self (fst b.Region.value.CST.inside))
        cond.ifnot
    in
    return
  ...
  | ETypeIn ti ->
    ...
    let ({ type_decl = { name; type_expr; _ }; semi = _; body } : CST.type_in) =
      ti
    in
  ...
  | EModIn mi ->
    ...
    let ({ mod_decl = { kwd_module; name; module_; rbrace; _ }; semi = _; body }
          : CST.mod_in)
      =
      mi
    in
    ...
    let rhs =
      let loc =
        Location.lift @@ CST.Region.cover kwd_module#region rbrace#region
      in
  ...
  | EModAlias ma ->
    let ma, loc = r_split ma in
    let ({ mod_alias = { alias; binders; _ }; semi = _; body } : CST.mod_alias) =
      ma
    in
...
and conv ~raise : CST.pattern -> AST.ty_expr option Pattern.t =
 fun p ->
  let open Pattern in
  match unepar p with
  | PVar { value = { variable; attributes = _ }; _ } ->
...
and compile_tuple_let_destructuring ~raise
    ...
    -> (CST.pattern, CST.comma) Utils.nsepseq CST.reg -> AST.expression
  =
 fun matchee body tuple ->
  ...
  let pattern = Location.wrap @@ Pattern.P_tuple nested_patterns in
  let cases : (AST.expression, AST.ty_expr option) Match_expr.match_case =
    { pattern; body }
  in
...
and compile_record_let_destructuring ~raise
    :  AST.expression -> AST.expression
    -> CST.field_pattern CST.reg CST.ne_injection CST.reg -> AST.expression
  =
 fun matchee body record ->
  let record, loc = r_split record in
  let aux : CST.field_pattern CST.reg -> Label.t * CST.pattern =
   fun field ->
    let ({ field_name; eq = _; pattern } : CST.field_pattern) = field.value in
    Label field_name.value, pattern
  in
  ...
  let pattern = Location.wrap @@ Pattern.P_record lps in
  let cases : (AST.expression, AST.ty_expr option) Match_expr.match_case =
    { pattern; body }
  in
  e_matching ~loc matchee [ cases ]
    ...
    | PVar { value = { variable = name; attributes = _ }; _ } ->
      ...
      let expr =
        compile_expression
          ...
          let_rhs
      in
      return_1 @@ (Binder.make fun_binder lhs_type, attributes, expr)
...
and compile_parameter ~raise
    : CST.pattern -> type_expression option Param.t * (expression -> expression)
  =
 fun pattern ->
  ...
  | PTuple tuple ->
    ...
    in
    let expr expr =
      e_param_matching_tuple (e_variable var) binder_lst @@ fun_ expr
    in
  ...
  | PTyped tp ->
    ...
    let binder = Param.map (Fn.const @@ Some ascr) binder in
    binder, exprs
  | _ -> raise.error @@ unsupported_pattern_type pattern


and compile_binder ~raise
    :  CST.pattern
    -> type_expression option Binder.t * (expression -> expression)
  =
 fun pattern ->
  ...
  | PTyped tp ->
    ...
    let ({ pattern; type_expr; colon = _ } : CST.typed_pattern) = tp in
    let ascr = compile_type_expression ~raise type_expr in
    let binder, exprs = compile_binder ~raise pattern in
...
and compile_declaration ~raise : CST.declaration -> _ =
 fun decl ->
  let return reg decl : AST.declaration list =
    List.map ~f:(Location.wrap ~loc:(Location.lift reg)) decl
  in
  ...
  | TypeDecl { value = { name; type_expr; params; _ }; region } ->
    ...
    let type_expr =
      ...
      | Some x ->
        ...
        let aux
            :  CST.type_var Region.reg -> AST.type_expression
            -> AST.type_expression
          =
         fun param type_ ->
          let param, loc = r_split param in
          let ty_binder =
            Type_var.of_input_var ~loc (quote_var param.name.value)
          in
  ...
  | ConstDecl { value = _kwd_let, kwd_rec, let_binding, attributes; region } ->
    ...
    | { binders; let_rhs; lhs_type = _; eq = _ } ->
      ...
      | CST.PTuple tuple ->
        ...
        let aux i binder =
          ( Z.add i Z.one
          , (binder, attributes, e_accessor expr @@ [ Access_tuple i ]) )
        in
        let lst = snd @@ List.fold_map ~f:aux ~init:Z.zero @@ lst in
        let aux (binder, attr, expr) : AST.declaration_content =
      ...
      | CST.PRecord record ->
        ...
        let aux
            ({ value = { field_name; eq = _; pattern }; _ } :
              CST.field_pattern CST.reg)
          =

...
let compile_attributes (attributes : string Region.reg list) : AST.Attr.t =
...
let rec compile_expression ~(raise:(Errors.abs_error,_) Simple_utils.Trace.raise) ?fun_rec : CST.expr -> AST.expr  = fun e ->
  ...
  | EFun func ->
    ...
    let ({binders; type_params; lhs_type; body; arrow=_; attributes=_} : CST.fun_expr) = func in
    ...
    let expr = match fun_rec with
      Some fun_binder ->
        ...
        let lhs_type = Option.map ~f:(Utils.uncurry t_arrow) @@ Option.bind_pair (Param.get_ascr lambda.binder, lambda.output_type) in
  ...
  | ELetIn li -> (
    ...
    let body = self body in
    let (binder,attr,rhs) = compile_let_binding ~raise ?kwd_rec attributes binding in
    return @@ e_let_in ~loc binder attr rhs body
  )
  | ETypeIn ti ->
...
and compile_pattern ~raise : CST.pattern -> AST.ty_expr option Pattern.t =
  ...
  let open Pattern in
  match unepar p with
  | CST.PVar {value={variable; attributes = _}; _} ->
  ...
  | CST.PRecord record ->
    ...
    let lst = Container.List.of_list (List.Ne.to_list lst) in
  ...
  | CST.PConstr pattern -> (
      ...
      let pv_opt = match p_opt with
        | Some pv -> compile_pattern ~raise pv
...
and compile_parameter ~raise : CST.pattern -> type_expression option Param.t * (expression -> expression) =
  fun pattern ->
  let return ?ascr fun_ var =
    (Param.make var ascr, fun_) in
  ...
  | PTuple tuple ->
    ...
    let expr = fun expr -> e_param_matching_tuple (e_variable var) binder_lst @@ fun_ expr in
...
and compile_binder ~raise : CST.pattern -> type_expression option Binder.t * (expression -> expression) =
  fun pattern ->
  let return ?ascr fun_ var =
  ...
  | PTuple tuple ->
    ...
    return ?ascr expr var
  | PPar par ->
    compile_binder ~raise par.value.inside
  | PRecord _ ->raise.error @@ unsupported_pattern_type pattern
  | PTyped tp ->
    let (tp, _loc) = r_split tp in
    let {pattern; type_expr; colon=_} : CST.typed_pattern = tp in
...
and compile_declaration ~raise : CST.declaration -> AST.declaration option = fun decl ->
  ...
  match decl with
    ...
    in
    return region @@ D_type {type_binder; type_expr; type_attr=[]}
...
and compile_module ~raise : CST.ast -> AST.module_ = fun t ->
    List.filter_map ~f:(compile_declaration ~raise) @@ nseq_to_list t.decl

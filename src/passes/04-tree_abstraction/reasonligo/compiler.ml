  ...
  | EFun func ->
    ...
    let expr =
      ...
      | Some fun_binder ->
        ...
        in
        let fun_type =
          trace_option ~raise (untyped_recursive_fun @@ Value_var.get_location fun_binder)
          @@ lhs_type
        in
    ...
    let expr =
      Option.value_map
        ~default:expr
        ~f:(fun tp ->
          let type_vars = List.Ne.map compile_type_var @@ npseq_to_ne_list tp.type_vars in
...
and compile_tuple_let_destructuring ~raise
    :  AST.expression -> AST.expression -> (CST.pattern, CST.comma) Utils.nsepseq CST.reg
    -> AST.expression
  =
...
and compile_binder ~raise
    : CST.pattern -> type_expression option Binder.t * (expression -> expression)
  =
 fun pattern ->

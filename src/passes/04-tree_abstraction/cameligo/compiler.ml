  ...
  | EFun func ->
    ...
    let expr = (match binders with
        ...
        | _ ->
          ...
          e_lambda ~loc param rhs_type @@ expr) in
    return expr
  | EConstr constr ->
    ...
    | [], None, None ->
      let matchee =
        ...
        | None ->
          ...
          | CST.PUnit _ -> e_annotation ~loc:let_rhs.location let_rhs (AST.t_unit ~loc ())
          | _ -> let_rhs)
      in
      let pattern = compile_pattern ~raise pattern in
      let pattern =
    ...
    | _, _, _ ->
      ...
      let let_rhs, rhs_type = (match binders with
          ...
          | _ ->
            ...
            let param = Param.make var tuple_type in
            e_lambda ~loc param rhs_type @@ expr, fun_type) in
        ...
        | Some lst ->
          ...
          let aux
           ...
           fun p acc ->
            ...
            Location.wrap ~loc (P_list (Cons (p', acc)))
          in
          let conscomb =

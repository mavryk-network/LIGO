...
let build_ins = ["Operation";"Test";"Tezos";"Crypto";"Bytes";"List";"Set";"Map";"Big_map";"Bitwise";"String";"Layout"]
...
and compile_instruction : ?next: AST.expression -> CST.instruction -> _ result  = fun ?next instruction ->
  ...
  | Loop (For (ForInt fl)) ->
    ...
    let* increment = Option.value ~default:(ok @@ e_int_z Z.one) @@
      Option.map ~f:(compile_expression <@ snd) fl.step
    in

...
let compile_variable var = let (var,loc) = w_split var in ValueVar.of_input_var ~loc var
...
let compile_var_opt : CST.expr -> AST.expression_variable option = fun expr ->
  Option.map (get_var expr) ~f:(fun (v,loc) -> ValueVar.of_input_var ~loc v)
...
let rec compile_expression ~(raise :Errors.abs_error Simple_utils.Trace.raise) : ?attr:CST.attribute list -> CST.expr -> AST.expr = fun ?(attr = []) e ->
  ...
  let compile_tuple_expression : CST.expr CST.tuple -> AST.expression = fun tuple_expr ->
    ...
    | _ ->
      ...
      e_tuple ~loc lst
  in
  let compile_tuple_expressions : CST.expr CST.tuple -> (AST.expression list * location) = fun tuple_expr ->
    let (lst, loc) = r_split tuple_expr in
    let (hd,tl) = lst.inside in
    let tl = List.map ~f:snd tl in
    match tl with
      ...
      let f : AST.expression -> (CST.expr, CST.expr) CST.field CST.reg -> AST.expression = fun acc x ->
        ...
        | CST.Punned {pun ; attributes} -> (
          ...
          let label,v =
            ...
            if String.equal label "_" then raise.raise (unexpected_wildcard @@ CST.expr_to_region pun) ;
            (label, ValueVar.of_input_var ~loc label)
          in
...
and conv ~raise : ?const:bool -> CST.pattern -> AST.ty_expr AST.pattern =
  fun ?(const = false) p ->
    ...
    | P_Var var -> (
      ...
      let b = { var ; ascr = None ; attributes } in
      Location.wrap ~loc:(ValueVar.get_location var) (P_var b)
    )
    | P_Tuple tuple -> (
      ...
      let aux : CST.field_pattern CST.reg -> AST.label * AST.ty_expr AST.pattern =
        fun x ->
          ...
          | Punned { pun ; attributes } ->
            ...
            let attributes = if const then Stage_common.Helpers.const_attribute else Stage_common.Helpers.var_attribute in
            let binder = { var = ValueVar.of_input_var ~loc label ; ascr = None ; attributes } in
...
and compile_parameter ~raise ?(attributes = Stage_common.Helpers.const_attribute) : CST.pattern -> _ binder * (_ -> _) =
  fun pattern ->
  ...
  match pattern with
  | P_Var pvar ->
    return_1 ~attributes @@ compile_variable pvar
  | P_Tuple tuple ->
    let (tuple, loc) = r_split tuple in
    let var = ValueVar.fresh ~loc () in
    let aux pattern (binder_lst, fun_) =
      let (binder,fun_') = compile_parameter ~raise ~attributes pattern in
      (binder :: binder_lst, fun_' <@ fun_)
    in
    let binder_lst, fun_ = List.fold_right ~f:aux ~init:([],fun e -> e) @@ npseq_to_list tuple.inside in
...
and compile_parameters ~raise : CST.parameters -> (AST.type_expression AST.binder) list * (expression -> expression) = fun params ->
  let aux : CST.param_decl CST.reg -> _ * (expression -> expression) -> (AST.type_expression AST.binder) list * (expression -> expression) = fun param (binders, k) ->
    let (param, _loc) = r_split param in
    let attributes = match param.param_kind with `Var _ -> Stage_common.Helpers.var_attribute
                                               | `Const _ -> Stage_common.Helpers.const_attribute in
    let binder, k' = compile_parameter ~raise ~attributes param.pattern in
  ...
  let binders, k = List.fold_right ~f:aux ~init:([], fun e -> e) params in
  binders, k
      ...
      fun (last_proj, updator) lst ->
        ...
        | access::tl -> (
          ...
          | Access_map k -> (
            ...
            let some_proj = ValueVar.fresh () in
            let some_body =
...
and compile_fun_decl loc ~raise : CST.fun_decl -> expression_variable * type_expression option * expression =
...
and compile_declaration ~raise : ?attr:CST.attribute list -> CST.declaration -> AST.declaration list =
  ...
  | D_Type { value = { name ; params ; type_expr; _ } ; region } ->
      ...
      let ast =
        AST.Declaration_type {type_binder= compile_type_var name;
                              type_expr; type_attr=[]}

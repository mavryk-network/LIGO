...
let compile_selection : CST.selection -> 'a Access_path.access * Location.t
  = function
  | FieldName name ->
 ...
 fun ?(attr = []) te ->
  ...
  | T_App { region; value = type_constant, args } ->
    ...
    | T_ModPath x ->
      raise.error
        (expected_variable
           (Location.lift @@ CST.type_expr_to_region x.value.field))
    | _ ->
      raise.error
        (expected_variable
           (Location.lift @@ CST.type_expr_to_region type_constant)))
  ...
  | T_ModPath { region; value = { module_path; selector = _; field } } ->
    ...
    | _ ->
      raise.error
        (expected_variable (Location.lift @@ CST.type_expr_to_region field)))
  ...
  | T_Record
      { region
      ; value =
          { kind = _loc; opening = _; elements; closing = _; terminator = _ }
      } ->
    let elements = Utils.sepseq_to_list elements in
    let f
        :  CST.field_decl CST.reg
        -> string * AST.type_expression * AST.attributes
      =
     fun decl ->
    ...
    t_record_ez_attr
      ~loc:(Location.lift region)
      ~attr:(compile_attributes attr)
      lst
...
let rec compile_expression
 ...
 fun ?(attr = []) e ->
  ...
  in
  let compile_bin_op
      : Constant.constant' -> _ CST.bin_op CST.reg -> AST.expression
    =
   fun op_type op ->
  ...
  in
  let compile_un_op
      : Constant.constant' -> _ CST.un_op CST.reg -> AST.expression
    =
   fun op_type op ->
    let op, loc = r_split op in
      ...
      | CST.Complete { field_lhs; field_lens; field_rhs; attributes } ->
        ...
        | Lens_Id _ ->
          let label, _ =
            trace_option
              ~raise
              (expected_field_name @@ CST.expr_to_region field_lhs)
            @@ get_var field_lhs
          in
          label, self field_rhs
        | _ ->
          raise.error
            (wrong_functional_lens @@ CST.field_lens_to_region field_lens))
    in
  ...
  | E_ModPath ma ->
    ...
    let module_path =
      List.map ~f:compile_mod_var (npseq_to_list ma.module_path)
    in
    (match ma.field with
      ...
      let var =
        let x, loc =
          trace_option
            ~raise
            (expected_variable
               (Location.lift @@ CST.expr_to_region proj.record_or_tuple))
    ...
    | _ ->
      raise.error
        (expected_variable (Location.lift @@ CST.expr_to_region ma.field)))
  | E_Update { value = { structure; kwd_with = _; update }; region } ->
    ...
    | E_Record record_lhs ->
      let f
          :  AST.expression -> (CST.expr, CST.expr) CST.field CST.reg
          -> AST.expression
        =
       fun acc x ->
        ...
        | CST.Complete { field_lhs; field_lens; field_rhs; attributes } ->
          ...
          | CST.E_Var x ->
            ...
            let self_accessor =
              e_accessor ~loc structure [ Access_record label ]
            in
            e_update
              ~loc
              acc
              [ Access_record label ]
              (func_update self_accessor)
          | CST.E_Proj
              { region; value = { record_or_tuple; selector = _; field_path } }
            ->
            let label, _ =
              trace_option
                ~raise
                (expected_variable
                   (Location.lift @@ CST.expr_to_region record_or_tuple))
            ...
            in
            let self_accessor =
              e_accessor ~loc:(Location.lift region) structure path
            in
      ...
      in
      List.fold_left
        (Utils.sepseq_to_list record_lhs.value.elements)
        ~f
        ~init:structure
  ...
  | E_Fun { value = { parameters; ret_type; return; _ }; region } ->
    ...
    let compile_param : CST.param_decl CST.reg -> _ Param.t =
     fun { value = { param_kind; pattern; param_type }; region = _ } ->
      ...
      let var =
        ...
        | x ->
          raise.error (unsuported_pattern_in_function @@ CST.pattern_to_region x)
      in
      let ascr =
        Option.map ~f:(compile_type_expression ~raise <@ snd) param_type
      in
    ...
    let lambda, fun_type =
      ...
      let ret_ty =
        Option.map ~f:(compile_type_expression ~raise <@ snd) ret_type
      in
      ...
      | params ->
        ...
        let expr =
          let body =
            e_param_matching_tuple ~loc:loc_par (e_variable binder) params body
          in
          e_lambda_ez ~loc binder ?ascr:input_tuple_ty ret_ty body
        in
        let ty_opt =
          Option.map
            ~f:(fun (a, b) -> t_arrow a b)
  ...
  | E_List lc ->
    ...
    let lst =
      Option.value ~default:[] @@ Option.map ~f:npseq_to_list lc.elements
    in
    let lst = List.map ~f:self lst in
    e_list ~loc lst
  | E_Cons cons ->
  ...
  | E_Set set ->
    ...
    let set =
      Option.value ~default:[] @@ Option.map ~f:npseq_to_list si.elements
    in
  ...
  | E_Map map ->
    ...
    let lst =
      Option.value ~default:[] @@ Option.map ~f:npseq_to_list mij.elements
    in
...
and conv ~raise : CST.pattern -> AST.ty_expr option Pattern.t =
 fun p ->
  ...
  | P_Record { region; value = { elements; _ } } ->
    ...
    let aux
        : CST.field_pattern CST.reg -> Label.t * AST.ty_expr option Pattern.t
      =
     fun x ->
      ...
      | Complete { field_lhs; field_rhs; attributes; _ } ->
        ...
        let lhs, _loc =
          ...
          | P_Var x -> w_split x
          | x ->
            raise.error (expected_field_or_access @@ CST.pattern_to_region x)
        in
  ...
  | P_Typed { region; value = { pattern; type_annot = _, ty_expr } } ->
    ...
    | P_var x ->
      Location.wrap ~loc (P_var (Binder.map (Fn.const (Some ty_expr)) x))
    | _ ->
      raise.error
        (unsupported_type_ann_on_patterns @@ CST.pattern_to_region pattern))
  | P_Ctor x ->
...
and compile_parameters ~raise
    : CST.parameters -> AST.type_expression option Param.t list
  =
 fun params ->
...
and compile_path
    : (CST.selection, CST.dot) Utils.nsepseq -> AST.expression Access_path.t
  =
 fun x ->
...
*)
and path_of_lvalue ~raise
    : CST.expr -> Value_var.t * AST.expression Access_path.t
  =
 fun expr ->
  let rec aux (lhs : CST.expr) (cpath : AST.expression Access_path.t) =
    ...
    | E_MapLookup { value = { map; keys }; _ } ->
      let keys =
        List.map
          ~f:(fun x ->
            Access_path.Access_map (compile_expression ~raise x.value.inside))
          (nseq_to_list keys)
      in
      ...
      | None -> aux map path)
    | E_ModPath _ ->
      raise.error (expected_field_or_access @@ CST.expr_to_region lhs)
...
and compile_assignment
    :  loc:Location.t -> last_proj_update:(expression -> expression)
    -> lhs:Value_var.t -> path:AST.expression Access_path.t
    -> default_rhs:AST.expression -> AST.expression
  =
...
and compile_instruction ~raise
    : ?next:AST.expression -> CST.instruction -> AST.expression
  =
 fun ?next instruction ->
  let return expr = Option.value_map next ~default:expr ~f:(e_sequence expr) in
  let compile_if_clause
      : ?next:AST.expression -> CST.test_clause -> AST.expression
    =
   fun ?next if_clause ->
  ...
  | I_Cond { region; value = { test; if_so; if_not; _ } } ->
    ...
    let ifnot =
      Option.value_map if_not ~default:(e_skip ()) ~f:(fun x ->
  ...
  | I_Case { region; value = { expr; cases; _ } } ->
    ...
    let cases =
      compile_matching_expr ~raise compile_if_clause (npseq_to_ne_list cases)
    in
  ...
  | I_Assign { region; value = { lhs; rhs; _ } } ->
    ...
    | last_access :: path ->
      ...
      | Access_map k ->
        let default_rhs =
          e_map_add ~loc k (compile_expression ~raise rhs) (e_variable var)
        in
      ...
      | Access_record _ | Access_tuple _ ->
        ...
        let last_proj_update last_proj =
          e_update ~loc last_proj [ last_access ] rhs
        in
        return
        @@ compile_assignment ~loc ~last_proj_update ~lhs:var ~path ~default_rhs))
  ...
  | I_ForIn
      (ForMap
        { region; value = { binding = key, _, value; collection; block; _ } })
    ->
    let loc = Location.lift region in
    let binder =
  ...
  | I_ForIn
      (ForSetOrList { region; value = { var; for_kind; collection; block; _ } })
    ->
    let loc = Location.lift region in
  ...
  | I_Patch { region; value = { collection; patch; patch_kind; _ } } ->
    ...
    let last_proj_update, default_rhs =
      ...
      | E_map kvl, `Map _ ->
        ...
        , List.fold kvl ~f ~init:(e_variable v) )
...
and compile_let_destructuring ~raise
    :  ?const:bool -> Location.t -> CST.expr -> CST.pattern -> AST.expression
    -> AST.type_expression option -> AST.expression
  =
...
and compile_data_declaration ~raise
    :  ?attr:CST.attribute list -> next:AST.expression -> CST.declaration
    -> AST.expression
  =
 fun ?(attr = []) ~next data_decl ->
  ...
  | D_Const const_decl ->
    ...
    let type_ =
      Option.map ~f:(compile_type_expression ~raise <@ snd) cd.const_type
    in
...
and compile_statement ~raise
 ...
 fun ?next statement ->
  ...
  | S_VarDecl var_decl ->
    ...
    let type_ =
      Option.map ~f:(compile_type_expression ~raise <@ snd) vd.var_type
    in
...
and compile_block ~raise
    : ?next:AST.expression -> CST.block CST.reg -> AST.expression
  =
 fun ?next block ->
...
and compile_fun_decl loc ~raise
 ...
 fun ({ kwd_recursive
  ...
  let lambda, fun_type =
    match param with
    | [ binder ] ->
      let lambda : (_, _ option) Lambda.t =
        { binder; output_type = ret_type; result }
      in
      ( lambda
  ...
  let func =
    ...
    | Some reg ->
      let fun_type =
        trace_option ~raise (untyped_recursive_fun loc) @@ fun_type
      in
...
and compile_declaration ~raise
    : ?attr:CST.attribute list -> CST.declaration -> AST.declaration list
  =
 fun ?(attr = []) decl ->
  ...
  | D_Type { value = { name; params; type_expr; _ }; region } ->
    ...
    in
    let ast =
      D_type { type_binder = compile_type_var name; type_expr; type_attr = [] }
    in
    return region ast
  | D_Const { value = { pattern; const_type; init; _ }; region } ->
    ...
    | P_Var name ->
      let var = compile_variable name in
      let ascr =
        Option.map ~f:(compile_type_expression ~raise <@ snd) const_type
      in
      let expr = compile_expression ~raise init in
      let binder = Binder.make var ascr in
      let ast = D_value { binder; attr; expr } in
      return region ast
    | _ -> raise.error (unsupported_top_level_destructuring region))
  | D_Fun { value; region } ->
    let var, ascr, expr =
      compile_fun_decl (Location.lift region) ~raise value
    in
    let binder = Binder.make var ascr in
...
and compile_module_expression ~raise : CST.module_expr -> AST.module_expr
  = function
...
and compile_declarations ~raise : CST.declaration Utils.nseq -> AST.module_ =
 fun decl ->
  let lst = List.map ~f:(compile_declaration ~raise) @@ nseq_to_list decl in
  List.concat lst

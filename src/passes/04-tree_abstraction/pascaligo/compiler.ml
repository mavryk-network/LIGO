...
and compile_assignment
 ...
 fun ~loc ~last_proj_update ~lhs ~path ~default_rhs ->
  let rec aux
    : AST.expr * (AST.expr -> AST.expr) -> AST.expression Access_path.t -> AST.expression
    =
...
and compile_instruction ~raise : ?next:AST.expression -> CST.instruction -> AST.expression
 ...
 fun ?next instruction ->
  ...
  | I_Cond { region; value = { test; if_so; if_not; _ } } ->
    ...
    let ifnot =
      Option.value_map if_not ~default:(e_skip ()) ~f:(fun x -> compile_if_clause (snd x))
    in
  ...
  | I_For { value = { index; init; bound; step; block; _ }; region } ->
    ...
    let increment =
      Option.value_map step ~default:(e_int_z Z.one) ~f:(compile_expression ~raise <@ snd)
    in
    let body = compile_block ~raise block in
  ...
  | S_VarDecl var_decl ->
    ...
    (match vd.pattern with
     | P_Var name ->
       ...
       (match next with
        | Some next -> Some (e_let_mut_in ~loc (Binder.make var type_) [] init next)
        | None -> None)
     | pattern ->
       (* not sure what to do with  attributes in that case *)
       (match next with
        | Some next ->
          let x =
            compile_let_destructuring ~raise ~const:false loc vd.init pattern next type_
          in
          Some x
        | None -> None))

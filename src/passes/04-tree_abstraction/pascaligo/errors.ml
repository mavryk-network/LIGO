 ...
 fun ~display_format f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Concrete_pascaligo_wrong_lvalue reg ->
      Format.fprintf
        f
        "@[<hv>%a@.Effectful updates must be performed on identified objects \
         that are not accessed through a module@]"
        Snippet.pp_lift
        reg
    | `Concrete_pascaligo_unsupported_type_ann_on_patterns reg ->
      Format.fprintf
        f
        "@[<hv>%a@.Type annotations on this kind of patterns are not supported \
         yet@]"
        Snippet.pp_lift
        reg
    | `Concrete_pascaligo_unsuported_pattern_in_function reg ->
      Format.fprintf
        f
        "@[<hv>%a@.These kind of patterns are not supported in function \
         parameters@]"
        Snippet.pp_lift
        reg
    ...
    | `Concrete_pascaligo_expected_field_name reg ->
      Format.fprintf f "@[<hv>%a@.Expected a field name@]" Snippet.pp_lift reg
    ...
    | `Concrete_pascaligo_ignored_attribute loc ->
      Format.fprintf f "@[<hv>%a@.Attribute being ignored@]" Snippet.pp loc
    | `Concrete_pascaligo_expected_variable loc ->
      Format.fprintf f "@[<hv>%a@.Expected a declaration name@]" Snippet.pp loc
    ...
    | `Concrete_pascaligo_unsupported_pattern_type pl ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid case pattern.@.Can't match on values. @]"
        Snippet.pp_lift
      @@ Raw.pattern_to_region pl
    ...
    | `Concrete_pascaligo_block_start_with_attribute block ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid attribute declaration.@.Attributes have to follow \
...
let error_json : abs_error -> Simple_utils.Error.t =
 fun e ->
  ...
  | `Concrete_pascaligo_wrong_lvalue reg ->
    let message =
      "Effectful updates must be performed on identified objects that are not \
       accessed through a module."
    in
  ...
  | `Concrete_pascaligo_unsupported_type_ann_on_patterns reg ->
    let message =
      "Type annotations on this kind of patterns are not supported yet."
    in
    let location = Location.lift reg in
    let content = make_content ~message ~location () in
    make ~stage ~content
  | `Concrete_pascaligo_unsuported_pattern_in_function reg ->
    let message =
      "These kind of patterns are not supported in function parameters."
    in
    let location = Location.lift reg in
    let content = make_content ~message ~location () in
    make ~stage ~content
  ...
  | `Concrete_pascaligo_unsupported_pattern_type pl ->
    let message =
      Format.sprintf "Invalid case pattern.@.Can't match on values."
    in
  ...
  | `Concrete_pascaligo_block_start_with_attribute block ->
    let message =
      "Invalid attribute declaration.@.Attributes have to follow the \

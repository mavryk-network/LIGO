 ...
 fun ~display_format f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Concrete_pascaligo_wrong_lvalue reg ->
      Format.fprintf
        f
        "@[<hv>%a@.Effectful updates must be performed on identified objects that are \
         not accessed through a module@]"
        Snippet.pp_lift
        reg
    ...
    | `Concrete_pascaligo_block_start_with_attribute block ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid attribute declaration.@.Attributes have to follow the \
...
let error_json : abs_error -> Simple_utils.Error.t =
 fun e ->
  ...
  | `Concrete_pascaligo_wrong_lvalue reg ->
    let message =
      "Effectful updates must be performed on identified objects that are not accessed \
       through a module."
    in
  ...
  | `Concrete_pascaligo_block_start_with_attribute block ->
    let message =
      "Invalid attribute declaration.@.Attributes have to follow the declaration it is \
       attached to."
    in

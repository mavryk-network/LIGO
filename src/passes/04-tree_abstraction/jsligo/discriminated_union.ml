...
let reset () = disc_unions := []
let nseq_to_list (hd, tl) = hd :: tl
let npseq_to_list (hd, tl) = hd :: List.map ~f:snd tl

let has_constructor (variants : disc_variant list) (variant_name : string)
    : bool
  =
...
let find_disc_union (variants : string list) =
  ...
  let rec aux = function
    | item :: tl when List.length item = variants_length ->
      let is_disc_union =
        List.fold_left
          ~f:(fun a i -> a && has_constructor item i)
          ~init:true
          variants
      in
...
let has_field (variant : disc_variant) (field_name, field_value) : bool =
  List.mem variant.fields field_name ~equal:String.equal
  || (String.equal variant.constructor_field field_name
     && String.equal variant.constructor field_value)
...
let is_disc_union (a : disc_variant list) (fields : (string * string) list) =
  let rec aux = function
    | variant :: tl when List.length variant.fields + 1 = List.length fields ->
      let result =
        List.fold_left
          ~f:(fun a i -> a && has_field variant i)
          ~init:true
          fields
      in
      if result then Some variant else aux tl
    | _ :: tl -> aux tl
    | [] -> None
  in
  aux a
...
let is_discriminated_union (a : CST.switch Region.reg) =
  ...
  then None
    ...
    in
    let check1, values =
      switch_fields (true, []) (nseq_to_list a.value.cases)
    in
    if not check1
    then None
    else
...
let find_disc_obj (a : CST.object_expr) =
  ...
  if check then find_variant fields else None


let get_shared_field ~raise (n : (CST.obj_type, CST.vbar) Utils.nsepseq) =
  let shared_fields =
    Utils.nsepseq_foldl
      (fun shared_fields (obj : CST.obj_type) ->
  ...
  let shared_field =
    ...
    | [] ->
      raise.error
      @@ no_shared_fields
           (CST.nsepseq_to_region (fun (r : CST.obj_type) -> r.region) n)
    | (hd, _) :: _ -> hd
  in
  shared_field


let add disc_union = disc_unions := disc_union :: !disc_unions

...
type abs_error =
  ...
  | `Concrete_jsligo_funarg_tuple_type_mismatch of
...
let error_ppformat
    :  display_format:string display_format -> Format.formatter -> abs_error
    -> unit
  =
 fun ~display_format f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Concrete_jsligo_untyped_recursive_fun reg ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid function declaration.@.Recursive functions are \
         required to have a type annotation (for now). @]"
        Snippet.pp_lift
        reg
    | `Concrete_jsligo_unsupported_pattern_type pl ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid pattern matching.@.  If this is pattern matching \
         over Booleans, then \"true\" or \"false\" is expected.@.  If this is \
         pattern matching on a list, then one of the following is \
         expected:@.    * an empty list pattern \"[]\";@.    * a cons list \
         pattern \"[head, ...tail]\".@.  If this is pattern matching over \
         variants, then a constructor of a variant is expected.@.@.  Other \
    ...
    | `Concrete_jsligo_unsupported_string_singleton te ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid type. @.It's not possible to assign a string to a \
         type. @]"
        Snippet.pp_lift
    ...
    | `Concrete_jsligo_michelson_type_wrong (texpr, name) ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid \"%s\" type.@.At this point, an annotation, in the \
         form of a string, is expected for the preceding type. @]"
        Snippet.pp_lift
    ...
    | `Concrete_jsligo_michelson_type_wrong_arity (loc, name) ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid \"%s\" type.@.An even number of 2 or more arguments \
         is expected, where each odd item is a type annotated by the following \
         string. @]"
        Snippet.pp
        loc
        name
    ...
    | `Concrete_jsligo_statement_not_supported_at_toplevel s ->
      Format.fprintf
        f
        "@[<hv>%a@.Statement not supported at toplevel.@.Only let, const, and \
         type statements are supported at the toplevel. @]"
        Snippet.pp_lift
    ...
    | `Concrete_jsligo_invalid_case (s, e) ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid '%s' field value. An anonymous arrow function was \
         expected, eg. `None: () => foo`.@]"
        Snippet.pp_lift
    ...
    | `Concrete_jsligo_invalid_constructor e ->
      Format.fprintf
        f
        "@[<hv>%a@.Invalid constructor. Expected a constructor like: \
    ...
    | `Concrete_jsligo_expected_a_variable reg ->
      Format.fprintf f "@[<hv>%a@.Expected a variable.@]" Snippet.pp_lift reg
    ...
    | `Concrete_jsligo_no_shared_fields r ->
      Format.fprintf
        f
        "@[<hv>%aAll the objects are expected to have a shared field with an \
         unique value.@]"
        Snippet.pp_lift
        r
    | `Concrete_jsligo_unexpected ->
    ...
    | `Concrete_jsligo_case_break_disc r ->
      Format.fprintf
        f
        "@[<hv>%aA discriminated union case needs to end with a `break` or \
         `return` statement. A fallthrough to another case is currently not \
         supported. @]"
        Snippet.pp_lift
        r
    | `Concrete_jsligo_not_implemented r ->
      Format.fprintf
        f
        "@[<hv>%aThis has not been implemented yet. @]"
        Snippet.pp_lift
        r)
...
let error_json : abs_error -> Simple_utils.Error.t =
 fun e ->
  ...
  | `Concrete_jsligo_untyped_recursive_fun reg ->
    let message =
      "Invalid function declaration.@.Recursive functions are required to have \
       a type annotation (for now)."
    in
  ...
  | `Concrete_jsligo_unsupported_pattern_type pl ->
    let message =
      Format.sprintf
        "Invalid pattern matching.@.  If this is pattern matching over \
         Booleans, then \"true\" or \"false\" is expected.@.  If this is \
         pattern matching on a list, then one of the following is \
         expected:@.    * an empty list pattern \"[]\";@.    * a cons list \
         pattern \"[head, ...tail]\".@.  If this is pattern matching over \
         variants, then a constructor of a variant is expected.@.@.  Other \
  ...
  | `Concrete_jsligo_unsupported_string_singleton te ->
    let message =
      "Invalid type. @.It's not possible to assign a string to a type."
    in
  ...
  | `Concrete_jsligo_recursion_on_non_function location ->
    let message =
      "Invalid let declaration.@.Only functions can be recursive."
    in
    let content = make_content ~message ~location () in
    make ~stage ~content
  | `Concrete_jsligo_michelson_type_wrong (texpr, name) ->
    let message =
      Format.sprintf
        "Invalid \"%s\" type.@.At this point, an annotation, in the form of a \
  ...
  | `Concrete_jsligo_michelson_type_wrong_arity (location, name) ->
    let message =
      Format.sprintf
        "Invalid \"%s\" type.@.An even number of 2 or more arguments is \
         expected, where each odd item is a type annotated by the following \
         string."
        name
    in
    let content = make_content ~message ~location () in
  ...
  | `Concrete_jsligo_funarg_tuple_type_mismatch (region, pattern, texpr) ->
    ...
    let message =
      Format.sprintf
        "The tuple \"%s\" does not have the expected type \"%s\"."
        p
        t
    in
  ...
  | `Concrete_jsligo_statement_not_supported_at_toplevel s ->
    let message =
      Format.sprintf
        "Statement not supported at toplevel.@.Only let, const, and type \
  ...
  | `Concrete_jsligo_invalid_case (s, e) ->
    let message =
      Format.sprintf
        "Invalid '%s' field value. An anonymous arrow function was expected, \
         eg. `None: () => foo`."
        s
    in
  ...
  | `Concrete_jsligo_invalid_constructor e ->
    let message =
      Format.sprintf
        "Invalid constructor. Expected a constructor like: `[\"Foo\"]` or \
         `[\"Foo\", int, string]`."
    in
  ...
  | `Concrete_jsligo_no_shared_fields r ->
    let message =
      Format.sprintf
        "All the objects are expected to have a shared field with an unique \
         value."
    in
  ...
  | `Concrete_jsligo_wrong_matchee_disc r ->
    let message =
      Format.sprintf "Expected a record field. For example: `field.kind`."
    in
    let location = Location.lift r in
    let content = make_content ~message ~location () in
    make ~stage ~content
  | `Concrete_jsligo_case_break_disc r ->
    let message =
      Format.sprintf
        "A discriminated union case needs to end with a `break` or `return` \

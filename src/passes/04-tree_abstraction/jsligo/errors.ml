  ...
  | Human_readable | Dev ->
    (match a with
     ...
     | `Concrete_jsligo_unknown_constructor (s, loc) ->
       Format.fprintf f "@[<hv>%a@.Unknown constructor in module: %s" Snippet.pp loc s
     ...
     | `Concrete_jsligo_unsupported_pattern_type pl ->
       Format.fprintf
         ...
         "@[<hv>%a@.Invalid pattern matching.@.  If this is pattern matching over \
          ...
          matching on a list, then one of the following is expected:@.    * an empty \
          list pattern \"[]\";@.    * a cons list pattern \"[head, ...tail]\".@.  If \
          this is pattern matching over variants, then a constructor of a variant is \
     ...
     | `Concrete_jsligo_not_implemented r ->
       Format.fprintf f "@[<hv>%aThis has not been implemented yet. @]" Snippet.pp_lift r)

...
module Compile_type = struct
  ...
  *)
  and try_type_compilers
      ...
      -> (unit -> AST.type_expression) -> AST.type_expression
    =
  ...
  and compile_type_function_args ~raise : CST.fun_type_args -> type_expression =
   fun args ->
  ...
  and compile_type_expression ~raise : CST.type_expr -> type_expression =
   fun te ->
    ...
    try_type_compilers
      [ compile_sapling ~raise; compile_michelson_pair_or ~raise ]
      te
    @@ fun () ->
    match te with
    | TSum sum ->
      ...
      let aux (v : CST.variant Region.reg)
          : string * type_expression * string list
        =
    ...
    | TModA ma ->
      ...
      let rec aux : Module_var.t list -> CST.type_expr -> AST.type_expression =
       fun acc exp ->
        ...
        | TModA ma ->
          aux
            (acc @ [ Module_var.of_input_var ma.value.module_name.value ])
            ma.value.field
        | _ ->
          raise.error (expected_a_variable (CST.type_expr_to_region ma.field))
      in
        ...
        Utils.nsepseq_map
          (fun (obj : CST.obj_type) ->
            ...
            let constructor, fields =
              List.partition_map
                (npseq_to_list obj.value.ne_elements)
                ~f:(fun x ->
            ...
            let type_expr =
              ...
              | hd :: tl ->
                ...
                let obj =
                  CST.TObject
                    { obj with value = { obj.value with ne_elements } }
                in
                self obj
              | [] -> t_unit ()
            in
            let fields =
              List.map ~f:(fun x -> x.value.field_name.value) fields
            in
...
let compile_selection ~raise
    : CST.selection -> _ Access_path.access * Location.t
  =
 fun selection ->
...
let arguments_to_expr_nseq (args : CST.arguments)
    : CST.expr Utils.nseq * Location.t
  =
  match args with
...
let rec compile_tuple_expression ~raise ?loc tuple_expr =
  let lst =
    List.map ~f:(fun e -> compile_expression ~raise e)
    @@ nseq_to_list tuple_expr
  in
  match lst with
  | [ hd ] -> hd
  | lst -> e_tuple ?loc lst
...
and compile_bin_op
    ~raise
    (op_type : Constant.constant')
    (op : _ CST.bin_op CST.reg)
  =
 ...
 fun e ->
      ...
      { value =
          ...
          , Multiple
              { value =
                  { inside =
                      ( EArray
                          { value = { inside; lbracket = _; rbracket = _ }
                          ; region = _
                          }
                      , [] )
                  ; _
  ...
  | ECall
    ...
    let compile_constr_pattern = function
      | CST.Property
          { value = { name = EVar { value = constr; region }; value; _ }; _ } ->
        (match value with
        | EFun { value = { parameters; body; _ }; _ } ->
          let parameters_opt, parameters_region =
            compile_simple_pattern parameters
          in
          let expr = compile_function_body_to_expression ~raise body in
          ( region
          , (Label.of_string constr, parameters_opt, parameters_region)
          , expr )
        | _ as e ->
          raise.error @@ invalid_case constr e (* TODO: improve error message *))
  ...
  | ECall
    ...
    let rec compile_parameter = function
      ...
      | ESeq
          { value =
              ( EAnnot
                  { value = EArray { value = { inside = None; _ }; _ }, _, _
                  ; _
                  }
              , _ )
          ; _
  ...
  | EConstr constr ->
    ...
    let args_o =
      Option.map
        ~f:(compile_tuple_expression ~raise <@ List.Ne.singleton)
        args_o
    in
    let args =
      Option.value
        ~default:(e_unit ~loc:(Location.lift constr.region) ())
        args_o
    in
  ...
  | EObject
      { value = { inside = Property_rest { value = { expr; _ }; _ }, rest; _ }
      ; _
      } ->
      ...
      let filtered_object =
        Utils.nsepseq_foldl
          (fun a i ->
            match i with
            | CST.Property
                { value = { value = EString (String { value = s; _ }); _ }; _ }
  ...
  | EFun func ->
    ...
    let lhs_type =
      Option.map ~f:(compile_type_expression ~raise <@ snd) lhs_type
    in
  ...
  | EAnnot
      { value = EArith (Int i), _, TVar { value = "mutez"; _ }; region = _ } ->
    let (_, i), loc = r_split i in
    return @@ e_mutez_z ~loc i
  | EAnnot
      { value = ECodeInj { value = { language; code }; _ }, kwd_as, type_expr
      ; region
      } ->
  ...
  | EAssign ((EVar { value; region } as e1), op, e2) ->
    ...
    (match e2 with
    | EAssign
        (EProj { value = { expr; selection }; region = _ }, { value = Eq; _ }, _)
      ->
    ...
    | _ ->
      e_assign
        ...
        e2x)
  | EAssign
      (e1, ({ value = Eq; _ } as op), (EAssign ((EProj _ as eproj), _, _) as e2))
    ->
    let e2 = self e2 in
  ...
  | EAssign
      ( EProj
          { value =
              ...
              ; selection =
                  Component { value = { inside = EArith (Int _); _ }; _ } as
                  selection
              }
          ; region = _
          }
    ...
    let outer_loc = Location.lift op.region in
    let sels, _ = compile_selection ~raise selection in
    e_assign_ez ~loc:outer_loc evar_value
    @@ e_update (e_variable_ez evar_value) [ sels ] e2
...
and compile_array_let_destructuring ~raise
    :  const:bool -> AST.expression -> CST.array_pattern -> AST.expression
    -> AST.expression
  =
    ...
    then body
      ...
      List.fold_left binders ~init:body ~f:(fun body binder ->
          e_let_mut_in
            ...
            binder
            []
            (e_variable ~loc (Binder.get_var binder))
            body))
  in
  fun body -> e_matching ~loc matchee [ { pattern; body = wrap body } ]


and compile_object_let_destructuring ~raise
    :  const:bool -> AST.expression -> CST.object_pattern -> AST.expression
    -> AST.expression
  =
 ...
 fun ~const expr ->
  ...
  let matching ~loc binder_lst fun_ =
    ...
    | _ ->
      ...
      let expr expr =
        e_param_matching_tuple ~loc (e_variable ~loc var) binder_lst
        @@ fun_ expr
      in
  ...
  | EArray array_items ->
    ...
    let binder_lst, fun_ =
      ...
      @@ List.map ~f:array_item arguments
    in
  ...
  | EObject obj' ->
    ...
    in
    let ascr =
      Option.all @@ List.map ~f:(Fn.compose Param.get_ascr snd) binder_lst
    in
    let ascr = Option.map ~f:t_tuple ascr in
...
and compile_function_body_to_expression ~raise : CST.body -> AST.expression =
 fun body ->
  match body with
  | FunctionBody statements ->
    compile_statements_to_expression ~raise statements.value.inside
  | ExpressionBody expr -> compile_expression ~raise expr


and compile_let_to_declaration ~raise
    : CST.attributes -> CST.val_binding Region.reg -> AST.declaration list
  =
 fun attributes let_binding ->
  let ({ binders; type_params; lhs_type; expr = let_rhs; _ } : CST.val_binding) =
    let_binding.value
  in
  let lst =
    compile_let_binding
  ...
  in
  let aux
      :  type_expression option Binder.t
         * Types.attributes
         * CST.type_generics option
         * expression
      -> declaration
    =
...
and compile_pattern ~raise
    : CST.pattern -> type_expression option Binder.t * (_ -> _)
  =
 fun pattern ->
  ...
  | PArray tuple ->
    ...
    let binder_lst, fun_ =
      List.fold_right ~f:aux ~init:([], fun e -> e)
      @@ Utils.nsepseq_to_list tuple.inside
    in
...
and compile_let_binding ~raise
    :  CST.attributes -> CST.expr -> (CST.colon * CST.type_expr) option
    -> CST.type_generics option -> CST.pattern -> Region.t
    -> (type_expression option Binder.t
       * Ast_imperative__.Types.attributes
       * _
       * expression)
       list
  =
 fun attributes let_rhs type_expr type_params binders region ->
  ...
  let lhs_type =
    Option.map ~f:(compile_type_expression ~raise <@ snd) type_expr
  in
  let aux = function
    | CST.PVar name ->
      (*function or const *)
      ...
      let expr =
        ...
        | CST.EFun _ ->
          let lambda =
            trace_option ~raise (recursion_on_non_function expr.location)
            @@ get_e_lambda expr.expression_content
          in
          let lhs_type =
            match lhs_type with
            ...
            | None ->
              ...
              @@ Option.bind_pair
                   (Param.get_ascr lambda.binder, lambda.output_type)
          in
          if is_recursive_lambda fun_binder lambda
          then (
            let fun_type =
              trace_option ~raise (untyped_recursive_fun name.region)
              @@ lhs_type
            in
            ...
            let lambda =
              Lambda.
                { binder = Param.map (Fn.const type1) binder
                ; result
                ; output_type = type2
                }
            in
            e_recursive
              ~loc:(Location.lift name.region)
              fun_binder
              fun_type
              lambda)
    ...
    | CST.PArray a ->
      (* tuple destructuring (for top-level only) *)
      let matchee = expr in
      ...
      let aux i binder =
        ...
        , ( binder
          , attributes
          , type_params
          , e_accessor expr @@ [ Access_tuple i ] ) )
      in
...
and compile_statements ?(wrap = false) ~raise
...
and compile_statement ?(wrap = false) ~raise : CST.statement -> statement_result
 ...
 fun statement ->
  ...
  let compile_initializer
      ~const
      attributes
      ({ value = { binders; type_params; lhs_type; expr = let_rhs; eq = _ }
       ; region
       } :
        CST.val_binding Region.reg)
    ...
    | _ ->
      ...
      let aux (binder, attr, type_params, rhs) expr =
        ...
        | E_assign { binder = b; _ } ->
          (* What??? *)
          let var =
            { expression_content = E_variable (Binder.get_var b)
        ...
        | _ ->
          ...
          in
          if const
          then e_let_in ~loc:(Location.lift region) binder attr rhs expr
          else e_let_mut_in ~loc:(Location.lift region) binder attr rhs expr
      in
      fun init -> List.fold_right ~f:aux ~init lst
  in
  let rec initializers
      ~const
      (result : expression -> expression)
      (rem : (CST.comma * CST.val_binding Region.reg) list)
      : expression -> expression
    =
    match rem with
    | (_, hd) :: tl ->
  ...
  | SCond cond ->
    ...
    let else_clause =
      Option.map ~f:(fun (_, s) -> self ~wrap:false s) cond.ifnot
    in
    ...
    | None ->
      ...
      | Return _ ->
        Binding (fun else_clause -> e_cond ~loc test then_clause else_clause)
  ...
  | SSwitch s' ->
    ...
    | Some data ->
      ...
      let rec check_return statements =
        ...
        | SCond { value = { ifso; ifnot; _ }; _ }, _ ->
          ...
          | None -> ())
        | (_ as s), _ ->
          raise.error @@ case_break_disc (CST.statement_to_region s)
      in
      let cases =
        List.map
          ~f:(fun f ->
            match f with
            | Switch_case
                { expr = EString (String v); statements = Some statements; _ }
              ->
              ...
              in
              let pattern =
                Location.wrap (Pattern.P_variant (Label v.value, ty))
              in
              Match_expr.
                { pattern
    ...
    | None ->
          ...
          (fun x ->
            e_let_mut_in
              ...
              (e_let_mut_in
                 ...
                 []
                 (e_false ())
                 (e_let_mut_in found_case_binder [] (e_false ()) x)))
      in
      ...
      let found_case_eq_true =
        eq_expr ~loc (e_variable found_case) (e_true ())
      in
      let fallthrough_eq_true =
        eq_expr ~loc (e_variable fallthrough) (e_true ())
      in
      ...
      let process_case case =
        ...
        | CST.Switch_case { kwd_case; expr; statements = None; colon = _ } ->
          ...
          let update_vars =
            e_sequence fallthrough_assign_true found_case_assign_true
          in
          Binding
            (fun x -> e_sequence (e_cond ~loc test update_vars (e_unit ())) x)
        | Switch_case
            { kwd_case; expr; statements = Some statements; colon = _ } ->
          let loc = Location.lift kwd_case#region in
          let case_expr = self_expr expr in
          let test = case_cond case_expr in
          ...
          let update_vars_break =
            e_sequence fallthrough_assign_false found_case_assign_true
          in
        ...
        | Switch_default_case { statements = None; kwd_default = _; colon = _ }
          -> Binding (fun x -> e_sequence (e_unit ()) x)
        | Switch_default_case
            { kwd_default; statements = Some statements; colon = _ } ->
          let loc = Location.lift kwd_default#region in
          let default_cond =
            or_expr fallthrough_eq_true (not_expr found_case_eq_true)
          in
          ...
          let statements =
            ...
            | Expr e | Break e ->
              Binding
                (fun x -> e_sequence (e_cond ~loc default_cond e (e_unit ())) x)
            | Return e ->
              Binding
                (fun x ->
                  match x with
  ...
  | SImport i' ->
    ...
    | Import_rename { alias; module_path; _ } ->
      ...
      let module_ =
        let path =
          List.Ne.map compile_mod_var @@ npseq_to_ne_list module_path
        in
...
and statement_result_to_expression : statement_result -> AST.expression =
...
and compile_statements_to_expression ~raise : CST.statements -> AST.expression =
 fun statements ->
...
and compile_statement_to_declaration ~raise ~export
 ...
 fun statement ->
  ...
  | SType { value; region } ->
    ...
    let d =
      D_type
        { type_binder = compile_type_var name
        ; type_expr
  ...
  | SLet { value = { bindings; attributes; _ }; region } ->
    ...
    let fst_binding =
      compile_let_to_declaration ~raise attributes fst_binding
    in
  ...
  | SNamespace
      { value =
          ( _
          , ident
          , { value = { inside = statements; _ }; region = region_in }
          , attributes )
      ; region
      } ->
    let attributes = if export then filter_private attributes else attributes in
    ...
    let module_ =
      ...
      Location.wrap ~loc
      @@ Module_expr.M_struct (compile_namespace ~raise statements)
    in
  ...
  | SExport { value = _, s; _ } ->
    compile_statement_to_declaration ~raise ~export:true s
...
and compile_namespace ~raise : CST.statements -> AST.module_ =
 fun statements ->
  ...
  let declarations =
    List.map
      ~f:(compile_statement_to_declaration ~raise ~export:false)
      statements
  in
  let lst = List.concat declarations in
  lst

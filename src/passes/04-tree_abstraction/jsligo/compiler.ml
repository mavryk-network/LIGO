...
and compile_expression ~raise : CST.expr -> AST.expr =
 fun e ->
  ...
  | EConstr constr ->
    ...
    let args_o =
      Option.map ~f:(compile_tuple_expression ~raise <@ List.Ne.singleton) args_o
    in
    let args =
...
and compile_array_let_destructuring ~raise
  : const:bool -> AST.expression -> CST.array_pattern -> AST.expression -> AST.expression
  =
...
and compile_object_let_destructuring ~raise
  : const:bool -> AST.expression -> CST.object_pattern -> AST.expression -> AST.expression
  =
...
and compile_pattern ~raise : CST.pattern -> type_expression option Binder.t * (_ -> _) =
 fun pattern ->
  ...
  | PArray tuple ->
    ...
    in
    let binder_lst, fun_ =
      List.fold_right ~f:aux ~init:([], fun e -> e) @@ Utils.nsepseq_to_list tuple.inside
    in
  ...
  let aux = function
    | CST.PVar name ->
      ...
      let expr =
        ...
        | EAssign ((EVar _ as v), _, _) -> e_sequence expr (compile_expression ~raise v)
    ...
    | CST.PArray a ->
      ...
      let aux i binder =
        ( Z.add i Z.one
        , (binder, attributes, type_params, e_accessor expr @@ [ Access_tuple i ]) )
      in
...
and compile_statement ?(wrap = false) ~raise : CST.statement -> statement_result =
 fun statement ->
  ...
  let compile_initializer
    ~const
    attributes
    ({ value = { binders; type_params; lhs_type; expr = let_rhs; eq = _ }; region } :
    ...
    | _ ->
      let lst =
        compile_let_binding ~raise attributes let_rhs lhs_type type_params binders region
      in
         ...
         List.map
           ~f:(fun f ->
             ...
             | Switch_case { expr = EString (String v); statements = Some statements; _ }
               ...
               Match_expr.
                 { pattern; body = compile_statements_to_expression ~raise statements }
             ...
             | Binding s ->
               Binding
                 (fun x ->
                   ...
                   e_sequence e x)
             | Expr e | Break e ->
               Binding (fun x -> e_sequence (e_cond ~loc default_cond e (e_unit ())) x)
             | Return e ->
               Binding
                 (fun x ->
                   match x with

...
and compile_bin_op ~raise (op_type : string) (op : _ CST.bin_op CST.reg) =
...
and compile_un_op ~raise (op_type : string) (op : _ CST.un_op CST.reg) =
  let self = compile_expression ~raise in
  let return e = e in
  let (op, loc) = r_split op in
...
and compile_expression ~raise : CST.expr -> AST.expr = fun e ->
  ...
  | EArith arth ->
    ( match arth with
      Add plus   -> compile_bin_op ~raise "#polymorphic_add_u" plus
    | Sub minus  -> compile_bin_op ~raise "#polymorphic_sub_u" minus
    | Mult times -> compile_bin_op ~raise "#mul_u" times
    | Div slash  -> compile_bin_op ~raise "#div_u" slash
    | Mod mod_   -> compile_bin_op ~raise "#mod_u" mod_
    | Neg minus  -> compile_un_op ~raise "#neg" minus
    | Int i ->
  ...
  | ELogic logic -> (
    match logic with
      BoolExpr be -> (
      match be with
        Or or_   -> compile_bin_op ~raise "#bool_or_u"  or_
      | And and_ -> compile_bin_op ~raise "#bool_and_u" and_
      | Not not_ -> compile_un_op ~raise "#not" not_
    )
    | CompExpr ce -> (
      match ce with
        Lt lt    -> compile_bin_op ~raise "#lt_u"  lt
      | Leq le   -> compile_bin_op ~raise "#le_u"  le
      | Gt gt    -> compile_bin_op ~raise "#gt_u"  gt
      | Geq ge   -> compile_bin_op ~raise "#ge_u"  ge
      | Equal eq -> compile_bin_op ~raise "#eq_u"  eq
      | Neq ne   -> compile_bin_op ~raise "#neq_u" ne
    )
  )
...
and conv ~raise : const:bool -> CST.pattern -> AST.ty_expr AST.pattern =
  ...
  | PObject record ->
    ...
    let labels = List.map
        ...
        ~f:(function PVar var -> AST.Label var.value.variable.value | _ -> raise.error @@ unsupported_pattern_type p)
    in
...
and compile_let_binding ~raise : const:bool -> CST.attributes -> CST.expr -> (CST.colon * CST.type_expr) option -> CST.type_generics option -> CST.pattern -> Region.t -> (type_expression binder * Ast_imperative__.Types.attributes * _ * expression) list =

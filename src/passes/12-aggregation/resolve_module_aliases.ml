...
let rec type_expression : Aliases.t -> type_expression -> type_expression =
 fun aliases te ->
...
let rec expression : Aliases.t -> expression -> expression =
 fun aliases e ->
...
and matching_cases : Aliases.t -> matching_expr -> matching_expr =
 fun scope me ->
...
and compile_declaration aliases (d : declaration)
    : Aliases.t * declaration option
  =
  ...
  | D_value { binder; expr; attr } ->
    ...
    return_s aliases @@ D_value { binder; expr; attr }
  | D_type { type_binder; type_expr; type_attr } ->
    let type_expr = type_expression aliases type_expr in
    return_s aliases @@ D_type { type_binder; type_expr; type_attr }
  | D_module { module_binder; module_; module_attr } ->
    ...
    | Some module_ ->
      return_s aliases @@ D_module { module_binder; module_; module_attr })


and compile_declaration_list aliases (program : program) : Aliases.t * program =
  let aliases, dcl =
    List.fold_map ~init:aliases ~f:compile_declaration program
  in
  let dcl = List.filter_opt dcl in
  aliases, dcl


and compile_decl : Aliases.t -> decl -> Aliases.t * decl option =
 fun s d -> compile_declaration s d


and compile_module aliases (m : module_) : Aliases.t * module_ =
...
and compile_module_expr
    :  Aliases.t -> module_expr
    -> Aliases.t * Module_var.t list option * module_expr option
  =
 fun aliases mexpr ->
...
let program : program -> Aliases.t * program =
 fun prg ->
  let aliases = Aliases.empty in
  compile_declaration_list aliases prg


let expression ?(aliases = Aliases.empty) : expression -> expression =
 fun e ->

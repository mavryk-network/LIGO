...
let rec decompile ~raise : Ast_aggregated.expression -> Ast_typed.expression =
 fun exp ->
    ...
    let aux : _ I.matching_content_case -> _ Ast_typed.Match_expr.match_case =
     fun { constructor; pattern; body } ->
      let pattern =
        ...
        let proj =
          Location.wrap @@ Ast_typed.Pattern.P_var (Binder.make pattern tv)
        in
        Location.wrap @@ Ast_typed.Pattern.P_variant (constructor, proj)
      in
      let body = decompile ~raise body in
      ({ pattern; body }
        : (O.expression, O.type_expression) Ast_typed.Match_expr.match_case)
    in
  ...
  | E_matching { matchee; cases = Match_record { fields; body; tv = _ } } ->
    let matchee = decompile ~raise matchee in
    let aux
        : I.type_expression Binder.t -> O.type_expression Ast_typed.Pattern.t
      =
     fun binder ->
      let proj =
        Location.wrap
        @@ Ast_typed.Pattern.P_var (Binder.map (decompile_type ~raise) binder)
      in
      proj
    in
    let body = decompile ~raise body in
    let case =
      match Record.is_tuple fields with
      | false ->

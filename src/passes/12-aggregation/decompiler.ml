...
let rec decompile ~raise : Ast_aggregated.expression -> Ast_typed.expression =
 fun exp ->
  ...
  let return expression_content : O.expression =
    ...
    ; type_expression = decompile_type ~raise exp.type_expression
    }
  in
  ...
  | E_matching { matchee; cases = Match_record { fields; body; tv } } ->
    ...
    let tv = decompile_type ~raise tv in
    let body = decompile ~raise body in
    let fields = Map.map ~f:(Binder.map (decompile_type ~raise)) fields in
    return (O.E_matching { matchee; cases = Match_record { fields; body; tv } })
  | E_matching { matchee; cases = Match_tuple { binders; body; tv } } ->
    let matchee = decompile ~raise matchee in
    let tv = decompile_type ~raise tv in
    let body = decompile ~raise body in
    let binders = List.map ~f:(Binder.map (decompile_type ~raise)) binders in
    return (O.E_matching { matchee; cases = Match_tuple { binders; body; tv } })
    (* Record *)
  | E_record map ->
    let map = Map.map ~f:(decompile ~raise) map in
  ...
  | I.E_tuple tuple ->
    let tuple = Tuple.map (decompile ~raise) tuple in
    return @@ O.E_tuple tuple


and decompile_type ~raise
    : Ast_aggregated.type_expression -> Ast_typed.type_expression
  =
 fun ty ->
  let module I = Ast_aggregated in
  let module O = Ast_typed in
  let decompile_row : _ I.Rows.t -> _ O.Rows.t =
  ...
  | T_tuple tuple ->
    let tuple = Tuple.map (decompile_type ~raise) tuple in

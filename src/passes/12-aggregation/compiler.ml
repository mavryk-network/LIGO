...
module PathVar (M : Simple_utils.Map.OrderedType) = struct
...
module PathVarMap = Simple_utils.Map.Make (PathVar (Value_var))
...
module Scope : sig
  ...
  module ValueVMap = Simple_utils.Map.Make (Value_var)
  module TypeVMap = Simple_utils.Map.Make (Type_var)
  module ModuleVMap = Simple_utils.Map.Make (Module_var)
  ...
  let empty =
    ...
    ; type_ = TypeVMap.empty
    ; module_ = ModuleVMap.empty
    ; name_map = PathVarMap.empty
    ; decl_list = []
    }
  ...
  let find_module scope m =
    Option.value
      ~default:(Path.empty, empty)
      (ModuleVMap.find_opt m scope.module_)
...
let rec compile_type_expression
    ...
    path
    scope
    (type_expression : I.type_expression)
    : O.type_expression
  =
  let self ?(path = path) ?(scope = scope) =
    compile_type_expression ~raise path scope
  in
  let compile_row : _ I.Rows.t -> _ O.Rows.t =
   fun { fields; attributes = { layout } } ->
    let fields =
      Map.map
        ~f:
          (fun { content = { associated_type; decl_pos }; michelson_annotation }
               : _ O.Rows.Elem.t ->
          { content = { associated_type = self associated_type; decl_pos }
...
let rec compile_expression ~raise path scope (expr : I.expression) =
  let self ?(path = path) ?(scope = scope) =
    compile_expression ~raise path scope
  in
  let self_type ?(path = path) ?(scope = scope) =
    compile_type_expression ~raise path scope
  in
  let self_cases ?(path = path) ?(scope = scope) =
    compile_cases ~raise path scope
  in
  ...
  | E_variable expression_variable ->
    ...
    let _, expression_variable =
      Scope.add_path_to_var scope path expression_variable
    in
  ...
  | E_let_in { let_binder; rhs; let_result; attributes } ->
    ...
    let scope =
      Scope.push_value scope let_binder rhs.type_expression attributes Path.empty
    in
  ...
  (* Record *)
  | E_record record ->
    let record = Map.map ~f:self record in
  ...
  | E_for_each
    ...
    let scope =
      List.fold_left
        ...
        ~init:scope
        ~f:Scope.push_func_or_case_binder
    in
  ...
  | I.E_sequence { expr1; expr2 } ->
    ...
    let let_binder =
      Binder.make (Value_var.fresh ~name:"()" ()) (O.t_unit ())
    in
    return
    @@ O.E_let_in
         { let_binder
  ...
  | I.E_map map ->
    ...
    let aux (k, v) prev =
      let k', v' = Simple_utils.Pair.map ~f:self (k, v) in
      return
      @@ E_constant { cons_name = C_MAP_ADD; arguments = [ k'; v'; prev ] }
    in
    let init =
      return @@ E_constant { cons_name = C_MAP_EMPTY; arguments = [] }
    in
  ...
  | I.E_big_map big_map ->
    ...
    in
    let init =
      return @@ E_constant { cons_name = C_BIG_MAP_EMPTY; arguments = [] }
    in
  ...
  | I.E_set set ->
    ...
    in
    let init =
      return @@ E_constant { cons_name = C_SET_EMPTY; arguments = [] }
    in
...
and compile_cases ~raise path scope cases : O.matching_expr =
  ...
  | Match_variant { cases; tv } ->
    let cases =
      List.map cases ~f:(fun I.{ constructor; pattern; body } ->
    ...
    let scope, fields =
      I.Record.fold_map
        (fun scope' binder ->
          ...
          , Binder.map (compile_type_expression ~raise path scope) binder ))
        scope
        fields
    in
    let body = compile_expression ~raise path scope body in
...
and compile_declaration
  ...
  | I.D_value { binder; expr; attr } ->
    let attr =
      ...
      ; public = attr.public && super_attr.public
      }
    in
    ...
    let scope, var =
      Scope.add_path_to_var scope path @@ Binder.get_var binder
    in
    let binder = Binder.set_var binder var in
    scope, O.context_decl binder expr attr
  | I.D_type _ -> scope, O.context_id
  | I.D_module { module_binder; module_; module_attr } ->
    let module_attr : O.Module_attr.t =
      { hidden = module_attr.hidden || super_attr.hidden
      ; public = module_attr.public && super_attr.public
      }
    in
    let path' = Path.add_to_path path module_binder in
    let mod_scope, decl_list =
      compile_module_expr ~raise ~module_attr path' scope module_
    in
    let scope = Scope.push_module scope module_binder path' mod_scope in
    scope, decl_list
...
and compile_declaration_list
  ...
  let scope, current =
    List.fold_map
      ~init:scope
      ~f:(compile_declaration ~raise ~super_attr path)
      program
  in
...
and compile_module_expr
 ...
 fun path scope mexpr ->
  let rec get_declarations_from_scope scope new_path old_path =
      ...
      List.fold_left ~init:O.context_id dcls ~f:(fun f dcl ->
          ...
          | Value (binder, ty, attr) ->
            let variable =
              O.e_a_variable
                (snd
                @@ Scope.add_path_to_var scope old_path
                @@ Binder.get_var binder)
                ty
            in
            let _, var =
              Scope.add_path_to_var scope new_path @@ Binder.get_var binder
            in
            O.(
              context_append
                (context_decl (Binder.set_var binder var) variable attr)
                f)
          | Module var ->
    ...
    module_
  in
  let super_attr : O.Module_attr.t =
...
let compile_program ~raise
 ...
 fun program ->
  ...
  let scope, decls =
    compile_declaration_list
      ~raise
      ~super_attr:{ public = true; hidden = false }
      Path.empty
      Scope.empty
      program
  in

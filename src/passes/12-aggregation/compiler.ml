...
and compile_type ~raise : I.type_expression -> O.type_expression =
  fun ty ->
    ...
    let return type_content : O.type_expression = { type_content ; orig_var = ty.orig_var ; location = ty.location ; source_type = Some ty ; from_external = ty.from_external } in

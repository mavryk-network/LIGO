...
module Scope : sig
  ...
  let open_module scope path mod_scope =
...
let rec compile_expression ~raise path scope (expr : I.expression) =
  ...
  | E_module_accessor { module_path; element } ->
    ...
    | hd :: tl ->
      ...
      let path, scope = Scope.find_module scope hd in
...
and compile_declaration
    ...
    (d : I.declaration)
  =
  let return scope decl_list = scope, decl_list in
  ...
  | D_value { binder; expr; attr } ->
    ...
    return scope @@ O.context_decl ~loc binder expr attr
  | D_irrefutable_match { pattern; expr; attr } ->
    ...
    let scope, pattern =
      O.Pattern.fold_map_pattern
        ...
        pattern
    in
    return scope @@ O.context_decl_pattern ~loc pattern expr attr
  ...
  | D_open (hd, tl) ->
    let mod_scope, decl_list = compile_module_path path scope ~loc (hd, tl) in
...
and compile_module_expr ~raise ?(module_attr = { public = true; hidden = false })
 ...
 fun path scope mexpr ->
  ...
  | M_variable v ->
    ...
    scope, module_
  | M_module_path path' -> compile_module_path ~loc path scope path'


and compile_module_path ~loc path scope (hd, tl) =

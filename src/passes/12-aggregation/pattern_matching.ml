...
let group_equations ~raise : equations -> equations Record.t =
 fun eqs ->
  let aux
      :  typed_pattern list * O.expression -> equations Record.t
      -> equations Record.t
    =
   fun (pl, body) m ->
    ...
    let upd
        : O.type_expression -> pattern -> equations option -> equations option
      =
    ...
    | P_list (Cons (p_hd, p_tl)) ->
      ...
      let pattern =
        Location.wrap ~loc:phd.location @@ I.Pattern.P_tuple [ p_hd; p_tl ]
      in
      let proj_t = extract_variant_type ~raise phd label t in
  ...
  | mhd :: mtl ->
    ...
    let grouped_eqs =
      ...
      | Some rows ->
        let eq_opt_map =
          Record.LMap.mapi
            (fun label _ -> Record.LMap.find_opt label eq_map)
            rows.fields
        in
 ...
 fun ~err_loc product_shape ms eqs def ->
  ...
  | mhd :: _ ->
    let lb : (_ * _ Binder.t) list =
      ...
      | P_record lps, t ->
        let aux
            : Label.t * _ I.Pattern.t -> Label.t * O.type_expression Binder.t
          =
         fun (l, proj_pattern) ->
          let field_t = extract_record_type ~raise p l t in
          let b =
...
let compile_matching
    ...
    matchee
    (eqs :
      (O.type_expression I.Pattern.t * O.type_expression * O.expression) list)
  =
  let eqs =
    List.map
  ...
  let missing_case_default =
    ...
    let t_fail =
      ...
      let type_ =
        let ty_binder = b in
        let kind = Ligo_prim.Kind.Type in
        let type_ = O.t_arrow (O.t_variable a ()) (O.t_variable b ()) () in

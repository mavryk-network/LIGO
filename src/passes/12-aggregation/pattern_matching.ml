...
let rec match_ ~raise
 ...
 fun ~err_loc ms eqs def ->
  ...
  | _ ->
    ...
    let aux
      (part_eq : equations)
      ((def, _, _) : O.expression_content * O.type_expression option * Location.t)
      =
...
and var_rule ~raise
  :  err_loc:Location.t -> typed_pattern option -> matchees -> equations -> rest
  -> O.expression
  =
...
and product_rule ~raise
  : err_loc:Location.t -> typed_pattern -> matchees -> equations -> rest -> O.expression
  =

...
type self_mini_c_error = [
  | `Self_mini_c_bad_self_address of Ligo_prim.Constant.constant'
...
let error_jsonformat : self_mini_c_error -> Yojson.Safe.t = fun a ->
  ...
  | `Self_mini_c_bad_self_address cst ->
    let msg = Format.asprintf "%a is only allowed at top-level" Ligo_prim.Constant.pp_constant' cst in
    let content = `Assoc [
      ("message", `String msg); ]
    in

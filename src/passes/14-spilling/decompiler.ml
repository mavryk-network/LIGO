...
let rec decompile
  ...
  | tc when AST.compare_type_content tc (t_bool ~loc ()).type_content = 0 ->
    let b = Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_bool v in
    return (e_bool ~loc b)
  | T_constant { language; injection; parameters } ->
    let () =
      Trace.Assert.assert_true
        ~raise
        (Errors.corner_case ~loc:__LOC__ ("unsupported language " ^ language))
        (String.equal language Backend.Michelson.name)
    in
    (match injection, parameters with
    | Unit, [] ->
      let () = Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_unit v in
      return (E_literal Literal_unit)
    | Int, [] ->
      let n = Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_int v in
      return (E_literal (Literal_int n))
    | Nat, [] ->
      let n = Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_nat v in
      return (E_literal (Literal_nat n))
    | Timestamp, [] ->
      let n =
        Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_timestamp v
      in
      return (E_literal (Literal_timestamp n))
    | Mav, [] ->
      let n = Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_mumav v in
    ...
    | Operation, [] ->
      let op =
        Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_operation v
      in
    ...
    | Map, [ k_ty; v_ty ] ->
      let map = Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_map v in
      let map' =
        let aux (k, v) =
    ...
    | Big_map, [ k_ty; v_ty ] ->
      let big_map =
        Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_big_map v
      in
      let big_map' =
        let aux (k, v) =
          let key = self k k_ty in
    ...
    | List, [ ty ] ->
      let lst = Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_list v in
      let lst' =
    ...
    | Set, [ ty ] ->
      let lst = Trace.trace_option ~raise (Errors.wrong_mini_c_value t v) @@ get_set v in
      let lst' =
        let aux e = self e ty in
        List.map ~f:aux lst
      in
    ...
    | Ticket, [ _ty ] ->
      ...
      raise.error @@ Errors.bad_decompile v
    | Contract, _ -> raise.error @@ Errors.bad_decompile v
    | (Michelson_pair | Michelson_or), _ ->
      raise.error
      @@ Errors.corner_case
           ~loc:"unspiller"
           "Michelson_combs t should not be present in mini-c"
    | ( ( Unit
        | Nat
        | Mav
        | Bytes
  ...
  | T_sum _ when Option.is_some (Ast_expanded.get_t_option t) ->
    ...
    | D_some v ->
      let tv =
        Trace.trace_option ~raise (Errors.corner_case ~loc:"unspiller" "impossible")
    ...
    | _ -> raise.error @@ Errors.corner_case ~loc:"unspiller" "impossible")
  | T_sum sum ->
  ...
  | T_arrow _ ->
    ...
    return (E_literal (Literal_string n))
  | T_variable _ ->
    raise.error @@ Errors.corner_case ~loc:__LOC__ "trying to decompile at variable type"
  | T_singleton _ ->
    raise.error @@ Errors.corner_case ~loc:__LOC__ "no value is of type singleton"
  | T_for_all _ ->
    raise.error
    @@ Errors.corner_case
         ~loc:__LOC__
         "trying to decompile a quantified type (no such thing ?)"

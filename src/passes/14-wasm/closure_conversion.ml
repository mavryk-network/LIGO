...
type env =
  ...
  ; exported_funcs :
      (replacements -> var_name list -> expression -> var_name list * expression) list
  ; replacements : replacements
  ; functions : var_name list
  }

let empty_env =
  { variables = []; missing = []; exported_funcs = []; replacements = []; functions = [] }
 ...
 fun env e ->
  ...
  | E_closure { binder; body } ->
    ...
    let rec find_body env2 a =
      ...
      | E_closure { binder; body } ->
        let env2 =
          { env2 with variables = (binder, a.type_expression) :: env2.variables }
        in
    ...
    let export_func remaining =
      List.fold_left
        ~f:(fun all binder ->
          ...
          let type_expression =
            { type_content = T_function (type_expression_item, all.type_expression)
        ...
        ~init:{ e with content = E_closure { binder; body } }
        remaining
    in
    let in_function i =
    ...
    let export replacements (funcs : var_name list) e =
      let v =
        match List.find replacements ~f:(fun (_, r) -> Value_var.equal r v) with
        | Some (x, _) -> x
        | None -> v
      in
  ...
  | E_variable v when variable_exists env v -> env, e
  ...
  | E_if_cons
      (e1, e2, (((var_name1, type_expression1), (var_name2, type_expression2)), e3)) ->
    let variables =
      (var_name1, type_expression1) :: (var_name2, type_expression2) :: env.variables
    in
    let env = { env with variables } in
    let env, e1 = lift env e1 in
    let env, e2 = lift env e2 in
  ...
  | E_if_left
      (e1, ((var_name1, type_expression1), e2), ((var_name2, type_expression2), e3)) ->
    let variables =
      (var_name1, type_expression1) :: (var_name2, type_expression2) :: env.variables
    in
    let env = { env with variables } in
  ...
  | E_let_in (e1, inline, ((var_name, type_expression), e2)) ->
    ...
    let env =
      ...
      | E_variable v -> { env with replacements = (var_name, v) :: env.replacements }
      | _ -> env
    in
    let env, e2 = lift env e2 in
    ( env
  ...
  | E_for_each (collection, collection_type, (fe_binder, fe_body)) ->
    ...
    ( env
    , { e with content = E_for_each (collection, collection_type, (fe_binder, fe_body)) }
    )
...
let rec toplevel_inner : env -> string -> expression -> expression =
 fun env entrypoint e ->
  ...
  | E_let_in
    ...
    let rec aux b =
      ...
      | _ ->
        lift
          ...
          ; variables = (binder, type_expression) :: env.variables
          }
          b
    in
    let env, body = aux body in
    let _, r =
      List.fold_left
        ~f:(fun (functions, prev) export -> export env.replacements functions prev)
        ~init:
          ( env.functions
          , { e with
              content =
      ...
      { e with
        content =
          E_let_in
            ( { e with content = e1 }
            , false
            , ( (ep, e.type_expression)
              , { content = E_variable ep

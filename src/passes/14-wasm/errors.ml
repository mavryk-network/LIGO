...
type wasm_error = [`Wasm_pass_michelson_insertion of Location.t]
 ...
 fun ~display_format f a ->
  ...
  | Human_readable | Dev -> (
    ...
    | `Wasm_pass_michelson_insertion l ->
      ...
      let s =
        Format.asprintf
          "Michelson insertion can't be used when targeting WebAssembly."
      in
...
let error_jsonformat : wasm_error -> Yojson.Safe.t =
 fun a ->
  ...
  | `Wasm_pass_michelson_insertion l ->
    ...
    let content = `Assoc [("message", `String msg)] in

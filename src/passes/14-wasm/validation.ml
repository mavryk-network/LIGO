...
let find_type w symbol =
  let t =
    List.find
      ~f:(fun f ->
        match f.it with
        | TypeSymbol { tname; _ } -> String.equal tname symbol
        | _ -> false)
      w.A.types
  in
  match t with
...
let stack_pop (env : Env.t) (v2 : T.value_type) =
  ...
  | Some (v1, Next operand_stack) when Poly.equal v1 v2 -> { env with operand_stack }
  | Some (_, _) ->
    failwith
      "Bug for the developers. The item popped from the operand stack does not match the \
       expected type, which should not happen."
  | None ->
    failwith
      "Bug for the developers. There's nothing on the operand stack, which should not \
       happen."


let stack_pop_any (env : Env.t) =
  match env.operand_stack with
  | Some (_, Next operand_stack) -> { env with operand_stack }
  | None ->
    failwith
      "Bug for the developers. There's nothing on the operand stack, which should not \
       happen."
...
let block_push (env : Env.t) b = { env with blocks = b :: env.blocks }
...
let rec check w env e : Env.t =
  ...
  | { it = Select _; _ } :: rest ->
    ...
    check w env rest
  | { it = Block (bt, il); _ } :: rest | { it = Loop (bt, il); _ } :: rest ->

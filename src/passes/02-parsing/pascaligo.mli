...
module Make (Options: Options.S) :
  sig
    ...
    (* All functions read a string buffer but they differ in the way
       ...
       comes originally from a string, and [expression] assumes that
       is contents is an expression and comes from a string. *)
    ...
    val parse_type_expression : CST.type_expr parser (* MAVRYK: PascaLIGO *)
...
(* MAVRYK: PascaLIGO — needed by the LSP (lib/ligo_lsp) for signature hover. *)

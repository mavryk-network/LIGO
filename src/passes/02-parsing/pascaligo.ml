...
module Make (Options : Options.S) =
  struct
    ...
    let pretty_print_file
      ...
      parse_file
        ?jsligo ?preprocess ?project_root ~preprocess_define ~raise buffer file_path
      |> pretty_print env
    ...
    let pretty_print_cst ?jsligo ?preprocess ~preprocess_define ?project_root
      ...
      let tree   = parse_file ?jsligo ?preprocess ~preprocess_define
                              ?project_root ~raise buffer file_path in

...
%[@recover.prelude
  ...
  let mk_ident    = Token.wrap_ident    Token.Plain "ghost_ident"
...
%token      <Token.id_kind * string Wrap.t> Ident     "<ident>"     [@recover.expr mk_ident     $loc] [@recover.cost 900]
...
%token <string Wrap.t> Entry     "entry"     [@recover.expr Token.wrap_entry     $loc]
...
%token <string Wrap.t> Originate "originate" [@recover.expr Token.wrap_originate $loc]
...
%token <string Wrap.t> Test      "test"      [@recover.expr Token.wrap_test      $loc]
...
%token <string Wrap.t> View      "view"      [@recover.expr Token.wrap_view      $loc]

...
val from_file   : raise:Errors.t Trace.raise -> Buffer.t -> file_path -> CST.t * LexerLib.Markup.t list
val from_string : raise:Errors.t Trace.raise -> Buffer.t -> CST.t
val expression  : raise:Errors.t Trace.raise -> Buffer.t -> CST.expr

(* Aliases *)

val parse_file       : raise:Errors.t Trace.raise -> Buffer.t -> file_path -> CST.t * LexerLib.Markup.t list

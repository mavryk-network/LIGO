...
(* MAVRYK: PascaLIGO. The token payload's lexeme is the source form (e.g. "1.5mav"),
   which round-trips exactly, so we print it directly. *)
let print_mav (node : (lexeme * Q.t) wrap) =
  fst node#payload |> string
...
and print_keys (node : expr brackets reg nseq) =
  let keys = nseq_to_list node in
...
and print_E_Mav (node : (lexeme * Q.t) wrap) = print_mav node

  ...
  let get_operators (protocol_version : protocol_type) c : predicate option =
    ...
    | C_DIV, _ ->
      Some
        ...
        @@ seq
             [ prim "EDIV"
             ; i_assert_some_msg (i_push_string "DIV by 0")
             ; i_car
             ])
    | C_MOD, _ ->
      Some
        ...
        @@ seq
             [ prim "EDIV"
             ; i_assert_some_msg (i_push_string "MOD by 0")
             ; i_cdr
             ])
    ...
    | C_MAP_FIND, _ ->
      Some
        (simple_binary
        @@ seq [ prim "GET"; i_assert_some_msg (i_push_string "MAP FIND") ])
    ...
    | C_SET_ADD, _ ->
      Some
        (simple_binary
        @@ seq [ dip (i_push (prim "bool") (prim "True")); prim "UPDATE" ])
    | C_SET_REMOVE, _ ->
      Some
        (simple_binary
        @@ seq [ dip (i_push (prim "bool") (prim "False")); prim "UPDATE" ])
    | C_SET_UPDATE, _ -> Some (simple_ternary @@ prim "UPDATE")
    | C_CONCAT, _ -> Some (simple_binary @@ prim "CONCAT")
    | C_SLICE, _ ->
      Some
        (simple_ternary
        @@ seq [ prim "SLICE"; i_assert_some_msg (i_push_string "SLICE") ])
    ...
    | C_MAP_REMOVE, _ ->
      Some
        (special (fun with_args ->
             seq [ dip (with_args "NONE"); prim "UPDATE" ]))
    | C_LEFT, _ -> Some (trivial_special "LEFT")
    | C_RIGHT, _ -> Some (trivial_special "RIGHT")
    | C_CREATE_CONTRACT, _ ->
      Some
        (special (fun with_args -> seq [ with_args "CREATE_CONTRACT"; i_pair ]))

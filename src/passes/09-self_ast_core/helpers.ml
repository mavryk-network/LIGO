...
type ('a, 'err, 'warn) folder =
  raise:('err, 'warn) raise -> 'a -> expression -> 'a

let rec fold_expression ~raise
  : ('a, 'err, 'warn) folder -> 'a -> expression -> 'a
  =
 fun f init e ->
  let self = fold_expression ~raise f in
  let idle acc _ = acc in
 ...
 fun self acc x ->
  ...
  | Module_expr.M_struct (decls : Ast_core.decl list) ->
    List.fold
      ...
      ~init:acc
      decls
  | M_module_path _ | M_variable _ -> acc


type ('err, 'warn) exp_mapper =
  raise:('err, 'warn) raise -> expression -> expression

type ('err, 'warn) ty_exp_mapper =
  raise:('err, 'warn) raise -> type_expression -> type_expression

let rec map_expression ~raise
  : ('err, 'warn) exp_mapper -> expression -> expression
  =
 fun f e ->
...
and map_expression_in_declaration
...
and map_expression_in_declarations
  : (expression -> expression) -> program -> program
  =
 fun self xs -> List.map ~f:(map_expression_in_declaration self) xs
...
and map_expression_in_module_expr
 ...
 fun self x ->
  ...
  | M_module_path _ | M_variable _ -> x


and map_type_expression ~raise
  : ('err, 'warn) ty_exp_mapper -> type_expression -> type_expression
  =
 fun f te ->
  let self = map_type_expression ~raise f in
  let te' = f ~raise te in
  let return type_content =
    { type_content; location = te.location; sugar = te.sugar }
  in
...
let rec fold_map_expression
  : type a. a fold_mapper -> a -> expression -> a * expression
  =
 fun f a e ->
...
and fold_map_expression_in_module_expr
  : type a.
    (a -> expression -> a * expression) -> a -> module_expr -> a * module_expr
  =
 fun self acc x ->
    ...
    let res, decls =
      List.fold_map
        ~f:(fun acc (x : decl) ->
          ...
          | D_module x ->
            let res, module_ =
              fold_map_expression_in_module_expr self acc x.module_
            in

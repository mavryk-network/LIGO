...
let translate_var (m : meta) (x : I.var_name) (env : I.environment) =
  let (_, idx) = match I.Environment.Environment.get_i x env with Some (v) -> v | None -> failwith @@ Format.asprintf "Corner case: %a not found in env" Ast_typed.PP.expression_variable x in

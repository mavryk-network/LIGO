...
let rec untype_type_expression (t:O.type_expression) : (I.type_expression, typer_error) result =
  ...
  | O.T_sum {fields ; layout} ->
     let aux ({associated_type ; michelson_annotation ; decl_pos} : O.row_element) =
       ...
       ok @@ v' in
     let* x' = Stage_common.Helpers.bind_map_lmap aux fields in
...
let rec untype_expression (e:O.expression) : (I.expression, typer_error) result =
  ...
  | E_constant {cons_name;arguments} ->
    let* arguments = bind_map_list untype_expression arguments in
  ...
  | E_record r ->
    let* r' = Stage_common.Helpers.bind_map_lmap untype_expression r in
    return @@ E_record r'
  ...
  | E_assign {lvalue; value; next} ->
    let lvalue = Location.map Var.todo_cast lvalue in
    let* value = untype_expression value in
    let* next = untype_expression next in
    return @@ E_assign {lvalue; value; next}
...
and untype_lambda {binder; output_type; result} : (_ O.lambda, typer_error) result =
    ...
    let* output_type = bind_map_option untype_type_expression output_type in

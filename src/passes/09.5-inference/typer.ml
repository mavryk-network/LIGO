  ...
  let check_has_no_unification_vars ((O.Module_With_Unification_Vars p) as pp) =
    ...
    let decl : O.declaration -> _ = fun d -> match d with
      ...
      | O.Module_alias { alias=_; binders=_} ->
        ...
        ok @@ () in
    let* () = bind_fold_list (fun () Location.{wrap_content;location=_} -> decl wrap_content) () p in
    ok @@ p
end
...
let rec type_declaration env state : I.declaration Location.wrap -> (environment * _ O'.typer_state * O.declaration Location.wrap * O.type_expression, typer_error) result = fun d ->
  let return : O.declaration -> _ -> _ -> _ O'.typer_state -> _ (* return of type_expression *) = fun expr ty e state constraints ->
    ...
    let* state = Solver.main state constraints in
  ...
  | Declaration_type {type_binder; type_expr} ->
    ...
    let* type_expr = evaluate_type env type_expr in
  ...
  | Declaration_constant {name; binder; attr={inline}; expr} -> (
    ...
    *)
    let* tv_opt = bind_map_option (evaluate_type env) binder.ascr in
    ...
    let* binder = Stage_common.Maps.binder (evaluate_type env)  binder in
  ...
  | Declaration_module {module_binder;module_} -> (
    let* (e,module_,t,state) = type_module ~init_env:env module_ in
...
and evaluate_type : environment -> I.type_expression -> (O.type_expression, typer_error) result = fun e t ->
  ...
  | T_sum {fields ; layout} ->
    ...
    in
    let* fields = Stage_common.Helpers.bind_map_lmap aux fields in
    let* () = trace_assert_fail_option (variant_redefined_error t.location) @@
  ...
  | T_app {type_operator;arguments} ->
    let* arguments = bind_map_list (evaluate_type e) arguments in
...
and type_expression : ?tv_opt:O.type_expression -> environment -> _ O'.typer_state -> I.expression -> (environment * _ O'.typer_state * O.expression * O.type_expression, typer_error) result = fun ?tv_opt e state ae ->
  let return : _ -> _ -> _ -> _ O'.typer_state -> _ (* return of type_expression *) = fun expr ty_expr e state constraints ->
    let* new_state = Solver.main state constraints in
    let () = Pretty_print_variables.flush_pending_print state in
    ok @@ (e,new_state, expr, ty_expr) in
  let* ((e,state,expr,ty_expr),constraints) = type_expression' ?tv_opt e state ae in
...
and type_expression' : ?tv_opt:O.type_expression -> environment -> _ O'.typer_state -> I.expression -> ((environment * _ O'.typer_state * O.expression * O.type_expression) * _, typer_error) result = fun ?tv_opt e state ae ->
  ...
  | E_constant {cons_name; arguments=lst} ->
    let* t = Typer_common.Constant_typers_new.Operators_types.constant_type cons_name in
    let* (e,state,constraints),lst = bind_fold_map_list
      (fun (e,state, c) l ->
  ...
  | E_lambda lambda ->
    let* lambda,_,state,constraints,wrapped = type_lambda e state lambda in
    ...
    let type_match_case (state,env) ({pattern;body} : _ I.match_case) : ((_ O'.typer_state * environment) * (_ O.match_case * O.type_constraint list * (O.type_expression * O.type_expression)),_) result =
            ...
            let* (arg_t_env , variant_t_env) =
              ...
              *)
              ...
              | Label "Some" -> 
                ...
                ok (option_content, p)
              | _ ->
                let* (x,y) = trace_option (unbound_constructor e constructor ae.location) @@
      ...
      let env = (* populate env with pattern binders (all P_var's) *)
        Stage_common.Helpers.fold_pattern
          ...
          new_pattern
      in
      let* (env, state, body, body_t),body_constraints = type_expression' env state body in
  ...
  | E_record m ->
    ...
    in
    let* (e,state',constraints), m' = Stage_common.Helpers.bind_fold_map_lmap aux (e,state,[]) m in
  ...
  | E_let_in {let_binder ; rhs ; let_result; inline} ->
    let* rhs_tv_opt = bind_map_option (evaluate_type e) (let_binder.ascr) in
    let* (e,state,rhs,t_r),c1 = self e state rhs in
    let* let_binder = Stage_common.Maps.binder (evaluate_type e) let_binder in
  ...
  | E_mod_in {module_binder; rhs ; let_result} ->
    let* (env,rhs,_,_) = type_module ~init_env:e rhs in
  ...
  | E_recursive {fun_name;fun_type;lambda} ->
    ...
    let* lambda,e,state,c1,(c2,t_var) = type_lambda e state lambda in
...
and type_lambda e state {
      ...
      let* input_type'  = bind_map_option (evaluate_type e) binder.ascr in
      let* output_type' = bind_map_option (evaluate_type e) output_type in
      let* binder = Stage_common.Maps.binder (evaluate_type e) binder in
      ...
      let* (e, state', result,t),constraints = type_expression' e' state result in
...
and type_module_returns_env ((env, state, p) : environment * _ O'.typer_state * I.module_) : (environment * _ O'.typer_state * O.module_with_unification_vars * O.type_expression, Typer_common.Errors.typer_error) result =
  let aux ((e : environment), (s : _ O'.typer_state) , (ds : O.declaration Location.wrap list), tys) (d:I.declaration Location.wrap) =
    let* (e , s' , d',t) = type_declaration_subst e s d in
...
and type_and_subst : type a b.
  ...
  let* node,ty,env =
    ...
    let substs : variable: O.type_variable -> O.type_content option = fun ~variable ->
      ...
      let* root = Solver.get_alias variable aliases in
      ...
      | `Constructor { tv ; c_tag ; tv_list } ->
        let* tv_root = Solver.get_alias tv aliases in

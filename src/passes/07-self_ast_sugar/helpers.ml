...
let rec fold_expression : ('a, 'err) folder -> 'a -> expression -> 'a =
 fun f init e ->
  ...
  | E_literal _ | E_variable _ | E_raw_code _ | E_skip _ | E_module_accessor _
    -> init
  ...
  | E_let_pattern_in   li -> Let_pattern_in.fold self self_type init li
...
let rec map_expression : exp_mapper -> expression -> expression =
 fun f e ->
  ...
  | E_let_in li ->
    ...
    return @@ E_let_in li
  | E_let_pattern_in li ->
    let li = Let_pattern_in.map self (fun a -> a) li in
...
and map_declaration m : declaration -> declaration =
 fun d ->
  let aux : declaration_content -> declaration_content =
   fun x ->
    match x, m with
...
and map_module : abs_mapper -> module_ -> module_ =
 fun m p -> List.map ~f:(map_decl m) p


let map_program : abs_mapper -> program -> program =
 fun m p -> List.map ~f:(map_declaration m) p
...
let rec fold_map_expression
 ...
 fun f a e ->
  ...
  then init, e'
    ...
    | E_map lst ->
      let res, lst' =
        List.fold_map ~f:(fun init -> Pair.fold_map ~f:self ~init) ~init lst
      in
    ...
    | E_let_pattern_in li ->
      let res,li = Let_pattern_in.fold_map self idle init li in
      res, return @@ E_let_pattern_in li
    | E_type_in ti ->
    ...
    | ( E_literal _
      ...
      | E_raw_code _
      | E_skip _
      | E_module_accessor _ ) as e' -> init, return e')

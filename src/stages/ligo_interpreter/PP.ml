...
let pp_ct : Format.formatter -> constant_val -> unit = fun ppf c ->
  ...
  | C_address c -> Format.fprintf ppf "%a" Tezos_protocol_010_PtGRANAD.Protocol.Alpha_context.Contract.pp c
  | C_contract c -> Format.fprintf ppf "%a(%a)" Tezos_protocol_010_PtGRANAD.Protocol.Alpha_context.Contract.pp c.address (PP_helpers.option PP_helpers.string) c.entrypoint
  ...
  | V_Failure err ->
     ...
     | Object_lang_ex {location;errors;calltrace = _} ->
        Format.fprintf ppf "@[<v 4>%a@.An uncaught error occured:@.%a@]"
          Snippet.pp location
          (Tezos_client_010_PtGRANAD.Michelson_v1_error_reporter.report_errors ~details:true ~show_source:true ?parsed:(None)) errors

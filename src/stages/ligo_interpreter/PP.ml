...
let pp_ct : Format.formatter -> constant_val -> unit = fun ppf c ->
  ...
  | C_address c -> Format.fprintf ppf "%a" Tezos_protocol_009_PsFLoren.Protocol.Alpha_context.Contract.pp c
  | C_contract c -> Format.fprintf ppf "%a(%a)" Tezos_protocol_009_PsFLoren.Protocol.Alpha_context.Contract.pp c.address (PP_helpers.option PP_helpers.string) c.entrypoint

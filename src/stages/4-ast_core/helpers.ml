...
module Free_type_variables = struct
  ...
  let unions : VarSet.t list -> VarSet.t =
   fun l -> List.fold l ~init:VarSet.empty ~f:(fun y1 y2 -> VarSet.union y1 y2)


  let rec map_type_expression : Type_var.t list -> type_expression -> VarSet.t =
   fun type_env te ->
    ...
    | T_app { arguments; _ } ->
      ...
      unions arguments
    | T_variable v
      when List.mem type_env v ~equal:(fun v1 v2 -> Type_var.compare v1 v2 = 0) ->
      VarSet.empty
    | T_variable _ -> VarSet.empty
    | T_module_accessor _ -> VarSet.empty (* self element *)
  ...
  let type_expression : Type_var.t list -> type_expression -> Type_var.t list =
   fun type_env t -> VarSet.fold (fun v r -> v :: r) (map_type_expression type_env t) []
end

...
[@@deriving
  ez
    { prefixes =
        [ "make_e", (Location.wrap : loc:Location.t -> expression_content -> expression)
        ; "get_e", Location.unwrap
        ]
    ; wrap_constructor =
        ( "expression_content"
...
[@@deriving
  ez
    { prefixes =
        [ "make_t", (Location.wrap : loc:Location.t -> type_content -> type_expression)
        ; "get_t", Location.unwrap
        ]
    ; wrap_constructor =
...
let rec get_e_applications t =
  ...
  | Some (lamb, args) ->
    (match get_e_applications (get_e lamb) with
    | [] -> [ lamb; args ]
    | apps -> apps @ [ args ])
  | None -> []

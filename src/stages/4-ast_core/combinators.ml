...
[@@deriving
  ez
    { prefixes =
        [ ( "make_e"
          , fun ?(loc = Location.generated) ?sugar expression_content
                : expression ->
    ...
    ; wrap_constructor =
        ( "expression_content"
        , fun expression_content ?loc ?sugar () ->
            make_e ?loc ?sugar expression_content )
...
[@@deriving
  ez
    { prefixes =
        [ ( "make_t"
          , fun ?(loc = Location.generated) ?sugar type_content
                : type_expression ->
...
let t__type_ ?loc ?sugar t : type_expression =
  t_constant ?loc ?sugar _type_ [ t ]
  [@@map _type_, ("list", "set", "contract", "ticket")]


let t__type_ ?loc ?sugar t t' : type_expression =
  t_constant ?loc ?sugar _type_ [ t; t' ]
...
let t_pair ?loc ?sugar a b : type_expression =
  ez_t_record
    ...
    ?sugar
    [ ( Label.of_int 0
      , { associated_type = a; michelson_annotation = None; decl_pos = 0 } )
    ; ( Label.of_int 1
      , { associated_type = b; michelson_annotation = None; decl_pos = 1 } )
    ]
...
let t_sum_ez ?loc ?sugar ?layout (lst : (string * type_expression) list)
    : type_expression
  =
  let lst =
    List.mapi
...
let e_bytes_string ?loc ?sugar (s : string) : expression =
  make_e ?loc ?sugar
  @@ E_literal (Literal_bytes (Hex.to_bytes (Hex.of_string s)))
...
let e_let_in ?loc ?sugar let_binder rhs let_result attr =
  e_let_in ?loc ?sugar { let_binder; rhs; let_result; attr } ()


let e_let_mut_in ?loc ?sugar let_binder rhs let_result attr =
  e_let_mut_in ?loc ?sugar { let_binder; rhs; let_result; attr } ()
...
let e_raw_code ?loc ?sugar language code =
  e_raw_code ?loc ?sugar { language; code } ()
...
let t_michelson_sum ?loc l l_ann r r_ann =
  let row decl_pos associated_type michelson_annotation
...
let t_michelson_pair ?loc l l_ann r r_ann =
  ...
  t_record
    ?loc
    (Record.of_list [ Label "0", row 0 l l_ann; Label "1", row 1 r r_ann ])

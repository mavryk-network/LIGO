...
let sum_set_t value sep ppf m =
  ...
  let new_pp
      ppf
      (k, { content = Rows.Elem.{ associated_type; _ }; attributes = _ })
    =
...
let record_sep_t
    ...
    sep
    ppf
    (m : (Label.t * type_expression Rows.Elem.t) list)
  =
  let attributes_2 (attr : string list) : string =
    List.map ~f:(fun s -> "[@@" ^ s ^ "]") attr |> String.concat
  in
  let lst = List.sort ~compare:(fun (a, _) (b, _) -> Label.compare a b) m in
  let new_pp ppf (k, { content = { Rows.Elem.associated_type; _ }; attributes })
    =
...
and expression_content ppf (ec : expression_content) =
  ...
  | E_constant c ->
    fprintf
      ...
      ((fun (Constant.Const c) -> c) c.cons_name)
      (list_sep expression (tag " ,"))
      c.arguments
  ...
  | E_let_mut_in li -> Let_in.pp_mut expression type_expression_option ppf li
...
and record ppf kvl =
  fprintf
    ...
    "@[<v>{ %a }@]"
    (list_sep
       (fun ppf (l, e) -> Format.fprintf ppf "%a : %a" Label.pp l expression e)
       (tag " ;"))
    kvl

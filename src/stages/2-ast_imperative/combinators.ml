...
[@@deriving
  ez
    { prefixes =
        [ ( "make_m"
          , fun ?(loc = Location.generated) wrap_content : module_expr ->
              { wrap_content; location = loc } )
...
let t_variable_ez ?loc n : type_expression =
  t_variable ?loc (Type_var.of_input_var n)
...
let t_record_ez_attr ?loc ?(attr = []) fields =
  let aux i (name, t_expr, attributes) =
    ...
    , { content = Rows.Elem.{ associated_type = t_expr; decl_pos = i }
      ; attributes
      } )
  in
...
let t_arrow ?loc type1 type2 : type_expression =
  make_t ?loc @@ T_arrow { type1; type2 }
...
let e_string ?loc s : expression =
  make_e ?loc @@ E_literal (Literal_string (Standard s))
...
let e_bytes_string ?loc (s : string) : expression =
  e_bytes_hex ?loc @@ Hex.of_string s


let e_string_cat ?loc sl sr : expression =
  make_e ?loc
  @@ E_constant { cons_name = Const C_CONCAT; arguments = [ sl; sr ] }


let e_map_add ?loc k v old : expression =
  make_e ?loc
  @@ E_constant { cons_name = Const C_MAP_ADD; arguments = [ k; v; old ] }
...
let e_matching_tuple ?loc matchee (binders : _ Binder.t list) body : expression =
  let pv_lst =
    List.map
      ~f:(fun (b : _ Binder.t) -> Location.wrap ?loc @@ Pattern.P_var b)
      binders
  in
...
let e_param_matching_tuple ?loc matchee (params : _ Param.t list) body
    : expression
  =
  let pv_lst =
    List.map
      ~f:(fun (p : _ Param.t) ->
        Location.wrap ?loc @@ Pattern.P_var (Param.to_binder p))
      params
  in
  let pattern = Location.wrap ?loc @@ Pattern.P_tuple pv_lst in
  let body =
...
let e_param_matching_record
    ?loc
    matchee
    (params : (string * _ Param.t) list)
    body
    : expression
  =
...
let e_update ?loc struct_ path update =
  make_e ?loc @@ E_update { struct_; path; update }
...
let e_bool ?loc b : expression =
  if b
  then e_constructor ?loc "True" (e_unit ())
...
let e_record_ez ?loc (lst : (string * expr) list) : expression =
  let map =
    List.fold_right
      ~f:(fun (x, y) acc -> (Label.of_string x, y) :: acc)
      ~init:[]
      lst
  in
  e_record ?loc map
...
let e_map_find_opt ?loc k map =
  e_constant ?loc (Const C_MAP_FIND_OPT) [ k; map ]
...
let e_map_remove ?loc ele map =
  e_constant ?loc (Const C_MAP_REMOVE) [ ele; map ]
...
let e_typed_big_map ?loc lst k v =
  e_annotation ?loc (e_big_map lst) (t_big_map k v)


let e_typed_set ?loc lst k = e_annotation ?loc (e_set lst) (t_set k)

let e_assign ?loc binder expression =
  make_e ?loc @@ E_assign { binder; expression }


let e_assign_ez ?loc variable expression =
  e_assign
    ?loc
    (Binder.make (Value_var.of_input_var ?loc variable) None)
    expression
...
let e_unopt ?loc matchee none_body (var_some, some_body) =
  let some_case =
    let pattern =
      ...
      @@ Pattern.P_variant
           ( Label "Some"
           , Location.wrap @@ Pattern.P_var (Binder.make var_some None) )
    in

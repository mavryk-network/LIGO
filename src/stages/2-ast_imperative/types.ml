...
module Attr = struct
  type t = string list [@@deriving eq, compare, hash, yojson]

  let pp ppf t =
    (Format.pp_print_list (fun ppf attr -> Format.fprintf ppf "[@@%s]" attr))
      ppf
      t
end
...
module Let_in = Let_in.Make (Attr)
...
type expression_content =
  ...
  (* Record *)
  | E_record of expr Record.List.t

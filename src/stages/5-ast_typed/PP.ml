...
and declaration ppf (d : declaration) =
  match d with
  | Declaration_constant {binder; expr; attr = { inline; no_mutation ; view ; public ; on_test } } ->
      fprintf ppf "const %a = %a%a%a%a%a%a" expression_variable binder expression expr option_inline inline option_no_mutation no_mutation option_view view option_public public option_on_test on_test

...
let map_row_element_mini_c
    : ('a -> 'b) -> 'a row_element_mini_c -> 'b row_element_mini_c
  =
...
module PP = struct
  ...
  let row_element f ppf (k, ({ associated_type; _ } : _ row_element)) =
    Format.fprintf ppf "@[<h>%a -> %a@]" Label.pp k f associated_type
  ...
  let tuple_or_record_sep_t
    ...
    then Format.fprintf ppf format_tuple (tuple_sep value (tag sep_tuple)) m
    else Format.fprintf ppf format_record (record_sep value (tag sep_record)) m


  let tuple_or_record_sep_type value =
    tuple_or_record_sep_t
      value
      "@[<hv 7>record[%a]@]"
      " ,@ "
      "@[<hv 2>( %a )@]"

...
let to_yojson v_to_yojson { ty_binder; kind; type_ } = `List [ Var.Type_var.to_yojson ty_binder; Kind.to_yojson kind; v_to_yojson type_ ]
let of_yojson v_of_yojson (yojson: Yojson.Safe.t) =
  match yojson with 
  | `List [ty_binder_yojson; kind_yojson; v_yojson ] ->
     (match Var.Type_var.of_yojson ty_binder_yojson, Kind.of_yojson kind_yojson, v_of_yojson v_yojson with

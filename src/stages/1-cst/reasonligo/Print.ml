...
and print_let_binding state node attr =
  ...
  let arity =
    ...
    print_pattern (state#pad 1 0) binders;
    0
  in
  let arity =
    match lhs_type with
  ...
  let () =
    ...
    then (
      ...
      List.iteri ~f:(apply length) attr)
  in
  ()
...
and print_let_in state node =
  ...
  let arity =
    ...
    | None -> 0
    | Some _ ->
      print_node (state#pad fields 0) "rec";
      1
  in
  let arity =
    let state = state#pad fields 0 in
...
and print_arguments state = function
  ...
  | Unit u -> print_loc_node state "Unit" u.region
...
and print_arith_expr state = function
  ...
  | Mutez m ->
    ...
    print_string state { m with value = fst m.value }
(* TODO *)
...
and print_cond_expr state (cond : cond_expr) =
  let () =
    ...
    print_test_expr (state#pad 1 0) cond.test
  in
  let () =
    let state = state#pad 3 1 in
    print_node state "<true>";
    print_expr (state#pad 1 0) (fst cond.ifso.value.inside)
  in
  let () =
...
and print_case : 'a. (state -> 'a -> unit) -> state -> 'a case -> unit =
 fun printer state case ->
  ...
  print_expr (state#pad length 0) case.expr;
...
and print_variant state { constr; args; attributes = attr } =
  ...
  let rank =
    ...
    | Some { value = { inside; _ }; _ } ->
      print_type_expr (state#pad arity rank) inside;
      rank + 1
  in
  let () =
    if not (List.is_empty attr)

...
type _ sing =
  ...
  | S_negation : negation sing
  | S_neq : neq sing
  | S_nsep_or_pref : 'a sing * 'b sing -> ('a, 'b) nsep_or_pref sing
  | S_nsep_or_term : 'a sing * 'b sing -> ('a, 'b) nsep_or_term sing
  ...
  | S_semi : semi sing
  | S_sep_or_term : 'a sing * 'b sing -> ('a, 'b) sep_or_term sing
...
let fold
  ...
  | S_expr -> process
    ...
    | E_Mod node -> node -| S_reg (S_bin_op S_modulo)
    | E_ModEq node -> node -| S_reg (S_bin_op S_mod_eq)
    ...
    | E_Not node -> node -| S_reg (S_un_op S_negation)
  ...
  | S_negation -> process @@ node -| S_wrap S_lexeme

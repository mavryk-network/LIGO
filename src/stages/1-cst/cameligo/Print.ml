...
and print_declaration state = function
  | Let { value = _kwd_let, kwd_rec, let_binding, attr; region } ->
    ...
    if Option.is_some kwd_rec then print_node (state#pad 0 0) "rec";
    (* Hack *)
...
and print_binders state patterns =
  ...
  List.iteri ~f:(apply arity) patterns
...
and print_type_params state (node : type_params par reg) =
  ...
  List.iteri ~f:(apply arity) vars


and print_type_decl state decl =
  let arity = if Option.is_none decl.params then 2 else 3 in
  let rank =
    print_ident (state#pad arity 0) decl.name;
    1
  in
  let rank =
    match decl.params with
    | Some params ->
      print_type_vars (state#pad arity rank) params;
      rank + 1
    | None -> rank
  in
...
and print_type_vars state = function
  ...
  | QParamTuple p ->
    ...
    let apply len rank = print_type_var (state#pad len rank) in
    List.iteri ~f:(apply arity) type_vars
...
and print_pattern state = function
  ...
  | PUnit { region; _ } -> print_loc_node state "PUnit" region
  | PList plist ->
...
and print_tuple_pattern state tuple =
  ...
  let apply len rank = print_pattern (state#pad len rank) in
  List.iteri ~f:(apply length) patterns
...
and print_injection : 'a. (state -> 'a -> unit) -> state -> 'a injection -> unit
 ...
 fun printer state inj ->
  ...
  let apply len rank = printer (state#pad len rank) in
...
and print_ne_injection :
 ...
 fun printer state inj ->
  ...
  if not (List.is_empty inj.attributes)
  then (
    let state = state#pad arity (arity - 1) in
    print_attributes state inj.attributes)
...
and print_expr state = function
  ...
  | EBytes b ->
    ...
    print_bytes state b
  | EUnit u -> print_loc_node state "EUnit" u.region
  | ETuple e_tuple ->
...
and print_code_inj state rc =
  let () =
    ...
    print_string (state#pad 1 0) rc.language.value
  in
  let () =
...
and print_mod_in state node =
  ...
  let () =
    ...
    print_cst (state#pad 1 0) module_
  in
  let () =
    let state = state#pad 3 2 in
    print_node state "<body>";
...
and print_arith_expr state = function
  ...
  | Land { value; region } -> print_bin_op "Land" region state value
  | Lor { value; region } -> print_bin_op "Lor" region state value
  | Lxor { value; region } -> print_bin_op "Lxor" region state value
  | Lsl { value; region } -> print_bin_op "Lsl" region state value
  | Lsr { value; region } -> print_bin_op "Lsr" region state value
  | Neg { value; region } ->
...
and print_cond_expr state (cond : cond_expr) =
  ...
  let () =
    ...
    print_expr (state#pad 1 0) cond.test
  in
  let () =
    let state = state#pad arity 1 in
    print_node state "<true>";
    print_expr (state#pad 1 0) cond.ifso
  in
  let () =
...
and print_case : 'a. (state -> 'a -> unit) -> state -> 'a case -> unit =
 fun printer state case ->
  ...
  let apply len rank = print_case_clause printer (state#pad len (rank + 1)) in
  print_expr (state#pad arity 0) case.expr;
...
and print_sum_type state { variants; attributes; _ } =
  ...
  let apply arity rank variant =
    let state = state#pad arity rank in
    print_variant state variant.value
  in
...
and print_variant state { constr; arg; attributes = attr } =
  ...
  let rank =
    ...
    | None -> rank
    | Some (_, c) ->
      print_type_expr (state#pad arity rank) c;
      rank + 1
  in
  let () =
    if not (List.is_empty attr)

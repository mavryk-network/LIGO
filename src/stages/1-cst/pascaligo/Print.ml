...
(* MAVRYK: PascaLIGO keeps [nseq] as a tuple (see Cst_pascaligo.CST); the shared
   Tree printer dropped [of_nseq], so we bridge to [of_ne_list]. *)
let of_nseq state label print (hd, tl) =
  Tree.of_ne_list state label print Simple_utils.Ne_list.(hd :: tl)
...
let payload_of_variable = function
  Var v -> v#payload
| Esc v -> "@" ^ v#payload
...
let rec print_cst state (node : cst) =
  of_nseq state "<cst>" print_declaration node.decl
...
and print_D_Fun state (node: fun_decl reg) =
  ...
  let children = Tree.[
    ...
    mk_child     print_variable    node.fun_name;
    mk_child_opt print_type_params node.type_params;
...
and print_type_params state (node : type_params chevrons reg) =
  Tree.of_nsepseq state "<type parameters>" print_variable
                  node.value.inside
...
and print_declarations state (node: declarations) =
  of_nseq state "<declarations>" print_declaration node
...
and print_D_Type state (node: type_decl reg) =
  ...
  let children = Tree.[
    mk_child     print_variable  node.name;
...
and print_type_vars state (node: variable tuple) =
  Tree.of_nsepseq state "<type variables>" print_variable
                  node.value.inside
...
and print_field_decl state (node: field_decl reg) =
  ...
  and root = payload_of_variable value.field_name
  in Tree.make state root ~region children
...
and print_ForMap state (node: for_map reg) =
  ...
  let print_binding state (source, _arrow, image) =
    let children = Tree.[
      mk_child print_variable source;
      mk_child print_variable image]
...
and print_P_Mumav state (node: mumav_literal) =
  Tree.make_mumav "P_Mumav" state node

and print_P_Mav state (node: mav_literal) =
...
and print_map_lookup state root (node: map_lookup reg) =
  ...
  let print_keys state (keys : expr brackets reg nseq) =
    of_nseq state "<keys>" print_key keys in
...
(* Literal mumav as expressions *)

and print_E_Mumav state (node: mumav_literal) =
  Tree.make_mumav "E_Mumav" state node

and print_E_Mav state (node: mav_literal) =

...
let print_attribute state (node : Attr.t reg) =
  ...
  match val_opt with
  | None -> Tree.print_unary state "<attribute>" Tree.print_node key
  | Some (String value) ->
    let children =
      [ Tree.mk_child Tree.print_node key; Tree.mk_child Tree.print_node value ]
    in
    Tree.print state "<attributes>" children
...
and print_D_Attr state (node : attribute * declaration) =
  ...
  in
  Tree.print state "D_Attr" children
...
and print_D_Const state (node : const_decl reg) =
  ...
  in
  Tree.print state "D_Const" children
...
and print_D_Fun state (node : fun_decl reg) =
  ...
  in
  Tree.print state "D_Fun" children


and print_recursive state (node : lexeme wrap) = Tree.print_literal state node

and print_type_params state (node : type_params chevrons reg) =
  print_nsepseq state "<type parameters>" Tree.print_literal node.value.inside


and print_parameters state (node : parameters) =
  print_sepseq state "<parameters>" print_param_decl node.value.inside
...
and print_param_decl state (node : param_decl reg) =
  ...
  in
  Tree.print state "<parameter>" ~region children
...
and print_D_Module state (node : module_decl reg) =
  ...
  in
  Tree.print state "D_Module" children
...
and print_D_Type state (node : type_decl reg) =
  let node = node.value in
  let print_type_expr state = Tree.print_unary state "<type>" print_type_expr in
  let children =
    [ Tree.mk_child Tree.print_literal node.name
  ...
  in
  Tree.print state "D_Type" children


and print_type_vars state (node : variable tuple) =
  print_nsepseq state "<type variables>" Tree.print_literal node.value.inside
...
and print_T_App state (node : (type_expr * type_tuple) reg) =
  ...
  in
  Tree.print state "T_App" ~region children
...
and print_T_Attr state (node : attribute * type_expr) =
  ...
  in
  Tree.print state "T_Attr" children
...
and print_T_Fun state (node : (type_expr * arrow * type_expr) reg) =
  ...
  in
  Tree.print state "T_Fun" ~region children
...
and print_compound :
 ...
 fun print label state { value; region } ->
  let children =
    List.map ~f:(Tree.mk_child print) @@ Utils.sepseq_to_list value.elements
  in
  Tree.print state label ~region children
...
and print_T_Sum state (node : sum_type reg) =
  ...
  in
  Tree.print state "T_Sum" variants
...
and print_of_type_expr state (_, type_expr) = print_type_expr state type_expr
...
and print_S_Attr state (node : attribute * statement) =
  ...
  in
  Tree.print state "S_Attr" children
...
and print_S_VarDecl state (node : var_decl reg) =
  ...
  in
  Tree.print state "S_VarDecl" children
...
and print_I_Assign state (node : assignment reg) =
  ...
  let children =
    [ Tree.mk_child print_lhs value.lhs; Tree.mk_child print_rhs value.rhs ]
  in
  Tree.print state "I_Assign" ~region children


(* Procedure calls *)

and print_I_Call state (node : call) = print_call state "I_Call" node

and print_call state label (node : call) =
  let Region.{ value; region } = node in
  let func, args = value
  and mk_func state = Tree.print_unary state "<function>" print_expr
  and mk_args state (node : (expr, comma) Utils.sepseq par reg) =
    print_sepseq state "<arguments>" print_expr node.value.inside
  in
  let children = [ Tree.mk_child mk_func func; Tree.mk_child mk_args args ] in
  Tree.print state label ~region children
...
and print_case : 'a. 'a Tree.printer -> label -> state -> 'a case reg -> unit =
 fun print label state { value; region } ->
  let print_case_test state = Tree.print_unary state "<condition>" print_expr in
  let cases =
    List.map ~f:(Tree.mk_child @@ print_case_clause print)
    @@ Utils.nsepseq_to_list value.cases
  in
  let children = Tree.mk_child print_case_test value.expr :: cases in
  Tree.print state label ~region children
...
and print_case_clause :
 ...
 fun print state { value; _ } ->
  let print_clause_pattern state =
    Tree.print_unary state "<pattern>" print_pattern
  in
  let children =
  ...
  in
  Tree.print state "<clause>" children
...
and print_conditional :
 ...
 fun state label ~if_so:print_if_so ~if_not:print_if_not node ->
  let Region.{ value; region } = node in
  let print_cond state = Tree.print_unary state "<condition>" print_expr
  and print_then state = Tree.print_unary state "<true>" print_if_so
  and print_else state (_, if_not) =
    Tree.print_unary state "<false>" print_if_not if_not
  in
  let children =
...
and print_I_For state (node : for_int reg) =
  ...
  let print_init state ((index, init) : variable * expr) =
    let children =
      [ Tree.mk_child Tree.print_literal index; Tree.mk_child print_expr init ]
    in
    Tree.print state "<init>" children
  and print_bound state = Tree.print_unary state "<bound>" print_expr
  and print_step state (_, expr) =
    Tree.print_unary state "<step>" print_expr expr
  in
  let children =
  ...
  in
  Tree.print state "I_For" ~region children
...
and print_ForMap state (node : for_map reg) =
  ...
  let print_binding state (source, _arrow, image) =
    ...
    in
    Tree.print state "<binding>" children
  ...
  in
  Tree.print state "I_ForIn" ~region children


and print_ForSetOrList state (node : for_set_or_list reg) =
  let Region.{ value; region } = node in
  let print_collection state = Tree.print_unary state "<collection>" print_expr
...
and print_I_Patch state (node : patch reg) =
  ...
  let children =
    ...
    ; Tree.mk_child print_expr value.patch
    ]
  in
...
and print_I_Skip state wrap = Tree.print_node ~region:wrap#region state "I_Skip"
...
and print_I_While state (node : while_loop reg) =
  ...
  in
  Tree.print state "<while>" children


and print_cond state = Tree.print_unary state "<condition>" print_expr
and print_block_expr state = Tree.print_unary state "<expr>" print_expr
...
and print_P_App state (node : (pattern * pattern tuple option) reg) =
  ...
  in
  Tree.print state "P_App" ~region children
...
and print_P_Attr state (node : attribute * pattern) =
  ...
  in
  Tree.print state "P_Attr" children
...
and print_P_Cons state (node : (pattern * sharp * pattern) reg) =
  ...
  let children =
    [ Tree.mk_child print_pattern head; Tree.mk_child print_pattern tail ]
  in
  Tree.print state "P_Cons" ~region children
...
and print_field_pattern state (node : field_pattern reg) =
  let print_lhs state = Tree.print_unary state "<lhs>" print_pattern
  and print_rhs state = Tree.print_unary state "<rhs>" print_pattern in
  print_field state ~lhs:print_lhs ~rhs:print_rhs node
...
and print_field :
 ...
 fun state ~lhs:print_lhs ~rhs:print_rhs { value; region } ->
  ...
  | Punned { pun; attributes } ->
    let children =
      [ Tree.mk_child print_lhs pun; Tree.mk_child print_attributes attributes ]
    in
    Tree.print state "<punned field>" ~region children
  | Complete { field_lhs; field_lens; field_rhs; attributes } ->
    ...
    in
    Tree.print state "<field>" ~region children


and print_lens state (node : field_lens) =
  match node with
  | Lens_Id l | Lens_Add l | Lens_Sub l | Lens_Mult l | Lens_Div l | Lens_Fun l
    -> Tree.print_literal state l
...
and print_P_Typed state (node : typed_pattern reg) =
  ...
  in
  Tree.print state "P_Typed" ~region children
...
and print_E_Add state (node : plus bin_op reg) = print_bin_op state "E_Add" node

and print_bin_op state label (node : 'op bin_op reg) =
  let Region.{ value; region } = node in
  let children =
    [ Tree.mk_child print_expr value.arg1; Tree.mk_child print_expr value.arg2 ]
  in
  Tree.print state label ~region children
...
and print_E_App state (node : (expr * expr tuple option) reg) =
  ...
  in
  Tree.print state "E_App" ~region children
...
and print_E_Attr state (node : attribute * expr) =
  ...
  let children =
    [ Tree.mk_child print_attribute attribute; Tree.mk_child print_expr expr ]
  in
  Tree.print state "E_Attr" children
...
and print_binding label state (node : binding reg) =
  ...
  let children =
    [ Tree.mk_child print_key value.key; Tree.mk_child print_value value.value ]
  in
  Tree.print state label ~region children
...
and print_E_Block state (node : block_with reg) =
  ...
  in
  Tree.print state "E_Block" ~region children
...
and print_E_Call state (node : call) = print_call state "E_Call" node
...
and print_E_CodeInj state (node : code_inj reg) =
  ...
  in
  Tree.print state "E_CodeInj" ~region children
...
and print_E_Cond state (node : expr conditional reg) =
  print_conditional state "E_Cond" ~if_so:print_expr ~if_not:print_expr node
...
and print_E_Fun state (node : fun_expr reg) =
  ...
  in
  Tree.print state "E_Fun" children


(* Greater or Equal *)

and print_E_Geq state (node : geq bin_op reg) = print_bin_op state "E_Geq" node

(* Greater Than *)

and print_E_Gt state (node : gt bin_op reg) = print_bin_op state "E_Gt" node
...
and print_E_Leq state (node : leq bin_op reg) = print_bin_op state "E_Leq" node
...
and print_E_Lt state (node : lt bin_op reg) = print_bin_op state "E_Lt" node
...
and print_map_lookup state label (node : map_lookup reg) =
  ...
  and print_key state (key : expr brackets reg) =
    Tree.print_unary state "<key>" print_expr key.value.inside
  in
  let print_keys state (keys : expr brackets reg Utils.nseq) =
    print_nseq state "<keys>" print_key keys
  in
  let children =
    [ Tree.mk_child print_map value.map; Tree.mk_child print_keys value.keys ]
  in
  Tree.print state label ~region children
...
and print_E_Neg state (node : minus un_op reg) = print_un_op state "E_Neg" node
...
and print_E_Neq state (node : neq bin_op reg) = print_bin_op state "E_Neq" node
...
and print_E_Or state (node : kwd_or bin_op reg) = print_bin_op state "E_Or" node
...
and print_E_Proj state (node : projection reg) =
  ...
  in
  Tree.print state "E_Proj" ~region children
...
and print_E_Record state (node : record_expr) =
  let print = print_field ~lhs:print_expr ~rhs:print_expr in
  print_compound print "E_Record" state node
...
and print_E_SetMem state (node : set_membership reg) =
  ...
  let children =
    [ Tree.mk_child print_set value.set; Tree.mk_child print_elt value.element ]
  in
  Tree.print state "E_SetMem" ~region children
...
and print_E_Typed state (node : typed_expr par reg) =
  ...
  in
  Tree.print state "E_Typed" ~region children
...
and print_E_Update state (node : update reg) =
  ...
  in
  Tree.print state "E_Update" ~region children

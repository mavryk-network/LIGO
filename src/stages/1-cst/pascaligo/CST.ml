...
let nseq_to_region to_region (hd, tl) = Region.cover (to_region hd) (last to_region tl)
...
let rec type_expr_to_region = function
  ...
  | T_Cart { region; _ } | T_Fun { region; _ } -> region
  | T_Int t -> t#region
  | T_ModPath { region; _ } | T_Par { region; _ } | T_Record { region; _ } -> region
  | T_String t -> t#region
...
let rec expr_to_region = function
  | E_Add { region; _ } | E_And { region; _ } | E_App { region; _ } -> region
  | E_Attr (_, e) -> expr_to_region e
  | E_BigMap { region; _ } | E_Block { region; _ } -> region
  | E_Bytes t -> t#region
  | E_Call { region; _ }
  | E_Case { region; _ }
  | E_Cat { region; _ }
  ...
  | E_Mutez t -> t#region
  | E_Nat t -> t#region
  | E_Neg { region; _ } | E_Neq { region; _ } -> region
  | E_Nil t -> t#region
  | E_Not { region; _ }
  | E_Or { region; _ }
  | E_Par { region; _ }
  ...
  | E_Var t | E_Verbatim t -> t#region


and module_expr_to_region = function
  | M_Body { region; _ } | M_Path { region; _ } -> region
  | M_Var e -> e#region
...
let rec pattern_to_region = function
  ...
  | P_Ctor t -> t#region
  | P_Int t -> t#region
  | P_List { region; _ } | P_ModPath { region; _ } -> region
  | P_Mutez t -> t#region
  | P_Nat t -> t#region
  | P_Nil t -> t#region
  | P_Par { region; _ } | P_Record { region; _ } -> region
  | P_String t -> t#region

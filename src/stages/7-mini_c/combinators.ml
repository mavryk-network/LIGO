...
let t_function ?loc x y : type_expression =
  Expression.make_t ?loc @@ T_function (x, y)


let t_pair ?loc x y : type_expression =
  Expression.make_t ?loc @@ T_tuple [ x; y ]
...
let e_unit ?loc () : expression =
  Expression.make_tpl
    ?loc
    (E_constant { cons_name = C_UNIT; arguments = [] }, t_unit ())
...
let e_let_in ?loc v tv inline expr body : expression =
  Expression.(
    make_tpl ?loc (E_let_in (expr, inline, ((v, tv), body)), get_type body))


let e_let_mut_in ?loc v tv expr body : expression =
  Expression.(
    make_tpl ?loc (E_let_mut_in (expr, ((v, tv), body)), get_type body))
...
let e_var ?loc vname t : expression =
  Expression.(make_tpl ?loc (E_variable vname, t))

...
let rec type_variable ppf : type_expression -> _ =
 fun te ->
  ...
  | T_function (a, b) ->
    fprintf ppf "@[(%a) ->@ (%a)@]" type_variable a type_variable b
  | T_map (k, v) ->
    fprintf ppf "@[<4>map(%a -> %a)@]" type_variable k type_variable v
  | T_big_map (k, v) ->
  ...
  | T_sapling_transaction t ->
    fprintf ppf "Sapling_transaction (%d)" (Z.to_int t)
...
let commas f =
  Format.(pp_print_list ~pp_sep:(fun ppf () -> pp_print_string ppf ", ") f)
...
and type_content ppf : type_content -> unit = function
  ...
  | T_or (a, b) ->
    fprintf
      ppf
      "or %a %a"
      type_expression_annotated
      a
      type_expression_annotated
      b
  | T_function (a, b) ->
    fprintf ppf "lambda (%a) %a" type_expression a type_expression b
  | T_base tc -> fprintf ppf "%a" type_constant tc
  | T_map (k, v) ->
    fprintf ppf "Map (%a,%a)" type_expression k type_expression v
  | T_big_map (k, v) ->
    fprintf ppf "Big_map (%a,%a)" type_expression k type_expression v
  ...
  | T_sapling_transaction x ->
    fprintf ppf "Sapling_transaction (%d)" (Z.to_int x)
...
and expression_content ppf (e : expression_content) =
  ...
  | E_for_each (coll, coll_type, (xs, body)) ->
    fprintf
      ppf
      "@[for_each %a %a %a -> %a@]"
      (* TODO use Cameligo style after it exists? *)
      expression
      coll
      type_expression
      coll_type

...
and expression_content ppf (ec : expression_content) =
  ...
  | E_let_in
      { let_binder; rhs; let_result; attr = { hidden = false; _ } as attr } ->
    fprintf
      ppf
      "@[let %a =@;<1 2>%a%a in@ %a@]"
...
and matching_variant_case
    :  (formatter -> expression -> unit) -> formatter
...
and matching : (formatter -> expression -> unit) -> _ -> matching_expr -> unit =
 fun f ppf m ->
  match m with
  ...
  | Match_record { fields; body; tv = _ } ->
    ...
    fprintf
      ppf
      "| @[%a@] ->@ @[%a@]"
      (tuple_or_record_sep_expr (Binder.pp type_expression_annot))
      fields
      f
      body

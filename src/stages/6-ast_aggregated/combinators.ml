...
[@@deriving
  ez
    { prefixes =
        [ ( "make_t"
          , fun ?(loc = Location.generated) ?source_type type_content
                : type_expression ->
    ...
    ; wrap_constructor =
        ( "type_content"
        , fun type_content ?loc ?source_type () ->
            make_t ?loc ?source_type type_content )
...
let t_constant ?loc injection parameters : type_expression =
  t_constant
    ?loc
    { language = Backend.Michelson.name; injection; parameters }
    ()
...
let t_triplet ?loc a b c : type_expression = t_tuple ?loc [ a; b; c ] ()

let t_unforged_ticket ?loc ty : type_expression =
  ez_t_record
    ?loc
    [ ( Label "ticketer"
      , { content = { associated_type = t_address (); decl_pos = 0 }
        ; michelson_annotation = None
        } )
    ; ( Label "value"
      , { content = { associated_type = ty; decl_pos = 1 }
        ; michelson_annotation = None
        } )
    ; ( Label "amount"
      , { content = { associated_type = t_nat (); decl_pos = 2 }
        ; michelson_annotation = None
        } )
    ]
...
let t_option ?loc typ : type_expression =
  t_sum_ez ?loc [ "Some", typ; "None", t_unit () ]
...
let get_t_base_inj (t : type_expression) (v : Ligo_prim.Literal_types.t)
    : unit option
  =
...
let get_t_map (t : type_expression) : (type_expression * type_expression) option
  ...
  | T_constant { language = _; injection; parameters = [ k; v ] }
    when Ligo_prim.Literal_types.equal injection Ligo_prim.Literal_types.Map ->
    Some (k, v)
  | _ -> None
...
let get_t_typed_address (t : type_expression)
  ...
  match t.type_content with
  | T_constant { language = _; injection; parameters = [ k; v ] }
    when Ligo_prim.Literal_types.equal
           injection
           Ligo_prim.Literal_types.Typed_address -> Some (k, v)
  | _ -> None
...
let is_t_bool t =
  match t.type_content with
  | T_sum { fields; _ } ->
    (match Map.keys fields with
    | [ Label "True"; Label "False" ] | [ Label "False"; Label "True" ] -> true
    | _ -> false)
  | _ -> false
    ...
    (t_record
       ...
       (Map.map
          ~f:(fun t ->
            ...
            { content = { Rows.Elem.associated_type; decl_pos = 0 }
            ; michelson_annotation = None
            })
          r))
    ...
    (ez_t_record
       ...
       (List.mapi
          ~f:(fun i (x, y) ->
            ( x
            , { content =
                  { Rows.Elem.associated_type = y.type_expression
                  ; decl_pos = i
                  }
              ; michelson_annotation = None
              } ))
...
let get_a_bool (t : expression) =
  ...
  | E_constructor { constructor = Label name; element }
    when String.equal name "True"
         && compare_expression_content element.expression_content @@ e_unit ()
            = 0 -> Some true
  | E_constructor { constructor = Label name; element }
...
let build_type_abstractions init =
  let t_for_all ty_binder kind type_ =
...
let build_type_insts init =
  let f av forall =
    ...
    make_e
      ...
      (Helpers.subst_type ty_binder type_ t)
  in

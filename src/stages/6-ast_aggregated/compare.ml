...
let cmp10 f a1 b1 g a2 b2 h a3 b3 i a4 b4 j a5 b5 k a6 b6 l a7 b7 m a8 b8 n a9 b9 o a10 b10 = match f a1 b1 with 0 -> (match g a2 b2 with 0 -> (match h a3 b3 with 0 -> (match i a4 b4 with 0 -> (match j a5 b5 with 0 -> (match k a6 b6 with 0 -> (match l a7 b7 with 0 -> (match m a8 b8 with 0 -> (match n a9 b9 with 0 -> o a10 b10 | c -> c) | c -> c) | c -> c) | c -> c) | c -> c) | c -> c) | c -> c) | c -> c) | c -> c
...
and let_in {let_binder=ba;rhs=ra;let_result=la;attr = { inline=aa;no_mutation=nma;entrypoint=ea;view=va;public=pua;thunk=ta;hidden=ha}} {let_binder=bb;rhs=rb;let_result=lb;attr = { inline=ab;no_mutation=nmb;entrypoint=eb;view=vb;public=pub;thunk=tb;hidden=hb}} =
  cmp10
    (binder type_expression) ba bb
    expression ra rb
    expression la lb

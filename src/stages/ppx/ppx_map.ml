            ...
            Pstr_eval
              ( { pexp_desc =
                    Pexp_tuple
                      ...
                      ; { pexp_desc = Pexp_constant (Pconst_string (rid, _, _))
                        ; _
                        }
                      ]
                ; _
          ...
          let attr =
            List.find_map expr.pvb_attributes ~f:(function
                | { attr_name; attr_payload; _ }
                  when String.equal attr_name.txt "map" -> Some attr_payload
                | _ -> None)
          in
          let attr = Option.bind attr ~f:extract_payload in
          Option.value_map attr ~default:[ expr ] ~f:(fun (id, ids) ->
              List.map ids ~f:(fun new_id ->
                  (replace_exprs id new_id)#value_binding expr))
        in

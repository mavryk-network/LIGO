...
let constructor_impl (cd : constructor_declaration) =
  ...
  let s =
    match get_payload "print" cd.pcd_attributes with
    | Some s -> s
    | None ->
      if String.is_prefix ~prefix:"C_" txt
      then String.drop_prefix txt 2
      else txt
  in
  ...
  in
  case
    ~lhs:(ppat_construct ~loc { loc = cloc; txt = Lident txt } pat)
    ~guard:None
    ~rhs

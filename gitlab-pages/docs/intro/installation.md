...
You can get the rolling release [here](https://gitlab.com/ligolang/ligo/-/jobs/4606441821/artifacts/raw/ligo), make it executable, and you are done!

```zsh
wget https://gitlab.com/ligolang/ligo/-/jobs/4606441821/artifacts/raw/ligo
chmod +x ./ligo
```
...
First, download [the package](https://gitlab.com/ligolang/ligo/-/jobs/4606441821/artifacts/raw/ligo.deb), and then install using: 
...
Linux or OSX:
> ```sh
> docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.69.0
> ```
> For convenience you can alias the above command
> ```sh
> alias ligo='docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.69.0'
> ```
...
Windows:
> ```dos
> docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.69.0`
> ```
> For convenience you can alias the above command
> ```dos
> doskey ligo=docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.69.0 $*
> ```
...
You can download the installer from [here hosted on our Gitlab](https://gitlab.com/ligolang/ligo/-/jobs/4606441821/artifacts/raw/ligo_installer.exe?inline=false).

...
Linux or OSX:
> ```sh
> docker run --rm -v "$PWD":"$PWD" -w "$PWD" mavrykdynamics/ligo:1.8.0
> ```
> For convenience you can alias the above command
> ```sh
> alias ligo='docker run --rm -v "$PWD":"$PWD" -w "$PWD" mavrykdynamics/ligo:1.8.0'
> ```
...
> ```dos
> docker run --rm -v "%CD%":/cd -w /cd mavrykdynamics/ligo:1.8.0`
> ```
> For convenience you can alias the above command
> ```dos
> doskey ligo=docker run --rm -v "%CD%":/cd -w /cd mavrykdynamics/ligo:1.8.0 $*
> ```

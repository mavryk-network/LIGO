...
However, they can also be tied to an implicit account and have an address that starts with `kt1`, `kt2` or `kt3`, [depending on the hashing function](https://protocol.mavryk.org/michelson-reference/#type-address).
...
can do it with `Mavryk.get_self_address`. When used inside a lambda,

...
:::note
The LIGO interpreter uses the [same library that Mavryk internally uses for testing](https://gitlab.com/mavryk-network/mavryk-protocol/-/tree/master/src/proto_alpha/lib_protocol/test/helpers).
:::
...
1. It calls the `increment` entrypoint with the `Test.Next.Contract.transfer_exn` function, passing the entrypoint, the parameter, and 0 mav.
...
let run_test1 =
  ...
  let orig = Test.Next.Originate.contract (contract_of MyContract.MyContract) initial_storage 0mav in
  let () = Assert.assert (Test.Next.Typed_address.get_storage(orig.taddr) = initial_storage) in
  let _: nat = Test.Next.Contract.transfer_exn (Test.Next.Typed_address.get_entrypoint "increment" orig.taddr) 32 0mav in
...
let test_failure =
  let initial_storage = 10 in
  let orig = Test.Next.Originate.contract (contract_of MyContract) initial_storage 0mav in
  let result: test_exec_result = Test.Next.Contract.transfer (Test.Next.Typed_address.get_entrypoint "increment" orig.taddr) 50 0mav in
  match result with
...
let test_admin =
  ...
  let result = Test.Next.Contract.transfer (Test.Next.Typed_address.get_entrypoint "reset" orig.taddr ) unit 0mav in
  ...
  let _: nat = Test.Next.Contract.transfer_exn (Test.Next.Typed_address.get_entrypoint "reset" orig.taddr) unit 0mav in
...
let test_accounts =
  let initial_balances : mav list = [] in

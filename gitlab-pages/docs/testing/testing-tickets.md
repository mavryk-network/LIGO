...
The Mavryk protocol has two types of operations:

- Internal operations are created from inside the chain, such as by smart contracts.
- External operations are created from outside the chain, such as by the Mavkit client, other Mavryk clients and SDKs, and in the context of LIGO testing, by functions such as `Test.Next.Originate.contract`.
For more information about the technical implementation, see [Semantics of smart contracts and transactions](https://protocol.mavryk.org/active/michelson.html#semantics-of-smart-contracts-and-transactions) in the Mavkit documentation.

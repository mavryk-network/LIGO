...
let threshold = (p : unit) =>
  if (Mavryk.get_amount() == 100tz) { 42; } else { 0; };
```
...
<SyntaxTitle syntax="reasonligo">
let transaction: ('param, mumav , contract('param)) => operation
</SyntaxTitle>
<SyntaxTitle syntax="jsligo">
...
<SyntaxTitle syntax="reasonligo">
let create_contract : (('param, 'storage) -> (list(operation), 'storage)) => option(key_hash) => mav => 'storage => (operation, address)
</SyntaxTitle>
<SyntaxTitle syntax="jsligo">
...
```reasonligo group=sap_t
let f = (tr : tr) =>
  switch (Mavryk.sapling_verify_update (tr, x)) {
    | Some (_, x) => x
    | None => failwith ("failed")
  }
```
...
const my_ticket1 = Mavryk.create_ticket (1, 10n)
const my_ticket2 = Mavryk.create_ticket ("one", 10n)
```
...
let my_ticket1 = Mavryk.create_ticket 1 10n
let my_ticket2 = Mavryk.create_ticket "one" 10n
```
...
let my_ticket1 = Mavryk.create_ticket(1, 10 as nat);
let my_ticket2 = Mavryk.create_ticket("one", 10 as nat);
```
...
```reasonligo group=manip_ticket
let v2 =
  let ((_addr, (v, _amt)), _ticket) = Mavryk.read_ticket (my_ticket2);
  v;
```
...
```reasonligo group=manip_ticket
let (ta,tb) =
  switch (Mavryk.split_ticket(my_ticket1, (6n, 4n))) {
  | None => failwith("amt_a + amt_v != amt")
  | Some split_tickets => split_tickets
  } ;
```
...
```pascaligo group=manip_ticket
const tc = {
  const ta = Mavryk.create_ticket (1, 10n);
  const tb = Mavryk.create_ticket (1, 5n)
...
let tc : int ticket option =
  let ta = Mavryk.create_ticket 1 10n in
  let tb = Mavryk.create_ticket 1 5n in
...
```reasonligo group=manip_ticket
let tc =
  let ta = Mavryk.create_ticket(1, 10n);
  let tb = Mavryk.create_ticket(1, 5n);
  Mavryk.join_tickets((ta, tb));
```
...
let ta = Mavryk.create_ticket(1, 10 as nat);
let tb = Mavryk.create_ticket(1, 5 as nat);
...
function main (const i : parameter ; const store : storage) : return is {
  const my_ticket1 = Mavryk.create_ticket (i, 10n);
...
let main (x : parameter * storage) : return =
  let i, store = x in
  let my_ticket1 = Mavryk.create_ticket i 10n in
...
let main = (x : (parameter , storage)) : return => {
  let (i,store) = x ;
  let my_ticket1 = Mavryk.create_ticket (i, 10n) ;
  let (_,x) = Big_map.get_and_update ("hello", Some(my_ticket1), store) ;
  ([], x)
};
```
...
let main = (x: [parameter, storage]): return_ => {
  let [i, store] = x ;
  let my_ticket1 = Mavryk.create_ticket (i, 10 as nat);
  let [_, x] = Big_map.get_and_update ("hello", Some(my_ticket1), store);
  return [list([]), x]
};
```

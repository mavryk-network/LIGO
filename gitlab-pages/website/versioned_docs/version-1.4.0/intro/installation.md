...
You can get the rolling release [here](https://gitlab.com/mavryk-network/ligo/-/jobs/6629577754/artifacts/raw/ligo), make it executable, and you are done!

```zsh
wget https://gitlab.com/mavryk-network/ligo/-/jobs/6629577754/artifacts/raw/ligo
chmod +x ./ligo
```
...
First, download [the package](https://gitlab.com/mavryk-network/ligo/-/jobs/6629577754/artifacts/raw/ligo.deb), and then install using: 
...
Linux or OSX:
> ```sh
> docker run --rm -v "$PWD":"$PWD" -w "$PWD" mavrykdynamics/ligo:1.6.0
> ```
> For convenience you can alias the above command
> ```sh
> alias ligo='docker run --rm -v "$PWD":"$PWD" -w "$PWD" mavrykdynamics/ligo:1.6.0'
> ```
...
> ```dos
> docker run --rm -v "%CD%":/cd -w /cd mavrykdynamics/ligo:1.6.0`
> ```
> For convenience you can alias the above command
> ```dos
> doskey ligo=docker run --rm -v "%CD%":/cd -w /cd mavrykdynamics/ligo:1.6.0 $*
> ```

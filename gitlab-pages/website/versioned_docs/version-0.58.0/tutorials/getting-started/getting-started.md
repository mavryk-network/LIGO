...
First, download [the package](https://gitlab.com/ligolang/ligo/-/jobs/3508308449/artifacts/raw/ligo.deb), and then install using:
...
If you've [installed 🐳 Docker](https://docs.docker.com/install/), you can run the latest [LIGO release 0.58.0](https://ligolang.org/docs/next/intro/changelog):

Linux or OSX:

> ```sh
> docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.58.0
> ```
>
> For convenience you can alias the above command
>
> ```sh
> alias ligo="docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.58.0"
> ```
>
...
> ```dos
> docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.58.0`
> ```
>
> For convenience you can alias the above command
>
> ```dos
> doskey ligo=docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.58.0 $*
> ```
>
...
let test_increment = {
  ...
  let (taddr, _, _) = Test.originate(main, initial_storage, 0mav);
  let contr = Test.to_contract(taddr);
  let _ = Test.transfer_to_contract_exn(contr, (Increment (1)), 1mumav);
